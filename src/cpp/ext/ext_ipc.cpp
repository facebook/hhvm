...
int64 f_ftok(CStrRef pathname, CStrRef proj) {
  if (pathname.empty()) {
    Logger::Warning("Pathname is empty");
    return -1;
  }
  if (proj.length() != 1) {
    Logger::Warning("Project identifier has to be one character int64: %s",
                    (const char *)proj);
    return -1;
  }
...
Variant f_msg_get_queue(int64 key, int64 perms /* = 0666 */) {
  ...
  if (id < 0) {
    ...
    if (id < 0) {
      Logger::Warning("Failed to create message queue for key 0x%lx: %s",
...
bool f_msg_remove_queue(CObjRef queue) {
  ...
  if (!q) {
    Logger::Warning("Invalid message queue was specified");
    return false;
  }
...
bool f_msg_send(CObjRef queue, int64 msgtype, CVarRef message,
  ...
  if (result < 0) {
    int err = errno;
    Logger::Warning("Unable to send message: %s",
...
bool f_msg_receive(CObjRef queue, int64 desiredmsgtype, Variant msgtype,
  ...
  if (maxsize <= 0) {
    Logger::Warning("Maximum size of the message has to be greater than zero");
    return false;
  }
  ...
  if (result < 0) {
    int err = errno;
    Logger::Warning("Unable to receive message: %s",
                    Util::safe_strerror(err).c_str());
    if (!errorcode.isNull()) {
      errorcode = err;
    }
  ...
  if (unserialize) {
    ...
    try {
      ...
      Logger::Warning("Message corrupted");
      return false;
    }
  } else {
...
public:
  ...
  bool op(bool acquire) {
    ...
    if (!acquire && count == 0) {
      Logger::Warning("SysV semaphore %d (key 0x%x) is not currently acquired",
                      o_getId(), key);
      return false;
    }
    ...
    while (semop(semid, &sop, 1) == -1) {
      if (errno != EINTR) {
        Logger::Warning("failed to %s key 0x%x: %s",
...
Variant f_sem_get(int64 key, int64 max_acquire /* = 1 */,
  ...
  if (semid == -1) {
    Logger::Warning("failed for key 0x%lx: %s", key,
  ...
  while (semop(semid, sop, 3) == -1) {
    if (errno != EINTR) {
      Logger::Warning("failed acquiring SYSVSEM_SETVAL for key 0x%lx: %s",
                      key, Util::safe_strerror(errno).c_str());
      break;
    }
  }
  ...
  while (semop(semid, sop, 1) == -1) {
    if (errno != EINTR) {
      Logger::Warning("failed releasing SYSVSEM_SETVAL for key 0x%lx: %s",
                      key, Util::safe_strerror(errno).c_str());
      break;
    }
  }
...
bool f_sem_remove(CObjRef sem_identifier) {
  ...
  if (semctl(sem_ptr->semid, 0, IPC_STAT, un) < 0) {
    Logger::Warning("SysV semaphore %d does not (any longer) exist",
                    sem_identifier->o_getId());
    return false;
  }

  if (semctl(sem_ptr->semid, 0, IPC_RMID, un) < 0) {
    Logger::Warning("failed for SysV sempphore %d: %s",
                    sem_identifier->o_getId(),
                    Util::safe_strerror(errno).c_str());
    return false;
  }
...
Variant f_shm_attach(int64 shm_key, int64 shm_size /* = 10000 */,
  ...
  if (shm_size < 1) {
    Logger::Warning("Segment size must be greater then zero.");
    return false;
  }
  ...
  if ((shm_id = shmget(shm_key, 0, 0)) < 0) {
    if (shm_size < (int)sizeof(sysvshm_chunk_head)) {
      Logger::Warning("failed for key 0x%x: memorysize too small", shm_key);
      free(shm_list_ptr);
      return false;
    }
    if ((shm_id = shmget(shm_key, shm_size, shm_flag | IPC_CREAT | IPC_EXCL))
        < 0) {
      Logger::Warning("failed for key 0x%x: %s", shm_key,
                      Util::safe_strerror(errno).c_str());
      free(shm_list_ptr);
      return false;
    }
...
bool f_shm_detach(int64 shm_identifier) {
  ...
  if (iter == g_shms.end()) {
    Logger::Warning("%lld is not a SysV shared memory index", shm_identifier);
    return false;
  }
  g_shms.erase(iter);
  delete *iter;
...
bool f_shm_remove(int64 shm_identifier) {
  ...
  if (shmctl(shm_list_ptr->id, IPC_RMID,NULL) < 0) {
    Logger::Warning("failed for key 0x%x, id %lld: %s", shm_list_ptr->key,
...
bool f_shm_put_var(int64 shm_identifier, int64 variable_key,
  ...
  if (ret == -1) {
    Logger::Warning("not enough shared memory left");
    return false;
  }
  return true;
}
...
bool f_shm_remove_var(int64 shm_identifier, int64 variable_key) {
  ...
  long shm_varpos = check_shm_data(shm_list_ptr->ptr, variable_key);
  if (shm_varpos < 0) {
    Logger::Warning("variable key %lld doesn't exist", variable_key);
    return false;
  }
