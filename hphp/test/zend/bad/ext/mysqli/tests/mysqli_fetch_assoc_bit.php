<?php
	...
	for ($bits = 1; $bits <= 46; $bits++) {
		...
		$tests = 0;
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_fetch_assoc_bit_table_1") ||
			!mysqli_query($link, $sql = sprintf('CREATE TABLE test_mysqli_fetch_assoc_bit_table_1(id BIGINT, bit_value BIT(%d) NOT NULL, bit_null BIT(%d) DEFAULT NULL) ENGINE="%s"', $bits, $bits, $engine)))
		...
		while ($tests < 10) {
			...
			$sql = sprintf("INSERT INTO test_mysqli_fetch_assoc_bit_table_1(id, bit_value) VALUES (%s, b'%s')", $value, $bin);
			...
			$sql = sprintf("SELECT id, BIN(bit_value) AS _bin, bit_value + 0 AS _bit_value0, bit_value, bit_null FROM test_mysqli_fetch_assoc_bit_table_1 WHERE id = %s", $value);
...
?>
<?php
	$test_table_name = 'test_mysqli_fetch_assoc_bit_table_1'; require_once("clean_table.inc");
?>