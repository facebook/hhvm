<?php
	$test_table_name = 'test_mysqli_stmt_bind_param_references_table_1'; require('table.inc');

	function findRow($offset, $link, $id, $label) {

		$sql = sprintf("SELECT id, label FROM test_mysqli_stmt_bind_param_references_table_1 WHERE id = '%d' AND label = '%s'",
				$id, $label);
		...
		$sql = sprintf("DELETE FROM test_mysqli_stmt_bind_param_references_table_1 WHERE id = '%d' AND label = '%s'",
				$id, $label);
	...
	mysqli_query($link, "ALTER TABLE test_mysqli_stmt_bind_param_references_table_1 DROP PRIMARY KEY");

	$stmt = mysqli_stmt_init($link);
	if (!mysqli_stmt_prepare($stmt, "INSERT INTO test_mysqli_stmt_bind_param_references_table_1(id, label) VALUES (?, ?)"))
...
?>
<?php
	$test_table_name = 'test_mysqli_stmt_bind_param_references_table_1'; require_once("clean_table.inc");
?>