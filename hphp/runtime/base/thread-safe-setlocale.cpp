...
/*
 * This class reimplements the setlocale() and localeconv() functions in a thread-safe manner 
 * and provides stronger symbols for them than would be available from the glibc shared library.
 * This is part of hphp/runtime/base so the symbols are always available in the main binary and
 * hence act as be a global replacement for calls even from other shared objects like ICU/etc.
 *
 * setlocale(): The implementation uses newlocale() and uselocale(). Because these don't provide
 * a human readable locale string as output, the implementation maintains a thread-local object
 * to persist the currently active locale settings. This design has meant some duplication to
 * what glibc already does in parsing the string locales but this should be OK because setlocale
 * would never be called enough to be a bottleneck. The object is hooked into hphp_session_init()
 * and hphp_context_shutdown() to be init'd and reset'd at the end of a request. I chose not to
 * use a RequestEventHandler because setlocale() may be called during static variable initialization
 * before any of the HHVM stuff like memory managers, etc. are even loaded. This is also why I used
 * std::string instead of the HHVM provided type.
 *
 * localeconv(): A thread-local struct lconv is maintained and it is filled by making multiple
 * calls to nl_langinfo() which is thread-safe.
 */
...
ThreadSafeLocaleHandler::ThreadSafeLocaleHandler() {
  m_category_locale_map = {
    #define FILL_IN_CATEGORY_LOCALE_MAP(category) {category, category ## _MASK, #category, ""}
...
const char* ThreadSafeLocaleHandler::actuallySetLocale(int category, const char* locale_cstr) {
  ...
  if (category == LC_ALL) {
    if (strchr(locale_cstr, ';') != nullptr) {
      /*
       * We need to parse out any semi-colon delimited categories and locales and store
       * them separately in the appropriate category_locale_map.locale_str
       *
       * We are not validating the string for correctness of format. The newlocale() call already
       * did that for us.
       */
      char *locale_cstr_copy = strdup(locale_cstr);

      for (char *start_ptr = locale_cstr_copy, *group_save = nullptr; ; start_ptr = nullptr) {
        ...
        int count = 0;
        for (char *item_ptr = group, *item_save = nullptr; ; count++, item_ptr = nullptr) {
        ...
        /* Completely naive search. If this is a bottleneck it can be converted into a
         * hash-map
         */
