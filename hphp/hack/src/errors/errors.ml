  ...
  | Context ->
     ...
     let truncated = match max_errors with
       | Some max_errors when dropped_count > 0 ->
          Printf.sprintf " (only showing first %d, dropped %d).\n" max_errors dropped_count
       | _ -> ".\n"
     in
...
let wrong_expression_kind_builtin_attribute expr_kind pos attr =
  let msg1 =
    Printf.sprintf "The %s attribute cannot be used on %s." (Utils.strip_ns attr) expr_kind
  in
...
let shape_field_non_existence_reason pos name = function
  ...
  | `Nothing reason ->
    (pos, "The type of the field '" ^ name ^ "' in this shape doesn't allow any values") :: reason
