...
BrotliCompressor::BrotliCompressor(BrotliEncoderMode mode, uint32_t quality, uint32_t lgWin) {
  if (quality < BROTLI_MIN_QUALITY || quality > BROTLI_MAX_QUALITY) {
    throw Exception("compression quality (%d) must be within %d..%d", quality, BROTLI_MIN_QUALITY, BROTLI_MAX_QUALITY);
  }

  if (lgWin < BROTLI_MIN_WINDOW_BITS || lgWin > BROTLI_MAX_WINDOW_BITS) {
    throw Exception("window size (%d) must be within %d..%d", lgWin, BROTLI_MIN_WINDOW_BITS, BROTLI_MAX_WINDOW_BITS);
  }
