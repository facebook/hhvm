...
let string_to_json message =
  let open Hh_json in
  try
    Ok (json_of_string message)
  with
    | Syntax_error e | Invalid_argument e -> Error (Parse_error e)

let check_service_framework3_rpc_protocol = function
  | "service_framework3_rpc" -> Ok Nuclide_rpc
  | x ->
    Error (Invalid_request (Printf.sprintf "Unknown protocol: %s" x))
