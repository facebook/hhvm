...
StructMetadata<::cpp2::Foo2>::gen(ThriftMetadata& metadata) {
  if (FLAGS_thrift_enable_schema_to_metadata_conversion) {
    return genStructMetadata<::cpp2::Foo2>(metadata, kGenerateAll).metadata;
  }

  auto res = genStructMetadata<::cpp2::Foo2>(metadata, {.genAnnotations = folly::kIsDebug});
  if (res.preExists) {
    return res.metadata;
  }
  ...
  for (const auto& f : *module_Foo2_fields) {
    genStructFieldMetadata<::cpp2::Foo2>(module_Foo2.fields()[i], f, i);
