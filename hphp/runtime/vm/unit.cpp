...
void Unit::merge() {
  ...
  if (UNLIKELY((m_mergeState.load(std::memory_order_relaxed) == MergeState::InitialMerged))) {
    mergeImpl<false>();
  }

  if (UNLIKELY((m_mergeState.load(std::memory_order_relaxed) == MergeState::NeedsNonPersistentMerged))) {
    if (isSystemLib()) {
