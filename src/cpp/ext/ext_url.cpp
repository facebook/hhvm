...
String f_http_build_query(CVarRef formdata,
                          CStrRef numeric_prefix /* = null_string */,
                          CStrRef arg_separator /* = "&" */) {
  if (!formdata.is(KindOfArray) && !formdata.is(KindOfObject)) {
    throw InvalidArgumentException("formdata", "(need Array or Object)");
  }

  StringBuffer ret(1024);
  std::set<void*> seen_arrs;
  url_encode_array(ret, formdata.toArray(), seen_arrs,
                   numeric_prefix, String(), String(), arg_separator);
  return ret;
}
...
Variant f_parse_url(CStrRef url, int component /* = -1 */) {
  ...
  if (!url_parse(resource, url.data(), url.size())) {
    Logger::Verbose("invalid url: %s", url.data());
    return false;
  }
  ...
  if (component > -1) {
    ...
    case PHP_URL_PORT:
      ...
      break;
    default:
      throw InvalidArgumentException("component", component);
    }
    return null;
  }
