...
#include <cpp/base/zend/zend_html.h>
#include <langinfo.h>
...
inline Array f_explode(CStrRef delimiter, CStrRef str, int limit = 0x7FFFFFFF) {
...
inline Array f_str_split(CStrRef str, int split_length = 1) {
  return StringUtil::Split(str, split_length);
}
inline String f_chunk_split(CStrRef body, int chunklen = 76,
                            CStrRef end = "\r\n") {
...
inline String f_wordwrap(CStrRef str, int width = 75, CStrRef wordbreak = "\n",
                         bool cut = false) {
  return StringUtil::WordWrap(str, width, wordbreak, cut);
}
...
inline String f_html_entity_decode(CStrRef str, int quote_style = k_ENT_COMPAT,
  ...
  // dropping charset parameter and see cpp/base/zend_html.h
...
inline String f_htmlentities(CStrRef str, int quote_style = k_ENT_COMPAT,
  ...
  // dropping charset and double_encode parameters and see cpp/base/zend_html.h
...
inline int f_printf(int _argc, CStrRef format, CArrRef _argv = null_array) {
  int len = 0; char *output = string_printf(format.data(), format.size(),
                                            _argv, &len);
  echo(output); free(output);
  return len;
}
inline int f_vprintf(CStrRef format, CArrRef args) {
  int len = 0; char *output = string_printf(format.data(), format.size(),
                                            args, &len);
  echo(output); free(output);
  return len;
}
inline String f_sprintf(int _argc, CStrRef format, CArrRef _argv = null_array) {
  char *output = string_printf(format.data(), format.size(), _argv, NULL);
  return String(output, AttachString);
}
inline String f_vsprintf(CStrRef format, CArrRef args) {
...
inline String f_money_format(CStrRef format, double number) {
  return StringUtil::MoneyFormat(format, number);
}
