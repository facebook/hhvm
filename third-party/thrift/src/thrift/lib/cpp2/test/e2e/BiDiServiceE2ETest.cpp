...
// EB-mode BiDi tests
//
// These mirror the CPU-pool BiDi tests above one-for-one, but with the handler
// annotated `@cpp.ProcessInEbThreadUnsafe` and implemented via `async_eb_*`
// instead of `co_*`. In EB mode, the user's coroutine starts inline on the IO
// thread via `ServerBiDiStreamFactory::startInlineUnsafe`, and the bridges'
// cross-thread hops degenerate into same-thread `runInLoop` callbacks.
// =============================================================================

CO_TEST_F(BiDiServiceE2ETest, EbMode_BasicStreamTransformation) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoEb(
        HandlerCallbackPtr<StreamTransformation<std::string, std::string>>
            callback) override {
      EXPECT_TRUE(callback->getEventBase()->inRunningEventBaseThread());
      callback->result(
          StreamTransformation<std::string, std::string>{
              [](folly::coro::AsyncGenerator<std::string&&> input)
                  -> folly::coro::AsyncGenerator<std::string&&> {
                while (auto item = co_await input.next()) {
                  co_yield std::move(*item);
  ...
  EXPECT_FALSE((co_await streamGen.next()).has_value());
}

CO_TEST_F(BiDiServiceE2ETest, EbMode_CustomSinkBufferSize) {
  constexpr int32_t kBufferSize = 5;
  constexpr int64_t kNumItems = 20;

  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_intStreamEb(
        HandlerCallbackPtr<StreamTransformation<int64_t, int64_t>> callback)
        override {
      callback->result(
          StreamTransformation<int64_t, int64_t>{
              .func = [](folly::coro::AsyncGenerator<int64_t&&> input)
                  -> folly::coro::AsyncGenerator<int64_t&&> {
                while (auto item = co_await input.next()) {
                  co_yield std::move(*item);
  ...
  auto client = makeClient<detail::test::TestBiDiService>();
  BidirectionalStream<int64_t, int64_t> stream =
      co_await client->co_intStreamEb();
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_BasicResponseAndStreamTransformation) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoWithResponseEb(
        HandlerCallbackPtr<ResponseAndStreamTransformation<
            std::string,
            std::string,
            std::string>> callback,
        std::unique_ptr<std::string> initial) override {
      callback->result(
  ...
  auto client = makeClient<detail::test::TestBiDiService>();
  ResponseAndBidirectionalStream<std::string, std::string, std::string> result =
      co_await client->co_echoWithResponseEb("Test");
  EXPECT_EQ(result.response, "Test");
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_MethodThrowsDeclaredException) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_canThrowEb(
        HandlerCallbackPtr<StreamTransformation<int64_t, int64_t>> callback)
        override {
      callback->exception(
          folly::exception_wrapper{
              detail::test::BiDiMethodException{"Method threw"}});
    }
  };
  ...
  auto client = makeClient<detail::test::TestBiDiService>();
  EXPECT_THROW(
      co_await client->co_canThrowEb(), detail::test::BiDiMethodException);
}

CO_TEST_F(BiDiServiceE2ETest, EbMode_MethodThrowsUndeclaredException) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_canThrowEb(
        HandlerCallbackPtr<StreamTransformation<int64_t, int64_t>> callback)
        override {
      callback->exception(
          folly::exception_wrapper{std::runtime_error{"Method threw"}});
    }
  };
  ...
  auto client = makeClient<detail::test::TestBiDiService>();
  EXPECT_THROW(
      co_await client->co_canThrowEb(), apache::thrift::TApplicationException);
}

CO_TEST_F(BiDiServiceE2ETest, EbMode_SinkThrowsDeclaredException) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_canThrowEb(
        HandlerCallbackPtr<StreamTransformation<int64_t, int64_t>> callback)
        override {
      callback->result(
          StreamTransformation<int64_t, int64_t>{
              [](folly::coro::AsyncGenerator<int64_t&&> input)
                ...
                CO_FAIL()
                    << "Expected BiDiSinkException but was never received";
              }});
    }
  };
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_SinkThrowsUndeclaredException) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_canThrowEb(
        HandlerCallbackPtr<StreamTransformation<int64_t, int64_t>> callback)
        override {
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_StreamThrowsDeclaredException) {
  constexpr int64_t kStreamItemsUntilThrow = 500;
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_StreamThrowsUndeclaredException) {
  constexpr int64_t kStreamItemsUntilThrow = 500;
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_IgnoreInputProduceOutput) {
  constexpr int64_t kTestLimit = 100;
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_ConsumeInputNoOutput) {
  constexpr int64_t kTestLimit = 10000;
...
CO_TEST_F(
    BiDiServiceE2ETest, EbMode_ClientDropsStreamWhileServerBlocksOnInput) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoEb(
        HandlerCallbackPtr<StreamTransformation<std::string, std::string>>
            callback) override {
  ...
  testConfig({std::make_shared<Handler>()});
  auto client = makeClient<detail::test::TestBiDiService>();
  auto bidi = co_await client->co_echoEb();
...
CO_TEST_F(
    BiDiServiceE2ETest, EbMode_ServerStopsOutputWhileClientBlocksOnStream) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoEb(
        HandlerCallbackPtr<StreamTransformation<std::string, std::string>>
            callback) override {
...
CO_TEST_F(
    BiDiServiceE2ETest,
    EbMode_ClientDropsStreamWhileServerProducesIndefinitely) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoEb(
        HandlerCallbackPtr<StreamTransformation<std::string, std::string>>
            callback) override {
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_CreditStarvationTimeout) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoEb(
        HandlerCallbackPtr<StreamTransformation<std::string, std::string>>
            callback) override {
...
CO_TEST_F(
    BiDiServiceE2ETest, EbMode_CreditStarvationTimeoutClientStopsConsuming) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_intStreamEb(
        HandlerCallbackPtr<StreamTransformation<int64_t, int64_t>> callback)
        override {
  ...
  auto client = makeClient<detail::test::TestBiDiService>();
  auto bidi = co_await client->co_intStreamEb();
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_SinkChunkTimeout) {
  struct Handler : public ServiceHandler<detail::test::TestBiDiService> {
    void async_eb_echoEb(
        HandlerCallbackPtr<StreamTransformation<std::string, std::string>>
            callback) override {
...
CO_TEST_F(
    BiDiServiceE2ETest, EbMode_NoSinkChunkTimeoutWhenClientSendsPromptly) {
  constexpr int64_t kNumItems = 20;
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_NoCreditStarvationWhenClientConsumes) {
  constexpr int64_t kStreamItems = 50;
...
CO_TEST_F(BiDiServiceE2ETest, EbMode_CustomBufferReplenishThreshold) {
  constexpr int32_t kBufferSize = 10;
  constexpr int64_t kNumItems = 20;
...
CO_TEST_F(
    BiDiServiceE2ETest, EbMode_ClientStreamCustomBufferReplenishThreshold) {
  constexpr int64_t kNumItems = 50;
  ...
  BidirectionalStream<int64_t, int64_t> stream =
      co_await client->co_intStreamEb(rpcOptions);
