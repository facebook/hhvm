...
type env = {
  ...
  server_start_options: Server_controller.server_start_options;
...
type t = env * Monitor_utils.monitor_config * Unix.file_descr
    ...
    setup_handler_for_signals
      begin
        fun _ ->
          ...
          Server_controller.kill_server ~violently:false process;
...
let kill_server_with_check_and_wait (server : server_process) : unit =
  ...
  | Alive server ->
    ...
    let rec kill_server_with_check_and_wait_impl () =
      Server_controller.kill_server
        ~violently:Float.(Unix.gettimeofday () -. start_t >= 4.0)
        server;
      let success =
        Server_controller.wait_for_server_exit
...
let kill_and_maybe_restart_server (env : env msg_update) exit_status :
  ...
  if Server_controller.is_saved_state_precomputed env.server_start_options then begin
    let reason =
...
let read_version_string_then_newline (fd : Unix.file_descr) :
    (Monitor_utils.VersionPayload.serialized, string) result =
  (* WARNING! Don't use the (slow) HackEventLogger here, in the inner loop non-failure path. *)
...
let ack_and_handoff_client (env : env msg_update) client_fd : env msg_update =
  ...
  >>= Monitor_utils.VersionPayload.deserialize
  |> Result.map_error ~f:(fun _err -> (env, "Malformed Build ID"))
  >>= fun {
            Monitor_utils.VersionPayload.client_version;
            tracker_id;
  ...
  if is_version_ok then begin
    ...
    Hh_logger.log
      "[%s] sending %s"
      tracker_id
      (Monitor_utils.show_connection_state connection_state);
    ...
    let connection_state =
      Build_id_mismatch_v3
        ( Monitor_utils.current_build_info,
          Monitor_utils.MismatchPayload.serialize ~monitor_will_terminate )
    in
    Hh_logger.log
      "[%s] sending %s"
      tracker_id
...
let update_status_ (env : env msg_update) monitor_config :
  ...
  let env =
    ...
    match env.server with
    | Alive process ->
      let (pid, proc_stat) = Server_controller.wait_pid process in
  ...
  begin
    ...
    | Some reason ->
      ...
      let telemetry =
        ...
        |> Telemetry.string_
             ~key:"server"
             ~value:(Server_process.show_server_process env.server)
        |> Telemetry.string_
             ~key:"new_server"
...
let rec check_and_run_loop
  ...
  let (env, consecutive_throws) =
    ...
    | Informant_notifier.Informant_edenfs_watcher_failed msg ->
...
let start_monitor
  ...
  Option.iter waiting_client ~f:(fun fd ->
      ...
      try
        Out_channel.output_string oc (Monitor_utils.ready ^ "\n");
