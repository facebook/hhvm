...
//!     bincode::serde::encode_to_vec(&WithIntern(v), bincode::config::standard())
//! }
//!
//! pub fn deserialize(encoded: &[u8]) -> Result<Vec<MyId>, bincode::error::DecodeError> {
//!     WithIntern::strip(
//!         bincode::serde::decode_from_slice(encoded, bincode::config::standard()).map(|(v, _)| v),
//!     )
//! }
//! ```
