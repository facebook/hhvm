...
module Ids = SharedMem.NoCache (SharedMem.Immediate) (IntKey) (struct
  type t = int array
...
let () =
  ...
  let handle = SharedMem.init ~num_workers { SharedMem.
    global_size = 0;
    heap_size = 10 * 1024 * 1024; (* 10 MiB *)
    dep_table_pow = 0;
    hash_table_pow = 14; (* 256 KiB *)
    shm_dirs = [];
    shm_min_avail = 0;
    log_level = 0;
    sample_rate = 0.0;
  ...
  let next () =
    let i = !job_id in
    if i < num_jobs
    then (incr job_id; Hh_bucket.Job i)
    else Hh_bucket.Done in
  let merge () () = () in
