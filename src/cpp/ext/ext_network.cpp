...
Variant f_inet_ntop(CStrRef in_addr) {
  ...
  if (in_addr.size() == 16) {
    ...
    Logger::Warning("Invalid in_addr value");
    return false;
  }
...
Variant f_inet_pton(CStrRef address) {
  ...
  if (strchr(saddress, ':')) {
    ...
    Logger::Warning("Unrecognized address %s", saddress);
    return false;
  }
...
bool f_dns_check_record(CStrRef host, CStrRef type /* = null_string */) {
  ...
  if (host.empty()) {
    throw ArgumentMissingException("host");
  }
  ...
  if (!strcasecmp("A", stype)) ntype = DNS_T_A;
    ...
    throw InvalidArgumentException("type", stype);
  }
...
Variant f_dns_get_record(CStrRef hostname, int type /* = -1 */,
  ...
  if (type & ~PHP_DNS_ALL && type != PHP_DNS_ANY) {
    Logger::Warning("Type '%d' not supported", type);
    return false;
  }
  ...
  for (int t = (type == PHP_DNS_ANY ? (PHP_DNS_NUM_TYPES + 1) : 0);
    ...
    if (n < 0) {
      Logger::Warning("res_nmkquery() failed");
      res_nclose(&res);
    ...
    if (n < 0) {
      Logger::Warning("res_nsend() failed");
      res_nclose(&res);
      php_dns_free_res(res);
      return false;
    }
    ...
    while (qd-- > 0) {
      ...
      if (n < 0) {
        Logger::Warning("Unable to parse DNS data received");
        res_nclose(&res);
        php_dns_free_res(res);
        return false;
      }
