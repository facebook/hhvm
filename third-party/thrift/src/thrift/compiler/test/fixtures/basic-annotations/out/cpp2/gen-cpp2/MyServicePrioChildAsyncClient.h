...
class Client<::cpp2::MyServicePrioChild> : public ::cpp2::MyServicePrioParentAsyncClient {
 ...
 private:
  ...
  folly::coro::Task<void> co_pang(apache::thrift::RpcOptions* rpcOptions) {
    ...
    if constexpr (hasRpcOptions) {
      fbthrift_serialize_and_send_pang(*rpcOptions, std::move(header), ctx.get(), std::move(wrappedCallback));
    } else {
      fbthrift_serialize_and_send_pang(*defaultRpcOptions, std::move(header), ctx.get(), std::move(wrappedCallback));
    }
    if (cancellable) {
 ...
 private:
  ...
  FOLLY_NOINLINE folly::SemiFuture<folly::Unit> fbthrift_semifuture_pang(folly::SemiFuture<typename apache::thrift::detail::FutureCallbackHelper<folly::Unit>::PromiseResult> semifuture, apache::thrift::RpcOptions& rpcOptions, apache::thrift::RequestClientCallback::Ptr callback, apache::thrift::ContextStack* contextStack, std::shared_ptr<::apache::thrift::transport::THeader> header);
 public:
};
