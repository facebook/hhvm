...
module WithToken (Token : TokenType) = struct
  module WithSyntaxValue (SyntaxValue : SyntaxValueType) = struct
    ...
    let has_kind syntax_kind node = kind node = syntax_kind
    ...
    let is_pipe_variable_expression =
      has_kind SyntaxKind.PipeVariableExpression
    ...
    let is_postfix_unary_expression =
      has_kind SyntaxKind.PostfixUnaryExpression
    ...
    let is_halt_compiler_expression =
      has_kind SyntaxKind.HaltCompilerExpression

    let is_isset_expression = has_kind SyntaxKind.IssetExpression

    let is_function_call_expression =
      has_kind SyntaxKind.FunctionCallExpression
    ...
    let is_array_creation_expression =
      has_kind SyntaxKind.ArrayCreationExpression

    let is_array_intrinsic_expression =
      has_kind SyntaxKind.ArrayIntrinsicExpression
    ...
    let is_xhp_children_declaration =
      has_kind SyntaxKind.XHPChildrenDeclaration

    let is_xhp_children_parenthesized_list =
      has_kind SyntaxKind.XHPChildrenParenthesizedList

    let is_xhp_category_declaration =
      has_kind SyntaxKind.XHPCategoryDeclaration
    ...
    let is_vector_array_type_specifier =
      has_kind SyntaxKind.VectorArrayTypeSpecifier
    ...
    let is_classname_type_specifier =
      has_kind SyntaxKind.ClassnameTypeSpecifier
    ...
    let is_pocket_identifier_expression =
      has_kind SyntaxKind.PocketIdentifierExpression

    let is_pocket_atom_mapping_declaration =
      has_kind SyntaxKind.PocketAtomMappingDeclaration

    let is_pocket_enum_declaration = has_kind SyntaxKind.PocketEnumDeclaration

    let is_pocket_field_type_expr_declaration =
      has_kind SyntaxKind.PocketFieldTypeExprDeclaration

    let is_pocket_field_type_declaration =
      has_kind SyntaxKind.PocketFieldTypeDeclaration

    let is_pocket_mapping_id_declaration =
      has_kind SyntaxKind.PocketMappingIdDeclaration

    let is_pocket_mapping_type_declaration =
      has_kind SyntaxKind.PocketMappingTypeDeclaration
    ...
    let fold_over_children f acc syntax =
      ...
      | ExpressionStatement
        ...
        acc
      | MarkupSection
          { markup_prefix; markup_text; markup_suffix; markup_expression } ->
      ...
      | ThrowStatement { throw_keyword; throw_expression; throw_semicolon } ->
        ...
        acc
      | BreakStatement { break_keyword; break_level; break_semicolon } ->
      ...
      | TypeConstant
        ...
        acc
      | PUAccess
          { pu_access_left_type; pu_access_separator; pu_access_right_type } ->
      ...
      | ListItem { list_item; list_separator } ->
        ...
        acc
      | PocketAtomExpression { pocket_atom_glyph; pocket_atom_expression } ->
      ...
      | PocketMappingIdDeclaration
          { pocket_mapping_id_name; pocket_mapping_id_initializer } ->
    ...
    let children_from_syntax s =
      ...
      | YieldFromExpression
        ...
        [yield_from_yield_keyword; yield_from_from_keyword; yield_from_operand]
      | PrefixUnaryExpression { prefix_unary_operator; prefix_unary_operand }
        ->
      ...
      | PocketAtomExpression { pocket_atom_glyph; pocket_atom_expression } ->
        [pocket_atom_glyph; pocket_atom_expression]
      | PocketIdentifierExpression
          {
            pocket_identifier_qualifier;
            pocket_identifier_pu_operator;
    ...
    let rec to_json ?(with_value = false) node =
      Hh_json.(
        let ch =
          ...
          | SyntaxList x ->
            [("elements", JSON_Array (List.map ~f:(to_json ~with_value) x))]
          | _ ->
            let rec aux acc c n =
              match (c, n) with
              | ([], []) -> acc
    ...
    let syntax_from_children kind ts =
      ...
      | ( SyntaxKind.ReturnStatement,
          [return_keyword; return_expression; return_semicolon] ) ->
        ReturnStatement { return_keyword; return_expression; return_semicolon }
      | (SyntaxKind.GotoLabel, [goto_label_name; goto_label_colon]) ->
        GotoLabel { goto_label_name; goto_label_colon }
    ...
    module WithValueBuilder (ValueBuilder : ValueBuilderType) = struct
      ...
      let make_goto_label goto_label_name goto_label_colon =
        let syntax = GotoLabel { goto_label_name; goto_label_colon } in
        let value = ValueBuilder.value_from_syntax syntax in
        make syntax value
      ...
      let make_break_statement break_keyword break_level break_semicolon =
        let syntax =
          BreakStatement { break_keyword; break_level; break_semicolon }
        in
        let value = ValueBuilder.value_from_syntax syntax in
      ...
      let make_pocket_atom_expression pocket_atom_glyph pocket_atom_expression
          =
        let syntax =
          PocketAtomExpression { pocket_atom_glyph; pocket_atom_expression }
        in
      ...
      let get_closure_type_specifier x =
        ...
        | _ ->
          failwith "get_closure_type_specifier: not a ClosureTypeSpecifier"
    end
  end
end
