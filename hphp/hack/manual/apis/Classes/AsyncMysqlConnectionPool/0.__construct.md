...
+ ` pool_connection_limit `: The maximum number of connections allowed in
  ...
  [` AsyncMysqlClient::setPoolsConnectionLimit() `](/docs/apis/Classes/AsyncMysqlClient/setPoolsConnectionLimit/).
...
The following example shows you how to create an [` AsyncMysqlConnectionPool `](/docs/apis/Classes/AsyncMysqlConnectionPool/) with various options, and then connect to MySQL asynchronously using those pool options. The various pool options that you can construct an AsyncMysqlConnectionPool with are:
