...
let utf32_to_utf8 k =
  if k < 0x80 then
    (* buf[0] = k; *)
    B.create 1 |> set 0 k
  else if k < 0x800 then
    (* buf[0] = 0xc0 | (k >> 6);
     buf[1] = 0x80 | (k & 0x3f); *)
    B.create 2 |> set 0 (0xc0 lor (k lsr 6)) |> set 1 (0x80 lor (k land 0x3f))
  else if k < 0x10000 then
    (* buf[0] = 0xe0 | (k >> 12);
     buf[1] = 0x80 | ((k >> 6) & 0x3f);
     buf[2] = 0x80 | (k & 0x3f); *)
    B.create 3
...
let decode_entity s =
  ...
  if s.[1] = '#' then (
    ...
    try
      ...
      |> int_of_string
      |> utf32_to_utf8
    with _ -> (* malformed entity - return empty string *)
              ""
  ) else (
...
let decode s =
  Str.global_substitute
    entity_regex
    (fun m -> decode_entity (Str.matched_string m))
    s
