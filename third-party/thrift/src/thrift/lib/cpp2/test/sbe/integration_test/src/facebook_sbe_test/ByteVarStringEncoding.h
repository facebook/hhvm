...
#ifndef _FACEBOOK_SBE_TEST_BYTEVARSTRINGENCODING_CXX_H_
#define _FACEBOOK_SBE_TEST_BYTEVARSTRINGENCODING_CXX_H_
...
public:
    ...
    ByteVarStringEncoding() = default;

    ByteVarStringEncoding(
        char *buffer,
        ...
        m_actingVersion(actingVersion)
    {
        ...
        {
            throw std::runtime_error("buffer too short for flyweight [E107] in ByteVarStringEncoding");
        }
    }
    ...
    ByteVarStringEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        ByteVarStringEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    ByteVarStringEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        ByteVarStringEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    ByteVarStringEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    ByteVarStringEncoding &length(const std::uint8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, ByteVarStringEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
