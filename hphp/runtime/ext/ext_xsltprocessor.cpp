...
static void xslt_ext_function_php(xmlXPathParserContextPtr ctxt, int nargs, int type) {
  c_XSLTProcessor *intern = NULL;
  int error = 0;
...
static void xslt_ext_function_string_php(xmlXPathParserContextPtr ctxt, int nargs) {
  xslt_ext_function_php(ctxt, nargs, 1);
}

static void xslt_ext_function_object_php(xmlXPathParserContextPtr ctxt, int nargs) {
...
xmlDocPtr c_XSLTProcessor::apply_stylesheet() {
  ...
  if (m_stylesheet == NULL || m_doc == NULL) {
  ...
  xmlDocPtr res = xsltApplyStylesheetUser(m_stylesheet, m_doc, NULL, NULL, profile, ctxt);
