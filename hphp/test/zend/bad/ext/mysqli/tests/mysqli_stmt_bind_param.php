<?php
	...
	*/
	...
	if (!mysqli_stmt_prepare($stmt, "INSERT INTO test_mysqli_stmt_bind_param_table_1(id, label) VALUES (?, ?)"))
	...
	*/
	...
	if (!($res = mysqli_query($link, "SELECT id, label FROM test_mysqli_stmt_bind_param_table_1 WHERE id = " . $id)))
...
/*
	...
	*/
	...
	if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_stmt_bind_param_table_1 WHERE id = 100"))
	...
	if (!mysqli_stmt_prepare($stmt, "INSERT INTO test_mysqli_stmt_bind_param_table_1(id, label) VALUES (?, ?)"))
		...
		if (!$res = mysqli_query($link, sprintf("SELECT label FROM test_mysqli_stmt_bind_param_table_1 WHERE id = %d", $values['id'])))
			printf("[2013] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
		if (!$row = mysqli_fetch_assoc($res))
