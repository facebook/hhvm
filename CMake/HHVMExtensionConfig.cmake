...
#
# HHVM_EXTENSION_#_EXTENSION_LIBRARY: <list>
...
# of values in this list, see the documentation of the DEPENDS_UPON parameter
...
#
# [REQUIRED]
# If passed, then this extension is integral to the functioning of HHVM, and
# a FATAL_ERROR should be triggered if dependencies fail to resolve.
#
# [PRETTY_NAME string]
...
function(HHVM_DEFINE_EXTENSION extNameIn)
  ...
  foreach (arg ${ARGN})
    ...
    elseif ("x${arg}" STREQUAL "xHEADERS")
      set(argumentState 4)
    elseif ("x${arg}" STREQUAL "xEXTENSION_LIBRARY")
      set(argumentState 5)
    elseif ("x${arg}" STREQUAL "xIDL")
      set(argumentState 6)
    elseif ("x${arg}" STREQUAL "xDEPENDS_UPON")
      set(argumentState 7)
    ...
    elseif ("x${arg}" STREQUAL "xOPTIONAL")
      if (${argumentState} EQUAL 7)
        ...
        message(FATAL_ERROR "The OPTIONAL modifier is only currently valid on items in the DEPENDS_UPON section of extension '${extensionPrettyName}'!")
      endif()
  ...
  set(HHVM_EXTENSION_${extensionID}_EXTENSION_LIBRARY ${extensionLibrary} CACHE INTERNAL "" FORCE)
...
function(HHVM_EXTENSION_INTERNAL_RESOLVE_DEPENDENCIES_OF_EXTENSION resolvedDestVar extensionID)
  ...
  while (i LESS depCount)
    ...
    string(FIND ${currentDependency} "ext_" listIDX)
    if (${listIDX} EQUAL 0)
      # Extension Dependency
      string(LENGTH ${currentDependency} depLength)
      math(EXPR depLength "${depLength} - 4")
      string(SUBSTRING ${currentDependency} 4 ${depLength} extName)
      
      set(foundExt OFF)
      set(i2 0)
      while (i2 LESS HHVM_EXTENSION_COUNT)
        if (${HHVM_EXTENSION_${i2}_NAME} STREQUAL ${extName})
          set(oldStack ${HHVM_EXTENSION_RESOLUTION_STACK})
          list(APPEND HHVM_EXTENSION_RESOLUTION_STACK "${HHVM_EXTENSION_${extensionID}_NAME}")
          HHVM_EXTENSION_INTERNAL_RESOLVE_DEPENDENCIES_OF_EXTENSION(extResState ${i2})
          set(HHVM_EXTENSION_RESOLUTION_STACK ${oldStack})
          set(foundExt ON)
          break()
        endif()
        math(EXPR i2 "${i2} + 1")
      ...
      if (NOT foundExt)
        message(FATAL_ERROR "Unable to find the extension '${extName}' while resolving the dependencies of '${HHVM_EXTENSION_${extensionID}_PRETTY_NAME}'!")
      endif()
      
      if (NOT ${extResState} EQUAL 1)
