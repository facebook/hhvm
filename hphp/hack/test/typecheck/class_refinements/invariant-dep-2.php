...
abstract class Outer<TBox as ArraykeyBox> {
  ...
  public abstract function boxGet<T as arraykey>(Invariant<TBox> $inv_box): T
  where
...
class ConcreteOuterGood extends Outer<self::TBox> {
  ...
  <<__Override>>
  public function boxGet(Invariant<self::TBox> $inv_box): int
...
class ConcreteOuterBad extends Outer<self::TBox> {
  ...
  <<__Override>>
  public function boxGet<T2 as int>(Invariant<self::TBox> $inv_box): T2 {
    return $inv_box->unwrap()->get();
