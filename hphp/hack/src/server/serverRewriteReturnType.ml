...
module Collector = struct
  ...
  let type_of_type_hint env (ty, _) =
    Pos.none, Tast_expand.expand_ty env ty
  (* reduce visitor used to traverse a tast and collect any inferred hint
    annotation. *)
  let declaration_visitor = object
    inherit [_] Tast_visitor.reduce
    method zero = (0, [])
    method plus (len1, list1) (len2, list2) =
      if len1 < len2 then len1 + len2, list1 @ list2
      else len1 + len2, list2 @ list1

    method! on_fun_ (env : Tast_env.env) fun_ =
      let ty = snd @@ type_of_type_hint (Tast_env.restore_fun_env env fun_) fun_.Aast.f_ret in
      1, [fun_.Aast.f_span, (Tast_env.tast_env_as_typing_env env, ty)]

    method! on_method_ (env : Tast_env.env) method_ =
      let ty = snd @@ type_of_type_hint (Tast_env.restore_method_env env method_) method_.Aast.m_ret in
  ...
  let compare_nonoverlap x y =
    ...
    if r <> 0 then r
      ...
      if xend < ystart then xend - ystart
      else if yend < xstart then xstart - yend
      else 0
  ...
  let get (collector : t) file node : string option =
    ...
    | Some pos ->
      Array.binary_search
        ...
        collector `First_equal_to pos
      >>| Array.get collector
      >>| snd
      >>= fun (env, ty) ->
        match print_ty ty with
        | None -> begin
          Hh_logger.log "[infer return]: type %s non denotable in %s at position %s"
...
let get_patches tcopt file =
  ...
  let get_patches node =
    ...
    | FunctionDeclarationHeader ({function_type; _} as fdh)
      ...
      in Option.to_list patch
      end
    | _ -> [] in
  let patches, _ = Rewriter.aggregating_rewrite_post
     (fun node patches ->
       let a = get_patches node @ patches in a, Rewriter.Result.Keep)
     root [] in
  patches
