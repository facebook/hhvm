
#
# Autogenerated by Thrift
#
# DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
#  @generated
#



cdef shared_ptr[_fbthrift_ctypes.cMyData] MyData_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.MyData?>inst)._cpp_obj


cdef object MyData_from_cpp(const shared_ptr[_fbthrift_ctypes.cMyData]& c_struct):
    return _fbthrift_ctypes.MyData._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cMyDataWithCustomDefault] MyDataWithCustomDefault_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.MyDataWithCustomDefault?>inst)._cpp_obj


cdef object MyDataWithCustomDefault_from_cpp(const shared_ptr[_fbthrift_ctypes.cMyDataWithCustomDefault]& c_struct):
    return _fbthrift_ctypes.MyDataWithCustomDefault._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cInnerUnion] InnerUnion_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.InnerUnion?>inst)._cpp_obj


cdef object InnerUnion_from_cpp(const shared_ptr[_fbthrift_ctypes.cInnerUnion]& c_struct):
    return _fbthrift_ctypes.InnerUnion._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cMyUnion] MyUnion_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.MyUnion?>inst)._cpp_obj


cdef object MyUnion_from_cpp(const shared_ptr[_fbthrift_ctypes.cMyUnion]& c_struct):
    return _fbthrift_ctypes.MyUnion._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cMyStruct] MyStruct_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.MyStruct?>inst)._cpp_obj


cdef object MyStruct_from_cpp(const shared_ptr[_fbthrift_ctypes.cMyStruct]& c_struct):
    return _fbthrift_ctypes.MyStruct._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cLateDefStruct] LateDefStruct_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.LateDefStruct?>inst)._cpp_obj


cdef object LateDefStruct_from_cpp(const shared_ptr[_fbthrift_ctypes.cLateDefStruct]& c_struct):
    return _fbthrift_ctypes.LateDefStruct._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cRecursive] Recursive_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.Recursive?>inst)._cpp_obj


cdef object Recursive_from_cpp(const shared_ptr[_fbthrift_ctypes.cRecursive]& c_struct):
    return _fbthrift_ctypes.Recursive._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cBar] Bar_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.Bar?>inst)._cpp_obj


cdef object Bar_from_cpp(const shared_ptr[_fbthrift_ctypes.cBar]& c_struct):
    return _fbthrift_ctypes.Bar._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cLoop] Loop_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.Loop?>inst)._cpp_obj


cdef object Loop_from_cpp(const shared_ptr[_fbthrift_ctypes.cLoop]& c_struct):
    return _fbthrift_ctypes.Loop._fbthrift_create(c_struct)
cdef shared_ptr[_fbthrift_ctypes.cRefFields] RefFields_convert_to_cpp(object inst):
    return (<_fbthrift_ctypes.RefFields?>inst)._cpp_obj


cdef object RefFields_from_cpp(const shared_ptr[_fbthrift_ctypes.cRefFields]& c_struct):
    return _fbthrift_ctypes.RefFields._fbthrift_create(c_struct)
