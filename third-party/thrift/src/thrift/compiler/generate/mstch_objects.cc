...
mstch::node mstch_type::get_sink_elem_type() {
  if (type_->is_sink()) {
    return context_.type_factory->make_mstch_object(
        dynamic_cast<const t_sink*>(type_)->get_sink_type(), context_, pos_);
  }
  return mstch::node();
}
  ...
  for (unsigned char c : str) {
    switch (c) {
      ...
      default:
        ...
        // languages. Hexadecimal ones have a problem of consuming all hex
        // digits after \x in C++, e.g. \xcafefe is a single escape sequence.
