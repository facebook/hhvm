...
final class Map<Tk, Tv> implements MutableMap<Tk, Tv> {
   ...
   *              a Map (e.g., array). If null, then an empty
   *              Map is created.
   */
   ...
   *
   * @return - an array containing the key and value pairs from the current Map.
   */
   ...
   *
   * @return - an integer-indexed array containing the values from the current
   *           Map.
   */
  ...
  /**
   * Returns an immutable Set (ImmSet) based on the values of the current Map.
   *
   ...
   *
   * @return - a KeyedIterable representing the lazy view into
   *           the current Map.
   *
   ...
   * mapWithKey(), unlike filterWithKey() where only values that meet a certain
   * criteria are affected.
   *
   * The keys will remain unchanged from this Map to the returned Map. The keys
   * are only used to help in the mapping operation.
   *
   *
   * @param $callback - The $callback containing the operation to apply to the
   ...
   *
   * The keys associated with the current Map remain unchanged in the returned
   * Map; the keys will be used in the filtering process only.
   *
   *
   * @param $callback - The $callback containing the condition to apply to the
   *                    current Map keys and values.
   ...
   *
   *  If the number of values of the current Map are not equal to the number of
   ...
   *
   *
   *  @param $traversable - The traversable to use to combine with the elements
   *                        of the current Map.
   *
   *  @return - The Map that combines the values of the current Map with the
   *            provided Traversable.
   */
  ...
  /**
   * Returns a Map containing the keys and values of the current Map up to but
   ...
   *
   * @return - A Map that is a proper subset of the current Map up until the
   *           callback returns false.
   */
  public function takeWhile((function(Tv): bool) $fn): Map<Tk, Tv>;

  /**
   * Returns a Map containing the values after the n-th element of the current
   * Map.
   *
   * The returned Map will always be a proper subset of the current Map.
   *
  ...
  /**
   * Returns a Map containing the values of the current Map starting after and
   * including the first value that produces true when passed to the specified
   * callback.
   *
   ...
   *              current Map.
   *
   * @return - A Map that is a proper subset of the current Map starting after
   *           the callback returns true.
   */
  public function skipWhile((function(Tv): bool) $fn): Map<Tk, Tv>;

  /**
   * Returns a subset of the current Map starting from a given key location up
   ...
   *
   * The provided Traversable is concatenated to the end of the current Map to
   * produce the returned Vector.
   *
   * @param $traversable - The Traversable to concat to the current Map.
   *
   ...
   *
   * @return - The first value in the current Map,  or null if the Map is empty.
   */
   ...
   *
   * @return - The first key in the current Map, or null if the Map is empty.
   */
   ...
   *
   * @return - The last value in the current Map, or null if the Map is empty.
   */
   ...
   *
   * @return - The last key in the current Map, or null if the Map is empty.
   */
  ...
  /**
   * Stores a value into the current Map with the specified key, overwriting the
   * previous value associated with the key.
   *
   * If the key is not present, an exception is thrown. If you want to add
   * a value even if a key is not present, use add().
   *
   * "$map->set($k,$v)" is semantically equivalent to "$map[$k] = $v"
   * (except that set() returns the current Map).
   *
   * Future changes made to the current Map ARE reflected in the returned, and
   * vice-versa.
   *
   * @param $k - The key to which we will set the value.
   * @param $v - The value to set.
   *
   * @return - A shallow copy of the current Map with the updated the value set.
   *           The current Map is also updated.
   */
   ...
   *
   * If a key is not present this Map that is present in the Traversable, an
   * exception is thrown. If you want to add a value even if a key is not
   * present, use addAll().
   *
   * Future changes made to the current Map ARE reflected in the
   ...
   * returned Map, and vice-versa.
   *
   * @return - A shallow copy of the current empty Map. The current Map is also
   *           empty.
   */
   ...
   *
   * @return - true if the specified key is present in the current Map; returns
   *           false otherwise.
   *
   * @guide /hack/generics/constraints
   */
   ...
   *
   * @return - A shallow copy of the current Map with the added key/value pair
   *           set. This Map is also updated.
   */
   ...
   * returned Map, and vice-versa.
   *
   * @param $k - The traversable with the new key/value Pair to set. If null is
   *             provided, no changes are made.
   *
   * @return - A shallow copy of the current Map with the added key/value pair
   *           set. This Map is also updated.
  ...
  /**
   * Returns a new Map with the keys that are in the current Map, but not in
   * the provided KeyedTraversable.
   *
   * @param $traversable - The KeyedTraversable on which to compare the keys.
   *
   * @return - A Map containing the keys (and associated values) of the current
   *           Map that are not in the KeyedTraversable.
   */
  ...
  /**
   * Creates a Map from the given Traversable, or an empty Vector
   * if "null" is passed.
   *
   ...
   *
   * @return - A Map with the key/value pairs from the Traversable; or an empty
   *           Map if the Traversable is null.
   */
   ...
   *
   * The Iterable returned is one that produces the key/values from the current
   * Map.
   *
