...
Returns a subset of the current [` ImmVector `](/docs/apis/Classes/HH/ImmVector/) starting from a given key up
...
` $start ` is 0-based. `` $len `` is 1-based. So [` slice(0, `](/docs/apis/Classes/HH/ImmVector/slice/)`` 2) `` would return the
...
proper subset) of the current [` ImmVector `](/docs/apis/Classes/HH/ImmVector/). If `` $start `` is greater than or
...
+ ` int $start ` - The starting key of the current [` ImmVector `](/docs/apis/Classes/HH/ImmVector/) at which to
  begin the returned [` ImmVector `](/docs/apis/Classes/HH/ImmVector/).
...
See [` Vector::slice `](/docs/apis/Classes/HH/Vector/slice/#examples) for usage examples.
<!-- HHAPIDOC -->
