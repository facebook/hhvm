...
let log_file_info_change
  ...
  let list_symbols_in_file_info file_info =
    ...
    | Some file_info ->
      Printf.sprintf "funs: %s, classes: %s, typedefs: %s, consts: %s"
  ...
  let verb =
    ...
    | (None, None) ->
      ...
      *)
      "spuriously updated"
  in
  log "File changed (%.3fs) %s %s: old: %s vs. new: %s"
    (end_time -. start_time)
...
let compute_fileinfo_for_path
  ...
  let new_file_info =
    ...
    | Some contents ->
      ...
      let facts = Facts_parser.from_text
        ~php5_compat_mode:false
        ~hhvm_compat_mode:true
        ~filename:path
        ~text:contents in
      let (funs, classes, typedefs, consts) = match facts with
        | None ->
          (* File failed to parse or was not a Hack file. *)
          ([], [], [], [])
        | Some facts ->
          let to_ids name_type names = List.map names ~f:(fun name ->
          ...
          let classes = facts.Facts.types
            ...
            |> Facts.InvSMap.keys
            |> to_ids FileInfo.Class in
          let typedefs = facts.Facts.types
            |> Facts.InvSMap.filter (fun _k v ->
              let open Facts in
              match v.kind with
              | TKTypeAlias ->
...
let process_changed_file
  ...
  | Some path ->
    ...
    if not (FindUtils.path_filter path)
    then Lwt.return env
    else
