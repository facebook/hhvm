...
let do_rpc
  ...
  try%lwt
    let success =
      Lwt_message_queue.push messages_to_send (Message_wrapper message) in
    (if not success
    then failwith "Could not send message (queue was closed)");

    let%lwt (response: response_wrapper option) =
      Lwt_message_queue.pop response_emitter in
    match response with
    | None ->
...
let initialize_from_saved_state
  ...
  let param = { ClientIdeMessage.Initialize_from_saved_state.
    root;
  ...
  let%lwt (_: int) = Marshal_tools_lwt.to_fd_with_preamble
    t.out_fd (ClientIdeMessage.Initialize_from_saved_state param) in
  let%lwt (response: ClientIdeMessage.message_from_daemon) =
    Marshal_tools_lwt.from_fd_with_preamble t.in_fd in

  match response with
  | ClientIdeMessage.Response (
      Ok _ (* expected to be `Ok ()`, but not statically-checkable *)
    ) ->
    log "Initialized IDE service process (log file at %s)"
  ...
  | ClientIdeMessage.Response (Error error_message) ->
    log "Failed to initialize IDE service process: %s"
      error_message;
    set_state t (Failed_to_initialize error_message);
    Lwt.return_error error_message
...
let rec serve (t: t): unit Lwt.t =
  let send_queued_up_messages ~out_fd messages_to_send: bool Lwt.t =
    ...
    | Some (Message_wrapper next_message) ->
      let%lwt (_: int) =
        Marshal_tools_lwt.to_fd_with_preamble out_fd next_message in
      Lwt.return true
  in

  let emit_messages_from_daemon
      ~in_fd
      response_emitter
    ...
    | ClientIdeMessage.Response response ->
      Lwt.return (Lwt_message_queue.push
        response_emitter (Response_wrapper response))
  in

  let%lwt should_continue =
    try%lwt
      (* We mutate the queues in `t`, which is why we don't return a new `t` here. *)
      let%lwt should_continue = Lwt.pick [
        send_queued_up_messages
          ~out_fd:t.out_fd
          t.messages_to_send;
...
let get_notifications (t: t): notification_emitter =
  t.notification_emitter
