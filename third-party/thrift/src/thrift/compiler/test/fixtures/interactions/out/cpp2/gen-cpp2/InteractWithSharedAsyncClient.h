...
class MyInteraction final : public apache::thrift::InteractionHandle {
 ...
 private:
  ...
  folly::coro::Task<::thrift::shared_interactions::DoSomethingResult> co_do_some_similar_things(apache::thrift::RpcOptions* rpcOptions) {
    ...
    if constexpr (hasRpcOptions) {
      fbthrift_serialize_and_send_do_some_similar_things(*rpcOptions, std::move(header), ctx.get(), std::move(wrappedCallback));
    } else {
      fbthrift_serialize_and_send_do_some_similar_things(*defaultRpcOptions, std::move(header), ctx.get(), std::move(wrappedCallback));
    }
    if (cancellable) {
      folly::CancellationCallback cb(cancelToken, [&] { CancellableCallback::cancel(std::move(cancellableCallback)); });
      co_await callback.co_waitUntilDone();
 ...
 private:
  template <typename Protocol_>
  apache::thrift::SerializedRequest fbthrift_serialize_do_some_similar_things(Protocol_* prot, const RpcOptions& rpcOptions, apache::thrift::transport::THeader& header, apache::thrift::ContextStack* contextStack);
  template <typename RpcOptions>
