<?php
	$test_table_name = 'test_mysqli_fetch_array_many_rows_table_1'; require("table.inc");

	// do as much as we can do in 5 seconds
	$start = microtime(true);
	for ($id = 100, $start = microtime(true); (microtime(true) - $start) < 5; $id++) {
		if (!mysqli_query($link, $sql = sprintf("INSERT INTO test_mysqli_fetch_array_many_rows_table_1(id, label) VALUES (%d, '%s')",
	...
	if (!$res = mysqli_query($link, 'SELECT id, label FROM test_mysqli_fetch_array_many_rows_table_1')) {
	...
	if (!$res = mysqli_query($link, 'SELECT count(*) AS num FROM test_mysqli_fetch_array_many_rows_table_1')) {
...
?>
<?php
	$test_table_name = 'test_mysqli_fetch_array_many_rows_table_1'; require_once("clean_table.inc");
?>