...
let expect_ty_equal env (pos, ty:Pos.t * locl ty) (expected_ty:locl ty_) =
  ...
  if not @@ ty_equal ~normalize_union:true ty expected_ty then
...
let refine ((_p, (_r, cond_ty)), cond_expr) _cond_is_true gamma =
  let is_refinement_fun fun_name =
    ...
    | "\\is_array" | "\\is_vec" | "\\is_dict" | "\\is_keyset"
    | "\\is_resource" -> true
    | _ -> false in
  ...
  | Call (
      _,
      (_, Class_const ((_, CI (_, "\\Shapes")), (_, "keyExists"))),
      _,
      _,
      _) ->
    raise Not_implemented
...
let check_expr env (expr:ETast.expr) (gamma:gamma) : gamma =
  ...
  let expr_checker = object (self)
    ...
    method! on_expr env (ty, expr as texpr) =
      ...
      | Int _ ->
        expect_ty_equal ty (Tprim Nast.Tint)
      | Float _ ->
        expect_ty_equal ty (Tprim Nast.Tfloat)
      | String _ ->
...
and check_block env (block:ETast.block) gamma =
  let rec go block gamma delta =
    ...
    | stmt :: block ->
      ...
      | None -> delta
      | Some gamma -> go block gamma delta in
...
let check_fun env (f:ETast.fun_def) =
  ...
  then raise Not_implemented else
...
let check_def env (def:ETast.def) =
  ...
  | Not_implemented ->
    Printf.printf "The TAST for this definition contains nodes which are not yet implemented.\n"
