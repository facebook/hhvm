...
let on_stmt on_error =
  let handler
        : 'a 'b.
          _ * (_ * ('a, 'b) Aast_defs.stmt_) ->
          ( _ * (_ * ('a, 'b) Aast_defs.stmt_),
            _ * (_ * ('a, 'b) Aast_defs.stmt_) )
          result =
   fun (env, stmt) ->
      ...
      | ( pos,
        ...
        | (pk, (cond_annot, cond_pos, cond)) :: exprs ->
          ...
          let violation =
            ( annot,
              expr_pos,
              Aast.Call (fn_expr, targs, exprs, unpacked_element) )
          in
          (match cond with
          | Aast.False ->
...
let pass on_error =
  Naming_phase_pass.(
    bottom_up Ast_transform.{ identity with on_stmt = Some (on_stmt on_error) })
