...
Variant MethodStatement::invokeInstance(CObjRef obj,
...
invokeInstanceDirect(CObjRef obj, VariableEnvironment &env,
  ...
  attemptAccess(env.currentContext());
...
Variant MethodStatement::invokeStatic(const char* cls, CArrRef params) const {
  MethScopeVariableEnvironment env(this, params.size());
...
void MethodStatement::getInfo(ClassInfo::MethodInfo &info) const {
  ...
  if (!(attr & ClassStatement::Protected || attr & ClassStatement::Private))
