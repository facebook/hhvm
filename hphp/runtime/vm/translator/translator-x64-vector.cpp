...
#define PREP_RESULT(useTvR)                                                   \
  if (!useTvR) {                                                              \
    ...
    a.  lea_reg64_disp_reg64(rVmSp, vstackOffset(ni, mResultStackOffset(ni)), \
                             r(rAsm));                                    \
    emitStoreUninitNull(a, 0, r(rAsm));                                   \
  }
...
void TranslatorX64::emitPropSpecialized(MInstrAttr const mia,
  ...
  a.    lea_reg64_disp_reg64(r(rBase), propOffset, r(rAsm));
  if (doWarn || doDefine) {
    emitCmpTVType(a, KindOfUninit, r(rAsm)[TVOFF(m_type)]);
    {
      ...
      if (doDefine) {
        emitStoreTVType(astubs, KindOfNull, r(rAsm)[TVOFF(m_type)]);
      } else {
        emitImmReg(astubs, uintptr_t(&init_null_variant), r(rAsm));
      }
    }
  }
          ...
          nLogicalRatchets(t, ni, mii));
  {
    ...
    // Copy tvRef to tvRef2.
    {
      ...
      for (size_t off = 0; off < sizeof(TypedValue); off += 8) {
        a.load_reg64_disp_reg64(mis_rsp, MISOFF(tvRef) + off, r(rAsm));
        a.store_reg64_disp_reg64(r(rAsm), MISOFF(tvRef2) + off, mis_rsp);
      }
    }
    // Reset tvRef.
...
void TranslatorX64::emitMPost(const Tracelet& t,
  ...
  if (forceMValIncDec(t, ni, mii)) {
    ...
    a.  load_reg64_disp_reg64(mis_rsp, MISOFF(tvVal) + TVOFF(m_data),
                              r(rAsm));
    emitDecRef(ni, r(rAsm), KindOfArray);
  }
  // Clean up ratchet.
