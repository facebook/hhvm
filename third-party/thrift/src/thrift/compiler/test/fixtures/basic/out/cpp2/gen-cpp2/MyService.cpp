...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_ping(apache::thrift::HandlerCallback<void>::Ptr callback) {
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_getRandomData(apache::thrift::HandlerCallback<std::unique_ptr<::std::string>>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_sink(apache::thrift::HandlerCallback<void>::Ptr callback, ::std::int64_t p_sink) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_putDataById(apache::thrift::HandlerCallback<void>::Ptr callback, ::std::int64_t p_id, std::unique_ptr<::std::string> p_data) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_hasDataById(apache::thrift::HandlerCallback<bool>::Ptr callback, ::std::int64_t p_id) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_getDataById(apache::thrift::HandlerCallback<std::unique_ptr<::std::string>>::Ptr callback, ::std::int64_t p_id) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_deleteDataById(apache::thrift::HandlerCallback<void>::Ptr callback, ::std::int64_t p_id) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_lobDataById(apache::thrift::HandlerCallbackBase::Ptr callback, ::std::int64_t p_id, std::unique_ptr<::std::string> p_data) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_invalid_return_for_hack(apache::thrift::HandlerCallback<std::unique_ptr<::std::set<float>>>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::test::fixtures::basic::MyService>::async_tm_rpc_skipped_codegen(apache::thrift::HandlerCallback<void>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
