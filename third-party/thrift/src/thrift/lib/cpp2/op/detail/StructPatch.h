...
void* intentionally_unimplemented_function();
...
void* typeErasedPatchImpl(Patch& patch) {
  ...
  if constexpr (folly::to_underlying(Ord) <= size_v<FieldPatchType>) {
    ...
    // This might happen when this function is explicitly instantiated, but it
    // would never happen during link-time since the caller will already
    // validate that the field is patchable.
