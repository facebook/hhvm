...
type refresh_env = {
  ...
  tvars: Errors.error_from_reasons_callback IMap.t;
...
let rec eliminate ~ty_orig ~rtv_pos ~name ~ubs ~lbs renv v =
  ...
  | Ast_defs.Invariant ->
    ...
    renv.on_error [(rtv_pos, "Rigid type variable " ^ name ^ " is escaping")];
...
let refresh_env_and_type ~remove:(types, remove) ~pos env ty =
  if List.is_empty types then
    ...
    Typing_log.log_escape
      ...
      types;
    let what = "lambda" in
    let on_error = Errors.rigid_tvar_escape_at pos what in
    let renv =
      {
        env;
        tvars = IMap.empty;
