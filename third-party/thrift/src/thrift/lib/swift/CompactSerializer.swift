...
/// Serializes/deserializes Thrift structs to/from Compact-protocol bytes.
/// Mirrors the per-protocol serializer convention of other Thrift languages
/// (e.g. C++ `CompactSerializer`, Rust `compact_protocol`).
public enum CompactSerializer {
  /// Serializes `value` to compact-protocol bytes.
  ...
  /// Deserializes a `T` from compact-protocol bytes. Throws if the input has
  ...
  public static func deserialize<T: ThriftSerializable>(
    ...
    let reader = CompactReader(data: data)
