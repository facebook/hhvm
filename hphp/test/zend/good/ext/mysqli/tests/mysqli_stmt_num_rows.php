<?php
	...
	function func_test_mysqli_stmt_num_rows_table_1_mysqli_stmt_num_rows($stmt, $query, $expected, $offset) {
	...
	func_test_mysqli_stmt_num_rows_table_1_mysqli_stmt_num_rows($stmt, "SELECT 1 AS a", 1, 10);
	func_test_mysqli_stmt_num_rows_table_1_mysqli_stmt_num_rows($stmt, "SHOW VARIABLES LIKE '%nixnutz%'", 0, 20);
	...
	// users do.
	func_test_mysqli_stmt_num_rows_table_1_mysqli_stmt_num_rows($stmt, "INSERT INTO test_mysqli_stmt_num_rows_table_1(id, label) VALUES (100, 'z')", 0, 30);

	if ($res = mysqli_query($link, 'SELECT COUNT(id) AS num FROM test_mysqli_stmt_num_rows_table_1')) {
		$row = mysqli_fetch_assoc($res);
		mysqli_free_result($res);
		func_test_mysqli_stmt_num_rows_table_1_mysqli_stmt_num_rows($stmt, "SELECT id, label FROM test_mysqli_stmt_num_rows_table_1", (int)$row['num'], 40);
	} else {
		printf("[050] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}
	...
	if (!mysqli_stmt_prepare($stmt, 'SELECT id FROM test_mysqli_stmt_num_rows_table_1'))
	...
	if (mysqli_stmt_execute($stmt)) {
		...
		any behaviour they like, including the one checked for in this test_mysqli_stmt_num_rows_table_1.
...
<?php
	$test_table_name = 'test_mysqli_stmt_num_rows_table_1'; require_once("clean_table.inc");
?>