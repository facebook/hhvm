...
StructMetadata<::cpp2::ReflectionStruct>::gen(ThriftMetadata& metadata) {
  if (FLAGS_thrift_enable_schema_to_metadata_conversion) {
    return genStructMetadata<::cpp2::ReflectionStruct>(metadata, kGenerateAll).metadata;
  }

  auto res = genStructMetadata<::cpp2::ReflectionStruct>(metadata, {.genAnnotations = folly::kIsDebug});
  if (res.preExists) {
    return res.metadata;
  }
  ::apache::thrift::metadata::ThriftStruct& reflection_ReflectionStruct = res.metadata;
  DCHECK_EQ(*reflection_ReflectionStruct.is_union(), false);
  ...
  for (const auto& f : *reflection_ReflectionStruct_fields) {
    genStructFieldMetadata<::cpp2::ReflectionStruct>(reflection_ReflectionStruct.fields()[i], f, i);
    ...
    reflection_ReflectionStruct.fields()[i++].type() = std::move(type);
  }
  ...
  DCHECK(structuredAnnotationsEquality(
    *res.metadata.structured_annotations(),
    newAnnotations,
    getAnnotationTypes<::cpp2::ReflectionStruct>()
  ));
  return res.metadata;
}
