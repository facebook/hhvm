  ...
  for (const auto& f : *module_CustomException_fields) {
    ...
    DCHECK(structuredAnnotationsEquality(
      *field.structured_annotations(),
      newAnnotations,
      getFieldAnnotationTypes<::cpp2::CustomException>(i, static_cast<std::int16_t>(f.id))
    ));
...
StructMetadata<::cpp2::ShouldBeBoxed>::gen(ThriftMetadata& metadata) {
  ...
  for (const auto& f : *module_ShouldBeBoxed_fields) {
    [[maybe_unused]] auto& field = module_ShouldBeBoxed.fields()[i];
    DCHECK_EQ(*field.id(), f.id);
    DCHECK_EQ(*field.name(), f.name);
    DCHECK_EQ(*field.is_optional(), f.is_optional);
    ...
    DCHECK(structuredAnnotationsEquality(
      *field.structured_annotations(),
      newAnnotations,
      getFieldAnnotationTypes<::cpp2::ShouldBeBoxed>(i, static_cast<std::int16_t>(f.id))
    ));
