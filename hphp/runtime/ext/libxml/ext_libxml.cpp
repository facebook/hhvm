...
class LibXMLExtension final : public Extension {
  ...
  private:
    ...
    void moduleInit() override {
      HHVM_RC_SAME_I(LIBXML_VERSION);
      HHVM_RC_SAME_S(LIBXML_DOTTED_VERSION);
      HHVM_RC_S(LIBXML_LOADED_VERSION, xmlParserVersion);

      // For use with loading xml
      HHVM_RC_I(LIBXML_NOENT, XML_PARSE_NOENT);
      HHVM_RC_I(LIBXML_DTDLOAD, XML_PARSE_DTDLOAD);
      HHVM_RC_I(LIBXML_DTDATTR, XML_PARSE_DTDATTR);
      HHVM_RC_I(LIBXML_DTDVALID, XML_PARSE_DTDVALID);
      HHVM_RC_I(LIBXML_NOERROR, XML_PARSE_NOERROR);
      HHVM_RC_I(LIBXML_NOWARNING, XML_PARSE_NOWARNING);
      HHVM_RC_I(LIBXML_NOBLANKS, XML_PARSE_NOBLANKS);
      HHVM_RC_I(LIBXML_XINCLUDE, XML_PARSE_XINCLUDE);
      HHVM_RC_I(LIBXML_NSCLEAN, XML_PARSE_NSCLEAN);
      HHVM_RC_I(LIBXML_NOCDATA, XML_PARSE_NOCDATA);
      HHVM_RC_I(LIBXML_NONET, XML_PARSE_NONET);
      HHVM_RC_I(LIBXML_PEDANTIC, XML_PARSE_PEDANTIC);
      HHVM_RC_I(LIBXML_COMPACT, XML_PARSE_COMPACT);
      HHVM_RC_I(LIBXML_NOXMLDECL, XML_SAVE_NO_DECL);
      HHVM_RC_I(LIBXML_PARSEHUGGE, XML_PARSE_HUGE);
      HHVM_RC_I(LIBXML_NOEMPTYTAG, LIBXML_SAVE_NOEMPTYTAG);

      // Schema validation options
#if defined(LIBXML_SCHEMAS_ENABLED)
      HHVM_RC_I(LIBXML_SCHEMA_CREATE, XML_SCHEMA_VAL_VC_I_CREATE);
#endif

      // Additional constants for use with loading html
#if LIBXML_VERSION >= 20707
      HHVM_RC_I(LIBXML_HTML_NOIMPLIED, HTML_PARSE_NOIMPLIED);
#endif

#if LIBXML_VERSION >= 20708
      HHVM_RC_I(LIBXML_HTML_NODEFDTD, HTML_PARSE_NODEFDTD);
#endif

      // Error levels
      HHVM_RC_I(LIBXML_ERR_NONE, XML_ERR_NONE);
      HHVM_RC_I(LIBXML_ERR_WARNING, XML_ERR_WARNING);
      HHVM_RC_I(LIBXML_ERR_ERROR, XML_ERR_ERROR);
      HHVM_RC_I(LIBXML_ERR_FATAL, XML_ERR_FATAL);
