...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  ...
  module WithLexer (Lexer : Lexer_S) = struct
    module type Parser_S =
      ParserSig.WithSyntax(Syntax).WithLexer(Lexer).Parser_S

    module WithParser (Parser : Parser_S) = struct
      open Parser

      let make_list parser items = Make.list parser (pos parser) items

      module NextToken : sig
        val next_token :
          ?tokenizer:(Lexer.t -> Lexer.t * Syntax.Token.t) ->
          t ->
          t * Syntax.Token.t
        ...
        let next_token ?(tokenizer = Lexer.next_token) parser =
          ...
          let (parser, token) =
             ...
             * followed by the leading trivia. *)
                ...
                let acc =
                  List.fold_left ~f:prepend_onto ~init:acc (Token.trailing t)
                in
                let acc =
                  Trivia.make_extra_token_error
                    (Lexer.source lexer)
                    (Lexer.start_offset lexer)
                    (Token.width t)
                  :: acc
                in
   ...
   *)
      ...
      let error_offsets ?(on_whole_token = false) parser =
        ...
        if on_whole_token then
          ...
          let start_offset =
            Lexer.end_offset lexer + Token.leading_width token
          in
      ...
      let skip_and_log_unexpected_token ?(generate_error = true) parser =
        let parser =
          if generate_error then
            ...
            with_error
              parser
              (SyntaxError.error1057 extra_str)
              ~on_whole_token:true
          else
            parser
        in
        ...
        if String.length str1 <> String.length str2 then
          ...
          let rec off_by_one str1 str2 =
            ...
            if str_len = 0 then
              ...
              if Char.equal str1.[0] str2.[0] then
                off_by_one rest_of_str1 rest_of_str2
              (* Allow only one mistake *)
              else
                String.compare rest_of_str1 rest_of_str2 = 0
          in
      ...
      let is_misspelled_kind kind token_str =
        ...
        if String.length tokenkind_str <= 1 then
          false
        else
      ...
      let is_misspelled_from kind_list token_str =
        List.exists
          ~f:(fun kind -> is_misspelled_kind kind token_str)
          kind_list
      ...
      let suggested_kind_from kind_list token_str =
        List.find_map kind_list ~f:(fun kind ->
            if is_misspelled_kind kind token_str then
              Some kind
            else
              None)
      ...
      let skip_and_log_misspelled_token parser required_kind =
        ...
        let parser =
          with_error
            parser
            (SyntaxError.error1058 received_str required_str)
            ~on_whole_token:true
        in
      ...
      let require_and_return_token parser kind error =
        ...
        if Token.kind token = kind then
          ...
          if next_kind = kind then
            ...
            is_misspelled_kind kind (current_token_text parser)
          then
      ...
      let require_token_one_of parser kinds error =
        let (parser1, token) = next_token parser in
        if List.mem kinds (Token.kind token) ~equal:( = ) then
     ...
     helper methods to deal with them. *)
      ...
      let is_next_name parser = Lexer.is_next_name (lexer parser)
     ...
     qualified name. *)
      ...
      let require_semicolon_token parser =
        ...
        | TokenKind.QuestionGreaterThan -> (parser, None)
        | _ ->
          require_and_return_token
            parser
            TokenKind.Semicolon
      ...
      let assert_left_angle_in_type_list_with_possible_attribute parser =
        ...
        let attr1 = Lexer.peek_char lexer 1 in
        let attr2 = Lexer.peek_char lexer 2 in
        if tparam_open = '<' && attr1 = '<' && attr2 = '<' then
      ...
      let assert_xhp_body_token =
        assert_token ~tokenizer:Lexer.next_xhp_body_token
  ...
  *)
      ...
      let parse_separated_list_predicate
          parser separator_kind list_kind close_predicate error parse_item =
         ...
         a close, or, if trailing separators are allowed, both *)
          ...
          if close_predicate kind || kind = TokenKind.EndOfFile then
           ...
           omitted and there was no error. *)
            let parser =
              if kind = TokenKind.EndOfFile || list_kind <> ItemsOptional then
                with_error parser error
              else
                parser
            in
            let (parser, missing1) = Make.missing parser (pos parser) in
            let (parser, missing2) = Make.missing parser (pos parser) in
            let (parser, list_item) =
              Make.list_item parser missing1 missing2
            in
            (* TODO(T25649779) *)
          ...
          parameter list.  *)
            let parser =
              if list_kind <> ItemsOptional then
                with_error parser1 error
              else
                parser1
            in
           ...
           to parse an item followed by a close or separator. *)
            ...
            if close_predicate kind then
             ...
             that comes next is a close? *)
              if allow_trailing && close_predicate (peek_token_kind parser)
              then
                (parser, acc)
              else
                aux parser acc
        ...
        let no_arg_is_missing =
          List.for_all items (fun c -> not (SC.is_missing c))
        in
        let (parser, item_list) = make_list parser (List.rev items) in
        (parser, item_list, no_arg_is_missing)

      let parse_separated_list
          parser separator_kind list_kind close_kind error parse_item =
      ...
      let parse_list_while
          parser (parse_item : Parser.t -> Parser.t * Parser.SC.r) predicate =
        let rec aux parser acc =
          if
            peek_token_kind parser = TokenKind.EndOfFile
          ...
          then
            ...
            if SC.is_missing result then
              (parser, result :: acc)
            else
              let current_lexer = Parser.lexer parser in
              (* INFINITE LOOP PREVENTION: If parse_item does not actually make
               * progress, just bail
               *)
              if
                Lexer.start_offset lexer_before
                = Lexer.start_offset current_lexer
                && Lexer.end_offset lexer_before
                   = Lexer.end_offset current_lexer
              then
                (parser, result :: acc)
              else
                aux parser (result :: acc)
          (* Or if nothing's wrong, recurse. *)
        in
        let (parser, items) = aux parser [] in
      ...
      let parse_list_until_none parser parse_item =
        let rec aux parser acc =
          ...
          | Some item when SC.is_missing item -> (parser, item :: acc)
