...
use oxidized::typing_defs::Exact;
use oxidized::typing_defs_core::{Ty as DTy, Ty_ as DTy_};
...
pub fn obj_get<'a>(
    ...
    member_id: &'a ast::Id,
    method_explicit_type_args: &Vec<ast::Targ>,
                ...
                Some(class_info) => {
                    ...
                    match member_info {
                        ...
                        Some(member_info) => {
                            ...
                            let DTy(r, member_decl_ty) =
                                typing_enum::member_type(env, &member_info);
                            let mut ety_env =
                                mk_ety_env(env, &r, class_info, class_id, exact, class_type_args);
                            let (member_ty, method_type_args, _et_enforced) =
                                match member_decl_ty.as_ref() {
                                    DTy_::Tfun(fun_type) if is_method => {
                                        ...
                                        (
                                            bld.fun(bld.alloc(r.into()), fun_type),
                                            method_type_args,
                                            false,
                                        )
                                    }
...
fn mk_ety_env<'a>(
    env: &Env<'a>,
    _r: &Reason,
    class_info: &'a ShallowClass,
    _class_id: &Id,
    _exact: &Exact,
    type_args: &[Ty<'a>],
) -> ExpandEnv<'a> {
    ExpandEnv {
        substs: subst::make_locl(env.bld(), &class_info.tparams, type_args.iter().copied()),
