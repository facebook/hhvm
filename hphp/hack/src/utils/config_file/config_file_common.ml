...
let apply_overrides
    ...
    : string SMap.t =
  if SMap.cardinal overrides = 0 then config else
...
let parse ~silent (fn:string): string * string SMap.t =
  ...
  if not silent then Printf.eprintf "%s on-file-system contents:\n%s\n" fn contents;
...
let parse_local_config ~silent (fn: string): string SMap.t =
    try let _hash, config = parse ~silent fn in config
    with e ->
