...
module WithToken (Token : Lexable_token_sig.LexableToken_S) = struct
  ...
  let empty = { expected = []; skipped_tokens = [] }

  let skipped_tokens context = context.skipped_tokens

  let with_skipped_tokens context skipped_tokens =
    { context with skipped_tokens }

  let with_expected context expected = { context with expected }

  let expects context token_kind =
    List.exists context.expected ~f:(fun scope -> Scope.mem token_kind scope)
  ...
  let pop_scope context token_kind_list =
    ...
    | [] when List.is_empty token_kind_list -> with_expected context []
    (* Failure conditions *)
    | current :: _others ->
      let failure_str =
        Printf.sprintf
          ...
          ^^ "actually: %s" )
          (Scope.to_string (Scope.of_list token_kind_list))
          (Scope.to_string current)
      in
      failwith failure_str
    | [] ->
      failwith
        ( "Error: tried to pop a list of token kinds off of an "
        ^ "empty context." )
  ...
  let print_expected context =
    ...
    let print_token_kind token_kind =
      Printf.printf "'%s', %!" (TokenKind.to_string token_kind)
    in
