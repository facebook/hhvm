 ...
 * a simple CC mapping for the given def and use. This is to be done in the
 * specializations below as general mappings are usually correct.
 */
...
void annotateSFUses(Vunit& unit) {
  ...
  for (auto& kv : env.sf_def_flags) {
    // Set the flags for the instruction that defs sf
    FlagVisitor fv {};
    visitOperands(*kv.second.def, fv);
    fv.setFlags(kv.second.flags);
    FTRACE(kVasmMapFlagsLevel, "VregSF: {} -- flags: {}\n", kv.first, flags);
  }

  printUnit(kVasmMapFlagsLevel, "after map SF", unit);
}
