...
unsafe fn parser_options_from_ocaml_only_for_parser_errors(
    ...
    unsafe {
        ...
        let po_const_static_props = bool::from_ocaml(*ocaml_opts.add(4)).unwrap();
        let po_const_default_func_args = bool::from_ocaml(*ocaml_opts.add(5)).unwrap();
        let po_abstract_static_props = bool::from_ocaml(*ocaml_opts.add(6)).unwrap();
        let po_disallow_func_ptrs_in_constants = bool::from_ocaml(*ocaml_opts.add(7)).unwrap();
        let po_enable_xhp_class_modifier = bool::from_ocaml(*ocaml_opts.add(8)).unwrap();
        let po_disable_xhp_element_mangling = bool::from_ocaml(*ocaml_opts.add(9)).unwrap();
        let po_disable_xhp_children_declarations = bool::from_ocaml(*ocaml_opts.add(10)).unwrap();
        let po_const_default_lambda_args = bool::from_ocaml(*ocaml_opts.add(11)).unwrap();
        let po_allow_unstable_features = bool::from_ocaml(*ocaml_opts.add(12)).unwrap();
        let po_interpret_soft_types_as_like_types = bool::from_ocaml(*ocaml_opts.add(13)).unwrap();
        let tco_is_systemlib = bool::from_ocaml(*ocaml_opts.add(14)).unwrap();
        let po_disallow_static_constants_in_default_func_args =
            bool::from_ocaml(*ocaml_opts.add(15)).unwrap();
        let use_legacy_experimental_feature_config = bool::from_ocaml(*ocaml_opts.add(16)).unwrap();
        ...
        let enable_class_pointer_hint = bool::from_ocaml(*ocaml_opts.add(19)).unwrap();
        let ignore_string_methods = bool::from_ocaml(*ocaml_opts.add(20)).unwrap();
        let enable_intrinsics_extension = bool::from_ocaml(*ocaml_opts.add(21)).unwrap();
        let disallow_bool_cast = bool::from_ocaml(*ocaml_opts.add(22)).unwrap();
        ...
        parser_options.disallow_bool_cast = disallow_bool_cast;
        (
            parser_options,
