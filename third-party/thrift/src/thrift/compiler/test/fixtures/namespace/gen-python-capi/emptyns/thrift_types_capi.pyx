...
import emptyns.thrift_types as __thrift_types

cdef api int can_extract__emptyns__Foo(object __obj) except -1:
    return 1 if isinstance(__obj, __thrift_types.Foo) else 0

cdef api __cIOBuf* extract__emptyns__Foo(object __obj) except NULL:
    cdef __IOBuf __buf = __serialize_iobuf(__obj, protocol=__Protocol.BINARY)
    return __buf._ours.release()

cdef api object construct__emptyns__Foo(__unique_ptr[__cIOBuf] __s):
