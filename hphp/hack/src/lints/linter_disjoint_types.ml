...
let check_non_disjoint ~is_dynamic_call env p name ty1 ty2 =
...
let rec check_non_disjoint_tyl ~is_dynamic_call env p name tyl =
  match tyl with
  | [] -> ()
  | ty :: tyl ->
    List.iter tyl ~f:(check_non_disjoint ~is_dynamic_call env p name ty);
    check_non_disjoint_tyl ~is_dynamic_call env p name tyl

let has_non_disjoint_attr tp =
  Attributes.mem SN.UserAttributes.uaNonDisjoint tp.tp_user_attributes

let check_function_type_args_non_disjoint ~is_dynamic_call env p name tal fun_ty
    =
...
let member_hook ~is_dynamic_call env p class_type method_name tal =
...
let check_instance_method_call env p function_type receiver_type method_name tal
    =
  ...
  |> List.iter ~f:(fun class_type ->
         member_hook ~is_dynamic_call env p class_type method_name tal)
