...
bool c_sqlite3::t_close() {
  if (m_raw_db) {
    ...
    if (errcode != SQLITE_OK) {
      Logger::Warning("Unable to close database: %d, %s", errcode,
...
bool c_sqlite3::t_loadextension(CStrRef extension) {
  ...
  if (translated.empty()) {
    Logger::Warning("Unable to load extension at '%s'", extension.data());
    return false;
  }
...
Variant c_sqlite3::t_querysingle(CStrRef sql, bool entire_row /* = false */) {
  ...
  if (!sql.empty()) {
    ...
    if (!same(stmt, false)) {
      ...
      default:
        Logger::Warning("Unable to execute statement: %s",
                        sqlite3_errmsg(m_raw_db));
      }
    }
  }
...
bool c_sqlite3::t_createfunction(CStrRef name, CVarRef callback,
  ...
  if (!f_is_callable(callback)) {
    Logger::Warning("Not a valid callback function %s",
...
void c_sqlite3stmt::t___construct(CObjRef dbobject, CStrRef statement) {
  if (!statement.empty()) {
    ...
    if (errcode != SQLITE_OK) {
      Logger::Warning("Unable to prepare statement: %d, %s",
...
bool c_sqlite3stmt::t_reset() {
  ...
  if (sqlite3_reset(m_raw_stmt) != SQLITE_OK) {
    Logger::Warning("Unable to reset statement: %s",
...
bool c_sqlite3stmt::t_clear() {
  ...
  if (sqlite3_clear_bindings(m_raw_stmt) != SQLITE_OK) {
    Logger::Warning("Unable to clear statement: %s",
                    sqlite3_errmsg(sqlite3_db_handle(m_raw_stmt)));
    return false;
  }
...
Variant c_sqlite3stmt::t_execute() {
  ...
  for (unsigned int i = 0; i < m_bound_params.size(); i++) {
    ...
    case SQLITE_BLOB:
      {
        ...
        if (p.value.isResource()) {
          ...
          if (same(blob, false)) {
            Logger::Warning("Unable to read stream for parameter %ld",
                            p.index);
            return false;
          }
          sblob = blob.toString();
    ...
    case SQLITE_NULL:
      ...
      break;
    default:
      Logger::Warning("Unknown parameter type: %ld for parameter %ld",
                      p.type, p.index);
      return false;
    }
  }
