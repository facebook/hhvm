...
#define REGINT(n, v) Native::registerConstant<KindOfInt64> \
    ...
    REGINT(CL_EXPUNGE, 32768);
    REGINT(NIL, 0);
    REGINT(OP_ANONYMOUS, 4);
    REGINT(OP_DEBUG, 1);
    REGINT(OP_EXPUNGE, 128);
    REGINT(OP_HALFOPEN, 64);
    REGINT(OP_PROTOTYPE, 32);
    REGINT(OP_READONLY, 2);
    REGINT(OP_SECURE, 256);
    REGINT(OP_SHORTCACHE, 8);
    REGINT(OP_SILENT, 16);
    REGINT(FT_INTERNAL, 8);
    REGINT(FT_NOT, 4);
    REGINT(FT_PEEK, 2);
    REGINT(FT_PREFETCHTEXT, 32);
    REGINT(FT_UID, 1);
    REGINT(ST_UID, 1);
    REGINT(ST_SET, 4);
    REGINT(ST_SILENT, 2);
    ...
    REGINT(CP_UID, 1);
    REGINT(SA_ALL, 31);
    REGINT(SA_MESSAGES, 1);
    REGINT(SA_RECENT, 2);
    REGINT(SA_UIDNEXT, 8);
    REGINT(SA_UIDVALIDITY, 16);
    REGINT(SA_UNSEEN, 4);
    REGINT(SE_FREE, 2);
    REGINT(SE_NOPREFETCH, 4);
    REGINT(SE_UID, 1);
    REGINT(SO_FREE, 8);
    REGINT(SO_NOSERVER, 16);
    REGINT(LATT_HASCHILDREN, 32);
    REGINT(LATT_HASNOCHILDREN, 64);
    REGINT(LATT_MARKED, 4);
    REGINT(LATT_NOINFERIORS, 1);
    REGINT(LATT_NOSELECT, 2);
    REGINT(LATT_REFERRAL, 16);
    REGINT(LATT_UNMARKED, 8);
    REGINT(SORTARRIVAL, 1);
    REGINT(SORTCC, 5);
    REGINT(SORTDATE, 0);
    REGINT(SORTFROM, 2);
    REGINT(SORTSIZE, 6);
    REGINT(SORTSUBJECT, 3);
    REGINT(SORTTO, 4);
    REGINT(TYPEAPPLICATION, 3);
    REGINT(TYPEAUDIO, 4);
    REGINT(TYPEIMAGE, 5);
    REGINT(TYPEMESSAGE, 2);
    REGINT(TYPEMODEL, 7);
    REGINT(TYPEMULTIPART, 1);
    REGINT(TYPEOTHER, 8);
    REGINT(TYPETEXT, 0);
    REGINT(TYPEVIDEO, 6);
    REGINT(ENC7BIT, 0);
