<?php
	...
	*/
	...
	if (!$link->query("DROP TABLE IF EXISTS test_mysqli_last_insert_id_table_1") ||
		!$link->query("CREATE TABLE test_mysqli_last_insert_id_table_1 (id INT auto_increment, label varchar(10) not null, PRIMARY KEY (id)) ENGINE=MyISAM") ||
		!$link->query("INSERT INTO test_mysqli_last_insert_id_table_1 (id, label) VALUES (null, 'a')")) {
		printf("[002] [%d] %s\n", $link->errno, $link->error);
	}
	...
	if (!($res = $link->query("SELECT 1 FROM test_mysqli_last_insert_id_table_2")))
		printf("[006] [%d] %s\n", $link->errno, $link->error);
	else
		$res->close();
	...
	if ($link->query("INSERT INTO test_mysqli_last_insert_id_table_1 (id, label) VALUES (null, null)")) {
	...
	if (!$link->query("UPDATE test_mysqli_last_insert_id_table_1 SET id=LAST_INSERT_ID(id+1)"))
	  printf("[012] [%d] %s\n", $link->errno, $link->error);
	...
	if (!$link->query("INSERT INTO test_mysqli_last_insert_id_table_1(id, label) VALUES (LAST_INSERT_ID(id + 1), 'b')"))
		printf("[019] [%d] %s\n", $link->errno, $link->error);
	...
	if (!$link->query("INSERT INTO test_mysqli_last_insert_id_table_1(label) VALUES ('c')"))
		printf("[021] [%d] %s\n", $link->errno, $link->error);
	...
	if (!($res = $link->query("SELECT id, label FROM test_mysqli_last_insert_id_table_1 ORDER BY id ASC")))
		printf("[023] [%d] %s\n", $link->errno, $link->error);
	...
	if (!$link->query("INSERT INTO test_mysqli_last_insert_id_table_1(label) SELECT CONCAT(label, id) FROM test_mysqli_last_insert_id_table_1 ORDER BY id ASC"))
	  printf("[024] [%d] %s\n", $link->errno, $link->error);
	...
	if ($link->query("INSERT INTO test_mysqli_last_insert_id_table_1(id, label) SELECT id, CONCAT(label, id) FROM test_mysqli_last_insert_id_table_1 ORDER BY id ASC"))
	...
	if (!$link->query(sprintf("INSERT INTO test_mysqli_last_insert_id_table_1(id, label) VALUES (%d, 'z') ON DUPLICATE KEY UPDATE id = 101", $sql_id)	))
	  printf("[029] [%d] %s\n", $link->errno, $link->error);
