...
let go
  ...
  let add_hot_classes acc cids =
    List.fold cids ~init:acc ~f:begin fun acc cid ->
      let deps = DepSet.singleton @@ Dep.make (Dep.Class cid) in
      let deps = Typing_deps.add_all_deps deps in
      if DepSet.cardinal deps > threshold then cid::acc else acc
    end
  in
  let classes_to_save =
    MultiWorker.call
  ...
  let result =
    ...
    json_to_string ~pretty:true @@ JSON_Object [
  ...
  let _ : float =
    Hh_logger.log_duration "Done generating hot classes file" start_t in
  result
