...
let parse_single_char ~add_bold (parsed, rest) =
  let formatter =
    if add_bold then
      Tty.apply_color (Tty.Bold Tty.Default)
    else
      default_highlighted
  in
  let (first, tail) = first_and_tail_exn rest in
...
let rec format_markdown_highlighted ?(add_bold = false) (msg : string) =
  if String.is_empty msg then
    ...
    | Some (parsed, rest) -> parsed ^ format_markdown_highlighted ~add_bold rest
...
let format_claim_highlighted
  ...
  in
  let pretty_msg = format_markdown_highlighted ~add_bold:true msg in
...
let format_reason_highlighted marked_msg : string =
  ...
  let pretty_msg =
    format_markdown_highlighted (Errors.get_message_str marked_msg.message)
  in
