...
var_dump( array_change_key_case( darray["ONE" => 1, "one" => 3, "One" => 4] ) ); 
try { var_dump( array_change_key_case( darray["ONE" => 1, "one" => 6, "One" => 5], "CASE_UPPER" ) );  } catch (Exception $e) { echo "\n".'Warning: '.$e->getMessage().' in '.__FILE__.' on line '.__LINE__."\n"; }

/* should return one value key pair with key being in uppercase */
var_dump( array_change_key_case( darray["ONE" => 1, "one" => 2, "One" => 3], CASE_UPPER ) ); 
var_dump( array_change_key_case( darray["ONE" => 1, "one" => 1, "One" => 2], 5 ) ); 
