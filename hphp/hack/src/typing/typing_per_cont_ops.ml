...
let union union_types env context1 context2 =
  ...
  let env, tpenv = Type_parameter_env_ops.join env context1.tpenv context2.tpenv in
  let fake_members = Typing_fake_members.join context1.fake_members context2.fake_members in
  env,
  { fake_members;
    local_types;
    tpenv;
