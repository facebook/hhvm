...
// Per-direction payload variants for the client side. Each lists only the
...
// Client-side outbound — what the client pipeline writes to the wire.
// Today: REQUEST_RESPONSE only.
using ThriftClientOutboundPayloadVariant =
    ThriftPayloadVariant<ThriftRequestResponsePayload>;

// Client-side inbound — what the client pipeline reads from the wire.
