...
let get_count_exn t k = match Hashtbl.find_opt t k with
  | None -> raise (NoSuchEntry ("count: " ^ k))
  | Some {count; _} -> count

let get_sum_exn t k = match Hashtbl.find_opt t k with
  | None -> raise (NoSuchEntry ("sum: " ^ k))
  | Some {sum; _} -> sum

let get_mean_exn t k = match Hashtbl.find_opt t k with
  | None -> raise (NoSuchEntry ("mean: " ^ k))
