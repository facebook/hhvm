...
StructMetadata<::some::ns::IncludedB>::gen(ThriftMetadata& metadata) {
  if (FLAGS_thrift_enable_schema_to_metadata_conversion) {
    return genStructMetadata<::some::ns::IncludedB>(metadata, kGenerateAll).metadata;
  }

  auto res = genStructMetadata<::some::ns::IncludedB>(metadata, {.genAnnotations = folly::kIsDebug});
  if (res.preExists) {
    return res.metadata;
  }
  ...
  for (const auto& f : *include2_IncludedB_fields) {
    genStructFieldMetadata<::some::ns::IncludedB>(include2_IncludedB.fields()[i], f, i);
  ...
  DCHECK(structuredAnnotationsEquality(
    *res.metadata.structured_annotations(),
    newAnnotations,
    getAnnotationTypes<::some::ns::IncludedB>()
  ));
  return res.metadata;
}
