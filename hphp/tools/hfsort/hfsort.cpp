...
void print(const std::vector<Cluster*>& clusters) {
  FILE* outfile = fopen("oss_hot_section_ordering", "wt");
  if (!outfile) error("opening output file oss_hot_section_ordering");
  ...
  fclose(outfile);
  printf("Output saved in oss_hot_section_ordering\n");
