...
and p_enumerator_list = "EnumeratorList", fun () -> [
  ...
  [NonTerm p_enumerator_list; NonTerm p_enumerator]]
...
and p_static_declarator = "StaticDeclarator", fun () -> [
  [NonTerm p_variable_name];
  [NonTerm p_variable_name; equal; NonTerm p_constant_expression]]
 ...
 * Should assignements appear in binary expr? *)
  [NonTerm p_unary_expression];
  [NonTerm p_binary_expression; NonTerm p_binary_op;
    NonTerm p_binary_expression];
  [NonTerm p_conditional_expression];
  [NonTerm p_instanceof_expression];]

and p_conditional_expression = "ConditionalExpression", fun () -> [
  [NonTerm p_binary_expression; question; NonTerm p_expression; colon;
    NonTerm p_binary_expression];
  [NonTerm p_binary_expression; question; colon;
    NonTerm p_binary_expression]]
...
and p_instanceof_expression = "InstanceofExpression", fun () -> [
  [NonTerm p_unary_expression];
  [NonTerm p_expression; instanceof; NonTerm p_instance_designator]]
...
and p_anonymous_fun_return = "AnonymousFunReturn", fun () ->
  [[colon; NonTerm p_type_spec]; [colon; noreturn]]
...
and p_simple_assignment_expression = "SimpleAssignmentExpression", fun () ->
  [[NonTerm p_unary_expression; equal; NonTerm p_assignment_expression];
  (* this does not appear in the grammar.md but this is necessary for the
   * grammar to terminate *)
  [NonTerm p_unary_expression; equal; NonTerm p_unary_expression];
  [NonTerm p_unary_expression; equal; NonTerm p_primary_expression]]
...
and p_compound_assignment_operator = "CompoundAssignmentOperator", fun () -> [
  [star_star_equal];[star_equal];[slash_equal];[percent_equal];[plus_equal];
...
and p_compound_assignment_expression = "CompoundAssignmentExpression",
  fun () -> [
  [NonTerm p_expression; NonTerm p_compound_assignment_operator;
    NonTerm p_assignment_expression];
  (* this does not appear in the grammar.md but this is necessary for the
   * grammar to terminate *)
  [NonTerm p_expression; NonTerm p_compound_assignment_operator;
