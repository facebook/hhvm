...
let test_rename_target_is_dir
    ?(append_slash_on_src = false) ?(append_slash_on_target = false) dir =
...
let test_rm_dir dir =
  ...
  let file_exists =
    Disk.file_exists (Path.to_string @@ Path.concat subdir "a.txt")
  in
...
let tests =
  [
    ...
    ( "test_write_needs_directory_tree",
      with_temp_dir test_write_needs_directory_tree );
