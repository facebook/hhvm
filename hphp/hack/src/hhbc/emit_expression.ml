...
let from_binop op =
  let check_int_overflow =
    Hhbc_options.check_int_overflow !Hhbc_options.compiler_options in
  match op with
  | Ast_defs.Plus -> instr (IOp (if check_int_overflow then AddO else Add))
  | Ast_defs.Minus -> instr (IOp (if check_int_overflow then SubO else Sub))
  | Ast_defs.Star -> instr (IOp (if check_int_overflow then MulO else Mul))
...
let binop_to_eqop op =
  let check_int_overflow =
    Hhbc_options.check_int_overflow !Hhbc_options.compiler_options in
  match op with
  | Ast_defs.Plus -> Some (if check_int_overflow then PlusEqualO else PlusEqual)
  | Ast_defs.Minus -> Some (if check_int_overflow then MinusEqualO else MinusEqual)
  | Ast_defs.Star -> Some (if check_int_overflow then MulEqualO else MulEqual)
...
let unop_to_incdec_op op =
  let check_int_overflow =
    Hhbc_options.check_int_overflow !Hhbc_options.compiler_options in
  match op with
  | Ast_defs.Uincr -> if check_int_overflow then PreIncO else PreInc
  | Ast_defs.Udecr -> if check_int_overflow then PreDecO else PreDec
  | Ast_defs.Upincr -> if check_int_overflow then PostIncO else PostInc
  | Ast_defs.Updecr -> if check_int_overflow then PostDecO else PostDec
...
and from_unop op =
  ...
  | Ast_defs.Uplus -> instr (IOp (if check_int_overflow then AddO else Add))
  | Ast_defs.Uminus -> instr (IOp (if check_int_overflow then SubO else Sub))
  | Ast_defs.Uincr
  | Ast_defs.Udecr
  | Ast_defs.Upincr
  | Ast_defs.Updecr
