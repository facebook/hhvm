<?php
	...
	$test_table_name = 'test_mysqli_select_db_table_1'; require_once("table.inc");
	...
	if (mysqli_select_db($link, 'mysql')) {
		...
		// ($db) was done by mysqli__connect() already and the previous test_mysqli_select_db_table_1
		// was quite useless
		if (!$res = mysqli_query($link, "SELECT DATABASE() AS dbname"))
			printf("[009] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	...
	if (!$res = $link->query("SELECT id FROM test_mysqli_select_db_table_1 WHERE id = 1"))
		printf("[018] [%d] %s\n");
