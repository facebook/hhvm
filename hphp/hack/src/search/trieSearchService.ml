...
module Make (S : SearchUtils.Searchable) = struct
  ...
  module WorkerApi = struct
    let process_term key name pos type_ trie_acc =
      let res = { pos; name; result_type = type_ } in
      (key, res) :: trie_acc
  ...
  module MasterApi = struct
    ...
    let lookup str = (try Hashtbl.find main_index str with Not_found -> [])

    let replace key value =
      if value = [] then
        Hashtbl.remove main_index key
      else
        Hashtbl.replace main_index key value
    ...
    let insert_fn fn key empty_keys =
      ...
      if current_val == [] && not (SSet.mem key empty_keys) then
    ...
    let remove_fn fn key empty_keys =
      ...
      replace key new_val;
      if new_val == [] then
        SSet.add key empty_keys
      else
        empty_keys

    let process_file fn =
      let old_keys =
        (try Hashtbl.find removal_index fn with Not_found -> [])
        (* This will happen when we haven't seen this file before *)
      in
      let new_keys =
        (try SearchKeys.find_unsafe fn with Not_found -> [])
      ...
      let old_keys_set =
        List.fold_left
          old_keys
          ~f:begin
               fun acc file -> SSet.add file acc
             end
          ~init:SSet.empty
      in
      let new_keys_set =
    ...
    let results_for_file results results_count fn str ~filter_map ~limit =
       ...
       * term is a match *)
      let defs = (try SearchUpdates.find_unsafe fn with Not_found -> []) in
      List.iter defs (fun (key, res) ->
          ...
          if string_starts_with key str then
    ...
    let search_query input type_ =
      ...
      let compute_score str key res =
        match type_ with
        | Some x when S.compare_result_type x res.result_type <> 0 -> None
        | _ ->
          let score =
            if string_starts_with (String.lowercase_ascii res.name) str then
              get_score res str
            else
              String.length key * 2
          in
