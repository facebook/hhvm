    ...
    method private on_list
      ...
      fun f env xs ->
        let rec aux env xs counter =
          ...
          | [y1] ->
              let z1 = f env y1 in
              if phys_equal y1 z1 then xs else [z1]
          | [y1; y2] ->
              let z1 = f env y1 in
              let z2 = f env y2 in
              if phys_equal y1 z1 && phys_equal y2 z2 then xs else [z1; z2]
          | [y1; y2; y3] ->
              let z1 = f env y1 in
              let z2 = f env y2 in
              let z3 = f env y3 in
          ...
          | [y1; y2; y3; y4] ->
              ...
              let z4 = f env y4 in
              if
                phys_equal y1 z1 && phys_equal y2 z2 && phys_equal y3 z3
                && phys_equal y4 z4
              then xs
              else [z1; z2; z3; z4]
          | [y1; y2; y3; y4; y5] ->
              ...
              if
                phys_equal y1 z1 && phys_equal y2 z2 && phys_equal y3 z3
                && phys_equal y4 z4 && phys_equal y5 z5
              then xs
              else [z1; z2; z3; z4; z5]
          | y1 :: y2 :: y3 :: y4 :: y5 :: ys ->
              let z1 = f env y1 in
              ...
              let z5 = f env y5 in
              let zs =
                if counter > 1000 then aux_slow env ys [] ys false
                else aux env ys (counter + 1)
              in
              if
                phys_equal y1 z1 && phys_equal y2 z2 && phys_equal y3 z3
                && phys_equal y4 z4 && phys_equal y5 z5 && phys_equal ys zs
              then xs
        ...
        and aux_slow env xs acc original_list has_new_elements =
          ...
          | y1 :: ys ->
              let z1 = f env y1 in
              aux_slow env ys (z1 :: acc) original_list
