...
bool
TranslatorX64::Lease::amOwner() const {
...
bool
TranslatorX64::Lease::acquire(bool blocking /* = false */ ) {
  TRACE_SET_MOD(txlease);
  if (amOwner()) {
    return true;
  }
...
void
TranslatorX64::Lease::drop(int64 hintExpireDelay) {
  TRACE_SET_MOD(txlease);
  ASSERT(amOwner());
...
TranslatorX64::LeaseHolderBase::LeaseHolderBase(Lease& l, LeaseAcquire acquire,
                                                bool blocking)
...
TranslatorX64::LeaseHolderBase::~LeaseHolderBase() {
...
bool
TranslatorX64::LeaseHolderBase::acquire() {
  ASSERT(!m_acquired);
...
emitDerefIfVariant(X64Assembler &a, PhysReg reg) {
  if (RuntimeOption::EvalJitCmovVarDeref) {
    a.cmp_imm32_disp_reg32(KindOfVariant, TVOFF(m_type), reg);
...
//   you probably want to lazily save these regs via recordInstrCall
//   and its ilk.
//
...
void TranslatorX64::emitDecRefGeneric(const NormalizedInstruction& i,
   ...
   */
  {
    ...
    callUnaryStub(a, i, m_dtorGenericStub, srcReg, disp);
  }
}
...
TCA TranslatorX64::retranslate(SrcKey sk, bool align) {
  ...
  LeaseHolder writer(m_writeLease);
...
TranslatorX64::getInterceptHelper() {
    ...
    if (!m_interceptHelper) {
      ...
      // Normally we'd like to recordInstrCall here, but the vmreg sync'ing
...
TranslatorX64::emitPrologueRedispatch(X64Assembler& a) {
  ...
  a.    load_reg64_disp_index_reg64(rax,
                                    // %rcx + 1
                                    Func::prologueTableOffset() + sizeof(TCA),
                                    rcx,
                                    rax);
  a.    jmp_reg(rax);
  a.    ud2();
...
TranslatorX64::emitFuncGuard(X64Assembler& a, const Func* func) {
  UNUSED TCA aStart = a.code.frontier;
 ...
 * entries into the FuncCache. So don't go around using the
...
TranslatorX64::funcPrologue(Func* func, int nPassed) {
  ...
  bool funcIsMagic = (func->isMagicCallMethod()
                      || func->isMagicCallStaticMethod());

  // Do a quick test before grabbing the write lease
  TCA prologue = (TCA)func->getPrologue(paramIndex);
  ASSERT(prologue);
  if (prologue != (TCA)fcallHelperThunk) return prologue;
  ...
  prologue = (TCA)func->getPrologue(paramIndex);
  ASSERT(prologue);
  if (prologue != (TCA)fcallHelperThunk) return prologue;
  ...
  // magic callees.
  if (!funcIsMagic) emitFuncGuard(a, func);
  ...
  if (funcIsMagic) {
    ...
    TCA magicStart = a.code.frontier;
    // Guard: we're in the right callee.
    emitFuncGuard(a, func);
    ASSERT(numParams == 2);
    emitRB(a, RBTypeFuncEntry, func->fullName()->data());
    ...
    if (nPassed == 2) {
      ...
      a.jcc8(CC_Z, start); // jz ==> not a magic call, go to prologue for nPassed
  ...
  recordGdbTranslation(skFuncBody, func->unit(), aStart,
                       a.code.frontier - aStart, false, true);
  Debug::recordBCInstr(OpFuncPrologue, start, a.code.frontier);
...
TranslatorX64::interceptPrologues(Func* func) {
  ...
  for (int i = 0; i < maxNumPrologues; i++) {
    ...
    TCA addr = a.code.frontier;
    emitInterceptPrologue(func, prologue);
    a. jmp(prologue);
    func->setPrologue(i, addr);
    TRACE(1, "%s.prologue[%d]=%p\n",
...
TranslatorX64::emitPrologue(Func* func, int nPassed) {
  int numParams = func->numParams();
  if (m_interceptsEnabled && !func->isPseudoMain()) {
...
TranslatorX64::emitCondJmp(const SrcKey &skTaken, const SrcKey &skNotTaken,
  ...
  emitServiceReq(false /* align */, REQ_BIND_JMPCC_FIRST, 4,
                 old, skTaken.offset(), skNotTaken.offset(), cc);
...
TranslatorX64::bindJmp(TCA toSmash, SrcKey destSk, bool isAddr) {
  ...
  sr->chainFrom(getAsmFor(toSmash), isAddr ?
                IncomingBranch((TCA*)toSmash) : IncomingBranch(toSmash));
  return tDest;
}
...
TranslatorX64::bindJmpccSecond(TCA toSmash, const Offset off,
  ...
  LeaseHolder writer(m_writeLease, NO_ACQUIRE);
...
TranslatorX64::checkType(X64Assembler& a,
  ...
  if (rtt.isValue() && rtt.outerType() == KindOfVariant) {
    ...
    if (rtt.innerType() != KindOfInvalid) {
      ...
      a.   cmp_imm32_disp_reg32(rtt.innerType(), TVOFF(m_type), rax);
      emitFallbackJmp(fail);
    }
  }
}
  ...
  for (;;) {
    ...
    switch (requestNum) {
      ...
      case REQ_STACK_OVERFLOW: {
        /*
         * Since the prologue can't reconstruct the actual FCall
         * instruction, it instead sets vmpc() to the instruction after
         * FCall. This is problematic for unwinding, because the
         * instruction after FCall has the stack at the wrong level.
         * FCall is a variable-length instruction, but the FPI region
         * query is range-based, so it suffices to get the pc somewhere
         * before the instruction after.
         *
         * Barf.
         */
        vmpc() -= 2;
        // XXX: Note the possible wrongness here when a chunk of the varint
        // happens to look like OpFCall. If anyone ever hits this bug, fire
        // me.
        if (*vmpc() != OpFCall) {
          // Maybe it was a four-byte offset?
          vmpc() -= 3;
          ASSERT(*vmpc() == OpFCall);
        }
...
TranslatorX64::emitServiceReqVA(bool align, ServiceRequest req, int numArgs,
   ...
   */
  astubs.    ret();
  Debug::recordBCInstr(OpServiceRequest, retval, astubs.code.frontier);
...
TranslatorX64::syncOutputs(int stackOff) {
  ...
  Debug::recordBCInstr(OpSyncOutputs, start, a.code.frontier);
}
...
TranslatorX64::emitUnboxTopOfStack(const NormalizedInstruction& i) {
  ...
  emitDecRef(i, rSrc, KindOfVariant);
...
planUnaryBooleanOp(const NormalizedInstruction& i) {
  ...
  if (inType == KindOfArray || rtt.isString()) {
    return Simple;
  }
...
void TranslatorX64::fuseBranchAfterStaticBool(const Tracelet& t,
  ...
  fuseBranchSync(t, i);
  bool isTaken = (resultIsTrue == (nexti.op() == OpJmpNZ));
  SrcKey taken, notTaken;
...
void TranslatorX64::fuseBranchAfterBool(const Tracelet& t,
  ...
  ASSERT(i.next);
  NormalizedInstruction &nexti = *i.next;
  if (i.next->op() == OpJmpZ) cc = ccNegate(cc);
...
TranslatorX64::translateCGetL(const Tracelet& t,
  ...
  if (inputs[0]->rtt.isUninit()) {
    ASSERT(outType == KindOfNull);
    ASSERT(inputs[0]->location.offset < curFunc()->numLocals());
    const StringData* name = curFunc()->pnames()[inputs[0]->location.offset];
...
TranslatorX64::translateCGetL2(const Tracelet& t,
  ...
  if (undefinedLocal) {
    ...
    const StringData* name = curFunc()->pnames()[ni.inputs[locIdx]->
      location.offset];
    ASSERT(name->isStatic());
...
TranslatorX64::translateAssignToLocalOp(const Tracelet& t,
  ...
  ASSERT(ni.inputs.size() == 2);
  ASSERT((op == OpBindL) ==
         (ni.inputs[rhsIdx]->outerType() == KindOfVariant));
  ...
  const bool affectInnerCell = op == OpSetL &&
                               oldLocalType == KindOfVariant;
  if (affectInnerCell) {
    ASSERT(rhsType != KindOfVariant);
...
TranslatorX64::analyzeAddElemC(Tracelet& t, NormalizedInstruction& i) {
  i.m_txFlags = simplePlan(i.inputs[2]->outerType() == KindOfArray &&
...
TranslatorX64::analyzeAddNewElemC(Tracelet& t, NormalizedInstruction& i) {
  ASSERT(i.inputs.size() == 2);
  i.m_txFlags = simplePlan(i.inputs[1]->outerType() == KindOfArray);
}
...
TranslatorX64::translateCns(const Tracelet& t,
  ...
  ASSERT((tv == NULL) == (outType == KindOfInvalid));
  using namespace TargetCache;
  if (tv) {
    m_regMap.allocOutputRegs(i);
    if (checkDefined) {
...
static void defCnsHelper(TargetCache::CacheHandle ch, Variant *inout,
  ...
  if (LIKELY(tv->m_type == KindOfUninit &&
    ...
    if (LIKELY(g_vmContext->m_constants.nvInsert(name, (TypedValue*)inout))) {
      ...
      *inout = true;
      if (setBit) {
        UNUSED bool alreadyDefined = testAndSetBit(bit);
...
planInstrAdd_Array(const NormalizedInstruction& i) {
  ASSERT(i.inputs.size() == 2);
  return simplePlan(i.inputs[0]->valueType() == KindOfArray &&
...
TranslatorX64::analyzeCastString(Tracelet& t, NormalizedInstruction& i) {
  i.m_txFlags =
    i.inputs[0]->isArray() ? Supported :
    i.inputs[0]->isObject() || i.inputs[0]->isInt() ? Simple :
    Native;
  i.funcd = NULL;
}
...
void TranslatorX64::toStringHelper(ObjectData *obj) {
  ...
  if (LIKELY(obj->isInstance())) {
    ...
    EXCEPTION_GATE_ENTER();
    TypedValue &tv = ar->m_r;
    tvWriteUninit(&tv);
    Variant &v = tvAsVariant(&tv);
    v = obj->t___tostring();
    EXCEPTION_GATE_LEAVE();
    // point our return address at a helper that will clean
    // up the stack
    rbp->m_savedRip = (uint64_t)tx64->m_toStringReturnHelper;
  }
}
...
TranslatorX64::translatePrint(const Tracelet& t,
  ...
  switch (type) {
    ...
    INTCASE():          EMIT_CALL1(a, print_int,     V(loc)); break;
...
TranslatorX64::translateSwitch(const Tracelet& t,
                               const NormalizedInstruction& i) {
  PhysReg labelReg = getReg(i.inputs[0]->location);
  syncOutputs(t); // this will mark labelReg as FREE but it still has
                  // its old value

  uint32_t* offsetAddr = (uint32_t*)(a.code.frontier + 3);
  a.lea_rip_disp_reg64(0x1badd00d, rScratch);
  TCA afterLea = a.code.frontier;

  a.jmp_reg64_index_displ(rScratch, labelReg, 0);
  translator_not_reached(a);
  moveToAlign(a, 8);

  ASSERT(*offsetAddr == 0x1badd00d);
  *offsetAddr = a.code.frontier - afterLea;
  const ImmVector& iv = i.immVec;
  for (int idx = 0; idx < iv.size(); ++idx) {
    ASSERT(isSmashable(a, sizeof(TCA)));
    SrcKey sk(curFunc(), i.offset() + iv.vec32()[idx]);
    TCA sr = emitServiceReq(REQ_BIND_ADDR, 2,
                            a.code.frontier, sk.offset());
    a.qword((uintptr_t)sr);
  }
}
...
TranslatorX64::translateRetC(const Tracelet& t,
  ...
  if (freeLocalsInline) {
    ...
    a.test_reg64_reg64(rScratch, rScratch);
    {
      ...
      astubs.call((TCA)&EventHook::FunctionExit);
      recordInstrStubCall(i);
      m_regMap.smashRegs(kCallerSaved);
    }
  } else {
  ...
  int retvalDestDisp = cellsToBytes(1 + nLocalCells) + AROFF(m_r);
  ...
  if (freeLocalsInline) {
    ...
    ASSERT(varEnvCheck);
    // 2:
    a.patchJcc(varEnvCheck, astubs.code.frontier);
    emitFrameRelease(astubs, i, noThis);
    astubs. jmp(localsFreed); // 1b
  }
}
...
static bool mapGenParams(ParamMap& map,
  ...
  for (unsigned i = 0; i < varNames.size(); ++i) {
    Id id;
    if (mapGet(genNames, varNames[i], &id)) {
      map[i] = id;
    } else {
      return false;
    }
...
void TranslatorX64::translateCreateCont(const Tracelet& t,
  ...
  if (origLocals <= kMaxInlineContLocals &&
    ...
    bool fillThis = origFunc->isNonClosureMethod() && !origFunc->isStatic() &&
      mapGet(genFunc->pnameMap(), thisStr, &thisId) &&
      !mapContains(origFunc->pnameMap(), thisStr);
...
void TranslatorX64::translateUnpackCont(const Tracelet& t,
  ...
  ASSERT(i.outStack->outerType() == BitwiseKindOfInt64);
  ...
  if (nCopy > kMaxInlineContLocals) {
    ...
    m_regMap.bind(rax, i.outStack->location, BitwiseKindOfInt64,
                  RegInfo::DIRTY);
    return;
  }
  ...
  m_regMap.bindScratch(rLabel, i.outStack->location, BitwiseKindOfInt64,
                       RegInfo::DIRTY);
}
...
void TranslatorX64::translatePackCont(const Tracelet& t,
  ...
  DiamondReturn astubsRet;
  {
    // TODO: We can probably prove that this is impossible in most
    // cases using information from hphpc
...
isNormalPropertyAccess(const NormalizedInstruction& i,
  ...
  return
    ...
    (i.immVecM[0] == MPC || i.immVecM[0] == MPL) &&
...
isSupportedCGetMProp(const NormalizedInstruction& i) {
  ...
  SKTRACE(2, i.source, "CGetM prop candidate: MPC-ness %d, "
                       "in[0] %s in[1] %s\n",
          i.immVecM[0] == MPC || i.immVecM[0] == MPL,
...
isSupportedCGetM_LE(const NormalizedInstruction& i) {
  ...
  return
    ...
    i.immVecM.size() == 1 &&
    (i.immVecM[0] == MEC || i.immVecM[0] == MEL) &&
...
isSupportedCGetM_LEE(const NormalizedInstruction& i) {
  ...
  return
    ...
    i.immVecM.size() == 2 &&
    (i.immVecM[0] == MEC || i.immVecM[0] == MEL) &&
    (i.immVecM[1] == MEC || i.immVecM[1] == MEL) &&
...
TranslatorX64::emitPropGet(const NormalizedInstruction& i,
  ...
  if (!base.isLocal()) {
    // Release the base
    PhysReg baseTmp = baseReg;
    if (base.isVariant()) {
      // Should only be possible for CGetM <LR ..>
      ASSERT(i.immVec.locationCode() == LR);
      emitDeref(a, baseReg, *scratch);
      baseTmp = *scratch;
    }
    emitDecRef(i, baseTmp, KindOfObject);
  }
}
...
TranslatorX64::translateCGetMProp(const Tracelet& t,
  ...
  if (propOffset != kInvalidSlot && i.immVec.locationCode() == LC) {
    ...
    // If PropCache::lookup returned NULL there's nothing left to do.
    a.test_reg64_reg64(rax, rax);
    {
      JccBlock<CC_Z> ifRaxNotNull(a);
      ScratchReg fieldAddr(m_regMap, rax);
      // Our base's register was probably blown away by the call up above
...
TranslatorX64::translateCheckTypeOp(const Tracelet& t,
  ...
  const bool isType    = checkTypeHelper(ni.op(), ni.inputs[0]->valueType());
  ...
  if (doUninit) {
    const StringData* name = curFunc()->pnames()[
      ni.inputs[0]->location.offset];
    ASSERT(name->isStatic());
    EMIT_CALL1(a, raiseUndefVariable, IMM((uintptr_t)name));
    recordInstrCall(ni);
  }
...
TranslatorX64::emitPropSet(const NormalizedInstruction& i,
  ...
  if (!base.isLocal()) {
    ...
    emitDecRef(i, baseReg, KindOfObject);
  }
}
...
TranslatorX64::translateSetMProp(const Tracelet& t,
  ...
  const DynLocation& base = *i.inputs[1];
  const DynLocation& prop = *i.inputs[2];
  const Slot propOffset    = getPropertyOffset(i, 2, 1);
  ASSERT(val.isStack() && val.outerType() != KindOfVariant);
  ...
  if (propOffset != kInvalidSlot && i.immVec.locationCode() == LC) {
    ...
    recordInstrCall(i);
    ScratchReg rField(m_regMap, rax);
    m_regMap.allocInputReg(i, kRhsIdx);

    // If PropCache::set returned NULL there's nothing left to do except
    // copy rhs to the output location
    a.    test_reg64_reg64(rax, rax);
    {
      JccBlock<CC_Z> ifRaxNotNull(a);
      emitPropSet(i, base, val, *rField);
...
uint64 TranslatorX64::reqLitHelper(const ReqLitStaticArgs* args,
                                   Cell *fp, Cell *sp) {
  vmfp() = fp;
  vmsp() = sp + 1;
  ...
  tl_regState = REGSTATE_DIRTY;
  if (!runPseudoMain) return (uint64)ec + 1;
  ec->m_fp->m_savedRip = (uint64)args->m_fallthrough;
  EXCEPTION_GATE_RETURN((uint64)ec);
}
...
TranslatorX64::translateReqLit(const Tracelet& t,
  ...
  args->m_efile = efile;
  args->m_pcOff = after;
  args->m_fallthrough = (TCA)kUninitializedRIP; // Patched below;
  args->m_local = local;
  emitImmReg(a, (uint64_t)args, argNumToRegName[0]);
  /* See description in processInit */
  a.call(m_requireHelper);
  const Opcode* destPC = unit->at(func->base());
  SrcKey funcBody(func, destPC);
  emitBindJmp(funcBody);
  args->m_fallthrough = a.code.frontier;
...
TranslatorX64::emitNativeTrampoline(TCA helperAddr) {
  ...
  Debug::recordBCInstr(OpNativeTrampoline, trampAddr,
                       atrampolines.code.frontier);
  return trampAddr;
}
...
TranslatorX64::analyzeFPushObjMethodD(Tracelet& t,
  ...
  i.m_txFlags = simplePlan(objLoc->valueType() == KindOfObject &&
                           isContextFixed());
}
...
TranslatorX64::translateFPushObjMethodD(const Tracelet &t,
  ...
  if (!func || !(func->attrs() & AttrStatic)) {
    ...
    emitVStackStoreImm(a, i, uintptr_t(baseClass) | 1, thisOff, sz::qword);
  }
...
TranslatorX64::analyzeFPushFuncD(Tracelet& t, NormalizedInstruction& i) {
  ...
  i.m_txFlags = simplePlan(func != NULL);
}
...
TranslatorX64::translateFCall(const Tracelet& t,
  ...
  a.test_reg64_reg64(rScratch, rScratch);
  {
    ...
    // Record a "before" fixup here.  After we've committed to the
    // call we'll record an "after" fixup (see emitBindCall).
    recordInstrStubCall(i);
    m_regMap.smashRegs(kCallerSaved);
  }
...
TranslatorX64::analyzeStaticLocInit(Tracelet& t, NormalizedInstruction& i) {
  ...
  // strategy.
  i.m_txFlags = simplePlan(!curFunc()->isClosureBody() &&
...
staticLocHelper(StringData* name, TargetCache::CacheHandle ch) {
  ...
  ASSERT(retval);
  if (retval->m_type != KindOfVariant) {
    tvBox(retval);
  }
...
TranslatorX64::translateStaticLocInit(const Tracelet& t,
  ...
  emitIncRef(*output, KindOfVariant);
  // Turn output into the local we just initialized.
  m_regMap.bindScratch(output, outLoc, KindOfVariant, RegInfo::DIRTY);
}
...
TranslatorX64::translateInstanceOfD(const Tracelet& t,
  ...
  DataType type = input0->outerType();
...
TranslatorX64::analyzeIterValueC(Tracelet& t, NormalizedInstruction& i) {
  i.m_txFlags = simplePlan(i.inputs[0]->rtt.iterType() == Iter::TypeArray);
}
...
bool
NormalizedInstruction::hasAliasableSuccessors() const {
  for (NormalizedInstruction* succ = next;
       succ; succ = succ->next) {
    for (size_t i = 0; i < succ->inputs.size(); ++i) {
      if (succ->inputs[i]->canBeAliased()) return true;
    }
  }
  return false;
}
...
#undef O
  ...
  // If we're mid-tracelet, and the instruction might have reentered,
  // *and* we have aliasable values in flight, prepare for a side-exit.
  bool maySideExit = !i.breaksBB && !(i.m_txFlags & NonReentrant) &&
    i.hasAliasableSuccessors();
  int canaryStackOff = -1;
  if (maySideExit) {
    canaryStackOff = -cellsToBytes(max(i.stackOff, i.next->stackOff) + 1);
    SKTRACE(1, i.source, "sideexit marker at %d\n", canaryStackOff);
    // Leave a canary just past the last cell read or used by this
    // instruction. Assumes individual instructions only move the stack in
    // one direction.
    a.  store_imm32_disp_reg(KindOfCanary,
                             canaryStackOff + TVOFF(m_type), rVmSp);
    if (debug) {
      // If we made an error finding a dead stack slot, we'll start
      // manipulating this cell as if it were data. Make it obvious.
      uint64 imm = 0xb100dfed00000000ul | i.source.offset();
      a.store_imm64_disp_reg64(imm, // non-canonical, will always fault
                               canaryStackOff + TVOFF(m_data.pstr), rVmSp);
    }
  }
...
#undef CASE
  ...
  if (maySideExit) {
    // If our canary is healthy, we have not reentered. All reentries
    // will first push an ActRec, then eventually either unwind (in
    // which case the code in the TC will be skipped over anyway), or
    // return via the RetC instruction. RetC always puts a valid TypedValue
    // where its invoking ActRec once was; since no valid TypedValue
    // has a KindOfCanary type, finding our canary intact after the call
    // means we have not re-entered.
    //
    // This is a bit harder to reason about than, e.g., a vmentry
    // generation counter, but that would require more in-TC code to
    // get right, and we'd still need somewhere to stash this value
    // during the instruction.
    //
    // Since we've just done a possibly-reentrant call, we know that any
    // dirty values are already sync'ed up. No need to clean here.
    a.    cmp_imm32_disp_reg32(KindOfCanary,
                               canaryStackOff + TVOFF(m_type),
                               rVmSp);
    {
  ...
  Debug::recordBCInstr(op, start, a.code.frontier);
  if (astubs.code.frontier != astart) {
    Debug::recordBCInstr(op + Op_count, astart, astubs.code.frontier);
  }
...
TranslatorX64::translateTracelet(const Tracelet& t) {
  ...
  try {
    ...
    if (a.code.frontier != start) {
      Debug::recordBCInstr(OpTraceletGuard, start, a.code.frontier);
    }
  ...
  addTranslation(TransRec(t.m_sk, curUnit()->md5(), t,
                          start, a.code.frontier - start,
                          stubStart, astubs.code.frontier - stubStart));

  recordGdbTranslation(sk, curUnit(), start, a.code.frontier - start,
                       false, false);
...
void
TranslatorX64::callUnaryStub(X64Assembler& a, const NormalizedInstruction& i,
...
TranslatorX64::requestExit() {
  if (m_writeLease.amOwner()) {
    m_writeLease.drop();
  }
  TRACE_MOD(txlease, 2, "%lx write lease stats: %15lld kept, %15lld grabbed\n",
            pthread_self(), m_writeLease.m_hintKept,
            m_writeLease.m_hintGrabbed);
  PendQ::drain();
...
void TranslatorX64::processInit() {
  ...
  static const size_t trampolinesSize = kTrampolinesBlockSize;
  // We want to ensure that the block for "a", "astubs", and "atrampolines" are
  // nearby so that we can short jump between them. Thus we allocate one slab
  // and divide it between "a", "astubs", and "atrampolines".
  uint8_t *base = allocSlab(aSize + astubsSize + trampolinesSize);
  atrampolines.init(base,trampolinesSize);
  a.init(base + trampolinesSize, aSize);
  astubs.init(base + trampolinesSize + aSize, astubsSize);
  ...
  // the return stack.
  m_callToExit = emitServiceReq(REQ_EXIT, 0);
  ...
  if (!RuntimeOption::EvalJitNoGdb) {
    m_debugInfo.recordTracelet(m_callToExit-1, astubs.code.frontier,
      NULL, NULL, true, false);
  }

  m_retHelper = emitRetFromInterpretedFrame();
  Debug::recordBCInstr(OpRetFromInterp, m_retHelper, astubs.code.frontier);
  if (!RuntimeOption::EvalJitNoGdb) {
    m_debugInfo.recordTracelet(m_retHelper, astubs.code.frontier,
      NULL, NULL, true, false);
  }
  ...
  astubs.   load_reg64_disp_reg64(rax, offsetof(VMExecutionContext, m_stack) +
                                       Stack::topOfStackOffset(), rVmSp);
  emitServiceReq(false, REQ_RESUME, 0);
  Debug::recordBCInstr(OpResumeHelper, m_resumeHelper, astubs.code.frontier);

  {
    /*
       Helper for literal require.

       On entry:
         - the return address is a stub that will branch
           to the pseudoMain of the unit;
         - arg 0 is the address of a pre-filled ReqLitStaticArgs.

       This stub passes its arguments, and fp and sp to
       reqLitHelper which checks to see if the require
       is needed, then calls evalUnit.
       reqLitHelper will return the ExecutionContext, with
       bit zero set if the pseudoMain does not need to be
       executed.
       */
    m_requireHelper = TCA(a.code.frontier);
    PhysReg rFallthrough = r13;
    PhysReg rDest = r15;
    a.   popr(rDest); // By default, continue into the body.
    if (false) { // typecheck
      ReqLitStaticArgs* a = NULL;
      Cell *fp = NULL;
      Cell *sp = NULL;
      UNUSED uint64 ret = reqLitHelper(a, fp, sp);
    }
    a.   load_reg64_disp_reg64(argNumToRegName[0],
                               offsetof(ReqLitStaticArgs, m_fallthrough), r13);
    a.   mov_reg64_reg64(rVmFp, argNumToRegName[1]);
    a.   mov_reg64_reg64(rVmSp, argNumToRegName[2]);
    a.   call((TCA)TranslatorX64::reqLitHelper);
    a.   test_imm32_reg64(1, rax);
    {
      JccBlock<CC_Z> ifSkip(a);
      // pseudoMain does not need to be executed, so
      // clear the bottom bit of rax to recover the EC
      a. sub_imm32_reg64(1, rax);
      // and move the post-pseudoMain address into rDest
      a. mov_reg64_reg64(rFallthrough, rDest);
    }
    // copy fp and sp back from the EC.
    a.   load_reg64_disp_reg64(rax, offsetof(VMExecutionContext, m_fp), rVmFp);
    a.   load_reg64_disp_reg64(rax, offsetof(VMExecutionContext, m_stack) +
                               Stack::topOfStackOffset(), rVmSp);
    a.   jmp_reg(rDest);
    Debug::recordBCInstr(OpRequireHelper, m_requireHelper, a.code.frontier);
  }
  ...
  Debug::recordBCInstr(OpDefClsHelper, m_defClsHelper, a.code.frontier);
  ...
  emitServiceReq(false, REQ_STACK_OVERFLOW, 0);
  ...
  m_dtorStubs[KindOfVariant]       = emitUnaryStub(a, vp(tv_release_var));
  m_dtorGenericStub                = genericRefCountStub(a);
  m_typedDtorStub                  = emitBinaryStub(a, vp(tv_release_typed));
  Debug::recordBCInstr(OpDtorStub, m_dtorStubs[BitwiseKindOfString],
                       a.code.frontier);
...
bool TranslatorX64::addDbgGuards(const Unit* unit) {
  ...
  bool locked = m_writeLease.acquire(true);
  if (!locked) {
    return false;
  }
...
void TranslatorX64::addDbgGuardImpl(const SrcKey& sk, SrcRec& srcRec) {
  ...
  {
    TCA fallback = emitServiceReq(REQ_INTERPRET, 2, sk.offset(), 0);
    a. jnz(fallback);
  }
...
bool TranslatorX64::dumpTC() {
  if (!m_writeLease.acquire(true)) return false;
...
bool TranslatorX64::dumpTCData() {
  ...
  for (size_t t = 0; t < m_translations.size(); t++) {
    m_translations[t].print(buf, getTransCounter(t));
    if (gzputs(tcDataFile, buf) == -1) {
      return false;
    }
  }
...
void TranslatorX64::invalidateSrcKey(const SrcKey& sk) {
   ...
   * to it.
   */
  TCA newDest = emitServiceReq(REQ_RETRANSLATE, 1, sk.offset());
