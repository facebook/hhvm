...
static xmlNodePtr serialize_parameter(sdlParamPtr param, Variant value,
  ...
  if (!value.isNull() && value.isObject()) {
    c_soapparam *p = value.toObject().getTyped<c_soapparam>();
    value = p->m_data;
    name = p->m_name.c_str();
  }
...
static bool do_request(c_soapclient *client, xmlDoc *request,
  ...
  response = client->t___dorequest(String(buf, buf_size, AttachLiteral),
...
static encodeMapPtr soap_create_typemap_impl(sdl *sdl, Array &ht) {
  ...
  for (ArrayIter iter(ht); iter; ++iter) {
    Variant tmp = iter.second();
    if (!tmp.isArray()) {
      Logger::Warning("Wrong 'typemap' option");
      return typemap;
    }
...
void c_soapserver::t_setclass(int _argc, CStrRef name,
  ...
  if (f_class_exists(name, true)) {
    ...
    Logger::Warning("Tried to set a non existant class (%s)", name.data());
  }
}
...
void c_soapserver::t_addfunction(CVarRef func) {
  ...
  if (func.isString()) {
    ...
    if (func.toInt64() == SOAP_FUNCTIONS_ALL) {
      ...
      Logger::Warning("Invalid value passed");
    }
    return;
  }
  ...
  if (m_type == SOAP_FUNCTIONS) {
    for (ArrayIter iter(funcs); iter; ++iter) {
      if (!iter.second().isString()) {
        Logger::Warning("Tried to add a function that isn't a string");
        return;
      }
      String function_name = iter.second().toString();
      if (!f_function_exists(function_name)) {
        Logger::Warning("Tried to add a non existant function '%s'",
                        function_name.data());
        return;
      }
  ...
  if (!request.isNull()) {
    ...
    if (g->gv__SERVER.toArray().exists("HTTP_CONTENT_ENCODING")) {
      ...
      if (encoding == "gzip" || encoding == "x-gzip") {
        ...
        Logger::Warning("Request is encoded with unknown compression '%s'",
                        encoding.data());
        return;
      }
      if (!ret.isString()) {
        Logger::Warning("Can't uncompress compressed request");
        return;
      }
      req = ret.toString();
    }
...
void c_soapserver::t_setpersistence(int64 mode) {
  ...
  if (m_type == SOAP_CLASS) {
    if (mode == SOAP_PERSISTENCE_SESSION || mode == SOAP_PERSISTENCE_REQUEST) {
      ...
      Logger::Warning("Tried to set persistence with bogus value (%ld)", mode);
    }
  } else {
    Logger::Warning("Tried to set persistence when you are using you "
...
Variant c_soapclient::t___soapcall(String name, Array args,
  ...
  if (input_headers.isNull()) {
    ...
    Logger::Warning("Invalid SOAP header");
    return null;
  }
...
Variant c_soapclient::t___dorequest(String buf, String location, String action,
  ...
  if (code != 200) {
    m_soap_fault =
      Object((NEW(c_soapfault)())->create
             ("HTTP", HttpProtocol::GetReasonString(code)));
    return null;
  }
...
void c_soapvar::t___construct(CVarRef data, CVarRef type,
  ...
  if (type.isNull()) {
    ...
    if (defEncIndex.find(ntype) != defEncIndex.end()) {
      ...
      Logger::Warning("Invalid type ID");
      return;
    }
  }
...
void c_soapfault::t___construct(CVarRef code, CStrRef message,
  ...
  if (code.isString()) {
    ...
    if (!fault_ns.isString() || !fault_code.isString()) {
      Logger::Warning("Invalid fault code");
      return;
    }
  } else  {
    Logger::Warning("Invalid fault code");
...
void c_soapparam::t___construct(CVarRef data, CStrRef name) {
  if (name.empty()) {
    Logger::Warning("Invalid parameter name");
    return;
  }
  m_name = name;
  m_data = data;
...
void c_soapheader::t___construct(CStrRef ns, CStrRef name,
  ...
  if (name.empty()) {
    Logger::Warning("Invalid header name");
    return;
  }
