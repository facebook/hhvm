...
final class ImmMap<Tk, +Tv> implements ConstMap<Tk, Tv>, Indexish<Tk, Tv> {
   ...
   *              an ImmMap (e.g., array). If null, then an empty
   *              ImmMap is created.
   */
  public function __construct(?KeyedTraversable<Tk, Tv> $it);

  /**
   * Returns an array containing the key/value pairs from the current ImmMap.
   *
   ...
   *
   * @return - an integer-indexed array where the values are the keys from the
   *           current ImmMap.
   */
  ...
  /**
   * Returns an ImmMap after an operation has been applied to each value in the
   * current ImmMap.
   *
   * Every value in the current ImmMap is affected by a call to map(), unlike
   ...
   *
   * @return - an ImmMap containing the values after a user-specified operation
   *           on the current ImmMap's keys and values is applied.
   */
  public function mapWithKey<Tu>((function(Tk, Tv): Tu) $callback):
    ImmMap<Tk, Tu>;

  /**
   * Returns an ImmMap containing the values of the current ImmMap that meet
   ...
   *                    current ImmMap values.
   *
   * @return - an ImmMap containing the values after a user-specified condition
   *           is applied.
   *
   * @guide /hack/collections/examples
   */
   ...
   *
   * @return - an ImmMap containing the values after a user-specified condition
   *           is applied to the keys and values of the current ImmMap.
   *
   */
   ...
   *
   *  If the number of values of the current ImmMap are not equal to the number
   *  of elements in the Traversable, then only the combined elements up to and
   ...
   *
   *  @return - The ImmMap that combines the values of the current ImmMap with
  ...
  /**
   * Returns an ImmMap containing the first n key/values of the current ImmMap.
   *
   * The returned ImmMap will always be a proper subset of the current ImmMap.
   *
   * n is 1-based. So the first element is 1, the second 2, etc.
   *
   * @param $n - The last element that will be included in the returned ImmMap.
   *
   * @return - An ImmMap that is a proper subset of the current ImmMap up to n
   *           elements.
   */
  public function take(int $n): ImmMap<Tk, Tv>;

  /**
   * Returns an ImmMap containing the keys and values of the current ImmMap up
   * to but not including the first value that produces false when passed to the
   * specified callback.
   *
   * The returned ImmMap will always be a proper subset of the current ImmMap.
   *
   ...
   *
   * @return - An ImmMap that is a proper subset of the current ImmMap up until
   ...
   *              ImmMap.
   *
   * @return - An ImmMap that is a proper subset of the current ImmMap starting
   *           after the callback returns true.
   */
  public function skipWhile((function(Tv): bool) $fn): ImmMap<Tk, Tv>;

  /**
   * Returns a subset of the current ImmMap starting from a given key location
   ...
   * ImmMap and the values of the provided Traversable.
   *
   * The provided Traversable is concatenated to the end of the current ImmMap
   ...
   *
   * @return - A KeyedIterator that allows you to traverse the current ImmMap.
   */
   ...
   *
   * @return - An ImmMap with the key/value pairs from the Traversable; or an
   *           empty ImmMap if the Traversable is null.
   */
