 ...
 * @param mixed $value - An array containing the values or a value if there
 * are more arguments.
 * @return mixed - min() returns the numerically lowest of the parameter
 * values.
 */
function min(... $numbers): mixed {
  if (!sizeof($numbers)) {
    trigger_error("min() expects at least 1 parameter, 0 given", E_WARNING);
  }
  /* min() can either have an array (or some array-like-thing)
   * as the only argument or variadic arguments.
   */
  if (sizeof($numbers) == 1) {
    if (sizeof($numbers[0])) {
      ...
      trigger_error("min(): When only one parameter is given, it must be an array or a collection", E_WARNING);
    }
  }
 ...
 */
function max(... $numbers): mixed {
  if (!sizeof($numbers)) {
    trigger_error("max() expects at least 1 parameter, 0 given", E_WARNING);
  }
  /* max() can either have an array (or some array-like-thing)
   * as the only argument or variadic arguments.
   */
  if (sizeof($numbers) == 1) {
    if (sizeof($numbers[0])) {
      ...
      trigger_error("max(): When only one parameter is given, it must be an array or a collection", E_WARNING);
    }
  }
 ...
 */
<<__Native>>
function base_convert(string $number,
                      int $frombase,
                      int $tobase): mixed;
 ...
 */
<<__Native>>
function srand(mixed $seed = null_variant): void;
 ...
 */
<<__Native>>
function rand(int $min = 0,
              int $max = RAND_MAX): int;
 ...
 */
<<__Native>>
function mt_srand(mixed $seed = null_variant): void;
