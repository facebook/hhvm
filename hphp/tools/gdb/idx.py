...
def compact_vector_at(vec, idx, hasher=None):
    ...
    elems = (vec["m_data"].cast(T("char").pointer()) + vec["elems_offset"]).cast(
        inner.pointer()
    )
    return elems[idx]
...
def idx_accessors():
    return {
        ...
        "boost::container::flat_map": boost_flat_map_at,
        "tbb::interface5::concurrent_hash_map": tbb_chm_at,
...
def idx(container, index, hasher=None):
    ...
    if container_type in accessors:
        ...
        try:
            ...
            print(
                "idx: Unrecognized container (%s - %s)." % (container_type, true_type)
            )
            return None
...
class IdxCommand(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        ...
        ty_parts = [
            x for x in re.split(r"(\s*(?:const\s*)?[*&](?!\s*[>,]))", ty, 1) if x
        ]
        ty_parts[0] = "'%s'" % ty_parts[0]

        gdb.execute("print *(%s)%s" % ("".join(ty_parts), str(value.address)))
