...
void first_pass(const Index& index,
  ...
  for (auto it = blk->hhbcs.begin(), end = blk->hhbcs.end(); it != end; ) {
    ...
    auto speculate = [&] (BlockId target, auto fun) {
      auto depth = interp.state.stack.size();
      auto const new_target = speculateHelper(interp, target);
      speculated = new_target != target || target == blk->fallthrough;
      auto bc = fun(new_target);
      if (bc) {
        gen(*bc);
        if (depth > interp.state.stack.size()) {
          auto state = interp.state;
          state.stack.insert(state.stack.end(),
                             depth - interp.state.stack.size(),
                             { Type{BTrue}, NoLocalId });
          auto const new_block = make_block(ainfo, blk, state);
      ...
      while (depth > interp.state.stack.size()) {
        gen(bc::PopC {});
        --depth;
      }
    };
    ...
    auto fixJmpDest = [&] {
      if (!options.RemoveDeadBlocks || flags.jmpDest == NoBlockId) {
        genOut(&op);
        return;
      }
      ...
      switch (op.op) {
        ...
        case Op::Switch:
          ...
          speculate(
            flags.jmpDest,
            [&] (BlockId jmpDest) -> folly::Optional<Bytecode> {
              gen(bc::PopC {});
            ...
            [&] (BlockId jmpDest) -> folly::Optional<Bytecode> {
              auto set_target = [&] (BlockId t) {
                if (op.op == Op::IterInit) {
                  op.IterInit.target = t;
                } else if (op.op == Op::IterInitK) {
                  op.IterInitK.target = t;
                } else if (op.op == Op::LIterInit) {
                  op.LIterInit.target = t;
                } else if (op.op == Op::LIterInitK) {
                  op.LIterInitK.target = t;
                } else {
                  assertx(false);
                }
              };
        ...
        case Op::LIterNextK:
          ...
          speculate(
            flags.jmpDest,
            [&] (BlockId fallthrough) -> folly::Optional<Bytecode> {
              /*
              ...
              if (op.op == Op::IterNext) {
                op.IterNext.target = fatal;
              } else if (op.op == Op::IterNextK) {
                op.IterNextK.target = fatal;
              } else if (op.op == Op::LIterNext) {
                op.LIterNext.target = fatal;
              } else {
                op.LIterNextK.target = fatal;
              }
              return op;
            }
          );
