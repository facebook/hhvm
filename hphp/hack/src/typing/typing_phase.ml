  ...
  | Tshape (Shape_splat { ss_elems }) ->
    ...
    let (env, ty, sd) =
      ...
      | Empty_shape sd ->
        ...
        in
        (env, mk (r, Tshape (Shape_simple shape)), sd)
      ...
      | Partial ([elem], sd) -> (env, elem, sd)
      | Partial (ss_elems, sd) ->
        (env, mk (r, Tshape (Shape_splat { ss_elems })), sd)
      | Union tys ->
        (* Union operand distributed: members are already finalised (supportdyn
