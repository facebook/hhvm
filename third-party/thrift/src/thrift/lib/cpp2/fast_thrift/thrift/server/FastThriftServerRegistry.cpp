...
namespace apache::thrift::fast_thrift::thrift::instrumentation {
namespace {
...
class TrackerCollection {
 public:
  ...
  TrackerCollection& operator=(const TrackerCollection&) = delete;
  ...
  void removeTracker(std::string_view key, ServerTracker& tracker) {
    servers_.withWLock([&](Map& map) {
      if (auto it = map.find(key); it != map.end()) {
        it->second.erase(&tracker);
      }
    });
  }
...
ServerTracker::~ServerTracker() {
  // join() blocks until every outstanding ServerTrackerRef observation has
  // released its lock, so the server is safe to destroy after this returns.
  cb_.join();
