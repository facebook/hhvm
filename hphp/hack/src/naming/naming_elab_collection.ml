...
let visitor =
  object (self)
    ...
    method! on_Collection env (pos, cname) c_targ_opt afields =
      let res =
        if Nast.is_vc_kind cname then
          ...
          let (fields, fields_err) =
            self#on_list (self#afield_key_value cname) env afields
          in
          let kvc_kind = Nast.get_kvc_kind cname in
          Ok
            ( Aast.KeyValCollection (kvc_kind, targs_opt, fields),
              self#plus targ_err fields_err )
        else if String.equal SN.Collections.cPair cname then
          match afields with
          | [] -> Error (Err.naming @@ Naming_error.Too_few_arguments pos)
          | [fst; snd] ->
            let (targs_opt, targ_err) =
              match c_targ_opt with
              | Some (Aast.CollectionTKV (tk, tv)) -> (Some (tk, tv), self#zero)
            ...
            and (snd, snd_err) =
              self#afield_value SN.Collections.cPair env snd
            in
            let err = self#plus targ_err (self#plus fst_err snd_err) in
            Ok (Aast.Pair (targs_opt, fst, snd), err)
          | _ -> Error (Err.naming @@ Naming_error.Too_many_arguments pos)
        else
          Error (Err.naming @@ Naming_error.Expected_collection { pos; cname })
      in
      match res with
      | Ok (expr_, err) ->
        let (expr_, super_err) = super#on_expr_ env expr_ in
        (expr_, self#plus err super_err)
      | Error err -> (Err.invalid_expr_ pos, err)
