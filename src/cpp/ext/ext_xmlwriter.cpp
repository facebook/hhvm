...
bool c_xmlwriter::t_openmemory() {
  ...
  if (m_output == NULL) {
    Logger::Warning("Unable to create output buffer");
    return false;
  }
...
bool c_xmlwriter::t_startelement(CStrRef name) {
  if (xmlValidateName((xmlChar*)name.data(), 0)) {
    Logger::Warning("invalid element name: %s", name.data());
    return false;
  }
  int ret = -1;
  if (m_ptr) {
...
bool c_xmlwriter::t_startattributens(CStrRef prefix, CStrRef name,
                                     CStrRef uri) {
  if (xmlValidateName((xmlChar*)name.data(), 0)) {
    Logger::Warning("invalid attribute name: %s", name.data());
    return false;
  }
  int ret = -1;
  if (m_ptr) {
...
bool c_xmlwriter::t_startpi(CStrRef target) {
  if (xmlValidateName((xmlChar*)target.data(), 0)) {
    Logger::Warning("invalid PI target: %s", target.data());
    return false;
  }
  int ret = -1;
  if (m_ptr) {
...
bool c_xmlwriter::t_startdtdelement(CStrRef qualifiedname) {
  if (xmlValidateName((xmlChar*)qualifiedname.data(), 0)) {
    Logger::Warning("invalid element name: %s", qualifiedname.data());
    return false;
  }
  int ret = -1;
  if (m_ptr) {
