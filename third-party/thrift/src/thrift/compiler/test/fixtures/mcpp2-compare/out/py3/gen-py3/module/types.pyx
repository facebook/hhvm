...
cdef class ComplexUnion(thrift.py3.types.Union):
    ...
    def __init__(
        ...
        if a_struct is not None:
            if not isinstance(a_struct, MyStruct):
                a_struct = _from_python_or_raise(a_struct, "a_struct", MyStruct)
        if a_union is not None:
            if not isinstance(a_union, SimpleUnion):
                a_union = _from_python_or_raise(a_union, "a_union", SimpleUnion)
        if opt_a_union is not None:
            if not isinstance(opt_a_union, SimpleUnion):
                opt_a_union = _from_python_or_raise(opt_a_union, "opt_a_union", SimpleUnion)
        if ref_field is not None:
            if not isinstance(ref_field, MyStruct):
                ref_field = _from_python_or_raise(ref_field, "ref_field", MyStruct)
        if ref_field2 is not None:
            if not isinstance(ref_field2, MyStruct):
                ref_field2 = _from_python_or_raise(ref_field2, "ref_field2", MyStruct)
        if excp_field is not None:
            if not isinstance(excp_field, AnException):
                excp_field = _from_python_or_raise(excp_field, "excp_field", AnException)
