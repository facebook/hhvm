...
class ReflectionParameter implements Reflector {
   ...
   *
   * @return     bool   TRUE if the parameter is named, otherwise FALSE
   */
  public function isNamed()[]: bool {
    return ($this->info['named'] ?? false);
  }
  ...
  public function allowsNull()[]: bool {
    return $this->info['type_hint_nullable'];
  }
...
class ReflectionType implements IPureStringishObject {
  ...
  public function isBuiltin()[]: bool {
    return self::isTypeBuiltin(idx($this->type_hint_info, 'name', ''));
  }

  public static function isTypeBuiltin(string $type_hint)[]: bool {
    if ($type_hint === '') return false;
    // Strip the soft (@) and nullable (?) prefixes (in that order, as
    // rendered by the runtime), then the HH\ namespace, then any generics.
    $stripped = $type_hint;
    if ($stripped[0] === '@') $stripped = substr($stripped, 1);
    if ($stripped[0] === '?') $stripped = substr($stripped, 1);
    if (strncmp($stripped, 'HH\\', 3) === 0) $stripped = substr($stripped, 3);
    $angle = strpos($stripped, '<');
    if ($angle !== false) $stripped = substr($stripped, 0, $angle);
    switch ($stripped) {
      case 'bool':
      case 'int':
      case 'float':
