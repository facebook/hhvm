...
let expect_absent key =
  expect
    ~msg:(Printf.sprintf "Expected key '%s' to be absent from hashtable" key)
  @@ (get_status key = -1)

let expect_removed key =
  expect
    ~msg:(Printf.sprintf "Expected key '%s' to be removed from hashtable" key)
  @@ (get_status key = -2)
...
let expect_get key expected =
  ...
  expect
    ~msg:
      (Printf.sprintf
         "Expected key '%s' to have value '%s', got '%s"
         key
         expected
         value)
    (value = expected)
