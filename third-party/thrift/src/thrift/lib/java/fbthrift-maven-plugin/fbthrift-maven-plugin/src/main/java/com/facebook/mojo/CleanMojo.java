...
public class CleanMojo extends AbstractMojo {
  ...
  private void deleteEmptyDirectory(File f) throws IOException {
    ...
    // // Note that `Files.delete()` promises to fail on non-empty directories.
    // try {
    //   Files.delete(generatedSourcesDirectory.toPath());
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    try {
      ...
      if (genChecksums.exists()) deleteFile(genChecksums);

      // Then delete the code
      File genSource = new File(generatedSourcesDirectory, "gen-java");
      if (genSource.exists()) FileUtils.deleteDirectory(genSource);
