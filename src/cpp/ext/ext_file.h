...
/**
 * Two departures from PHP:
 *
 * 1. Stream model: There are only 3 major types, local files/dirs, sockets and
 *    URL-based resources. Mixing them up doesn't always make sense. For
 *    example, many file system functions don't apply to URL at all, yet
 *    suffering extra layers of coding. We here break them up into separate
 *    classes and functions: local file functions are grouped under File
 *    class, URL handling is left to CURL extension functions, and socket
 *    functions are just plain socket functions.
 *
 * 2. Stream context/filter/protocol/bucket: not supported until we really
 *    find good use of it, then we may implement them in transport layers in
 *    a different way.
 */
namespace HPHP {
///////////////////////////////////////////////////////////////////////////////
...
bool f_mkdir(CStrRef pathname, int64 mode = 0, bool recursive = false,
             CObjRef context = null_object);
