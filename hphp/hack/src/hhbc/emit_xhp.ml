...
let emit_xhp_attribute_array ~ns xal =
  ...
  let get_enum_attributes enumo =
    ...
    | Some (_, es) -> Tast_annotate.make (T.Varray (None, es)) in
  let get_attribute_array_values id enumo =
    ...
    let class_name =
      ...
      | _ -> Tast_annotate.make T.Null in
    class_name, type_ident in
  let inner_array ho expo enumo is_req =
    ...
    let class_name, hint =
      ...
      | _ -> failwith "There are no other possible xhp attribute hints" in
    let is_required = Tast_annotate.make (T.Int (if is_req then "1" else "0")) in
  ...
  let aux xa =
    ...
    let v = Tast_annotate.make (T.Varray (None, (inner_array ho expo enumo is_req))) in
    (k, v)
  in
...
let emit_xhp_use_attributes xual =
  let aux xua (* A cute little palindrome :P *) =
    ...
    | _, Aast.Happly ((_, s), []) ->
      ...
      let e =
        Tast_annotate.make (T.Class_const
          ((Tast_annotate.make (T.CIexpr (Tast_annotate.make (T.Id (p, s))))),
          (p, "__xhpAttributeDeclaration"))) in
...
let properties_for_cache ~ns class_ class_is_const =
  ...
  let prop =
    Emit_property.from_ast
      ...
      ns
      None
      (p, (p, "__xhpAttributeDeclarationCache"), Some init_value) in
  [ prop ]
...
let from_attribute_declaration ~ns class_ xal xual =
  ...
  let cache =
    Tast_annotate.make
      (T.Class_get
        ...
        T.CGstring (p, "$__xhpAttributeDeclarationCache"))) in
  let token1 = p, T.Expr (Tast_annotate.make (T.Binop (A.Eq None, var_r, cache))) in
  (* if ($r === null) {
  ...
  let arg1 =
    Tast_annotate.make (T.Call
      (Aast.Cnormal,
      Tast_annotate.make (T.Class_const
        ((Tast_annotate.make (T.CIexpr (Tast_annotate.make (T.Id (p, "parent"))))),
        (p, "__xhpAttributeDeclaration"))),
      [],
      [],
      [])) in
  let args =
    arg1 :: emit_xhp_use_attributes xual @ [emit_xhp_attribute_array ~ns xal] in
  let array_merge_call =
    Tast_annotate.make (T.Call
      (Aast.Cnormal,
      Tast_annotate.make (T.Id (p, "__SystemLib\\merge_xhp_attr_declarations")),
      [],
      args,
      [])) in
  let set_cache = p, T.Expr (Tast_annotate.make (T.Binop (A.Eq None, cache, array_merge_call))) in
  let set_r = p, T.Expr (Tast_annotate.make (T.Binop (A.Eq None, var_r, cache))) in
...
let emit_xhp_children_paren_expr c =
  let l, op_num =
    ...
    | Aast.ChildUnary (Aast.ChildList l, op) -> l, xhp_child_op_to_int @@ Some op
    | _ ->
      failwith @@ "Xhp children declarations cannot be plain id, " ^
                  "plain binary or unary without an inside list" in
...
let get_category_array categories =
  ...
  List.map
    ~f:(fun s -> (Tast_annotate.make (T.String s), Tast_annotate.make (T.Int "1")))
    categories
