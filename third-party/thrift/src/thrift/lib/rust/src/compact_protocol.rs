...
/// Serialize a Thrift value using the compact protocol from a reference.
...
/// Serialize a Thrift value using the compact protocol to a pre-allocated buffer from a reference.
/// This will panic if the buffer is not large enough. A buffer at least as
/// large as the return value of `serialize_size` will not panic.
#[inline]
pub fn serialize_to_buffer_ref<T>(v: &T, buffer: BytesMut) -> CompactProtocolSerializer<BytesMut>
where
    T: SerializeRef,
{
    let mut buf = CompactProtocolSerializer {
