...
// A reference to a non-optional boxed field.
template <typename T>
class boxed_field_ref {
  static_assert(std::is_reference<T>::value, "not a reference");
  static_assert(
      detail::is_boxed_value_ptr<folly::remove_cvref_t<T>>::value,
      "not a boxed_value_ptr");
 ...
 public:
  ...
  template <
      ...
      std::enable_if_t<
          std::is_same<T, U&&>{} || std::is_same<T, const U&&>{},
          int> = 0>
  FOLLY_ERASE explicit boxed_field_ref(
      const boxed_field_ref<U&>& other) noexcept
      : value_(other.value_) {}

  template <typename U = value_type>
  FOLLY_ERASE std::
      enable_if_t<std::is_assignable<value_type&, U&&>::value, boxed_field_ref&>
      operator=(U&& value) {
    value_ = static_cast<U&&>(value);
    return *this;
  }

  // Assignment from boxed_field_ref is intentionally not provided to
  // prevent potential confusion between two possible behaviors, copying and
  // reference rebinding. This copy_from method is provided instead.
  template <typename U>
  FOLLY_ERASE void copy_from(const boxed_field_ref<U>& other) {
    value_ = T(other.value_);
  }
  ...
  FOLLY_ERASE bool is_set() const noexcept { return static_cast<bool>(value_); }
  ...
  FOLLY_ERASE value_type* operator->() { return &*value_; }
...
template <typename T1, typename T2>
bool operator==(boxed_field_ref<T1> a, boxed_field_ref<T2> b) {
  return *a == *b;
}
...
template <typename T, typename U>
bool operator==(boxed_field_ref<T> a, const U& b) {
  return *a == b;
}
