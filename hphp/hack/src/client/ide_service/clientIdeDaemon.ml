...
type message =
  | Message: 'a ClientIdeMessage.t -> message
...
let set_up_hh_logger_for_client_ide_service ~(root: Path.t): unit =
  ...
  begin try
    Sys.rename client_ide_log_fn (client_ide_log_fn ^ ".old")
  with _e ->
    ()
  end;
  Hh_logger.set_log client_ide_log_fn (Out_channel.create
    client_ide_log_fn
    ~append:true
  );
...
let load_naming_table_from_saved_state_info
    ...
    : ServerEnv.env Lwt.t =
  let path =
    Saved_state_loader.Naming_table_saved_state_info.(Path.to_string
      saved_state_info.naming_table_path) in
...
let load_saved_state
  ...
  let%lwt result =
    try%lwt
      let%lwt result = match naming_table_saved_state_path with
        | Some naming_table_saved_state_path ->
          ...
          Lwt.return_ok ({ Saved_state_loader.Naming_table_saved_state_info.
            naming_table_path = naming_table_saved_state_path;
          }, changed_files)
        | None ->
          let%lwt result = State_loader_lwt.load
            ~repo:root
            ~saved_state_type:Saved_state_loader.Naming_table in
          Lwt.return result
      in
      let%lwt new_state = match result with
        | Ok (saved_state_info, changed_files) ->
          log "[saved-state] Naming table path: %s"
            Saved_state_loader.Naming_table_saved_state_info.(Path.to_string
              saved_state_info.naming_table_path);

          let%lwt server_env = load_naming_table_from_saved_state_info
            env saved_state_info in
          log "[saved-state] Load succeeded";

          Lwt.return (Initialized {
            saved_state_info;
            hhi_root;
            server_env;
    ...
    with e ->
      ...
      Hh_logger.exc e
        ...
        ~stack;
      Lwt.return (Failed_to_initialize (Printf.sprintf
        "Uncaught exception in client IDE services: %s" stack))
  in
  Lwt.return result
...
let initialize
  ...
  let (server_config, server_local_config) = ServerConfig.load
    ServerConfig.filename server_args in
  ...
  let _: SharedMem.handle = SharedMem.init
    ~num_workers:0
    (ServerConfig.sharedmem_config server_config)
  in
  ...
  let namespace_map = GlobalOptions.po_auto_namespace_map server_env.ServerEnv.tcopt in
  let sienv = SymbolIndex.initialize
    ~globalrev_opt:None
    ~namespace_map
    ~provider_name:server_local_config.ServerLocalConfig.symbolindex_search_provider
    ~quiet:server_local_config.ServerLocalConfig.symbolindex_quiet
    ~savedstate_file_opt:server_local_config.ServerLocalConfig.symbolindex_file
    ~workers:None in
  let sienv = {
    sienv with
  ...
  let%lwt new_state = load_saved_state server_env
    ~root ~hhi_root ~naming_table_saved_state_path in
...
let make_context_from_document_location
  ...
  let (file_path, file_input) = match document_location with
    | { ClientIdeMessage.file_contents = None; file_path; _ } ->
      let file_input =
        ServerCommandTypes.FileName (Path.to_string file_path) in
...
let handle_message: type a.
  ...
  | ((Failed_to_initialize _ | Initializing), File_changed _) ->
    (* Should not happen. *)
    Lwt.return (state, Handle_message_result.Error (
  ...
  | (Initialized ({
      ...
      _
    } as state), File_changed path) ->
    let changed_files_to_process = Path.Set.add changed_files_to_process path in
  ...
  | (Initialized { server_env; _ },
     Completion { ClientIdeMessage.Completion.
       document_location = { ClientIdeMessage.
         file_path;
         file_contents;
         line;
         column;
    ...
    let path = file_path
      |> Path.to_string
      |> Relative_path.create_detect_prefix in
    let file_content = match file_contents with
      | Some file_contents ->
        file_contents
      | None ->
        file_path
          |> Path.to_string
          |> Sys_utils.cat_no_fail
    in
    let sienv = !(server_env.ServerEnv.local_symbol_table) in
    let matches = ServerAutoComplete.auto_complete_at_position_ctx
      ~line
      ~column
      ~file_content
      ~path
      ...
      ~is_manually_invoked
    in
    let result = { AutocompleteTypes.
      completions = matches.Utils.With_complete_flag.value;
      char_at_pos = ' ';
  ...
  | (Initialized { server_env; _ }, Completion_resolve param) ->
    let open ClientIdeMessage.Completion_resolve in
    let start_time = Unix.gettimeofday () in
    let result = ServerDocblockAt.go_docblock_for_symbol
      ~env:server_env
      ~symbol:param.symbol
      ~kind:param.kind
    in
    let sienv = !(server_env.ServerEnv.local_symbol_table) in
    if sienv.SearchUtils.sie_log_timings then begin
      let _t: float =
        Hh_logger.log_duration (Printf.sprintf "[docblock] Search for [%s] [%s]"
        param.symbol (SearchUtils.show_si_kind param.kind)) start_time in
      ()
    end;
    Lwt.return (state, Handle_message_result.Response result)
...
let write_message
    ...
    : unit Lwt.t =
  let%lwt _: int =
    Marshal_tools_lwt.to_fd_with_preamble out_fd message in
  Lwt.return_unit
...
let serve
  ...
  let rec pump_message_queue (message_queue: message_queue): unit Lwt.t =
    try%lwt
      let%lwt (message: a ClientIdeMessage.t) =
        Marshal_tools_lwt.from_fd_with_preamble in_fd in
      let is_queue_open =
        Lwt_message_queue.push message_queue (Message message) in
      match message with
      | ClientIdeMessage.Shutdown () ->
        Lwt.return_unit
      | _ when not is_queue_open ->
        Lwt.return_unit
      | _ ->
        pump_message_queue message_queue
    with e ->
  ...
  let rec handle_messages (t: t): unit Lwt.t =
    match t with
    | { message_queue;
        state = Initialized ({ server_env; changed_files_to_process; _ }
          as state); }
      ...
      let changed_files_to_process =
        Path.Set.remove changed_files_to_process next_file in
      let%lwt server_env =
        ClientIdeIncremental.process_changed_file server_env next_file in
      let%lwt () =
        if Path.Set.is_empty changed_files_to_process
        then
