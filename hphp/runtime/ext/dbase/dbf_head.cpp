...
int put_dbf_field(dbhead_t *dbh, dbfield_t *dbf)
{
	...
	dbase_strlcpy(dbfield.dbf_name, dbf->db_fname, DBF_NAMELEN + 1);
...
void put_dbf_info(dbhead_t *dbh)
{
	...
	if ((cp = db_cur_date(NULL))) {
		dbase_strlcpy(dbh->db_date, cp, 9);
		free(cp);
	}
	put_dbf_head(dbh);
	dbf = dbh->db_fields;
...
char *get_dbf_f_fmt(dbfield_t *dbf)
{
	...
	switch (dbf->db_type) {
	   ...
	   case 'F':
		...
		break;
	   case 'M':
		dbase_strlcpy(format, "%s", sizeof(format));
		break;
	   default:
		return NULL;
	}
...
dbhead_t *dbf_open(const char *dp, int o_flags)
{
	...
	if ((fd = open(dp, o_flags|O_BINARY)) < 0) {
		return NULL;
	}
