...
let update_json_data predicate json json_data_progress =
  ...
  | SymbolOccurrence ->
      { json_data_progress with symbolOccurrence = json::json_data_progress.symbolOccurrence }
...
let json_of_symbol_occurrence_shared symbol_occurrence progress symbol_def =
  ...
  let symbol_kind, class_attribute = match symbol_occurrence.type_ with
    | Class -> 0, JSON_Null
    | Function -> 1, JSON_Null
    | Method (s1, s2) -> 2, JSON_Object ([
        "class_name", key_ (JSON_String(s1));
        "property_name", key_ (JSON_String(s2));
      ])
    | LocalVar -> 3, JSON_Null
    | Property (s1, s2) -> 4, JSON_Object ([
        "class_name", key_ (JSON_String(s1));
        "property_name", key_ (JSON_String(s2));
      ])
    | ClassConst (s1, s2) -> 5, JSON_Object ([
        "class_name", key_ (JSON_String(s1));
        "property_name", key_ (JSON_String(s2));
      ])
    | Typeconst (s1, s2) -> 6, JSON_Object ([
        "class_name", key_ (JSON_String(s1));
        "property_name", key_ (JSON_String(s2));
      ])
    | GConst -> 7, JSON_Null
  in
  let type_object = match class_attribute with
...
let json_of_symbol_occurrence symbol_occurrence progress =
  ...
  let symbol_def_pos = match symbol_def with
    | None -> None
...
let json_of_localvars progress lv =
  ...
  List.fold lv.lvs ~init:progress ~f:begin fun acc sym_occ ->
    snd (json_of_symbol_occurrence_shared sym_occ acc symbol_def)
  end

let build_json tcopt symbols =
  let json_data_progress =
    List.fold symbols.decls ~init:default_json ~f:begin fun acc symbol ->
      match symbol with
      | Fun fd ->
  ...
  let json_data_progress =
    List.fold symbols.occurrences ~init:json_data_progress ~f:begin fun acc occ ->
      if occ.is_declaration then acc
      else
