...
/* TODO: mysqli.c needs to export a few more constants - see all the defined() calls! */
	...
	foreach ($columns as $column_def => $expected_flags) {
		...
		$create = sprintf('CREATE TABLE test_mysqli_fetch_field_flags_table_1(id INT, col1 %s)', $column_def);
		if (!mysqli_query($link, $create)) {
			// Server might not support it - skip
			continue;
		}

		if (!$res = mysqli_query($link, 'SELECT * FROM test_mysqli_fetch_field_flags_table_1')) {
		...
		results.The test_mysqli_fetch_field_flags_table_1 does not yet fully reflect all server changes/bugs etc.
		*/
		switch ($column_def) {
