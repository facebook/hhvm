...
let on_class_get visitor (_, cid_) cge ~is_call_target =
  ...
  | T.CIparent
  | T.CIself
  | T.CIstatic -> failwith "Expects CIexpr as class_id on aast where expr was on ast"
  | T.CI _sid -> failwith "Expects CIexpr as class_id on aast where expr was on ast"
  | T.CIexpr e ->
    ...
    begin
      ...
      | T.CGstring pstr ->
         ...
         *)
        if is_call_target
        then visitor#set_state (add_local ~barethis:Bare_this visitor#state pstr)
        else ()
      ...
      | Aast.Try (body, catch_list, finally) ->
         ...
         List.iter catch_list
           (fun (_exn_name, (pos, id), catch_body) ->
             state := add_local ~barethis:Bare_this_as_ref !state (pos, Local_id.get_name id);
...
let uls_from_ast
    ~is_closure_body ~has_this ~params ~is_toplevel ~is_in_static_method
    ~get_param_name ~get_param_default_value ~explicit_use_set_opt (b: Tast.program) =
  let visitor =
    declvar_visitor explicit_use_set_opt is_in_static_method is_closure_body in
...
let from_ast
    ...
    ~is_in_static_method ~explicit_use_set (b : Tast.program) =
  let needs_local_this, decl_vars =
    uls_from_ast
      ...
      ~get_param_default_value:
        (fun p -> Option.map (Hhas_param.default_value p) ~f:snd)
      ~explicit_use_set_opt:(Some explicit_use_set)
      b in
  needs_local_this, ULS.items decl_vars

let vars_from_ast
  ~is_closure_body ~has_this ~(params : Tast.fun_param list) ~is_toplevel ~is_in_static_method (b: Tast.program) =
  let _, decl_vars =
    uls_from_ast
      ~is_closure_body
      ~has_this
