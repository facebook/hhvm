...
Type getValType(IterSpecialization specialization) {
  switch (specialization.base_type) {
    case IterSpecialization::Packed: return TInitGen;
    case IterSpecialization::Mixed:  return TInitGen;
    case IterSpecialization::Vec:    return TInitCell;
    case IterSpecialization::Dict:   return TInitCell;
  }
  always_assert(false);
}
...
struct PackedAccessor : public Accessor {
  explicit PackedAccessor(IterSpecialization specialization) {
    ...
    arr_type = is_hack_arr ? TVec : Type::Array(ArrayData::kPackedKind);
    pos_type = is_ptr_iter ? getValType(specialization).ptr(Ptr::Elem) : TInt;
...
struct MixedAccessor : public Accessor {
  explicit MixedAccessor(IterSpecialization specialization) {
    ...
    arr_type = is_hack_arr ? TDict : Type::Array(ArrayData::kMixedKind);
    pos_type = is_ptr_iter ? getValType(specialization).ptr(Ptr::Elem) : TInt;
...
// Iterator output locations are usually cells, which we check in canSpecialize
// before specializing an IterNext. Hence, we can use a type-param of TCell.
...
void profileDecRefs(IRGS& env, const IterData& data, SSATmp* base,
  ...
  auto const val = gen(env, LdLoc, TGen, LocalId(data.valId), fp(env));
  ...
  auto const key = [&]{
    auto const tmp = gen(env, LdLoc, TGen, LocalId(data.keyId), fp(env));
...
void emitSpecializedHeader(IRGS& env, const Accessor& accessor,
  ...
  iterIfThen(env,
    ...
    [&]{
      finish(elm, unbox(env, val, nullptr));
...
bool canSpecialize(IRGS& env, const IterData& data) {
  return areLocalsCells(env, data) && !curFunc(env)->isPseudoMain();
}
...
ArrayIterProfile::Result getProfileResult(IRGS& env, const SSATmp* base) {
  ...
  if (result.value_type.maybe(TBoxedCell)) return generic;
...
// Returns true if the outputs of a given iterator are known to be cells.
bool areLocalsCells(IRGS& env, const IterData& data) {
  auto const cell = [&](uint32_t local) {
    if (local == kInvalidId) return true;
...
void specializeIterInit(IRGS& env, Offset doneOffset,
  ...
  if (!canSpecialize(env, data)) return despecialize();
...
bool specializeIterNext(IRGS& env, Offset loopOffset,
  ...
  if (!canSpecialize(env, data)) return false;
  ...
  if (iter->footer == nullptr) {
    ...
    emitSpecializedFooter(env, *accessor, data, iter->header, baseLocalId);
    iter->footer = footer;
  }
  return true;
}
