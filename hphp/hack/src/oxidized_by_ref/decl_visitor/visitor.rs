...
//
// @generated SignedSource<<33ab9702d02ee13094f27f98d176a563>>
//
...
pub trait Visitor<'a> {
    ...
    fn visit_decl_constraint_requirement(&mut self, p: &'a DeclConstraintRequirement<'a>) {
        p.recurse(self.object())
    }
    ...
    fn visit_witness_decl(&mut self, p: &'a WitnessDecl<'a>) {
        p.recurse(self.object())
    }
    ...
    fn visit_pessimise_reason(&mut self, p: &'a PessimiseReason) {
        p.recurse(self.object())
    }
    ...
    fn visit_ctor_kind(&mut self, p: &'a CtorKind) {
        p.recurse(self.object())
    }
