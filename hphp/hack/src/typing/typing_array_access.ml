...
let rec array_get ~array_pos ~expr_pos ?(lhs_of_null_coalesce=false)
  ...
  | Tintersection tyl ->
      (* If any of the types is valid for an array get, then this should be valid. *)
      let env, tyl = TUtils.run_on_intersection env tyl ~f:(fun env ty1 ->
        array_get ~array_pos ~expr_pos ~lhs_of_null_coalesce is_lvalue env ty1 e2 ty2) in
