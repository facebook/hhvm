...
let class_get_cursor_finder =
  object
    ...
    method! on_Class_const _ _ mid = matches_auto_complete_suffix (snd mid)

    method! on_Obj_get _ _ mid _ =
      match mid with
      | (_, Tast.Id mid) -> matches_auto_complete_suffix (snd mid)
      | _ -> false

    method! on_Class_get _ _ mid =
      match mid with
      | Tast.CGstring p -> matches_auto_complete_suffix (snd p)
      | _ -> false
  end
...
class ranking_context_extract =
  object (self)
    ...
    val mutable after_cursor : bool = false

    val mutable tokens : string list = []

    val mutable token_types : string list = []

    method get_context tast =
      self#go tast;
      ( tokens |> List.rev |> String.concat ~sep:" ",
        token_types |> List.rev |> String.concat ~sep:" " )

    method add_token token =
      if visit_method && not after_cursor then tokens <- token :: tokens

    method add_token_type token_type =
      if visit_method && not after_cursor then
    ...
    method! on_method_ env m =
      if class_get_cursor_finder#on_method_ () m then (
        ...
        self#add_token (snd m.Tast.m_name);
