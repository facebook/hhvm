...
struct lexer_scan_window {
  ...
  lexer_scan_window with_start(cursor start) const {
    assert(start <= head);
    return lexer_scan_window(*src, start, head);
  }
  lexer_scan_window with_head(cursor head) const {
    assert(start <= head);
    return lexer_scan_window(*src, start, head);
  }
...
class lexer {
 ...
 private:
  ...
  // parses raw text
  class state_text;
  // parses interpolation of variables "{{ ... }}"
