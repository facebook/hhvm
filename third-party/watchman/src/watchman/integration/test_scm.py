...
o  changeset:   0:b08db10380dd
   ...
   summary:     initial
    ...
    def test_scmHgIgnoredFile(self) -> None:
        self.skipIfNoFSMonitor()
        ...
        self.touchRelative(root, "ignored-foo")

        # Ensure the file is actually ignored by hg ( = it is not deleted by hg
        # purge). This guards against test environments where the ui.ignore
        # setting is not ".gitignore", which would make the test silently PASS.
        self.hg(["purge"], cwd=root)
        ...
        self.assertFileListsEqual(res["files"], ["ignored-foo"])
