...
class Client<::cpp2::MyService> : public apache::thrift::GeneratedAsyncClient {
  static_assert(!folly::is_detected_v<detail::st::detect_complete, ::cpp2::MyService>, "Definition collision with service tag. Either rename the Thrift service using @cpp.Name annotation or rename the conflicting C++ type.");
 public:
 ...
 private:
  ...
  folly::coro::Task<void> co_lobDataById(apache::thrift::RpcOptions* rpcOptions, ::std::int64_t p_id, const ::std::string& p_dataStr) {
    ...
    if constexpr (hasRpcOptions) {
      ...
      fbthrift_serialize_and_send_lobDataById(*defaultRpcOptions, header, ctx.get(), std::move(wrappedCallback), p_id, p_dataStr);
    }
    if (cancellable) {
      folly::CancellationCallback cb(cancelToken, [&] { CancellableCallback::cancel(std::move(cancellableCallback)); });
      co_await callback.co_waitUntilDone();
 ...
 private:
  ...
  folly::coro::Task<void> co_cppDoNothing(apache::thrift::RpcOptions* rpcOptions) {
    ...
    if constexpr (hasRpcOptions) {
      ...
      fbthrift_serialize_and_send_cppDoNothing(*defaultRpcOptions, header, ctx.get(), std::move(wrappedCallback));
    }
    if (cancellable) {
      folly::CancellationCallback cb(cancelToken, [&] { CancellableCallback::cancel(std::move(cancellableCallback)); });
      co_await callback.co_waitUntilDone();
 ...
 private:
  ...
  template <typename RpcOptions>
  void fbthrift_send_cppDoNothing(apache::thrift::SerializedRequest&& request, RpcOptions&& rpcOptions, std::shared_ptr<apache::thrift::transport::THeader> header, apache::thrift::RequestClientCallback::Ptr callback, std::unique_ptr<folly::IOBuf> interceptorFrameworkMetadata);
