...
val localize_with_self : env -> decl ty -> env * locl ty

val localize_with_self_possibly_enforceable :
  env -> decl ty -> env * locl possibly_enforced_ty

val localize : ety_env:expand_env -> env -> decl ty -> env * locl ty
...
val localize_hint_with_self : env -> Aast.hint -> env * locl ty

val localize_hint : ety_env:expand_env -> env -> Aast.hint -> env * locl ty
...
val localize_with_dty_validator :
  env -> decl ty -> (expand_env -> decl ty -> unit) -> env * locl ty

val sub_type_decl :
  env -> decl ty -> decl ty -> Errors.typing_error_callback -> unit

val unify_decl :
  env -> decl ty -> decl ty -> Errors.typing_error_callback -> unit

val check_tparams_constraints :
  use_pos:Pos.t -> ety_env:expand_env -> env -> decl tparam list -> env
...
val decl : decl ty -> phase_ty

val locl : locl ty -> phase_ty
