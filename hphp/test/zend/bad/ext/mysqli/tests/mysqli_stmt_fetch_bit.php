<?php
	...
	for ($bits = 1; $bits < 32; $bits++) {
		...
		$tests = 0;
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_stmt_fetch_bit_table_1") ||
			!mysqli_query($link, $sql = sprintf('CREATE TABLE test_mysqli_stmt_fetch_bit_table_1(id INT, label BIT(%d)) ENGINE="%s"', $bits, $engine)))
		...
		while ($tests < min($max_value, 20)) {
			...
			$sql = sprintf("INSERT INTO test_mysqli_stmt_fetch_bit_table_1(id, label) VALUES (%d, b'%s')", $value, decbin($value));
			...
			$sql = sprintf("SELECT id, label + 0 AS _label0, label FROM test_mysqli_stmt_fetch_bit_table_1 WHERE id = %d", $value);
			if (!mysqli_stmt_prepare($stmt, $sql) ||
					!mysqli_stmt_execute($stmt) ||
...
?>
<?php
	$test_table_name = 'test_mysqli_stmt_fetch_bit_table_1'; require_once("clean_table.inc");
?>