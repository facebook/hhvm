...
static std::vector<char> memcache_prepare_for_storage(const MemcacheData* data, const Variant& var, int &flag) {
  String v;
  if (var.isString()) {
    v = var.toString();
...
static bool HHVM_METHOD(Memcache, set, const String& key, const Variant& var,
  ...
  auto data = Native::data<MemcacheData>(this_);
  String serializedKey = memcache_prepare_key(key);
  std::vector<char> serializedVar = memcache_prepare_for_storage(data, var, flag);
