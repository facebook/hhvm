        ...
        | (r_super, Tdestructure { d_required; d_optional; d_variadic; d_kind })
          ...
          let destructure_tuple r ts env =
            ...
            if len_ts < len_required then
              ...
              let (ts_optional, ts_variadic) =
                List.split_n remain len_optional
              in
              List.fold2_exn
                ts_required
                d_required
...
let sub_type_with_dynamic_as_bottom env ty_sub ty_super on_error =
  ...
  in
  let (env, ty_err) =
    Subtype.prop_to_env (LoclType ty_sub) (LoclType ty_super) env prop on_error
  in
