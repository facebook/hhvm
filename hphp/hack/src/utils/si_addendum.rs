...
pub fn get_si_addenda(
    with_hashes: &oxidized::direct_decl_parser::ParsedFileWithHashes,
) -> Vec<SiAddendum> {
    with_hashes
        .iter()
...
/// From a `ParsedFileWithHashes<'a>` generated by some decl parse, collect all
/// the necessary information to update the symbol index.
pub fn get_si_addenda_obr<'a>(with_hashes: &ParsedFileWithHashes<'a>) -> Vec<SiAddendum> {
    with_hashes
        .iter()
        .filter_map(|(name, decl, _hash, _sort_text)| {
            use oxidized::file_info::SiKind::*;
