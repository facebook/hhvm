<?php
	...
	function func_mysqli_stmt_get_result_geom($link, $engine, $sql_type, $bind_value, $offset) {
		...
		while ($row = mysqli_fetch_assoc($res)) {
			...
			if (!$res_normal = mysqli_query($link, sprintf("SELECT id, label FROM test_mysqli_stmt_get_result_geom_table_1 WHERE id = %d",
					$row['id'] + 10))) {
...
?>
<?php
	$test_table_name = 'test_mysqli_stmt_get_result_geom_table_1'; require_once("clean_table.inc");
?>