...
let enum_contents_with_one_more_field =
  {|<?hh // strict
enum MyEnum : string {
  FIELD_1 = "field 1"
  FIELD_2 = "field 2"
...
let enum_users =
  List.init 10 ~f:(fun n -> (enum_user_name n, enum_user_contents n))
...
let init_disk_contents =
  [(enum_name, enum_contents); (enum_switch_name, enum_switch_contents)]
  @ enum_users
...
let test_ok saved_state_dir () =
  ...
  let (env, _) =
    Test.(
      run_loop_once
        env
        { default_loop_input with disk_changes = [(enum_name, enum_contents)] })
  in
