...
bool TestExtMysql::RunTests(const std::string &which) {
  ...
  return true;
// Some of the mysql methods have been moved to PHP, so they cannot be tested.
// As these tests aren't actually used, I've commented them out instead of
// changing the calls to go through the VM.
#if 0
  bool ret = true;
...
static bool CreateTestTable() {
  HHVM_FN(mysql_select_db)(TEST_DATABASE);
  HHVM_FN(mysql_query)("drop table test");
  return HHVM_FN(mysql_query)("create table test (id int not null auto_increment,"
...
bool TestExtMysql::test_mysql_connect() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!same(conn, false));
  return Count(true);
}

bool TestExtMysql::test_mysql_pconnect() {
  Variant conn = HHVM_FN(mysql_pconnect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!same(conn, false));
  return Count(true);
}

bool TestExtMysql::test_mysql_set_charset() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME,
                                 TEST_PASSWORD);
  VERIFY(HHVM_FN(mysql_set_charset)("utf8", conn));
  return Count(true);
}

bool TestExtMysql::test_mysql_ping() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(HHVM_FN(mysql_ping)());
  return Count(true);
}

bool TestExtMysql::test_mysql_escape_string() {
  String item = "Zak's Laptop";
  String escaped_item = HHVM_FN(mysql_escape_string)(item);
...
bool TestExtMysql::test_mysql_real_escape_string() {
  {
    Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME,
                                   TEST_PASSWORD);
    String item = "Zak's Laptop";
    String escaped_item = HHVM_FN(mysql_real_escape_string)(item);
    VS(escaped_item, "Zak\\'s Laptop");
    HHVM_FN(mysql_close)(conn);
    VS(HHVM_FN(mysql_real_escape_string)(item), false);
  }
  return Count(true);
}
...
bool TestExtMysql::test_mysql_client_encoding() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VS(HHVM_FN(mysql_client_encoding)(), "latin1");
  return Count(true);
}

bool TestExtMysql::test_mysql_close() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(HHVM_FN(mysql_close)(conn));
  return Count(true);
}

bool TestExtMysql::test_mysql_errno() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!HHVM_FN(mysql_select_db)("nonexistentdb").toBoolean());
  VS(HHVM_FN(mysql_errno)(conn), 1049);
  return Count(true);
}
...
bool TestExtMysql::test_mysql_error() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!HHVM_FN(mysql_select_db)("nonexistentdb").toBoolean());
  VS(HHVM_FN(mysql_error)(conn), "Unknown database 'nonexistentdb'");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_warning_count() {
  ...
  VS(HHVM_FN(mysql_warning_count)(conn), 0);
  VS(HHVM_FN(mysql_query)("INSERT INTO test (name) VALUES ('test'),('test2')"), true);
  VS(HHVM_FN(mysql_warning_count)(conn), 0);

  // Dropping a non-existent table with IF EXISTS generates a warning.
  VS(HHVM_FN(mysql_query)("DROP TABLE IF EXISTS no_such_table"), true);
  VS(HHVM_FN(mysql_warning_count)(conn), 1);

  // Dropping an existing table generates no warnings.
  VS(HHVM_FN(mysql_query)("DROP TABLE IF EXISTS test"), true);
  VS(HHVM_FN(mysql_warning_count)(conn), 0);
...
bool TestExtMysql::test_mysql_get_client_info() {
  VERIFY(!HHVM_FN(mysql_get_client_info)().empty());
  return Count(true);
}

bool TestExtMysql::test_mysql_get_host_info() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!HHVM_FN(mysql_get_host_info)().toString().empty());
  return Count(true);
}

bool TestExtMysql::test_mysql_get_proto_info() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VS(HHVM_FN(mysql_get_proto_info)(), 10);
  return Count(true);
}

bool TestExtMysql::test_mysql_get_server_info() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!HHVM_FN(mysql_get_server_info)().toString().empty());
  return Count(true);
}
...
bool TestExtMysql::test_mysql_info() {
  ...
  VERIFY(CreateTestTable());
  VS(HHVM_FN(mysql_query)("insert into test (name) values ('test'),('test2')"), true);
  VS(HHVM_FN(mysql_info)(), "Records: 2  Duplicates: 0  Warnings: 0");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_insert_id() {
  ...
  VERIFY(CreateTestTable());
  VS(HHVM_FN(mysql_query)("insert into test (name) values ('test'),('test2')"), true);
  VS(HHVM_FN(mysql_insert_id)(), 1);
  return Count(true);
}

bool TestExtMysql::test_mysql_stat() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(!HHVM_FN(mysql_stat)().toString().empty());
  return Count(true);
}

bool TestExtMysql::test_mysql_thread_id() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(more(HHVM_FN(mysql_thread_id)(), 0));
  return Count(true);
}

bool TestExtMysql::test_mysql_create_db() {
  try {
    HHVM_FN(mysql_create_db)("");
...
bool TestExtMysql::test_mysql_affected_rows() {
  ...
  VERIFY(CreateTestTable());
  VS(HHVM_FN(mysql_query)("insert into test (name) values ('test'),('test2')"), true);
  VS(HHVM_FN(mysql_affected_rows)(), 2);
  return Count(true);
}

bool TestExtMysql::test_mysql_set_timeout() {
  VERIFY(HHVM_FN(mysql_set_timeout)(10));
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD,
                                 true, 0, 20, 50);
  VERIFY(!same(conn, false));
  return Count(true);
...
bool TestExtMysql::test_mysql_subsecond_timeout() {
  ...
  Variant conn = HHVM_FN(mysql_connect)(UNROUTABLE_DESTINATION,
  ...
  VS(HHVM_FN(mysql_errno)(), CR_CONN_HOST_ERROR);
...
bool TestExtMysql::test_mysql_query() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  Variant row = HHVM_FN(mysql_fetch_assoc)(res);
  ...
  row = HHVM_FN(mysql_fetch_assoc)(res);
  VS(HHVM_FN(print_r)(row, true),
     "Array\n"
     "(\n"
     "    [id] => 2\n"
...
bool TestExtMysql::test_mysql_unbuffered_query() {
  ...
  Variant res = HHVM_FN(mysql_unbuffered_query)("select * from test");
  Variant row = HHVM_FN(mysql_fetch_assoc)(res);
  VS(HHVM_FN(print_r)(row, true),
     "Array\n"
     "(\n"
...
bool TestExtMysql::test_mysql_list_dbs() {
  ...
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  Variant res = HHVM_FN(mysql_list_dbs)();
  Variant db = HHVM_FN(mysql_fetch_assoc)(res);
...
bool TestExtMysql::test_mysql_list_tables() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  Variant res = HHVM_FN(mysql_list_tables)(TEST_DATABASE);
  Variant table = HHVM_FN(mysql_fetch_assoc)(res);
...
bool TestExtMysql::test_mysql_list_fields() {
  try {
    Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME,
                                   TEST_PASSWORD);
    HHVM_FN(mysql_list_fields)(TEST_DATABASE, "test");
  } catch (const NotSupportedException& e) {
    return Count(true);
  }
  return Count(false);
...
bool TestExtMysql::test_mysql_list_processes() {
  ...
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  Variant res = HHVM_FN(mysql_list_processes)();
  Variant process = HHVM_FN(mysql_fetch_assoc)(res);
...
bool TestExtMysql::test_mysql_db_name() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  Variant dbs = HHVM_FN(mysql_list_dbs)();
  if (HHVM_FN(mysql_db_name)(dbs, 0).toString().empty()) {
    return CountSkip();
  }
  return Count(true);
}
...
bool TestExtMysql::test_mysql_tablename() {
  Variant conn = HHVM_FN(mysql_connect)(TEST_HOSTNAME, TEST_USERNAME, TEST_PASSWORD);
  VERIFY(CreateTestTable());
  Variant tables = HHVM_FN(mysql_list_tables)(TEST_DATABASE);
  VERIFY(!HHVM_FN(mysql_tablename)(tables, 0).toString().empty());
  return Count(true);
}
...
bool TestExtMysql::test_mysql_num_fields() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_num_fields)(res), 2);
  return Count(true);
}
...
bool TestExtMysql::test_mysql_num_rows() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_num_rows)(res), 2);
  return Count(true);
}
...
bool TestExtMysql::test_mysql_fetch_row() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  Variant row = HHVM_FN(mysql_fetch_row)(res);
  VS(HHVM_FN(print_r)(row, true),
     "Array\n"
     "(\n"
     "    [0] => 1\n"
...
bool TestExtMysql::test_mysql_fetch_array() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  Variant row = HHVM_FN(mysql_fetch_array)(res);
  VS(HHVM_FN(print_r)(row, true),
     "Array\n"
     "(\n"
     "    [0] => 1\n"
...
bool TestExtMysql::test_mysql_fetch_lengths() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  Variant row = HHVM_FN(mysql_fetch_row)(res);
  Variant lengths = HHVM_FN(mysql_fetch_lengths)(res);
  ...
  HHVM_FN(mysql_query)("drop table testlen");
  VS(HHVM_FN(mysql_query)("create table testlen (id int not null auto_increment, "
                   "d decimal(10,5), t tinyint, i int, b bigint, f float, "
                   "db double, y2 year(2), y4 year(4), primary key (id)) "
                   "engine=innodb"), true);
  VS(HHVM_FN(mysql_query)("insert into testlen(d, t, i, b, f, db, y2, y4) values"
                   "(.343, null, 384, -1, 03.44, -03.43892874e101, 00, 0000)"),
     true);
  res = HHVM_FN(mysql_query)("select * from testlen");
  row = HHVM_FN(mysql_fetch_row)(res);
  lengths = HHVM_FN(mysql_fetch_lengths)(res);
  VS(HHVM_FN(print_r)(lengths, true),
     "Array\n"
     "(\n"
     "    [0] => 1\n"
...
bool TestExtMysql::test_mysql_fetch_object() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  Variant row = HHVM_FN(mysql_fetch_object)(res);
...
bool TestExtMysql::test_mysql_result() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_result)(res, 1, 1), "test2");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_fetch_field() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_fetch_field)(res, 1).toObject()->o_get("name"), "name");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_field_seek() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VERIFY(HHVM_FN(mysql_field_seek)(res, 1));
  VS(HHVM_FN(mysql_fetch_field)(res).toObject()->o_get("name"), "name");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_field_name() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_field_name)(res, 1), "name");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_field_table() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_field_table)(res, 1), "test");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_field_len() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_field_len)(res, 1), 255);
  return Count(true);
}
...
bool TestExtMysql::test_mysql_field_type() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_field_type)(res, 1), "string");
  return Count(true);
}
...
bool TestExtMysql::test_mysql_field_flags() {
  ...
  Variant res = HHVM_FN(mysql_query)("select * from test");
  VS(HHVM_FN(mysql_field_flags)(res, 0), "not_null primary_key auto_increment");
  return Count(true);
#endif
}
