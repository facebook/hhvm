...
TEventHandler::TEventHandler(TEventBase* eventBase, int fd) {
  ...
  if (eventBase != nullptr) {
    ...
    // Callers must set the TEventBase and fd before using this timeout.
...
bool TEventHandler::registerImpl(uint16_t events, bool internal) {
  ...
  // we intentionally don't allow an TEventHandler to also use a timeout.
  // Callers must maintain a separate TAsyncTimeout object if they want a
  // timeout.
  //
  ...
  // I/O callback could delete the TEventHandler.)  Additionally, if a caller
  ...
  if (event_add(&event_, nullptr) < 0) {
    LOG(ERROR) << "TEventBase: failed to register event handler for fd "
...
void TEventHandler::unregisterHandler() {
  if (isHandlerRegistered()) {
    event_del(&event_);
  }
}

void TEventHandler::attachEventBase(TEventBase* eventBase) {
  // attachEventBase() may only be called on detached handlers
  assert(event_.ev_base == nullptr);
  assert(!isHandlerRegistered());
  // This must be invoked from the TEventBase's thread
  assert(eventBase->isInEventBaseThread());

  setEventBase(eventBase);
}

void TEventHandler::detachEventBase() {
...
void TEventHandler::ensureNotRegistered(const char* fn) {
  // Neither the TEventBase nor file descriptor may be changed while the
...
void TEventHandler::libeventCallback(int fd, short events, void* arg) {
  TEventHandler* handler = reinterpret_cast<TEventHandler*>(arg);
