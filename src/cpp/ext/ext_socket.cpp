...
static void check_socket_parameters(int &domain, int &type) {
  if (domain != AF_UNIX && domain != AF_INET6 && domain != AF_INET) {
    Logger::Warning("invalid socket domain [%ld] specified for argument 1, "
                    "assuming AF_INET", domain);
    domain = AF_INET;
  }

  if (type > 10) {
    Logger::Warning("invalid socket type [%ld] specified for argument 2, "
...
static bool get_sockaddr(sockaddr *sa, Variant &address, Variant &port) {
  ...
  Logger::Warning("Unsupported address family %d", sa->sa_family);
  return false;
}
...
static bool php_set_inet6_addr(struct sockaddr_in6 *sin6, const char *address,
  ...
  if (inet_pton(AF_INET6, address, &tmp)) {
    ...
    if (addrinfo->ai_family != PF_INET6 ||
      ...
      Logger::Warning("Host lookup failed: Non AF_INET6 domain "
...
static bool php_set_inet_addr(struct sockaddr_in *sin, const char *address,
  ...
  if (inet_aton(address, &tmp)) {
    ...
    if (result.hostbuf.h_addrtype != AF_INET) {
      Logger::Warning("Host lookup failed: Non AF_INET domain "
...
static bool set_sockaddr(sockaddr_storage &sa_storage, Socket *sock,
  ...
  case AF_INET6:
    ...
    break;
  default:
    Logger::Warning("unsupported socket type '%d', must be "
...
Variant f_socket_create(int domain, int type, int protocol) {
  ...
  Object ret(sock);
  if (!sock->valid()) {
    Logger::Warning("Unable to create socket [%d]: %s", errno,
...
bool f_socket_create_pair(int domain, int type, int protocol, Variant fd) {
  ...
  if (socketpair(domain, type, protocol, fds_array) != 0) {
    Logger::Warning("unable to create socket pair [%d]: %s", errno,
                    Util::safe_strerror(errno).c_str());
    return false;
  }
  ...
  case SO_LINGER:
    {
      ...
      if (!value.exists("l_onoff")) {
        Logger::Warning("no key \"l_onoff\" passed in optval");
        return false;
      }
      if (!value.exists("l_linger")) {
        Logger::Warning("no key \"l_linger\" passed in optval");
        return false;
      }
  ...
  case SO_SNDTIMEO:
    {
      Array value = optval.toArray();
      if (!value.exists("sec")) {
        Logger::Warning("no key \"sec\" passed in optval");
        return false;
      }
      if (!value.exists("usec")) {
        Logger::Warning("no key \"usec\" passed in optval");
        return false;
      }
...
bool f_socket_connect(CObjRef socket, CStrRef address, int port /* = 0 */) {
  ...
  case AF_INET:
    if (port == 0) {
      Logger::Warning("Socket of type AF_INET/6 requires 3 arguments");
      return false;
    }
    break;
  default:
...
Variant f_socket_select(Variant read, Variant write, Variant except,
  ...
  if (retval == -1) {
    Logger::Warning("unable to select [%d]: %s", errno,
                    Util::safe_strerror(errno).c_str());
    free(fds);
    return false;
  }
...
Variant f_socket_sendto(CObjRef socket, CStrRef buf, int len, int flags,
  ...
  case AF_INET6:
    ...
    break;
  default:
    Logger::Warning("Unsupported socket type %d", sock->getType());
    return false;
  }
