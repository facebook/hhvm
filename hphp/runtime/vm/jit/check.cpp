...
bool checkTmpsSpanningCalls(const IRUnit& unit) {
  ...
  auto ignoreSrc = [&](IRInstruction& inst, SSATmp* src) {
    /*
     * ReDefSP, TakeStack, and FramePtr/StKptr-typed tmps are used only
     * for stack analysis in the simplifier and therefore may live across
     * calls. In particular, ReDef[Generator]SP are used to bridge the logical
     * stack of the caller when a callee is inlined so that analysis does not
     * scan into the callee stack when searching for a type of value in the
     * caller.
     *
