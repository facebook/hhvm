...
class TestServer:
    server: ThriftServer
    serve_task: Optional[asyncio.Task]

    def __init__(
        self,
        ip: Optional[str] = None,
        path: Optional["Path"] = None,
        handler: ServiceInterface | PythonServiceInterface | None = None,
    ) -> None:
        if handler is None:
            handler = Handler()
    ...
    async def __aexit__(self, *exc_info: Iterable[object]) -> None:
        self.server.stop()
        await typing_cast(asyncio.Task, self.serve_task)
...
class ClientMetadataTestingServiceTests(unittest.IsolatedAsyncioTestCase):
