...
#include <thrift/compiler/test/fixtures/nolegacy/src/gen-py3/module/services_wrapper.h>
#include <thrift/compiler/test/fixtures/nolegacy/src/gen-py3/module/services_api.h>
...
MyServiceWrapper::MyServiceWrapper(PyObject *obj, folly::Executor* exc)
  ...
  {
    import_test__fixtures__nolegacy__module__services();
  }


void MyServiceWrapper::async_tm_query(
  std::unique_ptr<apache::thrift::HandlerCallback<std::unique_ptr<::test::fixtures::nolegacy::TestStruct>>> callback
...
val = std::move(val)    ]() mutable {
        auto [promise, future] = folly::makePromiseContract<std::unique_ptr<::test::fixtures::nolegacy::TestStruct>>();
        call_cy_MyService_query(
            this->if_object,
            ctx,
            std::move(promise),
            std::move(val)        );
        std::move(future).via(this->executor).thenTry([callback = std::move(callback)](folly::Try<std::unique_ptr<::test::fixtures::nolegacy::TestStruct>>&& t) {
          (void)t;
