...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  ...
  module SimpleParser =
    SimpleParserSyntax.WithLexer (Full_fidelity_lexer.WithToken (Syntax.Token))
  ...
  module type ExpressionParser_S =
    Full_fidelity_expression_parser_type.WithSyntax(Syntax).WithLexer
      (Full_fidelity_lexer.WithToken(Syntax.Token))
    .ExpressionParser_S

  module type DeclarationParser_S =
    Full_fidelity_declaration_parser_type.WithSyntax(Syntax).WithLexer
      (Full_fidelity_lexer.WithToken(Syntax.Token))
    .DeclarationParser_S

  module type TypeParser_S =
    Full_fidelity_type_parser_type.WithSyntax(Syntax).WithLexer
      (Full_fidelity_lexer.WithToken(Syntax.Token))
    .TypeParser_S

  module type StatementParser_S =
    Full_fidelity_statement_parser_type.WithSyntax(Syntax).WithLexer
      (Full_fidelity_lexer.WithToken(Syntax.Token))
    .StatementParser_S
  ...
  module WithSmartConstructors
      (SCI : SCWithKind_S with module Token = Syntax.Token) =
  struct
    module WithExpressionAndDeclAndTypeParser
      ...
      let with_decl_parser :
            'a. t -> (DeclParser.t -> DeclParser.t * 'a) -> t * 'a =
       fun parser f ->
        let decl_parser =
          DeclParser.make
            parser.env
      ...
      let rec parse_statement parser =
        ...
        | Coroutine
        | Function ->
          parse_possible_php_function ~toplevel:false parser
        | Abstract
        | Final
        | Interface
        | Trait
        | Class ->
          ...
          with_decl_parser parser (fun parser ->
              DeclParser.parse_classish_declaration parser missing)
        ...
        | Let when Env.is_experimental_mode (env parser) ->
        ...
        | Static -> parse_expression_statement parser
        ...
        let (parser, suffix, is_echo_tag, has_suffix) =
          ...
          | Some (less_than_question, language_opt) ->
           ...
           section won't throw parser off the rails. *)
            ...
            in
            (parser, suffix, is_echo_tag, true)
          | None ->
            let (parser, missing) = Make.missing parser (pos parser) in
    ...
    *)
        ...
        let (parser, for_initializer_expr) =
      ...
      and parse_foreach_statement parser =
        ...
        let (parser, foreach_collection_name) =
          with_expr_parser parser (fun p ->
              ExpressionParser.with_as_expressions
                p
                ~enabled:false
        ...
        let (parser, foreach_key, foreach_arrow, foreach_value) =
          ...
          | _ ->
            let parser =
              with_error parser1 SyntaxError.invalid_foreach_element
            in
      ...
      and parse_do_statement parser =
        let (parser, do_keyword_token) = assert_token parser Do in
   ...
   *)
    ...
    *)
        ...
        | Semicolon ->
          ...
          Make.using_statement_function_scoped
            parser
            await_kw
            using_kw
            expr
        ...
        | _ ->
          ...
          Make.using_statement_block_scoped
            parser
            await_kw
            using_kw
            left_paren
    ...
    *)
        ...
        let parse_elseif_opt parser_elseif =
          if peek_token_kind parser_elseif = Elseif then
            let (parser_elseif, elseif_token) =
            ...
            let ( parser_elseif,
                  ...
                  elseif_right_paren,
                  elseif_statement ) =
              parse_if_body_helper parser_elseif
            in
            let (parser_elseif, elseif_syntax) =
              Make.elseif_clause
                parser_elseif
                elseif_token
                elseif_left_paren
        ...
        let (parser, if_left_paren, if_expr, if_right_paren, if_consequence) =
          parse_if_body_helper parser
        in
        let (parser, elseif_syntax) =
          parse_list_until_none parser parse_elseif_opt
        in
    ...
    *)
        ...
        let (parser, section_list) =
          ...
          | Semicolon when peek_token_kind parser1 = RightBrace ->
            make_list parser1 []
          | _ -> parse_terminated_list parser parse_switch_section RightBrace
        in
      ...
      and parse_possible_erroneous_fallthrough parser =
        if is_switch_fallthrough parser then
          let parser =
            with_error parser SyntaxError.error1055 ~on_whole_token:true
          in
    ...
    *)
        if peek_token_kind parser = Catch then
          ...
          let (parser, catch_var) =
            if Env.is_experimental_mode (env parser) then
    ...
    *)
    ...
    TODO: Implement that error. *)
      ...
      and parse_goto_label parser =
        let (parser, goto_label_name) =
          next_token_non_reserved_as_name parser
        in
          ...
          let (parser, token) =
            ...
            | Some t ->
              if SC.is_halt_compiler_expression expression then
                ...
                Make.token parser t
            | None -> Make.missing parser (pos parser)
          in
