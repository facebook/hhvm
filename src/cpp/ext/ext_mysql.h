...
public:
  ...
  static String GetDefaultSocket()   { return String();}
...
inline Variant f_mysql_errno(CVarRef link_identifier = null) {
  MYSQL *conn = MySQL::GetConn(link_identifier);
  if (!conn) return false;
  return (int64)mysql_errno(conn);
}
inline Variant f_mysql_error(CVarRef link_identifier = null) {
  MYSQL *conn = MySQL::GetConn(link_identifier);
  if (!conn) return false;
