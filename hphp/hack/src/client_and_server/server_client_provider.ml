...
let accept_client
  ...
  Monitor_rpc.write_server_receipt_to_monitor_file
    ~server_receipt_to_monitor_file:
      (Server_files.server_receipt_to_monitor_file (Unix.getpid ()))
...
let select ~idle_gc_slice fd_list timeout =
  ...
  match Server_idle_gc.select ~slice:idle_gc_slice ~timeout fd_list with
  | [] ->
...
let say_hello oc =
  ...
  let (_ : int) =
    Marshal_tools.to_fd_with_preamble fd Server_command_types.Hello
  in
  ()
...
let send_response_to_client client response =
  let (_ : int) =
    Marshal_tools.to_fd_with_preamble
      ...
      (Server_command_types.Response (response, client.tracker))
  in
  ()
