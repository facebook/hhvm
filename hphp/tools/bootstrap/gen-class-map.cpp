...
static void outputConstants(const std::string &invocation_trace,
                            const char *outputfn,
                            const fbvector<PhpConst>& consts) {
  std::ofstream out(outputfn);

  FileBrand::brandOutputFile(out, "gen-class-map.cpp", invocation_trace);
...
static void outputClassMap(const std::string &invocation_trace,
