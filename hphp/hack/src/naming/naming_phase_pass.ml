...
let combine_transform (ps : (_, _) pass list) ~select =
...
let merge_passes ps =
  {
    ...
    on_class_c_uses = combine_transform ps ~select:(fun t -> t.on_class_c_uses);
    on_class_c_xhp_attrs =
    ...
    on_class_c_reqs = combine_transform ps ~select:(fun t -> t.on_class_c_reqs);
    ...
    on_class_c_vars = combine_transform ps ~select:(fun t -> t.on_class_c_vars);
    on_class_c_enum = combine_transform ps ~select:(fun t -> t.on_class_c_enum);
    on_class_c_methods =
    ...
    on_class_var_cv_user_attributes =
      combine_transform ps ~select:(fun t -> t.on_class_var_cv_user_attributes);
    on_class_var_cv_expr =
    ...
    on_fun_f_params = combine_transform ps ~select:(fun t -> t.on_fun_f_params);
    ...
    on_method_m_ret = combine_transform ps ~select:(fun t -> t.on_method_m_ret);
    on_method_m_tparams =
    ...
    on_default_case = combine_transform ps ~select:(fun t -> t.on_default_case);
    on_fun_param_user_attributes =
...
let merge ts =
  ...
  (merge_passes top_downs, merge_passes bottom_ups)
...
type ('env, 'err) handler = {
  top_down: ('env, 'err list) pass;
  bottom_up: ('env, 'err list) pass;
  on_error: 'err -> unit;
}
...
and traverse_hint_ elem ~env ~handler =
  ...
  | Aast.(
      ( Hany | Herr | Hmixed | Hnonnull | Hprim _ | Hthis | Hdynamic | Hnothing
      | Hvar _ | Hfun_context _ )) ->
    elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_hint_fun_hf_return_ty) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_hint_fun_hf_param_tys) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_hint_fun_hf_variadic_ty) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_type_refinement_bounds) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem

and traverse_type_refinement_bounds Aast.{ tr_lower; tr_upper } ~env ~handler =
  Aast.
    {
...
and traverse_ctx_refinement ctx_refinement ~env ~handler =
  ...
  | Aast.CRloose ctx_refinement_bounds ->
    Aast.CRloose (on_ctx_refinement_bounds ctx_refinement_bounds ~env ~handler)
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_ctx_refinement_bounds) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_class_abstract_typeconst) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_class_concrete_typeconst) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
...
and traverse_expr_ expr_ ~env ~handler =
  ...
  | Shape flds ->
    Shape (on_list (on_tuple2 on_shape_field_name on_expr) flds ~env ~handler)
  ...
  | FunctionPointer (id, targs) ->
    FunctionPointer
      (on_function_ptr_id id ~env ~handler, on_list on_targ targs ~env ~handler)
...
and traverse_xhp_attribute elem ~env ~handler =
  match elem with
  | Xhp_simple xhp_simple -> Xhp_simple (on_xhp_simple xhp_simple ~env ~handler)
...
and traverse_as_expr elem ~env ~handler =
  ...
  | As_kv (e1, e2) -> As_kv (on_expr e1 ~env ~handler, on_expr e2 ~env ~handler)
...
and traverse_fun_ f ~env ~handler =
  ...
  and f_unsafe_ctxs = on_fun_f_unsafe_ctxs f.Aast.f_unsafe_ctxs ~env ~handler
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_fun_f_where_constraints) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_fun_f_user_attributes) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
  ...
  let (_, elem, errs) =
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          let elem = (on_list on_where_constraint_hint) elem ~env ~handler in
          (or_ok handler.bottom_up.on_where_constraint_hints) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_where_constraint_hint) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
...
and traverse_class c ~env ~handler =
  ...
  and c_xhp_attr_uses = on_class_c_xhp_attr_uses c.c_xhp_attr_uses ~env ~handler
  ...
  let (_, elem, errs) =
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          let elem = (on_list on_hint) elem ~env ~handler in
          (or_ok handler.bottom_up.on_class_c_xhp_attr_uses) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
  ...
  let (_, elem, errs) =
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          let elem = on_user_attributes elem ~env ~handler in
          (or_ok handler.bottom_up.on_class_c_user_attributes) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
    ...
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          ...
          (or_ok handler.bottom_up.on_class_c_file_attributes) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
...
and traverse_class_const_kind elem ~env ~handler =
  match elem with
  | CCAbstract expr_opt -> CCAbstract (on_option on_expr expr_opt ~env ~handler)
  ...
  let (_, elem, errs) =
    Result.(
      fold
        ...
        >>= fun (env, elem, errs) ->
          let elem = on_user_attributes elem ~env ~handler in
          (or_ok handler.bottom_up.on_method_m_user_attributes) (env, elem, errs)
        ))
  in
  List.iter ~f:handler.on_error errs;
  elem
...
let mk_visitor
    (passes : (Naming_phase_env.t, Naming_phase_error.t list) t list)
    ~(on_error : Naming_phase_error.t -> unit) =
  let (top_down, bottom_up) = merge passes in
