...
function test_preg_replace() :mixed{
  ...
  $expFoo[1] = "FOO";
  $expFoo[2] = "FOO";
  VS(preg_replace("/some pattern/", "", dict[]), dict[]);
  ...
  VS(preg_replace("/xxx/", "w", vec["xxxx"]), dict(vec["wx"]));
  VS(preg_replace("/xxx/", "w", vec["xxxx", "yyyy"]), dict(vec["wx", "yyyy"]));
  VS(preg_replace(vec["/xxx/", "/xxx"], "w", vec["xxxx"]), dict[]);
  VS(preg_replace(vec["/xxx/", "/xxx/"], "w", vec["xxxx"]), dict(vec["wx"]));

  VS(preg_replace("/xxx", vec["w"], vec["xxxx"]), false);
  VS(preg_replace(vec["/xxx"], vec["w"], vec["xxxx"]), dict[]);
  VS(preg_replace(vec["/xxx/"], vec["w"], vec["xxxx"]), dict(vec["wx"]));
}
