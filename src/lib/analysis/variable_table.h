...
public:
  ...
  bool isNestedGlobal(const std::string &name) const;
  ...
  bool isUnused(const std::string &name) const;
  ...
  void addNestedGlobal(const std::string &name);
  ...
  void addReferenced(const std::string &name);
  void addUnused(const std::string &name);
  ...
  void outputCPPPropertyTable(CodeGenerator &cg, AnalysisResultPtr ar,
                              const char *parent, int dynamicObject = 0);
...
private:
  ...
  std::set<std::string> m_nestedGlobal; // the name occurred in a
  // nested global statement
  ...
  std::set<std::string> m_referenced;   // the referenced variables
  std::set<std::string> m_unused;       // unused local variables
