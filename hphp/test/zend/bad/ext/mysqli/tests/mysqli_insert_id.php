<?php
	...
	$tmp    = NULL;
	$link   = NULL;
	$test_table_name = 'test_mysqli_insert_id_table_1'; require('table.inc');

	if (0 !== ($tmp = mysqli_insert_id($link)))
		printf("[003] Expecting int/0, got %s/%s\n", gettype($tmp), $tmp);

	if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_insert_id_table_1 ORDER BY id LIMIT 1")) {
	...
	if (!$res = mysqli_query($link, "INSERT INTO test_mysqli_insert_id_table_1(id, label) VALUES (100, 'a')")) {
		printf("[006] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}
	if (0 !== ($tmp = mysqli_insert_id($link)))
		printf("[007] Expecting int/0, got %s/%s\n", gettype($tmp), $tmp);

	if (!$res = mysqli_query($link, "ALTER TABLE test_mysqli_insert_id_table_1 MODIFY id INT NOT NULL AUTO_INCREMENT")) {
		printf("[008] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}

	if (!$res = mysqli_query($link, "INSERT INTO test_mysqli_insert_id_table_1(label) VALUES ('a')")) {
		printf("[009] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}
	if (($last_id = mysqli_insert_id($link)) <= 0)
		printf("[010] Expecting int/any >0, got %s/%s\n", gettype($last_id), $last_id);

	if (mysqli_query($link, "LOCK TABLE test_mysqli_insert_id_table_1 WRITE")) {
		...
		do {
			...
			if (!mysqli_query($link, "INSERT INTO test_mysqli_insert_id_table_1(label) VALUES ('b')")) {
				printf("[013] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
				break;
			}
			...
			$last_id = $next_id;
			if (!mysqli_query($link, "INSERT INTO test_mysqli_insert_id_table_1(label) VALUES ('c'), ('d'), ('e')")) {
				printf("[015] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
				break;
			}
			/*
		...
		mysqli_query($link, "UNLOCK TABLE test_mysqli_insert_id_table_1");
	}

	if (!$res = mysqli_query($link, "INSERT INTO test_mysqli_insert_id_table_1(id, label) VALUES (1000, 'a')")) {
		printf("[020] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}
	if (1000 !== ($tmp = mysqli_insert_id($link)))
		printf("[021] Expecting int/1000, got %s/%s\n", gettype($tmp), $tmp);

	if (!$res = mysqli_query($link, "INSERT INTO test_mysqli_insert_id_table_1(label) VALUES ('b'), ('c')")) {
		printf("[022] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}
	if (1000 >= ($tmp = mysqli_insert_id($link)))
...
?>
<?php
	$test_table_name = 'test_mysqli_insert_id_table_1'; require_once("clean_table.inc");
?>