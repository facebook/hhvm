...
File* DataStreamWrapper::open(CStrRef filename, CStrRef mode,
  ...
  if (comma != data) {
    ...
    if (!semi) {
      ...
      if (semi != data // (e.g. foo;base64,data)
          || meta_len != sizeof(";base64") - 1 // (e.g. ;something,data)
          || memcmp(data, ";base64", sizeof(";base64") - 1)) // (e.g. ;base65,data)
        {
          raise_warning("rfc2397: invalid meta data");
          return nullptr;
        }
    ...
    while (semi && (data == semi)) {
      ...
      if (!equals || (semi && semi < data)) {
        ...
        if (meta_len != sizeof("base64") - 1 || memcmp(data, "base64", sizeof("base64")-1)) {
  ...
  free(decoded); // probably not exception safe
  return file.release();
}
///////////////////////////////////////////////////////////////////////////////
}
