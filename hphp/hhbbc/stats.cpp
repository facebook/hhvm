...
#define SPEC_TYPES(X)                                                \
  ...
  X(obj_isect_exact, "isect exact obj", is_isect_exact_obj)          \
  X(cls_sub, "sub class", is_sub_cls)                                \
  X(cls_exact, "exact class", is_exact_cls)                          \
  X(cls_isect, "isect class", is_isect_cls)                          \
  X(cls_isect_exact, "isect exact cls", is_isect_exact_cls)          \
...
bool is_isect_exact_obj(const Type& t) {
  return
    is_specialized_obj(t) &&
    !is_specialized_wait_handle(t) &&
    dobj_of(t).isIsectAndExact();
}
...
bool is_isect_exact_cls(const Type& t) {
  return is_specialized_cls(t) && dobj_of(t).isIsectAndExact();
}
