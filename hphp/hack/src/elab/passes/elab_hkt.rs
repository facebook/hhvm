    ...
    fn on_ty_tparam_top_down<Ex: Default, En>(
        ...
        if !cfg.hkt_enabled() {
            if !elem.parameters.is_empty() {
                ...
                cfg.emit_error(NamingError::TparamWithTparam {
                    pos: pos.clone(),
                    tparam_name: tp_name.clone(),
                });
...
mod tests {
    ...
    fn test_hint() {
        ...
        elem.transform(&cfg_hkt_enabled, &mut pass);
        let Hint(_, hint_) = &elem;
        assert!(match &**hint_ {
            Hint_::Habstr(_, hints) => !hints.is_empty(),
            _ => false,
        });
        assert_eq!(cfg_hkt_enabled.into_errors().len(), 0);

        elem.transform(&cfg_hkt_disabled, &mut pass);
        let Hint(_, hint_) = &elem;
        assert!(match &**hint_ {
            Hint_::Habstr(_, hints) => hints.is_empty(),
            _ => false,
        });
        assert_eq!(cfg_hkt_disabled.into_errors().len(), 1);
    }
