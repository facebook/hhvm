...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  module WithSmartConstructors (SCI : SC_S with module Token = Syntax.Token) : sig
    ...
    val parse_header_only :
      Env.t -> Full_fidelity_source_text.t -> SCI.r option
  end = struct
    ...
    module ExpressionParserSyntax_ =
      Full_fidelity_expression_parser.WithSyntax (Syntax)
    module ExpressionParserSyntax =
      ExpressionParserSyntax_.WithSmartConstructors (SCWithToken)
    ...
    module StatementParserSyntax_ =
      Full_fidelity_statement_parser.WithSyntax (Syntax)
    module StatementParserSyntax =
      StatementParserSyntax_.WithSmartConstructors (SCWithToken)
    ...
    module DeclarationParserSyntax_ =
      Full_fidelity_declaration_parser.WithSyntax (Syntax)
    module DeclarationParserSyntax =
      DeclarationParserSyntax_.WithSmartConstructors (SCWithToken)
    ...
    module TypeParserSyntax_ = Full_fidelity_type_parser.WithSyntax (Syntax)
    module TypeParserSyntax =
      TypeParserSyntax_.WithSmartConstructors (SCWithToken)

    module rec ExpressionParser :
      (ExpressionParser_S with module SC = SCWithToken) =
    ...
    and TypeParser : (TypeParser_S with module SC = SCWithToken) =
      TypeParserSyntax.WithExpressionAndDeclParser
        (ExpressionParser)
        (DeclParser)
    ...
    let make (env : Env.t) text =
      let lexer =
        Lexer.make ~is_experimental_mode:(Env.is_experimental_mode env) text
      in
      {
        lexer;
        errors = [];
    ...
    let rust_parse_script parser =
      let (sc_state, root, errors, rust_tree) =
        SCI.rust_parse (parser.lexer |> Lexer.source) parser.env
      in
      ({ parser with errors; sc_state }, root, rust_tree)

    let parse_script parser =
      if Env.rust parser.env then
        rust_parse_script parser
      else
        let decl_parser =
          DeclParser.make
            parser.env
      ...
      begin
        ...
        | MarkupSection
          ...
          begin
            ...
            | Token t when Token.kind t = Full_fidelity_token_kind.Equal ->
              Some FileInfo.Mphp
            | _ ->
              let skip_length =
                full_width pfx
              ...
              if language = "php" then
                ...
                let s =
                  SourceText.sub text skip_length (trailing_width name)
                in
                ...
                in
                let mode =
                  try List.hd (Str.split (Str.regexp " +") mode) with _ -> ""
                in
