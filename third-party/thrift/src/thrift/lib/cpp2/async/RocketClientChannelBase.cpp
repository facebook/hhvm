...
folly::Try<FirstResponsePayload> decodeResponseError(
  ...
  switch (ex.getErrorCode()) {
    ...
    case rocket::ErrorCode::REJECTED:
      break;
    case rocket::ErrorCode::RESERVED:
    case rocket::ErrorCode::INVALID_SETUP:
    case rocket::ErrorCode::UNSUPPORTED_SETUP:
    case rocket::ErrorCode::REJECTED_SETUP:
    case rocket::ErrorCode::REJECTED_RESUME:
    case rocket::ErrorCode::CONNECTION_ERROR:
    case rocket::ErrorCode::CONNECTION_CLOSE:
    case rocket::ErrorCode::APPLICATION_ERROR:
    case rocket::ErrorCode::RESERVED_EXT:
    default:
...
class RocketClientChannelBase::SingleRequestSingleResponseCallback final
...
int32_t RocketClientChannelBase::getMetaKeepAliveTimeoutMs(
...
RocketClientChannelBase::~RocketClientChannelBase() {}

void RocketClientChannelBase::setFlushList(FlushList* flushList) {
  getRocketClientImpl().setFlushList(flushList);
}

void RocketClientChannelBase::sendRequestResponse(
...
void RocketClientChannelBase::sendRequestNoResponse(
    const RpcOptions& rpcOptions,
    apache::thrift::MethodMetadata&& methodMetadata,
    SerializedRequest&& request,
    std::shared_ptr<transport::THeader> header,
...
void RocketClientChannelBase::sendRequestStream(
    const RpcOptions& rpcOptions,
    apache::thrift::MethodMetadata&& methodMetadata,
    SerializedRequest&& request,
...
void RocketClientChannelBase::sendRequestSink(
    const RpcOptions& rpcOptions,
    apache::thrift::MethodMetadata&& methodMetadata,
    SerializedRequest&& request,
    std::shared_ptr<transport::THeader> header,
...
void RocketClientChannelBase::sendRequestBiDi(
...
void RocketClientChannelBase::sendThriftRequest(
  ...
  try {
    requestPayload = getRocketClientImpl().getPayloadSerializer()->packWithFds(
        &metadata,
        std::move(buf),
...
void RocketClientChannelBase::sendSingleRequestNoResponse(
    const RpcOptions& rpcOptions,
    RequestRpcMetadata&&,
    rocket::Payload requestPayload,
    RequestClientCallback::Ptr cb) {
  ...
  if (isSync && folly::fibers::onFiber()) {
    callback.onWrite(
        getRocketClientImpl().sendRequestFnfSync(std::move(requestPayload)));
  } else {
    getRocketClientImpl().sendRequestFnf(
        std::move(requestPayload),
        folly::copy_to_unique_ptr(std::move(callback)));
  }
}

void RocketClientChannelBase::sendSingleRequestSingleResponse(
    const RpcOptions& rpcOptions,
...
void RocketClientChannelBase::sendStreamRequest(
    const RpcOptions& rpcOptions,
    RequestRpcMetadata&& metadata,
    ...
    StreamClientCallback* clientCallback) {
  assert(metadata.name());
  getRocketClientImpl().sendRequestStream(
      std::move(requestPayload),
      ...
      new FirstRequestProcessorStream(
          ...
          clientCallback,
          evb_,
          rocket::RocketClient::DestructionGuardedClient(
              &getRocketClientImpl())));
}

void RocketClientChannelBase::sendSinkRequest(
    const RpcOptions&,
    RequestRpcMetadata&& metadata,
    std::chrono::milliseconds firstResponseTimeout,
    rocket::Payload requestPayload,
    ...
    SinkClientCallback* clientCallback) {
  assert(metadata.name());
  getRocketClientImpl().sendRequestSink(
      std::move(requestPayload),
      firstResponseTimeout,
...
void RocketClientChannelBase::sendBiDiRequest(
    const RpcOptions& rpcOptions,
    RequestRpcMetadata&& metadata,
    std::chrono::milliseconds firstResponseTimeout,
    rocket::Payload requestPayload,
    ...
    BiDiClientCallback* clientCallback) {
  assert(metadata.name());
  getRocketClientImpl().sendRequestBiDi(
      std::move(requestPayload),
      firstResponseTimeout,
      rpcOptions.getChunkBufferSize(),
...
bool RocketClientChannelBase::canHandleRequest(CallbackPtr& cb) {
  if (!getRocketClientImpl().isAlive()) {
...
RocketClientChannelBase::getClientTimeout(const RpcOptions& rpcOptions) const {
...
RocketClientChannelBase::getInteractionHandle(const RpcOptions& rpcOptions) {
...
ClientChannel::SaturationStatus RocketClientChannelBase::getSaturationStatus() {
  DCHECK(evb_);
  evb_->dcheckIsInEventBaseThread();
...
void RocketClientChannelBase::closeNow() {
  if (evb_) {
    evb_->dcheckIsInEventBaseThread();
  }
  closeNowImpl(
...
void RocketClientChannelBase::setCloseCallback(CloseCallback* closeCallback) {
  getRocketClientImpl().setCloseCallback([closeCallback] {
    if (closeCallback) {
...
folly::AsyncTransport* FOLLY_NULLABLE RocketClientChannelBase::getTransport() {
  auto* transportWrapper = getRocketClientImpl().getTransportWrapper();
  return transportWrapper
      ? transportWrapper->getUnderlyingTransport<folly::AsyncTransport>()
      : nullptr;
}

bool RocketClientChannelBase::good() {
  if (evb_) {
    evb_->dcheckIsInEventBaseThread();
  }
  return getRocketClientImpl().isAlive();
}

size_t RocketClientChannelBase::inflightRequestsAndStreams() const {
  return getRocketClientImpl().streams() + getRocketClientImpl().requests();
}

void RocketClientChannelBase::setTimeout(uint32_t timeoutMs) {
  if (evb_) {
    evb_->dcheckIsInEventBaseThread();
  }
...
void RocketClientChannelBase::attachEventBase(folly::EventBase* evb) {
  evb->dcheckIsInEventBaseThread();
  if (getRocketClientImpl().getTransportWrapper()) {
    getRocketClientImpl().attachEventBase(*evb);
  }
  evb_ = evb;
}

void RocketClientChannelBase::detachEventBase() {
  DCHECK(isDetachable());
  DCHECK(getDestructorGuardCount() == 0);

  if (getRocketClientImpl().getTransportWrapper()) {
    getRocketClientImpl().detachEventBase();
  }
  evb_ = nullptr;
}

bool RocketClientChannelBase::isDetachable() {
  if (evb_) {
    evb_->dcheckIsInEventBaseThread();
  }
  auto* transport = getTransport();
  return !evb_ || !transport ||
      (getRocketClientImpl().isDetachable() && pendingInteractions_.empty());
}

void RocketClientChannelBase::setOnDetachable(
    folly::Function<void()> onDetachable) {
  DCHECK(getRocketClientImpl().getTransportWrapper());
  ClientChannel::setOnDetachable(std::move(onDetachable));
  getRocketClientImpl().setOnDetachable([this] {
    if (isDetachable()) {
      notifyDetachable();
    }
  });
...
void RocketClientChannelBase::unsetOnDetachable() {
  ClientChannel::unsetOnDetachable();
  getRocketClientImpl().setOnDetachable(nullptr);
}

void RocketClientChannelBase::terminateInteraction(InteractionId id) {
  evb_->dcheckIsInEventBaseThread();
  ...
  getRocketClientImpl().terminateInteraction(id);
  releaseInteractionId(std::move(id));
}

InteractionId RocketClientChannelBase::registerInteraction(
  ...
  getRocketClientImpl().addInteraction();

  return createInteractionId(id);
}

int32_t RocketClientChannelBase::getServerVersion() const {
  return getRocketClientImpl().getServerVersion();
}
} // namespace apache::thrift
