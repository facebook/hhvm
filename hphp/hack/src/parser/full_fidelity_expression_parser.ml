...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  ...
  module PrecedenceSyntax = Full_fidelity_precedence_parser.WithSyntax (Syntax)
  module PrecedenceParser =
    PrecedenceSyntax.WithLexer (Full_fidelity_lexer.WithToken (Syntax.Token))
  ...
  struct
    module WithStatementAndDeclAndTypeParser
      ...
      ExpressionParser_S with module SC = SCI = struct
      open TokenKind
      open Syntax
      ...
      (* next line warning 32 unused variable pp_binary_expression_prefix_kind *)
      ...
      let with_type_parser :
            'a. t -> (TypeParser.t -> TypeParser.t * 'a) -> t * 'a =
       fun parser f ->
        let type_parser =
          TypeParser.make
            parser.env
      ...
      let parse_compound_statement parser =
        ...
        let (statement_parser, statement) =
          StatementParser.parse_compound_statement statement_parser
        in
      ...
      let parse_parameter_list_opt parser =
        let (parser, (left, token, right)) =
          with_decl_parser parser (fun decl_parser ->
              let (parser, left, token, right) =
      ...
      and parse_term parser =
        ...
        let allow_new_attr =
          Full_fidelity_parser_env.allow_new_attribute_syntax parser.env
        in
        match Token.kind token with
        | DecimalLiteral
        | OctalLiteral
        ...
        | Name ->
          ...
          begin
            ...
            | HeredocStringLiteral ->
              begin
                match
                  try_parse_specified_function_call parser qualified_name
                with
                | Some r -> r
                | None ->
                  let parser =
                    with_error parser SyntaxError.prefixed_invalid_string_kind
                  in
                  parse_name_or_collection_literal_expression
                    parser
                    qualified_name
              end
            | HeredocStringLiteralHead ->
        ...
        | Static -> parse_anon_or_awaitable_or_scope_resolution_or_name parser
      ...
      coroutine. Give an error in a later pass if this isn't the case. *)
        ...
        | LessThan ->
          parse_possible_xhp_expression ~in_xhp_body:false token parser1
       ...
       functions or lambdas *)
        ...
        | Require
        | Require_once ->
          parse_inclusion_expression parser
    ...
    *)
    ...
    identifier, so we do too; consider whether it should be reserved. *)
      ...
      and parse_braced_expression_in_string
          ~left_brace ~dollar_inside_braces parser =
        (*
    ...
    string text. *)
        ...
        let (parser, expr, right_brace) =
          match
            (Token.kind name_or_keyword_as_name, Token.kind after_name)
          with
          | (Name, RightBrace) ->
        ...
        expression, so we special-case only the second case here. *)
            let (parser, receiver) =
              Make.token parser1 name_or_keyword_as_name
            in
            let (parser, left_bracket) = Make.token parser after_name in
            let (parser, index) =
              parse_expression_with_reset_precedence parser
            in
            let (parser, right_bracket) = require_right_bracket parser in
            let (parser, expr) =
              Make.subscript_expression
                parser
                receiver
                left_bracket
                index
              ...
              then
                let error =
                  SyntaxError.make
                    ...
                    end_offset
                    SyntaxError
                    .illegal_interpolated_brace_with_embedded_dollar_expression
                in
    ...
    *)
    ...
    body or tail. *)
       ...
       an identifier, and so on, and then body text, etc. *)
          | Some head ->
            let k =
              match (Token.kind head, Token.kind token) with
          ...
          | None ->
            let token =
              ...
              | DoubleQuotedStringLiteralTail ->
                token
              | _ -> Token.with_kind token StringLiteralBody
            in
            Some token
        in
        ...
        let parse_embedded_expression parser token =
          ...
          | (MinusGreaterThan, Name, _) ->
            ...
            Make.embedded_member_selection_expression
              parser
              var_expr
              token1
              token2
          | (LeftBracket, Name, RightBracket) ->
            ...
            Make.embedded_subscript_expression
              parser
              var_expr
              token1
              token2
        ...
        the expression or closing bracket are missing. *)
            ...
            let parser =
              with_error parser SyntaxError.expected_simple_offset_expression
            in
            Make.embedded_subscript_expression
              parser
              var_expr
      ...
      whether there is trivia between the left brace and the $x which follows.*)
          let (parser1, left_brace) =
            next_token_in_string parser literal_kind
          in
      ...
      trivia after the {. That's why we use next_token_in_string here. *)
        ...
        and aux parser head acc =
          ...
          | LeftBrace -> handle_left_brace parser head acc
          | Variable ->
            let (parser, acc) = put_opt parser1 head acc in
            let (parser, expr) = parse_embedded_expression parser token in
            aux parser None (expr :: acc)
          | Dollar -> handle_dollar parser1 token head acc
          | _ -> aux parser1 (merge token head) acc
        in
    ...
    *)
      ...
      and try_parse_specified_function_call parser term =
        if not (can_term_take_type_args term) then
          None
        else
          match
            peek_token_kind_with_possible_attributized_type_list parser
          with
          | LessThan ->
            let (parser1, (type_arguments, no_arg_is_missing)) =
              parse_generic_type_arguments parser
            in
            if (not no_arg_is_missing) || parser.errors <> parser1.errors then
              None
            else
              let (parser, result) =
   ...
   *)
      and check_if_should_override_normal_precedence
          parser left_term operator left_precedence =
        (*
    ...
    - <term> <operator> does not look like a prefix of
      some assignment expression*)
          ...
          | _ ->
            ...
            | QuestionQuestion
            | QuestionQuestionEqual ->
              parse_remaining_binary_expression
                parser
                term
            ...
            | Instanceof ->
              ...
              (parser, term)
            | Is -> parse_is_expression parser term
            | As when allow_as_expressions parser ->
              parse_as_expression parser term
            | QuestionAs -> parse_nullable_as_expression parser term
            | QuestionMinusGreaterThan
            | MinusGreaterThan ->
              let (parser, result) =
                parse_member_selection_expression parser term
              in
            ...
            | ColonAt ->
              let (parser, result) =
                parse_pocket_identifier_expression parser term
              in
              parse_remaining_expression parser result
            | PlusPlus
            | MinusMinus ->
    ...
    *)
    ...
    leaf, rather than as a name or variable expression. Is that right? *)
        let (parser, name) =
          ...
          | Dollar -> parse_dollar_expression parser
          | _ -> require_xhp_class_name_or_name_or_variable parser
        in
       ...
       *)
        let (parser1, e) =
          let precedence =
            Operator.precedence parser.env Operator.IndexingOperator
          in
          parse_expression (with_precedence parser precedence)
        in
    ...
    *)
        ...
        | _ ->
          ...
          let (parser, index) =
            with_as_expressions parser ~enabled:true (fun parser ->
                with_reset_precedence parser parse_expression)
          in
          let (parser, right) =
            match Token.kind left with
            | LeftBracket -> require_right_bracket parser
            | _ -> require_right_brace parser
          in
          let (parser, result) =
    ...
    *)
        ...
        let f parser =
          with_reset_precedence parser parse_decorated_expression_opt
        in
      ...
      and parse_start_of_type_specifier parser start_token =
        let (parser, name) =
          if Token.kind start_token = Backslash then
            let (parser, missing) = Make.missing parser (pos parser) in
    ...
    *)
        ...
        | Self ->
          begin
            match
              peek_token_kind_with_possible_attributized_type_list parser1
            with
            | LeftParen -> Make.token parser1 token
            | LessThan ->
              let (parser, (type_arguments, no_arg_is_missing)) =
          ...
          begin
            ...
            | Some (parser, name) ->
             ...
             new C::$x() as an expression. *)
              with_type_parser
                parser
                (TypeParser.parse_remaining_type_specifier name)
            | None -> default parser
          end
        | _ -> default parser
        ...
        | _ ->
          begin
            ...
            | Some (parser, attribute_spec, async, coroutine, signature) ->
              parse_lambda_expression_after_signature
                parser
                attribute_spec
                async
                coroutine
                signature
            | None -> parse_parenthesized_expression parser
          end
    ...
    * Otherwise, it's a parenthesized expression. *)
        ...
        let is_cast =
          Token.kind right_paren = RightParen
          &&
    ...
    *)
        ...
        try
          let (parser, attribute_spec) = Make.missing parser (pos parser) in
          let (parser, async, coroutine, signature) =
            parse_lambda_header parser
          in
          if
            old_errors = errors parser
    ...
    *)
      ...
      and parse_lambda_expression_after_signature
          parser attribute_spec async coroutine signature =
    ...
    *)
      ...
      and parse_remaining_binary_expression_helper
       ...
       See comments above for more details. *)
        ...
        if
          Operator.is_trailing_operator_token kind
          && (kind <> As || allow_as_expressions parser)
        then
          let right_operator = Operator.trailing_from_token kind in
          let right_precedence =
            Operator.precedence parser.env right_operator
          in
          let associativity =
            Operator.associativity parser.env right_operator
          in
    ...
    *)
        let missing_consequence =
          kind = Colon && not (is_next_xhp_class_name parser)
        in
        ...
        let (parser, term) = parse_term parser in
        let precedence =
          Operator.precedence parser.env Operator.ConditionalQuestionOperator
        in
      ...
      and parse_name_or_collection_literal_expression parser name =
        ...
        | LessThan ->
          ...
          then
            ...
            (parser, name)
        | LeftBracket ->
          if peek_token_kind ~lookahead:2 parser = EqualGreaterThan then
            parse_record_creation_expression parser name
          else
            (parser, name)
        | At ->
          if peek_token_kind ~lookahead:1 parser = LeftBracket then
            parse_record_creation_expression parser name
          else
            (parser, name)
        | _ -> (parser, name)
    ...
    *)
      ...
      *)
      ...
      and parse_bracketed_collection_intrinsic_expression
          parser keyword_token parse_element_function make_intrinsic_function =
  ...
  *)
      and parse_array_creation_expression parser =
        let (parser, left_bracket, members, right_bracket) =
          parse_bracketted_comma_list_opt_allow_trailing
            ...
            parse_array_element_init
        in
        Make.array_creation_expression
          parser
          left_bracket
          members
          right_bracket
      ...
      and parse_array_element_init parser =
        let (parser, expr1) = with_reset_precedence parser parse_expression in
        let (parser1, token) = next_token parser in
        match Token.kind token with
        | EqualGreaterThan ->
          let (parser, arrow) = Make.token parser1 token in
    ...
    *)
    ...
    so we do too. We might consider making it reserved. *)
       ...
       them as appropriate. *)
        ...
        | Variable
        | LeftParen ->
          parse_lambda_expression parser1 attribute_spec
        | _ ->
          let (parser, static_or_async_or_coroutine_as_name) =
            next_token_as_name parser
          in
      ...
      and parse_async_block parser attribute_spec =
        ...
        Make.awaitable_creation_expression
          parser
          attribute_spec
          async
          coroutine
    ...
    *)
      ...
      and parse_xhp_spread_attribute parser =
        ...
        let (parser, left_brace) = Make.token parser left_brace in
        let (parser, ellipsis) =
          require_token parser DotDotDot SyntaxError.expected_dotdotdot
        in
        let (parser, expression) =
          parse_expression_with_reset_precedence parser
        in
        ...
        if Token.kind token != Equal then
         ...
         looking for the next attribute. *)
          ...
          | XHPStringLiteral ->
            let (parser, token) = Make.token parser'' token in
            let (parser, node) =
              Make.xhp_simple_attribute parser name equal token
            in
            (parser, Some node)
          | LeftBrace ->
            let (parser, expr) = parse_braced_expression parser' in
            let (parser, node) =
              Make.xhp_simple_attribute parser name equal expr
            in
            (parser, Some node)
          | _ ->
         ...
         belongs to the attribute and start looking for the next attribute. *)
            ...
            let (parser, missing) = Make.missing parser (pos parser'') in
            let (parser, node) =
              Make.xhp_simple_attribute parser name equal missing
            in
            (parser, Some node)
      ...
      tooling can flag it as suspicious. *)
        ...
        | LessThan ->
          let (parser, expr) =
            parse_possible_xhp_expression ~in_xhp_body:true token parser1
          in
          (parser, Some expr)
        | _ -> (parser, None)
      ...
      and parse_xhp_close ~consume_trailing_trivia parser _ =
        ...
        let (parser, less_than_slash_token) =
          Make.token parser less_than_slash
        in
          ...
          if Token.kind name = XHPElementName then
            ...
            let (parser2, greater_than, _) =
              next_xhp_element_token
                ~no_trailing:(not consume_trailing_trivia)
                parser1
            in
            if Token.kind greater_than = GreaterThan then
              let (parser, greater_than_token) =
                Make.token parser2 greater_than
              in
              Make.xhp_close
                parser
              ...
              let (parser, missing) = Make.missing parser (pos parser) in
              Make.xhp_close parser less_than_slash_token name_token missing
          else
            (* ERROR RECOVERY: *)
            let parser = with_error parser SyntaxError.error1039 in
            let (parser, missing1) = Make.missing parser (pos parser) in
            let (parser, missing2) = Make.missing parser (pos parser) in
            Make.xhp_close parser less_than_slash_token missing1 missing2
        else
      ...
      and parse_xhp_expression
          ~consume_trailing_trivia parser left_angle name name_text =
        let (parser, attrs) =
          parse_list_until_none parser parse_xhp_attribute
        in
        let (parser1, token, _) =
          next_xhp_element_token ~no_trailing:true parser
        in
        match Token.kind token with
        | SlashGreaterThan ->
      ...
      and parse_possible_xhp_expression ~in_xhp_body less_than parser =
        ...
        if Token.kind name = XHPElementName then
          ...
          parse_xhp_expression
            ~consume_trailing_trivia:(not in_xhp_body)
            parser
            less_than
            token
            text
    ...
    Otherwise, parse them as ordinary names.  *)
        ...
        if peek_token_kind parser1 = ColonColon then
          ...
          let (parser, parent_or_self_or_static_as_name) =
            next_token_as_name parser
          in
        ...
        let (parser, name) =
          ...
          | Variable when Env.php5_compat_mode (env parser) ->
           ...
           in other cases fall back to the regular parsing logic *)
            if
              peek_token_kind parser1 = LeftParen
    ...
    *)
        ...
        Make.pocket_identifier_expression
          parser
          qualifier
          op_pu
          field_name
