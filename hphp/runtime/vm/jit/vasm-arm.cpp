...
void Vgen::emit(const incqmlock& i) {
  // FIXME: vixl in hphp/ doesn't contain (ld|st)xr instructions
  not_implemented();
}
...
void Vgen::emit(const roundsd& i) {
  ...
  case RoundDirection::floor:
    // FIXME: a->frintm(D(i.d), D(i.s)); 'frintm' is missing in vixl
    not_implemented();
    break;
  case RoundDirection:: ceil:
    // FIXME: a->frintp(D(i.d), D(i.s)); 'frintp' is missing in vixl
    not_implemented();
    break;
  default:
