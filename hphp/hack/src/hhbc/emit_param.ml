...
object(_)
  ...
  method! on_CIexpr scope this e =
    ...
    | Class_id ((p, _) as cid) -> A.CIexpr (Tast_annotate.with_pos p (A.Id cid))
    | _ -> this
...
let default_type_check param_name param_type_info param_expr =
  ...
  Option.bind default_type
    (fun t ->
      ...
      | Some "class" ->
        Some ("Default value for parameter " ^ param_true_name ^
          " with a class type hint can only be NULL")
      | Some h ->
        Some ("Default value for parameter " ^ param_true_name ^ " with type " ^
          t ^ " needs to have the same type as the type hint " ^ h)
      | _ ->  None)
...
let from_ast ~tparams ~namespace ~generate_defaults ~scope p =
  ...
  let _ =
    ...
    | None    -> ()
    | Some s  -> Emit_fatal.raise_fatal_parse p.A.param_pos s in
  let param_expr =
    Option.map p.A.param_expr ~f:(resolver_visitor#on_expr scope) in
...
let rename_params params =
  let names =
    List.fold_left
      params
      ~init:SSet.empty
      ~f:(fun n p -> SSet.add (Hhas_param.name p) n) in
  let rec rename param_counts param =
    ...
    | Some count ->
      ...
      else param_counts, (Hhas_param.with_name newname param) in
...
let from_asts ~namespace ~tparams ~generate_defaults ~scope ast_params =
  let hhas_params =
    List.filter_map
      ast_params
      ~f:(from_ast ~tparams ~namespace ~generate_defaults ~scope) in
