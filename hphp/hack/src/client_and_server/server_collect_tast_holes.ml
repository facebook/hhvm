...
let filter_source hole_filter src =
  ...
  | (Server_command_types.Tast_hole.Any, _) -> true
  | (Server_command_types.Tast_hole.Typing, Aast.Typing) -> true
  | (Server_command_types.Tast_hole.Cast, Aast.(UnsafeCast _ | EnforcedCast _))
    ->
    true
  | _ -> false
...
let visitor hole_filter =
  object (self)
    ...
    method private zero = Tast_holes_service.Set.empty

    method private plus t1 t2 = Tast_holes_service.Set.union t1 t2
...
let tast_holes ctx tast hole_src_opt =
  Tast_holes_service.Set.elements ((visitor hole_src_opt)#go ctx tast)
