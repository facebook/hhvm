...
bool TestCodeRun::TestSimpleXML() {
  ...
  MVCR("<?php\n"
       "$node = new SimpleXMLElement('<foo><bar>whoops<baz/></bar></foo>');\n"
       "$arr = (array)$node->bar;\n"
       "$arr['baz'] = (array)$arr['baz'];\n"
       "var_dump($arr);\n");
  MVCR("<?php\n"
       "$node = new SimpleXMLElement('<foo><bar>first<baz/>after</bar></foo>');\n"
       "$arr = (array)$node->bar;\n"
       "$arr['baz'] = (array)$arr['baz'];\n"
       "var_dump($arr);\n");
  ...
  MVCR("<?php class AdditionalXml extends SimpleXMLElement {}\n"
       "$obj = simplexml_load_string(\"<root><foo/></root>\", \"AdditionalXml\");\n"
       "var_dump(get_class($obj));\n");

  MVCR("<?php\n"
       "function __autoload($className) {\n"
         "class AdditionalXml extends SimpleXMLElement {}\n"
       "}\n"
       "$obj = simplexml_load_string(\"<root><foo/></root>\", \"AdditionalXml\");\n"
       "var_dump(get_class($obj));\n");
  ...
  MVCR("<?php\n"
       ...
       "<?xml version=\"1.0\"?>\n"
       "<config root=\"ROOT\">\n"
         "<global global=\"GLOBAL\" other=\"OTHER\">\n"
           "<a>4</a>\n"
           "<a>AA</a>\n"
           "<a>AAA</a>\n"
           "<a/>\n"
       ...
       "EOM;\n"
       "\n"
       "$local = simplexml_load_string($local, 'Varien_Simplexml_Element');\n"
       "$config = simplexml_load_string($config, 'Varien_Simplexml_Element');\n"
       "\n"
       "function xmlInfo($xml, $childCount = 0) {\n"
         "$indent = str_repeat(\"\\t\", $childCount);\n"
         "foreach($xml as $tmp) {\n"
           "checkNode($tmp);\n"
           "$children   = $tmp->children();\n"
           "$attributes = $tmp->attributes();\n"
           "echo \"{$indent}Name: {$tmp->getName()} (C: \".count($children).\" A: \".count($attributes).\")\\n\";\n"
           "if ($tmp->children()) {\n"
             "xmlInfo($tmp, $childCount + 1);\n"
           "}\n"
         "}\n"
       "}\n"
       "\n"
       "function checkNode($node) {\n"
         "if (!empty($node) !== (bool)$node) {\n"
           "var_dump('!empty($node) !== (bool)$node');\n"
         "}\n"
         "\n"
         "$children = $node->children();\n"
         "if (!empty($children) !== (bool)$node->children()) {\n"
           "var_dump('!empty($children) !== (bool)$node->children()');\n"
         "}\n"
         "\n"
         "$attributes = $node->attributes();\n"
         "if (!empty($attributes) !== (bool)$node->attributes()) {\n"
           "var_dump('!empty($attributes) !== (bool)$node->attributes()');\n"
         "}\n"
       "}\n"
       "\n"
       "echo \"Local:\\n\";\n"
       ...
       "$local->extend($config);\n"
       "\n"
       "echo \"\n\nLocal merged with config:\\n\";\n"
       "xmlInfo($local);\n");

  MVCR("<?php\n"
       "$xml = simplexml_load_string('<root><foo><bar attr=\"ATTR\"/></foo></root>');\n"
       "$children = $xml->children()->children();\n"
       "function show($children) {\n"
         "var_dump($children->getName());\n"
         "var_dump((array)$children->attributes());\n"
         "$tmp = (array)$children->children();\n"
         "if(isset($tmp['addFirst']) && is_object($tmp['addFirst'])) {\n"
           "$tmp['addFirst'] = get_class($tmp['addFirst']);\n"
         "}\n"
         "var_dump($tmp);\n"
         "echo \"\\n\\n\";\n"
       "}\n"
       "var_dump($children->asXml());\n"
       "show($children);\n"
       "$children->addChild('addFirst');\n"
