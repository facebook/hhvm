...
static bool HHVM_METHOD(XMLWriter, setIndentString,
  ...
  int ret = -1;
  if (data->m_ptr) {
    ret = xmlTextWriterSetIndentString(data->m_ptr, (xmlChar*)indentstring.data());
  }
  return ret != -1;
}
...
static bool HHVM_METHOD(XMLWriter, startDTDElement,
  ...
  int ret = -1;
  if (data->m_ptr) {
    ret = xmlTextWriterStartDTDElement(data->m_ptr, (xmlChar*)qualifiedname.data());
  }
  return ret != -1;
}
...
static bool HHVM_METHOD(XMLWriter, startDTDEntity,
  ...
  int ret = -1;
  if (data->m_ptr) {
    ret = xmlTextWriterStartDTDEntity(data->m_ptr, isparam, (xmlChar*)name.data());
  }
  return ret != -1;
}
