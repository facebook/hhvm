...
and fmt_hint ~tparams ~namespace ?(strip_tparams=false) (pos, h) =
  ...
  | Aast.Hshape { Aast.nsi_field_map; _ } ->
    let fmt_field_name name =
      ...
      | A.SFclass_const (cid, (_, s2)) ->
        fmt_name_or_prim cid ^ "::" ^ s2 in
    let format { Aast.sfi_hint; Aast.sfi_optional; Aast.sfi_name } =
      let prefix = if sfi_optional then "?" else "" in
      prefix ^ fmt_field_name sfi_name ^ "=>"
        ^ fmt_hint ~tparams ~namespace sfi_hint in
    let shape_fields =
      List.map ~f:format nsi_field_map in
  ...
  | Aast.Hvarray_or_darray _ -> fmt_name_or_prim (pos, SN.Typehints.varray_or_darray)
...
let rec hint_to_type_constraint ~kind ~tparams ~skipawaitable ~namespace (p, h) =
  let happly_helper (pos, name as id) =
    if List.mem ~equal:(=) tparams name
    then
      let tc_name = Some "" in
      ...
      then
        ...
        TC.make (Some tc_name) tc_flags in
  match h with
...
let param_hint_to_type_info ~kind ~skipawaitable ~nullable ~tparams ~namespace h =
  let is_simple_hint =
    match snd h with
    | Aast.Hsoft _
    | Aast.Hoption _
  ...
  let name, flags =
    ...
    | Some t, Some ret ->
      ...
      let name =
        ...
        Some (vanilla_name @@ strip_nullable @@ strip_soft @@ strip_nullable t) in
      let flags = [TC.HHType; TC.ExtendedHint] in
      let rec get_flags (_, t) flags =
        match t with
        | Aast.Hoption x -> TC.Nullable :: TC.DisplayNullable :: (get_flags x flags)
