...
let check_arraykey_index env pos container_ty index_ty =
  if TypecheckerOptions.disallow_invalid_arraykey (Env.get_tcopt env)
  then
    let reason =
      match container_ty with
      | _, Tclass((_, cn), _, _) -> Reason.index_class cn
      | _ -> Reason.index_array in
...
let rec array_get ~array_pos ~expr_pos ?(lhs_of_null_coalesce=false) is_lvalue env ty1 e2 ty2 =
  let env, ty1 =
    Typing_solver.expand_type_and_narrow env
  ...
  GenericRules.apply_rules ~ignore_type_structure:true env ty1
  begin fun env ty1 ->
      ...
      || cn = SN.Collections.cKeyset ->
        if cn = SN.Collections.cKeyset && is_lvalue then begin
          ...
          let env =
            ...
            then check_arraykey_index env expr_pos ety1 ty2
...
let assign_array_get ~array_pos ~expr_pos ur env ty1 key tkey ty2 =
  ...
  GenericRules.apply_rules env ety1
  begin fun env ety1 ->
    let (r, ety1_) = ety1 in
    let arity_error (_, name) =
      Errors.array_get_arity expr_pos name (Reason.to_pos r) in
