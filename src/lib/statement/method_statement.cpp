...
FunctionScopePtr MethodStatement::onParseImpl(AnalysisResultPtr ar) {
    ...
    if (ar->isFirstPass()) {
      for (i = 0; i < maxParam; i++) {
        ...
        if (names.find(param->getName()) == names.end()) {
          ...
          ar->getCodeError()->record(self, CodeError::RedundantParameter, param);
  ...
  FunctionScopePtr funcScope
    (new FunctionScope(ar, m_method, m_name, stmt, m_ref, minParam, maxParam,
                       m_modifiers, m_attribute, ar->getFileScope()));
  if (!m_stmt) {
  ...
  if (ar->getClassScope()) {
    ...
    // ArrayAccess methods
      ...
      if (m_name == "offsetexists") {
        ...
        funcScope->setOverriding(TypePtr(), Type::Variant, Type::Variant);
      } else if (m_name == "offsetunset") {
        funcScope->setOverriding(TypePtr(), Type::Variant);
      }
    }
  }
...
void MethodStatement::outputCPP(CodeGenerator &cg, AnalysisResultPtr ar) {
  FunctionScopePtr funcScope = m_funcScope.lock();
  ...
  if (ac > 0 || varArgs || !isStatic || !ret && inClass
   || cg.getContext() == CodeGenerator::JavaFFIInterface) {
    ...
    cg.printf("public %s%s %s(",
              (isStatic ? "static " : ""),
              (!ret && !inClass ? "void" : "HphpVariant"), originalName.c_str());
    ostringstream args;
    bool first = true;
    if (!isStatic) {
