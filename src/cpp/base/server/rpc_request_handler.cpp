...
void RPCRequestHandler::handleRequest(Transport *transport) {
  ...
  if (RuntimeOption::SandboxMode && !sourceRootInfo.sandboxOn()) {
    transport->sendString("Sandbox configuration is incorrect.", 404);
    transport->onSendEnd();
    return;
  }
...
bool RPCRequestHandler::executePHPFunction(Transport *transport,
  ...
  if (!sparams.empty()) {
    Variant jparams = f_json_decode(String(sparams));
    if (jparams.isArray()) {
    ...
    if (!sparams.empty()) {
      for (unsigned int i = 0; i < sparams.size(); i++) {
        Variant jparams = f_json_decode(String(sparams[i]));
