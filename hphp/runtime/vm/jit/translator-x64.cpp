...
/**
 * The prologues are regenerated the first time one of the function
 * entries (either it's main body entry or one of it's DV funclets) is
 * retranslated.  So we can tell if the prologues have been already
 * regenerated by checking if any of the entries were already
 * retranslated/optimized.
 */
bool TranslatorX64::prologuesWereRegenerated(const Func* func) {
  // Check if main body was already optimized.
  if (m_profData->optimized(SrcKey(func, func->base()))) return true;

  // Check if any DVFunclet was already optimized.
  for (auto funcletPair : func->getDVFunclets()) {
    Offset funcletOffset = funcletPair.second;
    if (m_profData->optimized(SrcKey(func, funcletOffset))) {
      return true;
    }
  }
  return false;
...
TranslatorX64::createTranslation(const TranslArgs& args) {
  ...
  if (stubsize && RuntimeOption::EvalDumpTCAnchors) {
    ...
    addTranslation(tr);
    if (RuntimeOption::EvalJitUseVtuneAPI) reportTraceletToVtune(sk.unit(), sk.func(), tr);
    if (m_profData) {
...
TranslatorX64::getFuncPrologue(Func* func, int nPassed, ActRec* ar) {
  ...
  addTranslation(tr);
  if (RuntimeOption::EvalJitUseVtuneAPI) reportTraceletToVtune(func->unit(), func, tr);
 ...
 * regenerate the prologue (as a TransPrologue).
 */
void TranslatorX64::regeneratePrologue(TransID prologueTransId) {
 ...
 */
void TranslatorX64::regeneratePrologues(Func* func) {
  ...
  case JIT::REQ_BIND_CALL: {
    ...
    if (dest) {
      ...
      if (writer && JIT::callTarget(toSmash) != dest) {
        ...
        if (func->shouldPGO() && !prologuesWereRegenerated(func)) {
...
TranslatorX64::emitNativeTrampoline(TCA helperAddr) {
  ...
  if (RuntimeOption::EvalJitUseVtuneAPI) reportTrampolineToVtune(trampAddr, trampolinesCode.frontier() - trampAddr);
