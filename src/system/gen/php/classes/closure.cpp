  ...
  switch (hash & 15) {
    ...
    case 15:
      HASH_GUARD_LITSTR(0x0D31D0AC229C615FLL, NAMSTR(s_sys_ssa1b87da7, "__construct")) {
        ...
        int count __attribute__((__unused__)) = params.size();
        if (count < 2 || count > 3) return throw_wrong_arguments("Continuation::__construct", count, 2, 3, 2);
...
CallInfo c_Continuation::ci___construct((void*)&c_Continuation::i___construct, (void*)&c_Continuation::ifa___construct, 3, 4, 0x0000000000000000LL);
...
c_Continuation *c_Continuation::create(Variant v_func, Variant v_vars, Variant v_obj //  = null
  ...
  CountableHelper h(this);
  init();
  t___construct(v_func, v_vars, v_obj);
  return this;
}
ObjectData *c_Continuation::dynCreate(CArrRef params, bool construct /* = true */) {
  ...
  if (construct) {
    CountableHelper h(this);
    int count __attribute__((__unused__)) = params.size();
    if (count < 2 || count > 3) throw_wrong_arguments("Continuation::__construct", count, 2, 3, 2);
    do {
      ArrayData *ad(params.get());
      ssize_t pos = ad ? ad->iter_begin() : ArrayData::invalid_index;
      CVarRef arg0((ad->getValue(pos)));
...
void c_Continuation::dynConstructFromEval(Eval::VariableEnvironment &env, const Eval::FunctionCallExpression *caller) {
  ...
  int count __attribute__((__unused__)) = params.size();
  if (count < 2 || count > 3) throw_wrong_arguments("Continuation::__construct", count, 2, 3, 1);
...
/* SRC: classes/closure.php line 38 */
...
namespace hphp_impl_splitter {}
/* SRC: classes/closure.php line 42 */
  ...
  if (toBoolean(m_obj)) {
    {
      v_func = m_func;
      {
        ...
        mcp1.methodCall((m_obj.objectForCall()), toString(v_func), -1);
        const CallInfo *cit1  __attribute__((__unused__)) = mcp1.ci;
        Variant tmp2(((mcp1.bindClass(info)->getMeth1Args())(mcp1, 1, ((p_Continuation&)GET_THIS()))));
        m_value = tmp2;
      }
    }
  }
    ...
    {
      {
        const Variant &tmp3((x_call_user_func(2, m_func, Array(ArrayInit(1, true).setRef(Variant(GET_THIS())).create()))));
        m_value = tmp3;
      }
    }
  }
...
namespace hphp_impl_splitter {}
/* SRC: classes/closure.php line 61 */
void c_Continuation::t_rewind() {
  INSTANCE_METHOD_INJECTION_BUILTIN(Continuation, Continuation::rewind);
  m_label = NAMSTR(s_sys_ss00000000, "");
  m_index = -1LL;
  {
    MethodCallPackage mcp1;
    mcp1.methodCall((GET_THIS()), NAMSTR(s_sys_ss50652d33, "next"), 0x3C6D50F3BB8102B8LL);
    const CallInfo *cit1  __attribute__((__unused__)) = mcp1.ci;
    (mcp1.bindClass(info)->getMeth0Args())(mcp1, 0);
  }
}
namespace hphp_impl_splitter {}
/* SRC: classes/closure.php line 66 */
