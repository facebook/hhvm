    ...
    begin match snd e with
      | Call _ | Omitted ->
        error_at env pos begin
          Printf.sprintf "Invalid argument for an %s parameter (must be lvalue)"
      ...
      | List _ ->
        error_at env pos begin
          Printf.sprintf "Cannot pass list intrinsic for an %s parameter"
            (string_of_param_kind flavor)
        end
...
and xhp_body pos name env =
   ...
   * bit of markup *)
  let abs_start = env.lb.Lexing.lex_curr_pos in
  let start_pos = env.lb.Lexing.lex_curr_p in
  let start = Pos.make_from_lexing_pos env.file start_pos start_pos in
