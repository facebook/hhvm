...
let test_avg_mem () =
  ...
  avg_mem heap1 heap2 |> avg_mem ~size2:2 heap1 = {
    heap_major_words = (2. +. 5. +. 2.) /. 3.;
    heap_major_chunks = (6. +. 3. +. 6.) /. 3.;
  }

let test_profile_longer_than () =
  let user_time, nbr_runs, mem = profile_longer_than (fun () ->
  ...
  user_time >= 1e-8  (* to avoid flakiness, use 5 orders of magnitude smaller *)
    && mem.heap_major_words >= 0. && mem.heap_major_chunks >= 1.

let test_profile_longer_than_min_runs_gt1 () =
  let min_runs = 7 in
  let cnt = ref 0 in
  let _, nbr_runs, _ = profile_longer_than ~min_runs (fun () -> cnt := !cnt + 1) 0.0 in
