...
class TDebugProtocol : public TVirtualProtocol<TDebugProtocol> {
 ...
 public:
  ...
  static const int32_t DEFAULT_STRING_PREFIX_SIZE = 128;
...
class TDebugProtocolEx : public apache::thrift::protocol::TDebugProtocol {
 public:
  ...
  template <typename T>
  std::enable_if_t<!std::is_enum_v<T>, void> write(const T& t) {
    t.write(this);
  }

  template <typename T>
  std::enable_if_t<std::is_enum_v<T>, void> write(const T value) {
