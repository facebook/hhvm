...
final class ImmVector<+Tv> implements ConstVector<Tv> {
   ...
   *              the ImmVector (e.g., array). If null, then an empty
   ...
   *
   * @return - true if the specified key is present in the current ImmVector;
   *           false otherwise.
   *
   ...
   * constructor.
   *
   * @param $items - any traversable object from which to create an ImmVector
   *                 (e.g., array). If null, then an empty ImmVector is created.
   *
   * @return An ImmVector with the values from the Traversable; or an empty
   *         ImmVector if the Traversable is null.
   */
   ...
   * the next available integer key in the current ImmVector and makes that key
   * the value.
   *
  ...
  /**
   * Returns the string version of the current ImmVector, which is "ImmVector".
   *
 ...
 /**
   ...
   *
   * @return - an integer-keyed Map with the values of the current ImmVector.
   */
  ...
  /**
   * Returns an immutable Set (ImmSet) with the values of the current ImmVector.
   *
   ...
   *
   * Every value in the current ImmVector is affected by a call to map(), unlike
  ...
  /**
   * Returns an ImmVector containing the values of the current ImmVector that
   ...
   *  number of elements in the Traversable, then only the combined elements up
   *  to and including the final element of the one with the least number of
   *  elements is included.
   *
   *  @param $traversable - The traversable to use to combine with the elements
   *                        of the current ImmVector.
   *
   *  @return - The ImmVector that combines the values of this ImmVector with
  ...
  /**
   * Returns an ImmVector containing the first n values of the current ImmVector.
   *
   * The returned ImmVector will always be a proper subset of this ImmVector.
   *
   * n is 1-based. So the first element is 1, the second 2, etc.
   *
   * @param $n - The last element that will be included in the ImmVector.
   *
   * @return - An ImmVector that is a proper subset of the curretn ImmVector up
   *           to n elements.
   */
  public function take(int $n): ImmVector<Tv>;

  /**
   * Returns an ImmVector containing the values of the current ImmVector up to
   ...
   *
   * @return - An ImmVector that is a proper subset of the current ImmVector up
   *           until when the callback returns false.
   */
  public function takeWhile((function(Tv): bool) $fn): ImmVector<Tv>;

  /**
   * Returns an ImmVector containing the values after the n-th element of the
   * current ImmVector.
   *
   * The returned ImmVector will always be a proper subset of the current
   * ImmVector.
  ...
  /**
   * Returns a subset of the current ImmVector starting from a given key up to, '
   ...
   *
   * @return - The first key (an integer) in the current ImmVector, or null if
   *           the current ImmVector is empty.
   */
   ...
   *
   * @return - The last key (an integer) in the current ImmVector, or null if
   *           the current ImmVector is empty.
   */
