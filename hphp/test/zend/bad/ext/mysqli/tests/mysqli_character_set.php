<?php
	...
	foreach ($charsets as $charset) {
		...
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_character_set_table_1"))
			printf("[006 + %s] [%d] %s\n", $k, mysqli_errno($link), mysqli_error($link));

		$sql = sprintf("CREATE TABLE test_mysqli_character_set_table_1(id INT, label CHAR(1)) CHARACTER SET '%s' ", $charset['Charset']);
		...
		for ($i = 1; $i <= 3; $i++) {
			if (!mysqli_query($link, sprintf("INSERT INTO test_mysqli_character_set_table_1 (id, label) VALUES (%d, '%s')",
		...
		if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_character_set_table_1"))
			printf("[010 + %s] [%d] %s\n", $k, mysqli_errno($link), mysqli_error($link));
...
?>
<?php
	$test_table_name = 'test_mysqli_character_set_table_1'; require_once("clean_table.inc");
?>