...
#include <cpp/base/zend/intl_convert.h>
...
static ThreadLocal<UErrorCode> s_errcode;
...
static DataType collator_is_numeric(UChar *str, int length, long *lval,
  ...
  if (allow_errors == -1) {
    Logger::Info("A non well formed numeric value encountered");
  }
...
static String collator_convert_str_utf16_to_utf8(CStrRef utf16_str,
  ...
  intl_convert_utf16_to_utf8(&str, &str_len,
                             ...
                             status);
  if (U_FAILURE(*status)) {
    Logger::Verbose("Error converting utf16 to utf8");
  }
...
static bool collator_sort_internal(bool renumber, Variant &array,
  ...
  *s_errcode = U_ZERO_ERROR;
  ...
  if (U_FAILURE(*s_errcode)) {
    Logger::Verbose("Error converting array from UTF-8 to UTF-16");
    return false;
  }
  ...
  *s_errcode = U_ZERO_ERROR;
  ...
  if (U_FAILURE(*s_errcode)) {
    ...
    Logger::Error("Error converting array from UTF-16 to UTF-8");
    return false;
  }
  array = temp;
  return true;
