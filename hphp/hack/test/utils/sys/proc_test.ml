...
let systemd_cmdline = "/usr/lib/systemd/systemd\x00--switched-root\x00--system\x00--deserialize\x0030\x00"
(* This is a small subset of a Java tool's cmdline contents *)
let java_cmdline = "toold\x00-Xmx1000m\x00-Djava.awt.headless=true\x00-Djna.nosys=true\x00" ^
  "-Djava.util.logging.config.class=com.company.tool.abc.bootstrapper.SomeConfig\x00-Dtool.test_util_no_tests_dir=true"
let python_cmdline1 = "[xarexec]\x00/usr/local/bin/threshold-monitor\x00-tt\x00/mnt/xarfuse/uid-0/123/__run_xar_main__.py"
let python_cmdline2 = "[xarexec]\x00/usr/local/bin/threshold-monitor\x00-tt\x00/mnt/xarfuse/uid-0/456/__run_xar_main__.py"
...
let create_cmdline (pid : int) (cmdline : string) : unit =
  ...
  Disk.write_file
    (Printf.sprintf cmdline_path_format proc_dir)
    cmdline
...
let ok_or_assert (r : ('a, string) result) : 'a =
  match r with | Error e -> (Printf.eprintf "%s\n" e); assert false | Ok r -> r

let test_get_cmdline_self () : bool =
  let cmdline = Proc.get_cmdline proc_test_pid.contents in
  match cmdline with
  | Error e ->
    (Printf.eprintf "%s" e); assert false
  | Ok c ->
...
let test_get_proc_stat_systemd () : bool =
  let proc_stat = ok_or_assert (Proc.get_proc_stat 1) in
    ...
    Int_asserter.assert_equals
      0
      proc_stat.Proc.ppid
      "The process's parent PID should be correct!";
    true
