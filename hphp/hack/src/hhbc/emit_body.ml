...
let emit_method_prolog ~env ~pos ~params ~ast_params ~should_emit_init_this =
  let init_this =
    if not should_emit_init_this
    then empty
    else instr (IMisc (InitThisLoc (Local.Named "$this"))) in
  let ast_params =
    List.filter
      ast_params
      ~f:(fun p -> not (p.A.param_is_variadic && p.A.param_name = "...")) in
  let make_param_instr (param, ast_param) =
    if Hhas_param.is_variadic param
    then None
    else
  ...
  let param_instr =
    List.filter_map
      ~f:make_param_instr
      (List.zip_exn params ast_params) in
...
and emit_defs env defs =
  ...
  let rec emit_markup env defs =
    ...
    | A.Stmt (_, A.Markup ((_, s), echo_expr_opt)) :: defs ->
      let i1 =
        Emit_statement.emit_markup env s echo_expr_opt ~check_for_hashbang:true in
      let i2 = aux env defs in
      gather [i1; i2]
    | [_]
    | [] -> aux env defs
    | d :: defs ->
      ...
      else gather [i1; aux env defs] in
  emit_markup env defs

let make_body
    body_instrs decl_vars
...
let emit_return_type_info ~scope ~skipawaitable ~namespace ret =
  ...
  | Some h ->
    Emit_type_hint.hint_to_type_info
      ~kind:Emit_type_hint.Return ~nullable:false ~skipawaitable ~tparams ~namespace h
...
let emit_deprecation_warning scope deprecation_info =
  ...
  | Some args ->
    ...
    let fn_name =
      ...
      | _ -> failwith "deprecated functions must have names" in
    let deprecation_string = class_name ^ fn_name ^ ": " ^
      match args with
      | [] -> "deprecated function"
      | Typed_value.String s :: _ -> s
      | _ -> failwith "deprecated attribute first argument is not a string" in
    let sampling_rate =
      match args with
      | [] | [_] -> Int64.one
      | _ :: Typed_value.Int i :: _ -> i
      | _ -> failwith "deprecated attribute second argument is not an integer" in
    let error_code =
      if Emit_env.is_systemlib ()
      then 8192 (* E_DEPRECATED *)
      else 16384 (* E_USER_DEPRECATED *) in
    if Int64.to_int_exn sampling_rate <= 0
    then empty
    else
      gather [
        trait_instrs;
...
let rec is_awaitable h =
  ...
  | Aast.Happly ((_, "Awaitable"), [_]) -> true
  | Aast.Hsoft h
  | Aast.Hoption h -> is_awaitable h
  | _ -> false
...
let emit_body
    ...
    ~return_value
    ~namespace
    ~doc_comment immediate_tparams ast_params ret (body : Tast.program) =
  ...
  let tparams =
    List.map
      ~f:(fun t -> snd t.A.tp_name)
      (Ast_scope.Scope.get_tparams scope) in
  Label.reset_label ();
  ...
  let params =
    Emit_param.from_asts
      ~namespace ~tparams ~generate_defaults:(not is_memoize) ~scope ast_params in
  let params =
    if is_closure_body
    then List.map ~f:Hhas_param.switch_inout_to_reference params
    else params in
    ...
    then
      ...
      let filter_vars cvl =
        let aux cv =
          ...
          ("$" ^ id) in
        List.map ~f:aux cvl in
      let captured_vars = filter_vars ast_class.A.c_vars in
      "$0Closure" ::
        captured_vars @
        List.filter
          (move_this decl_vars)
          ~f:(fun v -> not (List.mem ~equal:(=) captured_vars v))
    else
      match scope with
  ...
  begin
    match SMap.get function_state_key @@ Emit_env.get_function_to_labels_map () with
    | Some s ->
  ...
  let should_emit_init_this =
    not is_in_static_method &&
      (needs_local_this || (is_toplevel && List.exists ~f:(fun x -> x = "$this") decl_vars)) in
  let header =
    ...
    let header_content = gather [
      if is_native
      then empty
      else emit_method_prolog ~env ~pos ~params ~ast_params ~should_emit_init_this;
