...
$extra_arg = 10;
$count = -1;
try { var_dump(preg_replace_callback($regex, fun('integer_word'), $subject, $limit, inout $count, $extra_arg)); } catch (Exception $e) { echo "\n".'Warning: '.$e->getMessage().' in '.__FILE__.' on line '.__LINE__."\n"; }
...
$regex = '/\d/';
try { var_dump(preg_replace_callback($regex, fun('integer_word'))); } catch (Exception $e) { echo "\n".'Warning: '.$e->getMessage().' in '.__FILE__.' on line '.__LINE__."\n"; }
echo "Done";
}
