...
class Reflection {
   ...
   * )
   *
   * Exports a reflection. Warning: This function is currently not documented;
   * only its argument list is available.
   *
   * @reflector  Reflector  The reflection to export.
   * @return     bool       Setting to TRUE will return the export, as opposed
   ...
   * only its argument list is available.
   *
   * @modifiers  int        The modifiers to get, which is from a numeric
   *                        value.
...
class ReflectionParameter implements Reflector {
    ...
    if ($func) {
      if(!$func instanceof ReflectionFunctionAbstract) {
        if(is_array($func) && count($func) == 2) {
          $func = new ReflectionMethod($func[0], $func[1]);
        } elseif(is_array($func)) {
          $func = new ReflectionFunction($func[0]);
        } else {
      ...
      if(!$this->info) {
        throw new ReflectionException('Unknown parameter ' . $param);
      }
      $this->function = $func;
  ...
  public function __toString() {
    ...
    if($this->isOptional()) {
      $flag = 'optional';
      $default = sprintf(' = %s', var_export($this->getDefaultValue(), true));
    } else {
      $flag = 'required';
      $default = '';
    }
    return sprintf("Parameter #%d [ <%s> $%s%s ]", $index, $flag, $name,
                    $default);
  }
...
class ReflectionFunctionAbstract {
  ...
  public function getStaticVariables() {
    $vars = $this->info['static_variables'];
    foreach($vars as &$val) {
      if($val === 'null') {
        $val = null;
      } else {
...
class ReflectionClass implements Reflector {
  ...
  public function __toString() {
    ...
    $str .= sprintf("%s %s ] {\n", implode(' ', $types), $this->info['name']);
    if(!empty($this->info['file'])) {
      $str .= sprintf("  @@ %s %d-%d\n\n", $this->info['file'],
              $this->info['line1'], $this->info['line2']);
    }

    // Constants
    $constants = $this->getConstants();
    $str .= sprintf("  - Constants [%d] {\n", count($constants));
    foreach($constants as $name => $value) {
      $str .= sprintf("    Constant [ %s %s ] { %s }\n", gettype($value),
                      $name, $value);
    }
    $str .= "  }\n\n";
  ...
  // Methods
  $str .= sprintf("  - Methods [%d] {\n", count($methods));
  $body = [];
  foreach($methods as $m) {
    $render = (string)$m;
    $render = preg_replace('/^(.)/m', '    \1', $render);
    $body[] = $render;
  }
  $str .= implode($body, "\n");
  $str .= "  }
  ...
  public function getMethod($name) {
    if (func_num_args() != 1) {
      if(func_num_args() > 1) {
        user_error(sprintf('%s expects exactly 1 parameter, %d given',
                            __FUNCTION__, func_num_args()), E_USER_WARNING);
      }
      return null;
    } elseif(is_array($name)) {
      user_error(sprintf(
        'Argument 1 passed to %s() must be an instance of string, array given',
        __METHOD__), E_USER_WARNING);
      return null;
    } elseif(is_object($name) && !method_exists($name, '__toString')) {
      user_error(sprintf(
        'Argument 1 passed to %s() must be an instance of string, object given',
        __METHOD__), E_USER_WARNING);
      return null;
    }
    if (!$this->info) {
  ...
  public function getProperties($filter = 0xFFFF) {
    if(!is_int($filter)) {
      user_error(sprintf('%s() expects parameter 1 to be long, %s given',
                          __METHOD__, gettype($filter)), E_USER_WARNING);
      $filter = (int)$filter;
    }
    $ret = array();
  ...
  public function getDefaultProperties() {
    ...
    foreach ($this->getProperties() as $prop) {
      if ($prop->isDefault()) {
        $ret[$prop->name] = $prop->info['default'];
      }
    }
    return $ret;
  }
...
class ReflectionProperty implements Reflector {
  ...
  public function isDefault() {
    ...
    return array_key_exists('default', $this->info) || $this->isStatic();
  }
  ...
  public function getValue($obj = null) {
    ...
    if (!$this->isPublic() && !$this->forceAccessible) {
      throw new ReflectionException(sprintf(
          'Cannot access non-public member %s::%s', $this->class,
          $this->info['name']
        ));
    }
    if ($this->isStatic()) {
    ...
    if ($obj && is_object($obj)) {
      ...
      user_error(sprintf('First parameter to %s must be an object, %s given',
                          __FUNCTION__, gettype($obj)), E_USER_WARNING);
    } else {
      user_error(sprintf('%s expects exactly 1 parameter, 0 given',
  ...
  public function setValue($obj, $value = null) {
    ...
    if (($this->isStatic() && $num < 1) ||
        ...
        $num > 2) {
      if ($num) {
        $expected = $this->isStatic() ? 1 : 2;
        user_error(sprintf('%s expects exactly %d parameters, %d given',
                            __FUNCTION__, $expected, $num), E_USER_WARNING);
      }
      return null;
    }
    if ($num == 1) {
      $value = $obj;
    } elseif (!$this->isStatic() && !is_object($obj)) {
      user_error(sprintf('First parameter to %s must be an object, %s given',
                          __FUNCTION__, gettype($obj)), E_USER_WARNING);
      return null;
    }
...
implements Reflector {
   ...
   * @return     mixed   No value is returned.
   */
  public function __construct($cls, $name = null) {
    ...
    if ($name === null) {
      ...
      if (count($arr) == 2) {
        ...
        throw new ReflectionException("Invalid method name $cls");
      }
    }
    ...
    if(is_string($cls)) {
      if(!class_exists($cls) && !interface_exists($cls)
          && !trait_exists($cls)) {
        throw new ReflectionException("Class $cls does not exist");
      }
    ...
    $this->_class = is_object($cls) ? get_class($cls) : $cls;
  }
  ...
  public function __set($name, $value) {
    if($name == 'class' || $name == 'name') {
      if($name == 'class' && $this->_class === null) {
        $this->_class = $value;
        return;
      } elseif($name == 'name' && $this->_name === null) {
        $this->_name = $value;
        return;
      }
      throw new ReflectionException(sprintf(
        'Cannot set read-only property %s::$%s', get_class($this), $name
      ));
    } else {
      $this->$name = $value;
    }
  ...
  public function __toString() {
    ...
    $start = $this->info['class'];
    $parent = get_parent_class($start);

    if($parent) {
      $info = hphp_get_method_info($parent, $this->name);
      if($info) {
        $params[] = 'overwrites ' . $info['class'];
      }
    }

    $prototype = $this->getPrototype();
    if(strcasecmp($prototype->class, $this->_class)) {
      $params[] = 'prototype ' . $prototype->class;
    }
    ...
    $str .= implode(' ', $flags) . ' method ' . $this->name;
    $str .= " ] {\n  @@ ";
    $str .= $this->info['file'] . ' ';
    $str .= $this->info['line1'] . ' - ' . $this->info['line2'];
  ...
  public function invoke($obj) {
    if ($this->info['accessible']) {
      if($this->isStatic()) {
        ...
        if(!is_object($obj)) {
          throw new ReflectionException('Non-object passed to Invoke()');
        } elseif(!is_a($obj, $this->info['class'])) {
          throw new ReflectionException('Given object is not an instance of the class this method was declared in');
        }
      }
      $args = func_get_args();
      array_shift($args);
      return hphp_invoke_method($obj, $this->_class, $this->info['name'],
                                $args);
    }
    else {
  ...
  public function isConstructor() {
    if(!empty($this->info['constructor'])) {
  ...
  public function getPrototype() {
    ...
    return new ReflectionMethod($base, $this->name);
  }
...
class ReflectionExtension implements Reflector {
  ...
  public function getClasses() {
    $names = $this->info['classes'];
    $classes = [];
    foreach($names as $name) {
      $classes[$name] = new ReflectionClass($name);
    }
    return $classes;
  }
  ...
  /**
   * ( excerpt from http://php.net/reflectionextension.getdependencies )
   *
