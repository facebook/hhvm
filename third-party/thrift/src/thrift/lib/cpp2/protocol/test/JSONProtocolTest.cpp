...
TEST_F(JSONProtocolTest, writeDouble) {
  const vector<ProtocolWriteTestCase<double>> kTests = {
      ...
      {numeric_limits<double>::lowest(), "-1.7976931348623157e+308"},
...
TEST_F(JSONProtocolTest, writeFloat) {
  const vector<ProtocolWriteTestCase<float>> kTests = {
      ...
      {-1234.f, "-1234"},
      {1234567.f, "1234567"},
      {numeric_limits<float>::max(), "3.4028235e+38"},
      {nextafter(numeric_limits<float>::max(), 0.f), "3.4028233e+38"},
      {numeric_limits<float>::lowest(), "-3.4028235e+38"},
