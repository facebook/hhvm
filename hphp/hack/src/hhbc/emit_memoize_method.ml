...
let make_info ast_class class_id ast_methods =
  let check_method ast_method =
    ...
    else if ast_method.T.m_abstract
    then
  ...
  let instance_count =
    List.count
      ast_methods
      ~f:(fun ast_method -> is_memoize ast_method && not ast_method.T.m_static) in
  {
...
let make_memoize_static_method_with_params_code ~pos
    env info scope deprecation_info method_id with_lsb params ast_params is_async is_reified =
  let param_count = List.length params in
  let notfound = Label.next_regular() in
  let suspended_get = Label.next_regular() in
  let eager_set = Label.next_regular() in
...
let make_memoize_static_method_code
    ~pos env info scope deprecation_info method_id with_lsb params ast_params is_async is_reified =
...
let make_memoize_instance_method_code
    ~pos env scope deprecation_info method_id params ast_params is_async is_reified =
  if List.is_empty params && not is_reified
  then
...
let emit_memoize_wrapper_body env memoize_info ast_method
    ~namespace scope deprecation_info params ret is_async is_reified =
  let is_static = ast_method.T.m_static in
  let tparams =
    List.map
      (Ast_scope.Scope.get_tparams scope)
      ~f:(fun t -> snd t.T.tp_name) in
  ...
  let with_lsb =
    Emit_attribute.ast_any_is_memoize_lsb ast_method.T.m_user_attributes in
  emit ~pos env memoize_info return_type_info scope deprecation_info
    params ast_method.T.m_params is_static method_id with_lsb is_async is_reified
...
let make_memoize_wrapper_method env info ast_class ast_method =
  ...
  let ret =
    ...
    else T.hint_of_type_hint ast_method.T.m_ret in
  let method_id = Hhbc_id.Method.from_ast_name original_name in
  let scope =
...
let emit_wrapper_methods env info ast_class ast_methods =
  ...
  let hhas_methods =
    List.fold_left ast_methods
      ~init:[]
      ~f:(fun acc ast_method ->
        ...
        then
          ...
          let hhas_method =
            make_memoize_wrapper_method env info ast_class ast_method in
          hhas_method :: acc
        else acc) in
  hhas_methods
