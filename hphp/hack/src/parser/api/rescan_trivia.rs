...
use parser::{
    compact_token::CompactToken, lexable_token::LexableToken, minimal_trivia::MinimalTrivium,
    positioned_trivia::PositionedTrivium, source_text::SourceText,
...
impl RescanTrivia<MinimalTrivium> for CompactToken {
    fn scan_leading(&self, source_text: &SourceText<'_>) -> Vec<MinimalTrivium> {
        let f = if self.kind() == TokenKind::XHPBody {
            minimal_parser::scan_leading_xhp_trivia
        } else {
            minimal_parser::scan_leading_php_trivia
        };
        f(
            source_text,
            self.leading_start_offset(),
            self.leading_width(),
        )
    }
    ...
    fn scan_trailing(&self, source_text: &SourceText<'_>) -> Vec<MinimalTrivium> {
        let f = if self.kind() == TokenKind::XHPBody {
            minimal_parser::scan_trailing_xhp_trivia
        } else {
            minimal_parser::scan_trailing_php_trivia
        };
        f(source_text, self.leading_start_offset())
    }
}
