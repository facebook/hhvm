...
//
// @generated SignedSource<<1201a214173cd6863965e72b441db43e>>
//
...
impl<P: Params> Node<P> for Expr_<P::Ex, P::En> {
    ...
    fn recurse<'node>(
        ...
        fn helper13<'node, P: Params + Params<Ex = Ex> + Params<En = En>, Ex, En>(
            a: &'node Box<(ast_defs::Bop, Expr<Ex, En>, Expr<Ex, En>)>,
            c: &mut P::Context,
            v: &mut dyn Visitor<'node, Params = P>,
        ) -> Result<(), P::Error> {
            a.0.accept(c, v)?;
...
impl<P: Params> Node<P> for Stmt_<P::Ex, P::En> {
    ...
    fn recurse<'node>(
        ...
        #[inline]
        fn helper0<'node, P: Params + Params<Ex = Ex> + Params<En = En>, Ex, En>(
            a: &'node Box<(Vec<(Option<Lid>, Expr<Ex, En>)>, Block<Ex, En>)>,
            c: &mut P::Context,
            v: &mut dyn Visitor<'node, Params = P>,
        ) -> Result<(), P::Error> {
            a.0.accept(c, v)?;
        ...
        #[inline]
        fn helper7<'node, P: Params + Params<Ex = Ex> + Params<En = En>, Ex, En>(
            a: &'node Box<(Block<Ex, En>, Vec<Catch<Ex, En>>, Block<Ex, En>)>,
            c: &mut P::Context,
            v: &mut dyn Visitor<'node, Params = P>,
        ) -> Result<(), P::Error> {
            a.0.accept(c, v)?;
