...
#include <cpp/base/zend/zend_pack.h>
...
#define INC_OUTPUTPOS(a,b)                                              \
  if ((a) < 0 || ((INT_MAX - outputpos)/((int)b)) < (a)) {              \
    throw InvalidArgumentException                                      \
      (0, "Type %c: integer overflow in format string", code);          \
  }                                                                     \
  outputpos += (a)*(b);
...
String ZendPack::pack(CStrRef fmt, CArrRef argv) {
  ...
  for (int i = 0; i < formatlen; ) {
    ...
    case '@':
      if (arg < 0) {
        throw InvalidArgumentException(0, "Type %c: '*' ignored", code);
        arg = 1;
      }
      break;
    ...
    case 'H':
      if (currentarg >= argc) {
        throw InvalidArgumentException(0, "Type %c: not enough arguments",
                                       code);
      }
    ...
    case 'd':
      ...
      if (currentarg > argc) {
        throw InvalidArgumentException(0, "Type %c: too few arguments", code);
      }
      break;

    default:
      throw InvalidArgumentException(0, "Type %c: unknown format code", code);
    }
  ...
  if (currentarg < argc) {
    throw InvalidArgumentException(0, "%d arguments unused",
                                   (argc - currentarg));
  }
  ...
  for (int i = 0; i < (int)formatcodes.size(); i++) {
    ...
    case 'X':
      ...
      if (outputpos < 0) {
        throw InvalidArgumentException(0, "Type %c: outside of string", code);
        outputpos = 0;
      }
      break;
  ...
  for (int i = 0; i < (int)formatcodes.size(); i++) {
    ...
    case 'H': {
      ...
      if(arg > slen) {
        throw InvalidArgumentException
          (0, "Type %c: not enough characters in string", code);
        arg = slen;
      }
      ...
      while (arg-- > 0) {
        ...
        if (n >= '0' && n <= '9') {
          ...
          throw InvalidArgumentException(0, "Type %c: illegal hex digit %c",
                                         code, n);
          n = 0;
        }
...
Array ZendPack::unpack(CStrRef fmt, CStrRef data) {
  const char *format = fmt;
  int formatlen = fmt.size();
  ...
  while (formatlen-- > 0) {
    ...
    default:
      throw InvalidArgumentException(0, "Invalid format type %c", type);
    }
    ...
    for (int i = 0; i != arg; i++ ) {
      ...
      if (size != 0 && size != -1 && INT_MAX - size + 1 < inputpos) {
        throw InvalidArgumentException(0, "Type %c: integer overflow", type);
        inputpos = 0;
      }
      ...
      if ((inputpos + size) <= inputlen) {
        ...
        case 'X':
          if (inputpos < size) {
            ...
            if (arg >= 0) {
              throw InvalidArgumentException(0, "Type %c: outside of string",
                                             type);
            }
          }
          break;
        ...
        throw InvalidArgumentException
          (0, "Type %c: not enough input, need %d, have %d",
