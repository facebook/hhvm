...
/**
 * ImmSet is an immutable, ordered set-style collection. HHVM provides a native
...
final class ImmSet<+Tv> implements ConstSet<Tv> {
   ...
   *              the ImmSet (e.g., array). If null, then an empty
   *              ImmSet is created.
   */
   ...
   *
   * @return - an array containing the values from the current ImmSet, where
   ...
   *
   * @return - A KeyedIterator that allows you to traverse the current ImmSet.
   */
   ...
   * if "null" is passed.
   *
   * This is the static method version of the ImmSet __construct() constructor.
   *
   * @param $items - any traversable object from which to create an ImmSet
   *                 (e.g., array). If null, then an empty ImmSet is created.
   *
   * @return An ImmSet with the values from the Traversable; or an empty ImmSet
   ...
   *
   * @param ... - The arrays to convert to an ImmSet.
   *
   * @return - An ImmSet with the values from the passed arrays.
   */
   ...
   *
   * @param $container - The container with the keys used to create the ImmSet.
   *
   ...
   *
   * @return - a Vector (integer-indexed) that contains the values of this
   *           ImmSet.
   */
   ...
   *
   * @return - a Map that that contains the values of the current ImmSet, with
   ...
   * Each key of the Map will be the same as its value.
   *
   * @return - an ImmMap that that contains the values of the current ImmSet,
   *           with each key of the ImmMap being the same as its value.
   */
   ...
   *
   * ImmSets don't have keys, so this will return the values.
   *
  ...
  /**
   * Returns an ImmSet containing the values after an operation has been applied
   * to each value in the current ImmSet.
   *
   * Every value in the current ImmSet is affected by a call to map(), unlike
  ...
  /**
   * Returns an ImmSet containing the values after an operation has been applied
   * to each "key" and value in the current ImmSet.
   *
   * Since ImmSets don't have keys, the callback uses the values as the keys
   * as well.
   *
   * Every value in the current ImmSet is affected by a call to mapWithKey(),
   ...
   *
   * @return - an ImmSet containing the values after a user-specified operation
   *           on the current ImmSet's values is applied.
   */
  public function mapWithKey<Tu>((function(mixed, Tv): Tu) $callback):
    ImmSet<Tu>;

  /**
   * Returns an ImmSet containing the values of the current ImmSet that meet
   ...
   *                    current ImmSet values.
   *
   * @return - an ImmSet containing the values after a user-specified condition
   *           is applied.
   *
   * @guide /hack/collections/examples
   */
   ...
   *
   * @return - an ImmSet containing the values after a user-specified condition
   *           is applied to the values of the current ImmSet.
   *
   */
   ...
   *
   *  Since ImmSets only support integers or strings as values, we cannot have
   *  a Pair as an ImmSet value. So in order to avoid an
   *  InvalidArgumentException, either the current ImmSet or the Traversable
   ...
   *
   *  @return - The ImmSet that combines the values of the current ImmSet with
   ...
   *
   * @param $n - The last element that will be included in the returned ImmSet.
   *
   * @return - An ImmSet that is a proper subset of the current ImmSet up to n
   *           elements.
   */
  public function take(int $n): ImmSet<Tv>;

  /**
   * Returns an ImmSet containing the values of the current ImmSet up to but not
   ...
   *
   * @return - An ImmSet that is a proper subset of the current ImmSet up until
   ...
   *              ImmSet.
   *
   * @return - An ImmSet that is a proper subset of the current ImmSet starting
   *           after the callback returns true.
   */
  public function skipWhile((function(Tv): bool) $fn): ImmSet<Tv>;

  /**
   * Returns a subset of the current ImmSet starting from a given key up to, but
   ...
   * The returned ImmSet will always be a proper subset of the current ImmSet.
   *
   * @param $start - The starting value in the current ImmSet for the returned
   *                 ImmSet.
  ...
  /**
   * Returns an ImmVector that is the concatenation of the values of the current
   * ImmSet and the values of the provided Traversable.
   *
   * The values of the provided Traversable is concatenated to the end of the
   * current ImmSet to produce the returned ImmVector.
   *
   * @param $traversable - The Traversable to concatenate to the current ImmSet.
   *
   ...
   *
   * @return - The first value in the current ImmSet, or null if the current
   *           ImmSet is empty.
   */
