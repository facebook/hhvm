...
for FILE in $PHP_FILES; do
  HHVM_TMP=$(mktemp /tmp/measure_hh_codegen.XXXXXX)
  HH_TMP=$(mktemp /tmp/measure_hh_codegen.XXXXXX)
  "$HHVM" -v Eval.DumpHhas=1 "$FILE" >"$HHVM_TMP"
  buck run //hphp/hack/src:hh_single_type_check -- --hhas-codegen "$FILE" >"$HH_TMP"
