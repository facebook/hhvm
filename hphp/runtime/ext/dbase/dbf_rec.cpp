...
#include <stdio.h>
#include <fcntl.h>
#include <runtime/base/runtime-error.h>
...
void pack_dbf(dbhead_t *dbh)
{
	...
	if (ftruncate(dbh->db_fd, out_off) != 0) {
	    HPHP::raise_warning("dbase_pack() couldn't truncate the file to the right size. Some deleted records may still be left in there");
	}
...
char *get_field_val(char *rp, dbfield_t *fldp, char *cp)
{
	...
	if ( cp ) {
		dbase_strlcpy(cp, &rp[fldp->db_foffset], flen + 1);
	}
	return cp;
}
