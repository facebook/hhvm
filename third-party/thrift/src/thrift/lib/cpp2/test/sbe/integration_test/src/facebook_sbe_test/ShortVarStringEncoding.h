...
#ifndef _FACEBOOK_SBE_TEST_SHORTVARSTRINGENCODING_CXX_H_
#define _FACEBOOK_SBE_TEST_SHORTVARSTRINGENCODING_CXX_H_
...
public:
    ...
    ShortVarStringEncoding() = default;

    ShortVarStringEncoding(
        char *buffer,
        ...
        m_actingVersion(actingVersion)
    {
        ...
        {
            throw std::runtime_error("buffer too short for flyweight [E107] in ShortVarStringEncoding");
        }
    }
    ...
    ShortVarStringEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        ShortVarStringEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    ShortVarStringEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        ShortVarStringEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    ShortVarStringEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    ShortVarStringEncoding &length(const std::uint16_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, ShortVarStringEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
