...
static bool setInitialCapabilities() {
  ...
  if (cap_d != NULL) {
    ...
    if (cap_set_flag(cap_d, CAP_PERMITTED, 3, cap_list, CAP_SET) < 0 ||
      ...
      HPHPLOG_ERROR("cap_set_flag failed");
      return false;
    }

    if (cap_set_proc(cap_d) == -1) {
      HPHPLOG_ERROR("cap_set_proc failed");
      return false;
    }

    if (cap_free(cap_d) == -1) {
      HPHPLOG_ERROR("cap_free failed");
      return false;
    }

    if (prctl(PR_SET_KEEPCAPS, 1, 0, 0, 0) < 0) {
      HPHPLOG_ERROR("prctl(PR_SET_KEEPCAPS) failed");
      return false;
    }
...
bool Capability::ChangeUnixUser(uid_t uid) {
  if (setInitialCapabilities()) {
    if (setuid(uid) < 0) {
      HPHPLOG_ERROR("unable to drop privs: %s", strerror(errno));
      return false;
    }

    if (!setMinimalCapabilities()) {
      HPHPLOG_ERROR("unable to set minimal server capabiltiies");
      return false;
    }
    return true;
  }
