...
/**
 * Base class/interface for all 7 types of specialized array data.
 */
class ArrayData : public Countable {
 ...
 public:
  ...
  /**
   * Starting a virtual table that's unavoidable with 7 types of array data.
   */
  virtual ~ArrayData();
   ...
   */
  virtual Variant get(int64   k, int64 prehash = -1) const = 0;
  virtual Variant get(litstr  k, int64 prehash = -1) const = 0;
  virtual Variant get(CStrRef k, int64 prehash = -1) const = 0;
  virtual Variant get(CVarRef k, int64 prehash = -1) const = 0;
  ...
  /**
   * Array function: insert a new item at specified position.
   */
  virtual ArrayData *insert(ssize_t pos, CVarRef v, bool copy) = 0;
