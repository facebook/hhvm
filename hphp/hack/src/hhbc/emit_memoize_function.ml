...
let make_memoize_function_no_params_code
  ...
  let fcall_args =
    if is_async
    then make_fcall_args ~async_eager_label:eager_set 0
    else make_fcall_args 0 in
  gather [
    deprecation_body;
    if is_async then
      gather [
...
let make_memoize_function_with_params_code
    ~pos ~deprecation_info env params ast_params renamed_method_id is_async is_reified =
  ...
  let fcall_args =
    if is_async
    then make_fcall_args ~async_eager_label:eager_set param_count
    else make_fcall_args param_count in
  let fcall_instrs =
    if not is_reified
  ...
  let fun_ =
    ...
    then
      ...
      make_memoize_function_with_params_code
        ~pos ~deprecation_info env params ast_params renamed_method_id is_async is_reified in
...
let emit_wrapper_function
  ...
  Emit_memoize_helpers.check_memoize_possible (fst ast_fun.T.f_name)
  ...
  let params =
    Emit_param.from_asts ~namespace ~tparams ~generate_defaults:true
      ~scope ast_fun.T.f_params in
  let function_attributes =
    Emit_attribute.from_asts namespace ast_fun.T.f_user_attributes in
  let function_attributes = Emit_attribute.add_reified_attribute
    function_attributes ast_fun.T.f_tparams in
  ...
  let is_reified =
    List.exists
      ...
      ast_fun.T.f_tparams in
  let body_instrs =
    make_memoize_function_code
      ~pos ~deprecation_info env params ast_fun.T.f_params renamed_id function_is_async is_reified in
