...
$q = sodium_crypto_generichash_final(inout $state);
var_dump(bin2hex($q));
$state = sodium_crypto_generichash_init();
sodium_crypto_generichash_update(inout $state, 'm');
sodium_crypto_generichash_update(inout $state, 'sg');
$q = sodium_crypto_generichash_final(inout $state);
var_dump(bin2hex($q));
...
sodium_crypto_generichash_update(inout $state, 'sg');
$state2 = '' . $state;
$q = sodium_crypto_generichash_final(inout $state, 64);
var_dump(bin2hex($q));
sodium_crypto_generichash_update(inout $state2, '2');
$q = sodium_crypto_generichash_final(inout $state2, 64);
$exp = bin2hex($q);
var_dump($exp);
$act = bin2hex(
