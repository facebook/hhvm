...
module Value = struct
  ...
  type t = {
    ...
    leading_width: int;
    width: int;
    (* Width of node, not counting trivia *)
    trailing_width: int;
...
module TypedValueBuilder = struct
  let value_from_children _ _ _ _ = assert false

  let value_from_token _ = assert false

  let value_from_syntax _ = assert false
end

include TypedSyntax
include TypedSyntax.WithValueBuilder (TypedValueBuilder)

let source_text node = (value node).Value.source_text

let leading_width node = (value node).Value.leading_width

let width node = (value node).Value.width

let trailing_width node = (value node).Value.trailing_width

let full_width node = leading_width node + width node + trailing_width node

let leading_start_offset node = (value node).Value.offset
