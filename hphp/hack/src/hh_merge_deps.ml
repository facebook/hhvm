...
let sharedmem_config =
  ...
  { SharedMem.
    global_size    = gig;
    heap_size      = 40 * gig;
    dep_table_pow  = 28; (* 1 << 28 *)
    hash_table_pow = 26; (* 1 << 26 *)
...
let merge_worker ((filename: string), (ignore_hh_version: bool)) : unit =
  try
    ...
    let (num_rows: int) = SharedMem.load_dep_table_blob
      filename
      ignore_hh_version
    in
  ...
  with e ->
    ...
    Hh_logger.exc
      ~prefix:(Printf.sprintf "Error in '%s'" filename)
      ~stack
      e
...
let merge_input
  ...
  let (input: (string * bool) list) =
    List.map (fun filename -> (filename, ignore_hh_version)) filenames in
  let () = List.iter merge_worker input in
  let (edges_added: int) = SharedMem.save_dep_table_sqlite
    db_name
...
let () =
  let (_handle: SharedMem.handle) = SharedMem.init ~num_workers:0 sharedmem_config in
  ...
  in
  let usage =
    "Usage: " ^ Sys.executable_name ^ " [dep_table_fragment_1 ... dep_table_fragment_N]" in
