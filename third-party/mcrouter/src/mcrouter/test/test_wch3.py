        ...
        while not self.is_stopped():
            ...
            if cmd.decode().startswith("get"):
                reply = "VALUE hit 0 {size}\r\n{payload}\r\nEND\r\n".format(
                    size=len(str(self.port)), payload=self.port
                )
...
class TestWCH3(McrouterTestCase):
    ...
    def setUp(self):
        ...
        self.mcrouter = self.add_mcrouter(
            self.config, "/test/A/", extra_args=self.extra_args
        )
    ...
    def test_wch3(self):
        ...
        # within a tolerance
        expected_fractions = {0: 0, 1: 1, 2: 1, 3: 0.0, 4: 0.5, 5: 1, 6: 0.3, 7: 0.5}
        tolerance = 0.075
        ...
        for i, weight in expected_fractions.items():
            ...
            if expected_frac > 0:
                self.assertAlmostEqual(measured_frac, expected_frac, delta=tolerance)
            else:
