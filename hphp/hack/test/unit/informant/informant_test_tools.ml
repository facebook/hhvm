...
let set_hg_to_global_rev_map ?delay_rev_200 () =
  ...
  Hg.Mocking.closest_global_ancestor_bind_value hg_rev_200
...
let set_xdb ~state_global_rev ~for_global_rev ~everstore_handle  =
  ...
  let hg_hash = match state_global_rev with
    | 1 -> hg_rev_1
    | 5 -> hg_rev_5
    | 200 -> hg_rev_200
    | 230 -> hg_rev_230
    | _ ->
      ...
      assert false
  in
  let hhconfig_hash, _config = Config_file.parse_hhconfig ~silent:true "/tmp/.hhconfig" in
  let result = {
  ...
  Xdb.Mocking.find_nearest_returns ~db:Xdb.hack_db_name
    ...
    ~global_rev:for_global_rev ~hh_version:(Some hh_version) result
