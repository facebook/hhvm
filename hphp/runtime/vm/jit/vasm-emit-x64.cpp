...
void optimize(Vunit& unit, const Abi& abi, bool regalloc) {
  ...
  if (unit.needsRegAlloc()) {
    ...
    if (regalloc) {
      ...
      doPass(
        "VOPT_POST_RA_SIMPLIFY",
        [&] (Vunit& u) { postRASimplify(u, abi); }
      );
    }
  }
...
void emit(Vunit& unit, Vtext& text, CGMeta& fixups, AsmInfo* asmInfo) {
