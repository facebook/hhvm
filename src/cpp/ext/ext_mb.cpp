...
#include <cpp/ext/php_unicode.h>
#include <cpp/ext/unicode_data.h>
...
static RequestLocal<MBGlobals> s_mb_globals;
...
static char *php_mb_convert_encoding(const char *input, size_t length,
  ...
  if (_to_encoding && strlen(_to_encoding)) {
    ...
    if (to_encoding == mbfl_no_encoding_invalid) {
      Logger::Warning("Unknown encoding \"%s\"", _to_encoding);
      return NULL;
    }
  } else {
  ...
  if (_from_encodings) {
    ...
    if (size == 1) {
      ...
      if (from_encoding != mbfl_no_encoding_invalid) {
        ...
        Logger::Warning("Unable to detect character encoding");
        from_encoding = mbfl_no_encoding_pass;
        to_encoding = from_encoding;
        string.no_encoding = from_encoding;
      }
    } else {
      Logger::Warning("Illegal character encoding specified");
    }
    if (list != NULL) {
      free((void *)list);
    }
  ...
  if (convd == NULL) {
    Logger::Warning("Unable to create character encoding converter");
    return NULL;
  }
...
static int php_mb_stripos(int mode,
  ...
  do {
    ...
    if (haystack.no_encoding == mbfl_no_encoding_invalid) {
      Logger::Warning("Unknown encoding \"%s\"", from_encoding);
      break;
    }

    if (offset < 0 || (unsigned long)offset > haystack.len) {
      Logger::Warning("Offset not contained in string.");
      break;
    }
...
Variant f_mb_list_encodings_alias_names(CStrRef name /* = null_string */) {
  ...
  if (name.isNull()) {
    ...
    if (no_encoding == mbfl_no_encoding_invalid) {
      Logger::Warning("Unknown encoding \"%s\"", name.data());
      return false;
    }
...
bool f_mb_check_encoding(CStrRef var /* = null_string */,
  ...
  if (!encoding.isNull()) {
    ...
    if (no_encoding == mbfl_no_encoding_invalid ||
      ...
      Logger::Warning("Invalid encoding \"%s\"", encoding.data());
      return false;
    }
  }
  ...
  if (convd == NULL) {
    Logger::Warning("Unable to create converter");
    return false;
  }
  mbfl_buffer_converter_illegal_mode
    (convd, MBSTRG(current_filter_illegal_mode));
...
Variant f_mb_convert_kana(CStrRef str, CStrRef option /* = null_string */,
  ...
  if (!encoding.empty()) {
    ...
    if (string.no_encoding == mbfl_no_encoding_invalid) {
      Logger::Warning("Unknown encoding \"%s\"", encoding.data());
      return false;
    }
  }
...
Variant f_mb_convert_variables(int _argc, CStrRef to_encoding,
  ...
  if (_to_encoding == mbfl_no_encoding_invalid) {
    Logger::Warning("Unknown encoding \"%s\"", to_encoding.data());
    return false;
  }
...
Variant f_mb_encode_mimeheader(CStrRef str, CStrRef charset /* = null_string */,
  ...
  if (!charset.empty()) {
    ...
    if (charsetenc == mbfl_no_encoding_invalid) {
      Logger::Warning("Unknown encoding \"%s\"", charset.data());
      return false;
    }
  } else {
...
Variant f_mb_language(CStrRef language /* = null_string */) {
  ...
  if (no_language == mbfl_no_language_invalid) {
    Logger::Warning("Unknown language \"%s\"", language.data());
    return false;
  }
...
Variant f_mb_preferred_mime_name(CStrRef encoding) {
  ...
  if (preferred_name == NULL || *preferred_name == '\0') {
    Logger::Warning("No MIME preferred name corresponding to \"%s\"",
                    encoding.data());
    return false;
  }
...
Variant f_mb_strimwidth(CStrRef str, int start, int width,
  ...
  if (start < 0 || start > str.size()) {
    Logger::Warning("Start position is out of reange");
    return false;
  }

  if (width < 0) {
    Logger::Warning("Width is negative value");
    return false;
  }
...
Variant f_mb_stristr(CStrRef haystack, CStrRef needle, bool part /* = false */,
  ...
  if (!mbs_needle.len) {
    Logger::Warning("Empty delimiter.");
    return false;
  }
...
Variant f_mb_strpos(CStrRef haystack, CStrRef needle, int offset /* = 0 */,
  ...
  case 1:
    break;
  case 2:
    Logger::Warning("Needle has not positive length.");
    break;
  case 4:
    Logger::Warning("Unknown encoding or conversion error.");
    break;
  case 8:
    Logger::Warning("Argument is empty.");
    break;
  default:
    Logger::Warning("Unknown error in mb_strpos.");
    break;
  }
  return false;
}
...
Variant f_mb_strrpos(CStrRef haystack, CStrRef needle,
  ...
  if (!enc_name && !*enc_name) {
    ...
    if (mbs_haystack.no_encoding == mbfl_no_encoding_invalid) {
      Logger::Warning("Unknown encoding \"%s\"", enc_name);
      return false;
    }
  }
...
Variant f_mb_substitute_character(CVarRef substrchar /* = null_variant */) {
  ...
  if (n < 0xffff && n > 0) {
    ...
    Logger::Warning("Unknown character.");
    return false;
  }
  return true;
}
...
Variant f_mb_substr_count(CStrRef haystack, CStrRef needle,
  ...
  if (mbs_needle.len <= 0) {
    Logger::Warning("Empty substring.");
    return false;
  }
...
static php_mb_regex_t *php_mbregex_compile_pattern(CStrRef pattern,
  ...
  if (!rc || rc->options != options || rc->enc != enc ||
    ...
    if ((err_code = onig_new(&rc, (OnigUChar *)pattern.data(),
      ...
      Logger::Warning("mbregex compile err: %s", err_str);
      return NULL;
    }
...
static Variant _php_mb_regex_ereg_replace_exec(CVarRef pattern,
  ...
  while (err >= 0) {
    ...
    if (err <= -2) {
      ...
      Logger::Warning("mbregex search failure: %s", err_str);
      break;
    }
    if (err >= 0) {
#if moriyoshi_0
      if (regs->beg[0] == regs->end[0]) {
        Logger::Warning("Empty regular expression");
        break;
      }
#endif
...
bool f_mb_ereg_search_setpos(int position) {
  if (position < 0 || position >= (int)MBSTRG(search_str).size()) {
    Logger::Warning("Position is out of range");
    MBSTRG(search_pos) = 0;
    return false;
  }
...
static Variant _php_mb_regex_ereg_search_exec(CStrRef pattern, CStrRef option,
  ...
  if (MBSTRG(search_re) == NULL) {
    Logger::Warning("No regex given");
    return false;
  }

  if (str == NULL) {
    Logger::Warning("No string given");
    return false;
  }
  ...
  if (err == ONIG_MISMATCH) {
    ...
    OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
    onig_error_code_to_str(err_str, err);
    Logger::Warning("mbregex search failure in mbregex_search(): %s", err_str);
    ret = false;
  } else {
...
Variant f_mb_split(CStrRef pattern, CStrRef str, int count /* = -1 */) {
  ...
  if (err <= -2) {
    OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
    onig_error_code_to_str(err_str, err);
    Logger::Warning("mbregex search failure in mbsplit(): %s", err_str);
    return false;
  }
...
static int php_mail(const char *to, const char *subject, const char *message,
  ...
  if (sendmail == NULL) {
    Logger::Warning("Could not execute mail delivery program '%s'",
                    sendmail_path);
    return 0;
  }

  if (EACCES == errno) {
    Logger::Warning("Permission denied: unable to execute shell to run "
...
bool f_mb_send_mail(CStrRef to, CStrRef subject, CStrRef message,
        ...
        if ((param_name = strtok_r(p, "= ", &tmp)) != NULL) {
          if (strcasecmp(param_name, "charset") == 0) {
            ...
            if (_tran_cs == mbfl_no_encoding_invalid) {
              Logger::Warning("Unsupported charset \"%s\" - "
  ...
  if (!s.isNull()) {
    ...
    default:
      Logger::Warning("Unsupported transfer encoding \"%s\" - "
  ...
  if (!to.empty()) {
    ...
    Logger::Warning("Missing To: field");
    err = 1;
  }
  ...
  if (!subject.isNull()) {
    ...
    Logger::Warning("Missing Subject: field");
    err = 1;
  }
