...
#define TEST_IMPL_TAG(Tag, Expected, ...)                        \
  do {                                                           \
    std::ostringstream out;                                      \
    if constexpr (std::is_void_v<Tag>) {                         \
      facebook::thrift::pretty_print(out, __VA_ARGS__);          \
    } else {                                                     \
      facebook::thrift::pretty_print<Tag>(out, __VA_ARGS__);     \
    }                                                            \
...
TEST(PrettyPrint, tc_binary_iobuf) {
  ...
  TEST_IMPL_TAG(tag, expectedStr, xb->to<std::string>());
  TEST_IMPL_TAG(tag, expectedStr, xb->to<folly::fbstring>());
  TEST_IMPL_TAG(tag, expectedStr, xb);
  TEST_IMPL_TAG(tag, expectedStr, *xb);
  TEST_IMPL_TAG(tag, R"()", std::unique_ptr<folly::IOBuf>());
  TEST_IMPL_TAG(tag, R"("0x")", std::make_unique<folly::IOBuf>());
}
