...
Variant File::Open(CStrRef filename, CStrRef mode,
  ...
  if (!strncmp(filename.data(), http_prefix, sizeof(http_prefix) - 1) ||
    ...
    if (options.isNull()) {
      ...
      Array opts = options.rvalAt("http");
      String method = "GET";
      if (opts.exists("method")) method = opts["method"].toString();
      Array headers;
      if (opts.exists("header")) {
        headers = StringUtil::Explode(opts.rvalAt("header").toString(),
                                      "\r\n");
        if (opts.exists("user_agent") && !headers.exists("User-Agent")) {
          headers.set("User_Agent", opts.rvalAt("user_agent"));
        }
      }
      int max_redirs = 20;
      if (opts.exists("max_redirects"))
        max_redirs = opts.rvalAt("max_redirects");
      int timeout = -1;
      if (opts.exists("timeout"))
        timeout = opts.rvalAt("timeout");
    ...
    if (!ret) {
      Logger::Verbose("%s", Util::safe_strerror(errno).c_str());
      return false;
    }
    return obj;
  }
...
String File::read(int length) {
  if (length <= 0) {
    throw InvalidArgumentException("length", length);
  }
...
bool File::lock(int operation, bool &wouldblock /* = false */) {
  ...
  if ((operation & 3) == 0) {
    throw InvalidArgumentException("operation", operation);
  }
