...
static RequestLocal<PCREData> s_pcre_data;
...
static pcre_cache_entry *pcre_get_compiled_regex_cache(CStrRef regex) {
  ...
  if (isalnum((int)*(unsigned char *)&delimiter) || delimiter == '\\') {
    Logger::Warning("Delimiter must not be alphanumeric or backslash");
    return NULL;
  }
  ...
  if (start_delimiter == end_delimiter) {
    ...
    if (*pp == 0) {
      Logger::Warning("No ending delimiter '%c' found: [%s]", delimiter,
                      regex.data());
      return NULL;
    }
  } else {
    ...
    if (*pp == 0) {
      Logger::Warning("No ending matching delimiter '%c' found: [%s]",
  ...
  while (*pp != 0) {
    ...
    default:
      Logger::Warning("Unknown modifier '%c': [%s]", pp[-1], regex.data());
      return NULL;
    }
  }
  ...
  if (re == NULL) {
    Logger::Warning("Compilation failed: %s at offset %d", error, erroffset);
    if (tables) {
      free((void*)tables);
    }
    return NULL;
  ...
  if (do_study) {
    ...
    if (error != NULL) {
      Logger::Warning("Error while studying pattern");
    }
  }
...
static int *create_offset_array(pcre_cache_entry *pce, int &size_offsets) {
  ...
  extra->match_limit = BACKTRACE_LIMIT;
  extra->match_limit_recursion = RECURSION_LIMIT;

  /* Calculate the size of the offsets array, and allocate memory for it. */
  int num_subpats; // Number of captured subpatterns
  int rc = pcre_fullinfo(pce->re, extra, PCRE_INFO_CAPTURECOUNT, &num_subpats);
  if (rc < 0) {
    Logger::Warning("Internal pcre_fullinfo() error %d", rc);
    return NULL;
  }
  num_subpats++;
...
Variant preg_grep(CStrRef pattern, CArrRef input, int flags /* = 0 */) {
  ...
  for (ArrayIter iter(input); iter; ++iter) {
    ...
    if (count == 0) {
      Logger::Warning("Matched, but too many substrings");
...
Variant preg_match_impl(CStrRef pattern, CStrRef subject,
  ...
  if (flags) {
    ...
    if ((global && (subpats_order < PREG_PATTERN_ORDER ||
      ...
      Logger::Warning("Invalid flags specified");
      return false;
    }
  }
    ...
    if (name_cnt > 0) {
      ...
      while (ni++ < name_cnt) {
        ...
        if (is_numeric_string(subpat_names[name_idx],
                              ...
                              NULL, NULL, 0) > 0) {
          Logger::Warning("Numeric named subpatterns are not allowed");
          free(offsets);
          free(subpat_names);
          return false;
        }
  ...
  do {
    ...
    if (count > 0) {
      ...
      if (pcre_get_substring_list(subject.data(), offsets, count,
        ...
        free(offsets);
        free(subpat_names);
        Logger::Warning("Get subpatterns list failed");
        return false;
      }
...
Variant preg_replace_impl(CVarRef pattern, CVarRef replacement,
  ...
  if (!is_callable &&
    ...
    Logger::Warning("Parameter mismatch, pattern is a string while "
    ...
    if (count > 0) {
      ...
      if (g_notempty != 0 && start_offset < ssubject.size()) {
        if (pce->compile_options & PCRE_UTF8) {
          if (re_bump == NULL) {
            int dummy;
            if ((re_bump = pcre_get_compiled_regex("/./u", &extra_bump,
                                                   &dummy)) == NULL) {
              return false;
            }
          }
