...
impl FunsTable {
    ...
    pub fn insert(&self, items: &[FunItem]) {
...
mod tests {
    ...
    #[test]
    fn test_add_fun() {
        let fun_table = FunsTable::new(Arc::new(Mutex::new(Connection::open_in_memory().unwrap())));
        fun_table.create();
        let funs = [FunItem {
            name: "Foo".to_string(),
            file_info_id: 123,
        }];
        fun_table.insert(&funs);
