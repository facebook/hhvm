...
module rec TypeSpecification: sig
  type t =
    | Id of Fact_id.t
    | Key of key
  [@@deriving ord]
...
and Literal: sig
  ...
  and to_json_key  = function
     ...
     | Binary_ _binary_ -> assert false (* TODO JSON_Object [("binary_", JSON_String (List.map ~f:Base64.encode_string binary_|> String.concat ""))] *)
...
and StructFieldValValue: sig
  ...
  let to_json  = function
     ...
     | Nothing nothing -> JSON_Object [("nothing", (ignore nothing; JSON_Object []) (* unsupported *))]
