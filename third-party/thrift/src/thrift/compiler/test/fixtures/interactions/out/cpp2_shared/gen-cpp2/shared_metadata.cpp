...
void ServiceMetadata<::apache::thrift::ServiceHandler<::thrift::shared_interactions::InteractLocally>>::gen(::apache::thrift::metadata::ThriftServiceMetadataResponse& response) {
  if (FLAGS_thrift_enable_schema_to_metadata_conversion) {
    genServiceMetadataResponse<::thrift::shared_interactions::InteractLocally>(response);
    DCHECK_EQ(*response.context()->module()->name(), "shared");
    DCHECK_EQ(*response.context()->service_info()->name(), "shared.InteractLocally");
    return;
  }
