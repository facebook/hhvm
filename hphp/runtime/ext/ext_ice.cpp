...
//ICE
c_ICE::c_ICE(Class* cls):ExtObjectData(cls){
}
...
/*
	...
	可以抛出异常
*/
Object c_ICE::t_stringtoproxy(CStrRef str /* = null_string */){
...
Object f_ice_find(CStrRef ice_name /* = null_string */){
...
bool f_ice_register(CObjRef ice_object /*= null_object*/, CStrRef ice_name /*= null_string*/, int64_t expires /*= 0*/){
	//printf("ice_register!==============\n");
	return true;
}
...
//IcePHP_class
c_IcePHP_class::c_IcePHP_class(Class* cls):ExtObjectData(cls){
...
c_IcePHP_class* get_hiphopice_classmap(){
		...
		String var_name="Hiphop_Ice_t_classMap";
		Variant iceclassmap = dvarname[var_name];
...
//Proxy
c_Proxy::c_Proxy(Class* cls):ExtObjectDataFlags<ObjectData::HasCall>(cls){
...
//new ice hzg 20130603
  try{	
	...
	//params process
		  const char * name_char=name.toString().data();
		  ...
		  for (ArrayIter iter(arg.toArray()); iter; ++iter) {
		  ...
		  if(!objectPrx->ice_invoke(methodname, Ice::OperationMode(method->sendMode), inParams, outParams))
		  {
				  status=false;
		  }
		  //捕获ice返回值
...
Object c_Proxy::t_ice_timeout(int64_t lasttime/* = 0 */){
...
Object c_Proxy::t_ice_context(CArrRef ctx  /* = null_variant */){
  //printf("ice_context is call \n");
		...
		for (ArrayIter iter(ctx); iter; ++iter) {
				...
				//20130903
				ice_ctx[ctx_key] = value.toString().data();
		}
		try{
...
Object c_Proxy::t_ice_checkedcast(CStrRef classid/* = null_string */, CVarRef facetOrCtx /*= null_variant*/, CVarRef ctx /*= null_variant*/){
  //printf("ice_checkedcast is call \n");
		...
		if(!facetOrCtx.isNull()){
				//objectPrx = objectPrx->ice_facet(facetOrCtx.toCArrRef());
		}

		if(!ctx.isNull()){
				Ice::Context ice_ctx;
				for (ArrayIter iter(ctx.toArray()); iter; ++iter) {
						Variant key=iter.first();
						Variant value = iter.second();
						std::string ctx_key=key.toString().data();
						ice_ctx[ctx_key] = value.toString().data();
...
//
Object c_Proxy::t_ice_uncheckedcast(CStrRef classid /* = null_string */, CVarRef facet /*= null_variant*/){
...
*/
Object f_icephp_defineexception(CStrRef id /* = null_string */, CStrRef name /* = null_string */, CVarRef base /* = null_variant */, CVarRef members /* = null_variant */) {
		ExceptionInfo* ex = NEWOBJ(ExceptionInfo)();
		ex->id = id.data();

        	///////////wh add at 20130530/////////////////
        //	P_ExceptionInfoPtr pcp = c_TypeInfoToP_TypeInfoPtr(ex);
        	P_ExceptionInfoPtr pcp =  dynamic_cast<P_ExceptionInfoPtr>(c_TypeInfoToP_TypeInfoPtr(ex));
...
*/
String f_icephp_stringifyexception(CObjRef value /* = null_object */, CObjRef target /* = null_object */) {
...
*/
String f_icephp_stringify(CObjRef value /* = null_object */, CObjRef target /* = null_object */) {
...
Object f_icephp_definesequence(CStrRef id /* = null_string */, CVarRef elementtype /* = null_variant */) {
...
//		P_SequenceInfoPtr psi = c_TypeInfoToP_TypeInfoPtr(type);
		P_SequenceInfoPtr psi = dynamic_cast<P_SequenceInfoPtr>(c_TypeInfoToP_TypeInfoPtr(type));
		psi->id = id.data();//psi->id = type->id;
...
Object f_icephp_defineproxy(CObjRef classobj /* = null_object */) {
	return NULL;
}

Object f_icephp_declareclass(CStrRef id /* = null_string */){
	return NULL;
}

Object f_icephp_defineclass(CStrRef id /* = null_string */, CStrRef name /* = null_string */, bool isabstract /* = true */, CObjRef base /* = null_object */, CVarRef interfaces /* = null_variant */, CVarRef members /* = null_variant */) {
	Ice_ClassInfo* ci = NEWOBJ(Ice_ClassInfo)();
	ci->id = id.data();
	///20130529 new struct////////////////////////////////////////////
//	P_ClassInfoPtr pcp=c_TypeInfoToP_TypeInfoPtr(ci);
	P_ClassInfoPtr pcp=dynamic_cast<P_ClassInfoPtr>(c_TypeInfoToP_TypeInfoPtr(ci));
	pcp->clearDataMemberList();
        pcp->id=ci->id;
	pcp->name=name.data();
...
//
void f_icephp_defineoperation(CObjRef classobj /* = null_object */, CStrRef funname /* = null_string */, int mode /* = 0 */, int sendmode /* = 0 */, CVarRef inparams /* = null_variant */, CVarRef outparams /* = null_variant */, CVarRef returntype /* = null_variant */, CVarRef exceptions /* = null_variant */) {
...
//		P_ClassInfoPtr pcp=c_TypeInfoToP_TypeInfoPtr(cic);
		P_ClassInfoPtr pcp=dynamic_cast<P_ClassInfoPtr>(c_TypeInfoToP_TypeInfoPtr(cic));
		...
		if(!exceptions.isNull()){
				...
				for(int i = 0 ; i < exp.size() ; i++ ){
						...
						////hzg new ice///////////////////////////////////////////////////////
						p_exceptionList.push_back(dynamic_cast<P_ExceptionInfoPtr>(c_TypeInfoToP_TypeInfoPtr(ei)));	
						/////////////////////////////////////////////////////////////////////
				}
				////hzg new ice///////////////////////////////////////////////////////
...
c_TypeInfo::c_TypeInfo(Class* cls):ExtObjectData(cls){
...
std::string getHPHPStringToStdString(HPHP::String hphpstring){
		const char * std_char=hphpstring.data();
...
char * getCurrentTid(){
		pthread_t tid;
		tid = pthread_self();
		Variant tidv=Variant((int64_t)tid);
		const char * tidc=tidv.toString().data();
		return  const_cast<char*>(tidc);
}
		...
		case c_TypeInfo::PrimitiveInfo:
			{
				...
				char* primitiveId=const_cast<char*>(c_typeInfoObj->getId().c_str());
		...
		case c_TypeInfo::Ice_ClassInfo:
			{
				...
				char * typeId=const_cast<char*>(c_typeInfoObj->id.c_str());
...
string  P_ClassInfo::print(int objcount,Variant &inParams){
	...
	string printValue="";
	Variant classVars=f_get_object_vars(inParams.toObject());
	String count=String(objcount);
	printValue.append("object #");
	printValue.append(count.data());
	printValue.append(" (");
...
void P_IceObjectWriter::write(const Ice::OutputStreamPtr& out) const{	
		...
		Variant classVars;
		if(!classObj.isNull()){
			classVars=f_get_object_vars(classObj.toObject());		
		}else{
			classVars=Variant();
		}
		out->startSlice();
...
P_ClassInfoPtr getP_ClassInfoPtrById(const string &classId){
	char * typeId=const_cast<char*>(classId.c_str());
...
P_ExceptionInfoPtr getP_ExceptionInfoPtrById(std::string &exceptionId){
	exceptionId="::"+exceptionId;
	char * typeId=const_cast<char*>(exceptionId.c_str());
