...
void RTTIInfo::translate_rtti(const char *rttiDirectory) {
  ...
  for (int i = 0; i < m_count; i++) {
    int total = 0;
    for (int j = 0; j < KindOfLast; j++) total += m_profData[i][j];
    if (!total) continue;
    printf("%s(%d):", m_id2name[i].c_str(), total);
    if (m_profData[i][KindOfNull]) {
      printf(" n/%u", m_profData[i][KindOfNull]);
    }
    if (m_profData[i][KindOfBoolean]) {
      printf(" b/%u", m_profData[i][KindOfBoolean]);
    }
    int totalInt = m_profData[i][KindOfByte] + m_profData[i][KindOfInt16] +
                   m_profData[i][KindOfInt32] + m_profData[i][KindOfInt64];
    if (totalInt) printf(" i/%u", totalInt);
    if (m_profData[i][KindOfDouble]) {
      printf(" d/%u", m_profData[i][KindOfDouble]);
    }
    int totalStr = m_profData[i][LiteralString] + m_profData[i][KindOfString];
    if (totalStr) printf(" s/%u", totalStr);
    if (m_profData[i][KindOfArray]) {
      printf(" a/%u", m_profData[i][KindOfArray]);
    }
    if (m_profData[i][KindOfObject]) {
      printf(" o/%u", m_profData[i][KindOfObject]);
    }
    if (m_profData[i][KindOfVariant]) {
      printf(" v/%u", m_profData[i][KindOfVariant]);
    }
    printf("\n");
  }
  free(m_profData);
...
public:
  ...
  virtual void requestShutdown() {
    ...
    if (m_data) {
      ...
      if (fd == -1) {
        Logger::Error("%s", Util::safe_strerror(errno).c_str());
      } else {
        write(fd, m_data, (m_count * sizeof(RTTICounter)));
        close(fd);
      }
    }
  }
...
static RequestLocal<RTTICounters> s_rtti_counters;
