...
Whenever possible, use `<<__Memoize>>` over manual approach.
...
## (Do Not) Consider Batching

Traditionally, batching is used to reduce per-call scheduling overhead, which might be high in some programming language runtimes.
In Hack, this is not necessary, as async functions have very little overhead, much lower than the cost of batching. Batching is considered
an anti-pattern, as it creates an unnecessary synchronization point, worsening the latency. Batching is also bad for error isolation, as
an exception during batch processing will fail the whole batch rather than a single item.

However, some backends (such as MySQL) have very high per-request overhead, or an extremely inefficient connection and session management.
In these situations batching might be considered at the lowest possible layer close to the actual I/O call.

Batching could be achieved via rescheduling of awaitables. A special Awaitable created via `\HH\Asio\later()` will block until the scheduler
queue becomes empty, waiting until all other awaitables have run and got a chance to add more items to the batch.
...
In the example above, we reduce the number of requests to the server containing the data information to two by batching the first lookup
...
So, `concurrent { ... }` has two pending awaitables. If `b_one` is called first, it calls `Batcher::lookup`, which
