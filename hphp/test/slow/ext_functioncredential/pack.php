...
function free_function(): FunctionCredential {
  return __FUNCTION_CREDENTIAL__;
}
...
<<__EntryPoint>>
function main(): void {
  // Test pack/unpack of free function
  $cred = free_function();
  $packed = $cred->pack();
  echo "Packed free function credential\n";
  echo "  Format valid: " . (validate_format($packed, null, 'free_function') ? "yes" : "no") . "\n";

  $unpacked = FunctionCredential::unpack($packed);
  echo "Unpacked free function:\n";
  echo "  Class: " . ($unpacked->getClassName() ?? "(null)") . "\n";
  echo "  Function: " . $unpacked->getFunctionName() . "\n";
  echo "\n";

  // Test pack/unpack of static method
  $cred2 = TestClass::static_method();
  $packed2 = $cred2->pack();
  echo "Packed static method credential\n";
  echo "  Format valid: " . (validate_format($packed2, 'TestClass', 'static_method') ? "yes" : "no") . "\n";

  $unpacked2 = FunctionCredential::unpack($packed2);
  echo "Unpacked static method:\n";
  echo "  Class: " . ($unpacked2->getClassName() ?? "(null)") . "\n";
  echo "  Function: " . $unpacked2->getFunctionName() . "\n";
  echo "\n";

  // Test pack/unpack of instance method
  $obj = new TestClass();
  $cred3 = $obj->instance_method();
  $packed3 = $cred3->pack();
  echo "Packed instance method credential\n";
  echo "  Format valid: " . (validate_format($packed3, 'TestClass', 'instance_method') ? "yes" : "no") . "\n";

  $unpacked3 = FunctionCredential::unpack($packed3);
  echo "Unpacked instance method:\n";
  ...
  echo "Verification:\n";
  echo "  Free function class matches: " . ($cred->getClassName() === $unpacked->getClassName() ? "yes" : "no") . "\n";
  echo "  Free function name matches: " . ($cred->getFunctionName() === $unpacked->getFunctionName() ? "yes" : "no") . "\n";
  echo "  Static method class matches: " . ($cred2->getClassName() === $unpacked2->getClassName() ? "yes" : "no") . "\n";
  echo "  Static method name matches: " . ($cred2->getFunctionName() === $unpacked2->getFunctionName() ? "yes" : "no") . "\n";
  echo "  Instance method class matches: " . ($cred3->getClassName() === $unpacked3->getClassName() ? "yes" : "no") . "\n";
  echo "  Instance method name matches: " . ($cred3->getFunctionName() === $unpacked3->getFunctionName() ? "yes" : "no") . "\n";
}
