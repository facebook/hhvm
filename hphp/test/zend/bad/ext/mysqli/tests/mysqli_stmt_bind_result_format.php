<?php
	...
	function create_table($link, $column, $min, $max, $engine, $offset) {
		...
		if (!mysqli_query($link, 'DROP TABLE IF EXISTS test_mysqli_stmt_bind_result_format_table_1')) {
			printf("[%03d] Cannot drop table test_mysqli_stmt_bind_result_format_table_1, [%d] %s\n",
				$offset,
				mysqli_errno($link), mysqli_error($link));
			return array();
		}

		$sql = sprintf("CREATE TABLE test_mysqli_stmt_bind_result_format_table_1(id INT AUTO_INCREMENT PRIMARY KEY, col1 %s) ENGINE=%s",
			$column, $engine);
		if (!mysqli_query($link, $sql)) {
			printf("[%03d] Cannot create table test_mysqli_stmt_bind_result_format_table_1, [%d] %s\n",
				$offset + 1,
				mysqli_errno($link), mysqli_error($link));
			return array();
		}
		...
		for ($i = 1; $i <= 100; $i++) {
			...
			$values[$i] = $col1;
			$sql = sprintf("INSERT INTO test_mysqli_stmt_bind_result_format_table_1(id, col1) VALUES (%d, %f)",
				$i, $col1);
			if (!mysqli_query($link, $sql)) {
	...
	test_format($link, 'FORMAT(1.01, 0)', 'test_mysqli_stmt_bind_result_format_table_2', null, '1', 10);
	test_format($link, 'FORMAT(1.23, 1)', 'test_mysqli_stmt_bind_result_format_table_2', null, '1.2', 20);
	test_format($link, 'FORMAT(1.23, 2)', 'test_mysqli_stmt_bind_result_format_table_2', null, '1.23', 30);
	test_format($link, 'FORMAT(1234.567, 3)', 'test_mysqli_stmt_bind_result_format_table_2', null, '1,234.567', 40);
	/* no typo! */
	test_format($link, 'FORMAT(1234.567, 4)', 'test_mysqli_stmt_bind_result_format_table_2', null, '1,234.5670', 50);

	mysqli_close($link);
	$test_table_name = 'test_mysqli_stmt_bind_result_format_table_1'; require_once('table.inc');

	/* select from existing table */
	test_format($link, 'FORMAT(id, 0)', 'test_mysqli_stmt_bind_result_format_table_1', null, '1', 60);
	test_format($link, 'FORMAT(id + 0.1, 1)', 'test_mysqli_stmt_bind_result_format_table_1', null, '1.1', 70);
	test_format($link, 'FORMAT(id + 0.01, 2)', 'test_mysqli_stmt_bind_result_format_table_1', null, '1.01', 80);
	...
	test_format($link, 'id AS order_by_col, FORMAT(col1, 0)', 'test_mysqli_stmt_bind_result_format_table_1', 'id', $expected, 100);

	$expected = create_table($link, 'FLOAT UNSIGNED', 0, 10000, $engine, 110);
	foreach ($expected as $k => $v)
		$expected[$k] = number_format(round($v), 0, '.', ',');
	test_format($link, 'id AS order_by_col, FORMAT(col1, 0)', 'test_mysqli_stmt_bind_result_format_table_1', 'id', $expected, 120);

	$expected = create_table($link, 'TINYINT', -128, 127, $engine, 130);
	foreach ($expected as $k => $v)
		$expected[$k] = number_format(round($v), 0, '.', ',');
	test_format($link, 'id AS order_by_col, FORMAT(col1, 0)', 'test_mysqli_stmt_bind_result_format_table_1', 'id', $expected, 140);

	$expected = create_table($link, 'SMALLINT UNSIGNED', 0, 65535, $engine, 150);
	foreach ($expected as $k => $v)
		$expected[$k] = number_format(round($v), 0, '.', ',');
	test_format($link, 'id AS order_by_col, FORMAT(col1, 0)', 'test_mysqli_stmt_bind_result_format_table_1', 'id', $expected, 160);
  ...
  PRIMARY KEY  (`targetport`)
		...
		do {
			...
			for ($i = 0; $i < 200; $i++) {
				...
				$sql = sprintf('INSERT INTO test_mysqli_stmt_bind_result_format_table_1(targetport, current_targets, maxreports, trend) VALUES (%d, %f, %s, %s)',
					$i,
					$current_targets,
			...
			if (!mysqli_stmt_prepare($stmt, 'SELECT trend, targetport, FORMAT(trend, 2) FROM test_mysqli_stmt_bind_result_format_table_1 WHERE current_targets > 0 AND trend IS NOT NULL ORDER BY trend DESC LIMIT 100')) {
			...
			if (!$stmt->prepare('SELECT trend, targetport, FORMAT(trend, 2) FROM test_mysqli_stmt_bind_result_format_table_1 WHERE current_targets > 0 AND trend IS NOT NULL ORDER BY trend DESC LIMIT 100')) {
				printf("[308] [%d] %s\n", mysqli_stmt_errno($link), mysqli_stmt_error($link));
				break;
			}
...
?>
<?php
	$test_table_name = 'test_mysqli_stmt_bind_result_format_table_1'; require_once("clean_table.inc");
?>