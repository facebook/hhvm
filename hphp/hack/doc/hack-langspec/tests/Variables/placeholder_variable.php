...
function g(
  int $_,
): void { // checker accepts $_ as a parameter name, but probably shouldn't
  echo "\$_ is >".$_."<\n";
}

class C {
  public float $_ =
    0.0; // checker accepts $_ as an instance field name, but probably shouldn't
}
...
function main(): void {
  ...
  foreach ($a as $key => $value) {
    var_dump(
      ...
      $value,
      $_,
    ); // checks and works, as expected; 'a', 10, "hello", and 'f', 30, "hello"
    //    var_dump($_);	// You are using the return value of a void function (Typing[4119])
    // A void function doesn't return a value ($_ is a placeholder variable not meant to be used)
  }
