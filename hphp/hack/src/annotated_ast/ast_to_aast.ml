...
module AnnotatedAST (AastAnnotations : Aast.ASTAnnotationTypes) =
struct
  include Aast
  include Aast.AnnotatedAST(AastAnnotations)
end

module MapAstToAast (Env : AstToAastEnvType) =
struct
  ...
  and on_expr (p, e) : Aast.expr =
    ...
    | Class_get (e1, e2) -> Aast.Class_get ((annot, Aast.CIexpr (on_expr e1)), Aast.CGexpr (on_expr e2))
    | Class_const (e, s) -> Aast.Class_const ((annot, Aast.CIexpr (on_expr e)), s)
    | Call (e, tl, el, uel) ->
