...
var_dump( disk_total_space( __SystemLib\hphp_test_tmppath('dir1') )); // Invalid directory

$fh = fopen( __SystemLib\hphp_test_tmppath('disk_total_space.tmp'), "w" );
fwrite( $fh, (string)" Garbage data for the temporary file" );
var_dump( disk_total_space( __SystemLib\hphp_test_tmppath('disk_total_space.tmp') )); // file input instead of directory
fclose($fh);

echo"\n--- Done ---";

unlink(__SystemLib\hphp_test_tmppath('disk_total_space.tmp'));
}
