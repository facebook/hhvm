 ...
 * down. The connection layer drives lifecycle in two phases:
 *
 ...
 *   close() — initiate teardown; returns immediately. The connection is
 *             responsible for bounding its own termination (draining
 *             in-flight work, reaping stuck callbacks) and firing its
 *             close callback when fully torn down.
 *
