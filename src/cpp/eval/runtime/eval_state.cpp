...
void RequestEvalState::requestInit() {}

void RequestEvalState::requestShutdown() {
...
// Should run last
int RequestEvalState::priority() const {
  return 0x7FFFFFFF;
}

static RequestLocal<RequestEvalState> s_res;
...
bool RequestEvalState::includeFile(Variant &res, CStrRef path, bool once,
  ...
  time_t modtime;
  if (!FileRepository::findFile(spath, modtime, currentDir)) return false;
  ...
  if (it != self->m_evaledFiles.end()) {
    ...
    efile = FileRepository::checkoutFile(spath, modtime);
    if (efile) self->m_evaledFiles[spath] = efile;
  }
  if (efile) {
