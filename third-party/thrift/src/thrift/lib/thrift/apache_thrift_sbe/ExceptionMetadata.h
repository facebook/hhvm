...
class ExceptionMetadata {
 ...
 public:
  ...
  ExceptionMetadata& wrapForEncode(
      char* buffer,
      const std::uint64_t offset,
      const std::uint64_t bufferLength) {
    m_buffer = buffer;
  ...
  ExceptionMetadata& wrapAndApplyHeader(
      char* buffer,
      const std::uint64_t offset,
      const std::uint64_t bufferLength) {
  ...
  SBE_NODISCARD static const char* payloadExceptionMetadataMetaAttribute(
  ...
  payloadExceptionMetadataSinceVersion() SBE_NOEXCEPT {
    return 0;
  }
  ...
  payloadExceptionMetadataEncodingOffset() SBE_NOEXCEPT {
    return 0;
  }
  ...
  void onPayloadExceptionMetadataAccessed() const {
    if (codecState() == CodecState::NOT_WRAPPED) {
      throw AccessOrderError(
          ...
          "Cannot access field \"payloadExceptionMetadata\" in state: " +
 ...
 public:
...
#endif
    m_payloadExceptionMetadata.wrap(
        m_buffer, m_offset + 0, m_actingVersion, m_bufferLength);
    return m_payloadExceptionMetadata;
  }

  SBE_NODISCARD static const char* name_utf8MetaAttribute(
      const MetaAttribute metaAttribute) SBE_NOEXCEPT {
    switch (metaAttribute) {
      case MetaAttribute::PRESENCE:
        return "required";
  ...
  bool name_utf8InActingVersion() SBE_NOEXCEPT { return true; }

  static SBE_CONSTEXPR std::uint16_t name_utf8Id() SBE_NOEXCEPT { return 2; }
  ...
  void onName_utf8LengthAccessed() const {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot decode length of var data \"name_utf8\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum ExceptionMetadata::CodecState.");
  ...
  void onName_utf8Accessed() {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot access field \"name_utf8\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum ExceptionMetadata::CodecState.");
 ...
 public:
  ...
  SBE_NODISCARD static const char* what_utf8MetaAttribute(
      const MetaAttribute metaAttribute) SBE_NOEXCEPT {
    switch (metaAttribute) {
      case MetaAttribute::PRESENCE:
        return "required";
  ...
  bool what_utf8InActingVersion() SBE_NOEXCEPT { return true; }

  static SBE_CONSTEXPR std::uint16_t what_utf8Id() SBE_NOEXCEPT { return 3; }
  ...
  void onWhat_utf8LengthAccessed() const {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot decode length of var data \"what_utf8\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum ExceptionMetadata::CodecState.");
  ...
  void onWhat_utf8Accessed() {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot access field \"what_utf8\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum ExceptionMetadata::CodecState.");
 ...
 public:
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
      ...
      const ExceptionMetadata& _writer) {
    ...
    builder << ", ";
    builder << R"("name_utf8": )";
    builder << '"' << writer.getName_utf8AsJsonEscapedString().c_str() << '"';

    builder << ", ";
    builder << R"("what_utf8": )";
    builder << '"' << writer.getWhat_utf8AsJsonEscapedString().c_str() << '"';
  ...
  SBE_NODISCARD static std::size_t computeLength(
      std::size_t name_utf8Length = 0, std::size_t what_utf8Length = 0) {
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    if (name_utf8Length > 1073741824LL) {
      throw std::runtime_error(
          "name_utf8Length too long for length type [E109]");
    }
    ...
    if (what_utf8Length > 1073741824LL) {
      throw std::runtime_error(
          "what_utf8Length too long for length type [E109]");
    }
