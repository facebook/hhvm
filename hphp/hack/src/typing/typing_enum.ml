...
let enum_check_const ty_exp env cc t =
  ...
  Typing_ops.sub_type p Reason.URenum env t ty_exp
...
let enum_check_type env pos ur ty on_error =
  let ty_arraykey =
    MakeType.arraykey (Reason.Rimplicit_upper_bound (pos, "arraykey")) in
  Errors.try_
    (fun () ->
      Typing_ops.sub_type pos ur env ty ty_arraykey on_error)
    (fun _ ->
...
let enum_class_check env tc consts const_types =
  ...
  let enum_info_opt =
    Decl_enum.enum_kind (pos, Cls.name tc) (Cls.enum_type tc) (Cls.get_ancestor tc) in
  ...
  | Some (ty_exp, _, ty_constraint) ->
    ...
    let env = enum_check_type env pos Reason.URenum_underlying ty_exp Errors.enum_underlying_type_must_be_arraykey in
    ...
    let env =
      ...
      | Some ty ->
        ...
        let env = enum_check_type env pos Reason.URenum_cstr ty Errors.enum_constraint_must_be_arraykey in
        Typing_ops.sub_type pos Reason.URenum_incompatible_cstr env ty_exp ty
          Errors.enum_subtype_must_have_compatible_constraint in
