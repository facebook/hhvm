...
let set_line_end line pos =
  ...
  | Pos_large { pos_file; pos_start; pos_end } ->
    ...
    let (_, column_start, offset_start) = File_pos_large.line_column_offset pos_start in
    let (_, column_end, offset_end) = File_pos_large.line_column_offset pos_end in
    let pos_start = File_pos_large.of_line_column_offset ~line ~column:column_start ~offset:offset_start in
    let pos_end = File_pos_large.of_line_column_offset ~line ~column:column_end ~offset:offset_end in
    Pos_large { pos_file; pos_start; pos_end }
  | _ -> pos
