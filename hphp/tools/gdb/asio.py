...
class WaitHandle:
    ...
    def __init__(self, wh):
        ...
        if not str(deref(wh).type).endswith("WaitHandle") and not str(
            deref(wh).type
        ).endswith("Awaitable"):
...
class InfoAsioCommand(gdb.Command):
    ...
    def invoke(self, args, from_tty):
        ...
        vmfp = TL("HPHP::rds::tl_base").cast(header_ptype)["vmRegs"][
            "fp"
        ]  # gdb.Value[HPHP::ActRec]
        ...
        print(
            "%d other resumable%s queued" % (int(queue_size), plural_suffix(queue_size))
        )
        ...
        print("")
        print("%d pending sleep event%s" % (int(num_sleeps), plural_suffix(num_sleeps)))
        print(
            "%d pending external thread event%s"
