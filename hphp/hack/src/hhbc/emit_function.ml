...
let emit_function (ast_fun, hoisted) : Hhas_function.t list =
  ...
  let original_id =
    Hhbc_id.Function.elaborate_id namespace ast_fun.T.f_name in
  let function_is_async =
    ast_fun.T.f_fun_kind = Ast_defs.FAsync
    || ast_fun.T.f_fun_kind = Ast_defs.FAsyncGenerator in
  ...
  let renamed_id =
    if is_memoize
    then
      Hhbc_id.Function.add_suffix original_id Emit_memoize_helpers.memoize_suffix
  ...
  let function_rx_level = Rx.rx_level_from_ast ast_fun.T.f_user_attributes
  ...
  let ast_body, is_rx_body, function_rx_disabled =
    ...
    else ast_fun.T.f_body.T.fb_ast, false, false in
...
let emit_functions_from_program (ast : (Closure_convert.hoist_kind * Tast.def) list) =
  let aux (is_top, def) =
    match def with
