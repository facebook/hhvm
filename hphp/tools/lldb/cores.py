...
class ExtractCommand(utils.Command):
    command = "extract"
    description = "Extract information like stacktrace, JIT profile, etc. from the core file"
    ...
    def __call__(self, debugger: lldb.SBDebugger, command: str, exe_ctx: lldb.SBExecutionContext, result: lldb.SBCommandReturnObject):
        ...
        if start.GetError().Fail():
            result.SetError(f"Unable to access start variable '{start_var}': {start.GetError()}")
            return
        ...
        if end.GetError().Fail():
            result.SetError(f"Unable to access end variable '{end_var}': {end.GetError()}")
            return
        ...
        err = lldb.SBError()
        memory = exe_ctx.target.ReadMemory(lldb.SBAddress(start_addr, exe_ctx.target), table_size, err)
        if err.Fail():
