...
void EventHook::onFunctionExit(const ActRec* ar) {
  ...
  // If we have a pending exception, then we're in the process of
  // unwinding for that exception. We avoid raising more exceptions
  // for surprises (including the pending exception).
