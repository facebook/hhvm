 ...
 */
  ...
  private function _yamlize($key, $value, $indent, $previous_key = -1,
    ...
    if (is_array($value)) {
      ...
      $string = $this->_dumpNode($key, self::REMPTY, $indent, $previous_key,
                                 $first_key, $source_array);
      // Add the indent
  ...
  private function _dumpNode($key, $value, $indent, $previous_key = -1,
    ...
    if (is_array ($source_array) && array_keys($source_array) ===
        range(0, count($source_array) - 1)) {
      // It's a sequence
      $string = $spaces.'- '.$value."\n";
    } else {
  ...
  private static function getTranslations(array $words) {
    $result = array();
    foreach ($words as $i) {
      $result = array_merge($result, array(ucfirst($i), strtoupper($i),
                            strtolower($i)));
    }
    return $result;
  }
  ...
  private function loadWithSource($Source) {
    ...
    for ($i = 0; $i < $cnt; $i++) {
      ...
      if ($literalBlockStyle) {
        ...
        $literal_block_indent = strlen($Source[$i+1]) -
                                       strlen(ltrim($Source[$i+1]));
        while (++$i < $cnt &&
  ...
  private function addLiteralLine ($literalBlock, $line,
   ...
   function revertLiteralPlaceHolder ($lineArray, $literalBlock) {
     foreach ($lineArray as $k => $_) {
      ...
      else if (substr($_, -1 * strlen ($this->LiteralPlaceHolder)) ==
...
do {
  ...
  if (empty ($_SERVER['PHP_SELF']) ||
      FALSE === strpos ($_SERVER['PHP_SELF'], 'Spyc.php') ) break;
  $file = $argv[1];
