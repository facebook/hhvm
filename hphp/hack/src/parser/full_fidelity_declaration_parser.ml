...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  ...
  struct
    module WithExpressionAndStatementAndTypeParser
      ...
      let parse_type_specifier ?(allow_var = false) ?(allow_attr = true) parser
        ...
        with_type_parser
          parser
          (TypeParser.parse_type_specifier ~allow_var ~allow_attr)
      ...
      let parse_expression parser =
        ...
        let (expr_parser, expr) =
          ExpressionParser.parse_expression expr_parser
        in
        let env = ExpressionParser.env expr_parser in
        let lexer = ExpressionParser.lexer expr_parser in
        let errors = ExpressionParser.errors expr_parser in
    ...
    *)
      ...
      and parse_namespace_body parser =
        ...
        | LeftBrace ->
          ...
          let (parser, body) =
            parse_terminated_list parser parse_declaration RightBrace
          in
    ...
    we should give an informative error message about that. *)
      ...
      *exactly* token if there is nothing except it in the name. *)
      ...
      and parse_group_use parser =
        ...
        in
        let (parser, semi) = require_semicolon parser in
        Make.namespace_group_use_declaration
          parser
          use_token
          use_kind
          prefix
    ...
    *)
    ...
    and detect the errors in a later pass. *)
      ...
      and parse_classish_declaration parser attribute_spec =
        let (parser, modifiers) = parse_classish_modifiers parser in
        let (parser, token) = parse_classish_token parser in
        ...
        let (parser, classish_implements, classish_implements_list) =
          parse_classish_implements_opt parser
        in
        let (parser, classish_where_clause) =
          parse_classish_where_clause_opt parser
        in
      ...
      and parse_classish_token parser =
        ...
        | Name when is_misspelled_from spellcheck_tokens token_str ->
          ...
          let suggested_kind =
            Option.value
              (suggested_kind_from spellcheck_tokens token_str)
              ~default:Name
          in
      ...
      and parse_classish_implements_opt parser =
        ...
        if Token.kind implements_token <> Implements then
          ...
          let (parser, implements_token) =
            Make.token parser1 implements_token
          in
      ...
      not every type is legal here. *)
        ...
        | Shape
        | Self
          when Env.hhvm_compat_mode (env parser) ->
  ...
  *)
        ...
        | LessThanLessThan ->
          ...
          parse_methodish_or_property_or_type_constant parser attr
        | At
          when Full_fidelity_parser_env.allow_new_attribute_syntax parser.env
          ->
          (* Parse methods, constructors, properties, or type constants. *)
          let (parser, attr) = parse_attribute_specification_opt parser in
          parse_methodish_or_property_or_type_constant parser attr
        ...
        | _ ->
         ...
         modifier, accept it here, but emit an error in a later pass. *)
          let (parser1, property) =
            let (parser, missing1) = Make.missing parser (pos parser) in
            let (parser, missing2) = Make.missing parser (pos parser) in
            parse_property_declaration parser missing1 missing2
          in
    ...
    *)
        ...
        | XHPClassName
        | XHPCategoryName ->
          parse_xhp_children_trailing parser1 name
        | LeftParen ->
      ...
      and parse_xhp_children_bar parser left =
        ...
        | Bar ->
          ...
          let (parser, result) =
            Make.binary_expression parser left token right
          in
    ...
    *)
      ...
      Note that these are two tokens. They can have whitespace between them. *)
        if peek_token_kind parser = At then
          ...
          | _ ->
            let parser = with_error parser SyntaxError.error1051 in
            Make.missing parser (pos parser)
        else
          Make.missing parser (pos parser)
    ...
    *)
      ...
      and parse_qualified_name_type parser =
       ...
       argument list; if we don't have a name, give an error. *)
        ...
        | Backslash
        | Name ->
          parse_simple_type_or_generic parser
    ...
    *)
       ...
       *)
       ...
       missing. *)
        ...
        let (parser, req_kind) =
          match Token.kind req_kind_token with
          | Implements
          | Extends ->
            Make.token parser1 req_kind_token
          | _ ->
        ...
        let (parser, name) =
          if is_next_xhp_class_name parser then
            ...
            parse_qualified_name_type parser
        in
        let (parser, semi) = require_semicolon parser in
      ...
      and parse_methodish_or_property_or_type_constant parser attribute_spec =
        ...
        | _ -> parse_methodish_or_property parser attribute_spec
  ...
  *)
      ...
      and parse_methodish_or_property parser attribute_spec =
        ...
        | (_, (Async | Coroutine | Function))
          ...
          ->
          let parser =
            with_error parser SyntaxError.error1056 ~on_whole_token:true
          in
          let parser =
            skip_and_log_unexpected_token parser ~generate_error:false
          in
          parse_methodish parser attribute_spec modifiers
        (* Otherwise, continue parsing as a property (which might be a lambda). *)
        | (_, _) -> parse_property_declaration parser attribute_spec modifiers
      ...
      and parse_trait_use_precedence_item parser name =
        ...
        let (parser, removed_names) =
          parse_trait_name_list parser (function
              | Semicolon -> true
              | _ -> false)
        in
  ...
  *)
      and parse_trait_use_conflict_resolution parser use_token trait_name_list
        ...
        let (parser, right_brace) =
          require_token parser RightBrace SyntaxError.error1006
        in
  ...
  *)
      and parse_const_declaration parser modifiers const =
        ...
        in
        let (parser, semi) = require_semicolon parser in
        Make.const_declaration parser modifiers const type_spec const_list semi
  ...
  *)
      and parse_type_const_declaration parser attributes modifiers const =
        ...
        let (parser, equal_token, type_specifier) =
          if peek_token_kind parser = Equal then
   ...
   *)
   ...
   (Both these work items are tracked by spec issue 106.) *)
      and parse_old_attribute_specification_opt parser =
        if peek_token_kind parser = LessThanLessThan then
          let (parser, left, items, right) =
            parse_double_angled_comma_list_allow_trailing
              parser
              parse_old_attribute
          in
      ...
      and parse_new_attribute_specification_opt parser =
        let (parser, attributes) =
          parse_list_while parser parse_new_attribute (fun parser ->
              peek_token_kind parser = At)
        in
      ...
      and parse_new_attribute parser =
        ...
        let (parser, constructor_call) =
          match Token.kind token with
          | Name ->
            with_expression_parser
              parser
              ExpressionParser.parse_constructor_call
          | _ ->
            let parser =
              with_error parser SyntaxError.expected_user_attribute
            in
            Make.missing parser (pos parser)
        in
      ...
      and parse_file_attribute_specification_opt parser =
        if peek_token_kind parser = LessThanLessThan then
          ...
          in
          let (parser, right) =
            require_token parser GreaterThanGreaterThan SyntaxError.error1029
          in
          Make.file_attribute_specification
            parser
            left
            keyword
            colon
    ...
    *)
      ...
      and parse_decorated_variable_opt parser =
        ...
        | DotDotDot
        | Ampersand ->
          parse_decorated_variable parser
  ...
  *)
      ...
      and parse_function_declaration parser attribute_specification =
        let (parser, modifiers) = parse_modifiers parser in
        let (parser, header) =
          parse_function_declaration_header
            parser
            ~is_methodish:false
            modifiers
        in
    ...
    *)
      ...
      and parse_where_constraint_list_item parser =
        ...
        | _ ->
          ...
          let (parser, comma) = optional_token parser Comma in
          let (parser, result) =
            Make.list_item parser where_constraint comma
          in
          (parser, Some result)
    ...
    *)
        ...
        let (parser, constraints) =
          parse_list_until_none parser parse_where_constraint_list_item
        in
    ...
    *)
       ...
       at parse time but rather by a later pass. *)
        ...
        let (parser, label) = parse_function_label_opt parser ~is_methodish in
        let (parser, generic_type_parameter_list) =
          parse_generic_type_parameter_list_opt parser
        in
        ...
        let (parser, colon_token, return_type) =
          parse_return_type_hint_opt parser
        in
   ...
   *)
      and parse_methodish parser attribute_spec modifiers =
        let (parser, header) =
          parse_function_declaration_header parser modifiers ~is_methodish:true
        in
        let (parser1, token) = next_token parser in
        match Token.kind token with
        | LeftBrace ->
        ...
        | Equal ->
          ...
          in
          let (parser, semi) = require_semicolon parser in
          Make.methodish_trait_resolution
            parser
            attribute_spec
            header
            equal
      ...
      begin with an attribute. *)
        let (parser1, attribute_specification) =
          parse_attribute_specification_opt parser
        in
        ...
        | Type
        | Newtype ->
          parse_alias_declaration parser1 attribute_specification
        | Async
        | Coroutine
        | Function ->
        ...
        | Trait
        | Class ->
          parse_classish_declaration parser1 attribute_specification
        | _ ->
      ...
      from the tree. *)
          let parser =
            with_error parser2 (SyntaxError.error1057 (Token.text token))
          in
          let (parser, token) = Make.token parser token in
          Make.error parser token
      ...
      and parse_declaration parser =
        ...
        let (parser, result) =
          ...
          | Require
          | Require_once ->
            parse_inclusion_directive parser
          | Type
          | Newtype
            when match peek_token_kind parser1 with
                 | Name
                 | Classname ->
                   true
                 | _ -> false ->
        ...
        the current namespace.      *)
          ...
          | _ -> with_statement_parser parser StatementParser.parse_statement
    ...
    *)
        ...
        | Type ->
          let (parser, typ) =
            require_token parser Type SyntaxError.type_keyword
          in
          let (parser, tyname) = require_name parser in
          let (parser, equal) = require_equal parser in
          let (parser, ty) = parse_type_specifier parser in
          Make.pocket_mapping_type_declaration parser typ tyname equal ty
        | Name ->
          let (parser, id) = require_name parser in
          let (parser, equal) = require_equal parser in
          let (parser, simple_init) = parse_expression parser in
          let (parser, sc_init) =
            Make.simple_initializer parser equal simple_init
          in
          Make.pocket_mapping_id_declaration parser id sc_init
        | _ ->
          let parser =
            with_error parser SyntaxError.pocket_universe_invalid_field
          in
          Make.missing parser (pos parser)
        ...
        | ColonAt ->
          ...
          | LeftParen ->
            let (parser, left_paren, mappings, right_paren) =
              parse_parenthesized_comma_list parser parse_pocket_mapping
            in
            let (parser, semi) = require_semicolon parser in
            Make.pocket_atom_mapping_declaration
              parser
              glyph
              enum_name
              left_paren
              ...
              semi
          | _ ->
            let (parser, missing_left) = Make.missing parser (pos parser) in
            let (parser, missing_mappings) =
              Make.missing parser (pos parser)
            in
            ...
            Make.pocket_atom_mapping_declaration
              ...
              semi)
        | Case ->
          let (parser, case_tok) = assert_token parser Case in
          (match peek_token_kind parser with
          | Type ->
            let (parser, type_tok) = assert_token parser Type in
            let (parser, name) = require_name parser in
            let (parser, semi) = require_semicolon parser in
            Make.pocket_field_type_declaration
              parser
              case_tok
              type_tok
              name
          ...
          | _ ->
            ...
            let (parser, name) = require_name parser in
            let (parser, semi) = require_semicolon parser in
            Make.pocket_field_type_expr_declaration
              parser
              case_tok
              ty
              name
    ...
    *)
        ...
        let (parser, final_tok) =
          if final then
            require_token
              parser
              Final
              SyntaxError.pocket_universe_final_expected
          else
            Make.missing parser (pos parser)
        in
        (* read Enum *)
        let (parser, enum_tok) =
          require_token parser Enum SyntaxError.pocket_universe_enum_expected
        in
        let (parser, name) = require_name parser in
        let (parser, left_brace, pocket_fields, right_brace) =
          parse_braced_list parser parse_pocket_fields_opt
        in
        Make.pocket_enum_declaration
          parser
          final_tok
          enum_tok
          name
      ...
      let parse_leading_markup_section parser =
        let (parser1, (markup_section, has_suffix)) =
          with_statement_parser parser (fun p ->
              let (p, s, has_suffix) = StatementParser.parse_header p in
              (p, (s, has_suffix)))
        in
      ...
      the error on a later pass *)
        if has_suffix then
          ...
          let parser =
            if
              SourceText.length text > 0
              && String_utils.string_ends_with suffix ".php"
            then
              with_error parser SyntaxError.error1001
            else
              parser
          in
          (parser, None)
