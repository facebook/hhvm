...
private:
     ...
     *       V0_OTHERMETADATA_DONE -> V0_EXCEPTIONMETADATA_DONE [label="  exceptionMetadata(?)  "];
     *       V0_EXCEPTIONMETADATA_DONE -> V0_EXCEPTIONMETADATA_DONE [label="  optionalMetadataLength()  "];
     *       V0_EXCEPTIONMETADATA_DONE -> V0_OPTIONALMETADATA_DONE [label="  optionalMetadata(?)  "];
     *   }
     * }</pre>
     */
...
public:
    ...
    ResponseRpcMetadata(char *buffer, const std::uint64_t bufferLength) :
        ResponseRpcMetadata(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ResponseRpcMetadata(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ResponseRpcMetadata(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ResponseRpcMetadata &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ResponseRpcMetadata &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ResponseRpcMetadata skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion, m_codecState);
        skipper.skip();
    ...
    void onStreamIdAccessed() const
    {
        if (codecState() == CodecState::NOT_WRAPPED)
        {
            throw AccessOrderError(std::string("Illegal field access order. ") +
                "Cannot access field \"streamId\" in state: " + codecStateName(codecState()) +
...
public:
    ...
    SBE_NODISCARD static const char *exceptionMetadataMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint64_t exceptionMetadataSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool exceptionMetadataInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }
    ...
    static SBE_CONSTEXPR std::uint64_t exceptionMetadataHeaderLength() SBE_NOEXCEPT
    {
        return 4;
    }
...
private:
    void onExceptionMetadataLengthAccessed() const
    {
        switch (codecState())
        {
            ...
            default:
                throw AccessOrderError(std::string("Illegal field access order. ") +
                    "Cannot decode length of var data \"exceptionMetadata\" in state: " + codecStateName(codecState()) +
                    ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                    "]. Please see the diagram in the docs of the enum ResponseRpcMetadata::CodecState.");
        }
    }
...
private:
    void onExceptionMetadataAccessed()
    {
        switch (codecState())
        {
            ...
            default:
                throw AccessOrderError(std::string("Illegal field access order. ") +
                    "Cannot access field \"exceptionMetadata\" in state: " + codecStateName(codecState()) +
                    ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                    "]. Please see the diagram in the docs of the enum ResponseRpcMetadata::CodecState.");
        }
    }
...
public:
    ...
    SBE_NODISCARD static const char *optionalMetadataMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint64_t optionalMetadataSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool optionalMetadataInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }
    ...
    static SBE_CONSTEXPR std::uint64_t optionalMetadataHeaderLength() SBE_NOEXCEPT
    {
        return 4;
    }
...
private:
    void onOptionalMetadataLengthAccessed() const
    {
        switch (codecState())
        {
            ...
            default:
                throw AccessOrderError(std::string("Illegal field access order. ") +
                    "Cannot decode length of var data \"optionalMetadata\" in state: " + codecStateName(codecState()) +
                    ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                    "]. Please see the diagram in the docs of the enum ResponseRpcMetadata::CodecState.");
        }
    }
...
private:
    void onOptionalMetadataAccessed()
    {
        switch (codecState())
        {
            ...
            default:
                throw AccessOrderError(std::string("Illegal field access order. ") +
                    "Cannot access field \"optionalMetadata\" in state: " + codecStateName(codecState()) +
                    ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                    "]. Please see the diagram in the docs of the enum ResponseRpcMetadata::CodecState.");
        }
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ResponseRpcMetadata &_writer)
{
    ...
    builder << '"' <<
        writer.skipExceptionMetadata() << " bytes of raw data\"";
    builder << ", ";
    builder << R"("optionalMetadata": )";
    builder << '"' <<
        writer.skipOptionalMetadata() << " bytes of raw data\"";
    builder << '}';
