<?php
	...
	$test_table_name = 'test_mysqli_explain_metadata_table_1'; require_once('table.inc');

	if (!$res = mysqli_query($link, 'EXPLAIN SELECT t1.*, t2.* FROM test_mysqli_explain_metadata_table_1 AS t1, test_mysqli_explain_metadata_table_1 AS t2'))
	...
	if ($stmt->prepare('EXPLAIN SELECT t1.*, t2.* FROM test_mysqli_explain_metadata_table_1 AS t1, test_mysqli_explain_metadata_table_1 AS t2') && $stmt->execute()) {
		...
		if (function_exists('mysqli_stmt_get_result') &&
			$stmt->prepare('EXPLAIN SELECT t1.*, t2.* FROM test_mysqli_explain_metadata_table_1 AS t1, test_mysqli_explain_metadata_table_1 AS t2') &&
			$stmt->execute()) {
...
?>
<?php
	$test_table_name = 'test_mysqli_explain_metadata_table_1'; require_once("clean_table.inc");
?>