...
class TBufferedTransport
 ...
 protected:
  virtual void shrinkWriteBuffer();
...
class TMemoryBuffer : public TVirtualTransport<TMemoryBuffer, TBufferBase> {
 private:
  ...
  void initCommon(uint8_t* buf, uint32_t size, bool owner, uint32_t wPos) {
    ...
    // TODO(dreiss): Investigate nullptr-ing this if !owner.
    wBase_ = buffer_ + wPos;
    wBound_ = buffer_ + bufferSize_;

    owner_ = owner;

    linkedBuffer_ = nullptr;
    observerCount_ = 0;
 ...
 public:
  ...
  std::unique_ptr<folly::IOBuf> cloneBufferAsIOBuf() const {
    if (buffer_ == nullptr) {
      return nullptr;
    }
    uint8_t* buf;
    uint32_t sz;
    getBuffer(&buf, &sz);
    return folly::IOBuf::copyBuffer(buf, sz);
  }

  std::unique_ptr<folly::IOBuf> wrapBufferAsIOBuf() const {
    if (buffer_ == nullptr) {
      return nullptr;
    }
    uint8_t* buf;
    uint32_t sz;
    getBuffer(&buf, &sz);
    return folly::IOBuf::wrapBuffer(buf, sz);
  }
