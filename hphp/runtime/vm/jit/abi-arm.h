...
/*
 * ARM    int meaning                   fp meaning                         bit tests            JIT mapping        JIT meaning
 * ------------------------------------------------------------------------------------------------------------------
 * eq     equal                         equal                              Z == 1               CC_E/CC_Z          equal
 * ne     not equal                     not equal or unordered             Z == 0               CC_NE/CC_NZ        not equal
 * cs/hs  carry set                     greater than, equal, or unordered  C == 1               CC_AE/CC_NB/CC_NC  unsigned higher or same
 * cc/lo  carry clear                   less than                          C == 0               CC_B/CC_NAE        unsigned lower
 * mi     minus, negative               less than                          N == 1               CC_S               minus (sign set)
 * pl     plus, positive, or zero       greater than, equal, or unordered  N == 0               CC_NS              plus (sign clear)
 * vs     overflow                      unordered                          V == 1               CC_O               overflow set
 * vc     no overflow                   ordered                            V == 0               CC_NO              overflow clear
 * hi     unsigned higher               greater than, or unordered         C == 1 && Z == 0     CC_A/CC_NBE        unsigned higher
 * ls     unsigned lower or same        less than or equal                 !(C == 1 && Z == 0)  CC_BE/CC_NA        unsigned lower or same
 * ge     signed greater than or equal  greater than or equal              N == V               CC_GE/CC_NL        signed greater or equal
 * lt     signed less than              less than, or unordered            N != V               CC_L/CC_NGE        signed less than
 * gt     signed greater than           greater than                       Z == 0 && N == V     CC_G/CC_NLE        signed greater than
 * le     signed less than or equal     less than, equal, or unordered     !(Z == 0 && N == V)  CC_LE/CC_NG        signed less or equal
 * al     always (not used)             always (not used)                  any                  CC_P               parity
 * nv     not valid                                                        any                  CC_NP              no parity
 */
