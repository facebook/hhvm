...
struct AccessPoint {
  ...
  const std::shared_ptr<const std::string>& getTwJobPtr() const {
    return twJob_;
  }

  void setTwJob(std::shared_ptr<const std::string> twJob) {
 ...
 private:
  ...
  std::shared_ptr<const std::string> twJob_{nullptr};
};
