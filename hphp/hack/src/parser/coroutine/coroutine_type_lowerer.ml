...
let rewrite_return_type return_type =
  if is_missing return_type then
    mixed_syntax
  else if is_void return_type then
    unit_type_syntax
  else
    return_type
...
let rewrite_function_header_return_type
  ...
  let function_colon =
    if is_missing function_colon then
      colon_syntax
    else
      function_colon
  in
...
let rewrite_anon_function_return_type
  ...
  let anonymous_colon =
    if is_missing anonymous_colon then
      colon_syntax
    else
      anonymous_colon
  in
...
let rewrite_lambda_return_type ({ lambda_colon; lambda_type; _ } as node) =
  let lambda_type = rewrite_return_type lambda_type in
  let lambda_colon =
    if is_missing lambda_colon then
      colon_syntax
    else
      lambda_colon
  in
