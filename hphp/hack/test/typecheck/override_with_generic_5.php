...
class B<T as I> extends A {
  <<__Override>>
  public function numInCovar_Sub<TTi>(Contra<TTi> $_): void
  where
    T as I with { type Ti super TTi } {
  }
  ...
  <<
    __Override,
  >> // CURRENT desugaring of polymorphic coeffects -> this must type-check but it doesn't!!!
  public function numInCovar_Eq<TTi>(Contra<TTi> $_): void
  where
    T as I with { type Ti = TTi } {} // OK
  // iff Contra<num> as Contra<TTi> for some TTi such that TTi=T::Ti
  ...
  <<__Override>>
  public function numInCovar_Super<TTi>(Contra<TTi> $_): void
  where
    T as I with { type Ti as TTi } {} // ERROR
  // iff Contra<num> as Contra<TTi> for some TTi such that T::Ti <: TTi
  // iff TTi <: num
  // Just let TTi = num and we're done.

  <<__Override>>
  public function numInContravar_Super<TTi>(TTi $_): void
  where
    T as I with { type Ti as TTi } {} // ERROR
  ...
  <<__Override>>
  public function numInContravar_Sub<TTi>(TTi $_): void
  where
    T as I with { type Ti super TTi } {} // ERROR
  // iff num <: TTi for some TTi such that TTi <: T::Ti
  // So we have num <: TTi <: T::Ti which has no solution

  <<__Override>>
  public function numInContravar_Eq<TTi>(TTi $_): void
  where
    T as I with { type Ti = TTi } {} // ERROR
  // iff num <: TTi for some TTi such that TTi = T::Ti
  // There is no solution

  <<__Override>>
  public function numInInvar_Super<TTi>(In<TTi> $_): void
  where
    T as I with {
      type Ti super TTi } {} // wrongly OK (was ERROR before D34600464)
  // Need In<num> <: In<TTi> such that TTi <: T::Ti
  // No solution

  <<__Override>>
  public function numInInvar_Sub<TTi>(In<TTi> $_): void
  where
    T as I with { type Ti as TTi } {} // OK
  // Need In<num> <: In<TTi> such that T::Ti <: TTi
  // Just set TTi = num and we're done.

  <<__Override>>
  public function numInInvar_Eq<TTi>(In<TTi> $_): void
  where
    T as I with { type Ti = TTi } {} // ERROR
  // Need In<num> <: In<TTi> such that TTi = T::Ti
  // No solution
