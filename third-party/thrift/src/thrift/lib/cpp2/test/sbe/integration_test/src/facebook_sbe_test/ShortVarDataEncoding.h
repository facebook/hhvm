...
#ifndef _FACEBOOK_SBE_TEST_SHORTVARDATAENCODING_CXX_H_
#define _FACEBOOK_SBE_TEST_SHORTVARDATAENCODING_CXX_H_
...
public:
    ...
    ShortVarDataEncoding() = default;

    ShortVarDataEncoding(
        char *buffer,
        ...
        m_actingVersion(actingVersion)
    {
        ...
        {
            throw std::runtime_error("buffer too short for flyweight [E107] in ShortVarDataEncoding");
        }
    }
    ...
    ShortVarDataEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        ShortVarDataEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    ShortVarDataEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        ShortVarDataEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    ShortVarDataEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    ShortVarDataEncoding &length(const std::uint16_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, ShortVarDataEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
