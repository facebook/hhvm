...
#define REGINT(n, v) Native::registerConstant<KindOfInt64> \
    ...
    REGINT(APACHE_MAP, 200);
    REGINT(SOAP_1_1, 1);
    REGINT(SOAP_1_2, 2);
    REGINT(SOAP_ACTOR_NEXT, 1);
    REGINT(SOAP_ACTOR_NONE, 2);
    REGINT(SOAP_ACTOR_UNLIMATERECEIVER, 3);
    REGINT(SOAP_AUTHENTICATION_BASIC, 0);
    REGINT(SOAP_AUTHENTICATION_DIGEST, 1);
    REGINT(SOAP_COMPRESSION_ACCEPT, 32);
    REGINT(SOAP_COMPRESSION_DEFLATE, 16);
    REGINT(SOAP_COMPRESSION_GZIP, 0);
    REGINT(SOAP_DOCUMENT, 2);
    REGINT(SOAP_ENCODED, 1);
    REGINT(SOAP_ENC_ARRAY, 300);
    REGINT(SOAP_ENC_OBJECT, 301);
    REGINT(SOAP_FUNCTIONS_ALL, 999);
    REGINT(SOAP_LITERAL, 2);
    REGINT(SOAP_PERSISTENCE_REQUEST, 2);
    REGINT(SOAP_PERSISTENCE_SESSION, 1);
    REGINT(SOAP_RPC, 1);
    REGINT(SOAP_SINGLE_ELEMENT_ARRAYS, 1);
    REGINT(SOAP_USE_XSI_ARRAY_TYPE, 4);
    REGINT(SOAP_WAIT_ONE_WAY_CALLS, 2);
    REGINT(UNKNOWN_TYPE, 7);
    REGINT(WSDL_CACHE_BOTH, 3);
    REGINT(WSDL_CACHE_DISK, 1);
    REGINT(WSDL_CACHE_MEMORY, 2);
    REGINT(WSDL_CACHE_NONE, 0);
    REGINT(XSD_1999_TIMEINSTANT, 401);
    REGINT(XSD_ANYTYPE, 145);
    REGINT(XSD_ANYURI, 117);
    REGINT(XSD_ANYXML, 147);
    REGINT(XSD_BASE64BINARY, 116);
    REGINT(XSD_BOOLEAN, 102);
    REGINT(XSD_BYTE, 137);
    REGINT(XSD_DATE, 109);
    REGINT(XSD_DATETIME, 107);
    REGINT(XSD_DECIMAL, 103);
    REGINT(XSD_DOUBLE, 105);
    REGINT(XSD_DURATION, 106);
    REGINT(XSD_ENTITIES, 130);
    REGINT(XSD_ENTITY, 129);
    REGINT(XSD_FLOAT, 104);
    REGINT(XSD_GDAY, 113);
    REGINT(XSD_GMONTH, 114);
    REGINT(XSD_GMONTHDAY, 112);
    REGINT(XSD_GYEAR, 111);
    REGINT(XSD_GYEARMONTH, 110);
    REGINT(XSD_HEXBINARY, 115);
    REGINT(XSD_ID, 126);
    REGINT(XSD_IDREF, 127);
    REGINT(XSD_IDREFS, 128);
    REGINT(XSD_INT, 135);
    REGINT(XSD_INTEGER, 131);
    REGINT(XSD_LANGUAGE, 122);
    REGINT(XSD_LONG, 134);
    REGINT(XSD_NAME, 124);
    REGINT(XSD_NCNAME, 125);
    REGINT(XSD_NEGATIVEINTEGER, 133);
    REGINT(XSD_NMTOKEN, 123);
    REGINT(XSD_NMTOKENS, 144);
    REGINT(XSD_NONNEGATIVEINTEGER, 138);
    REGINT(XSD_NONPOSITIVEINTEGER, 132);
    REGINT(XSD_NORMALIZEDSTRING, 120);
    REGINT(XSD_NOTATION, 119);
    REGINT(XSD_POSITIVEINTEGER, 143);
    REGINT(XSD_QNAME, 118);
    REGINT(XSD_SHORT, 136);
    REGINT(XSD_STRING, 101);
    REGINT(XSD_TIME, 108);
    REGINT(XSD_TOKEN, 121);
    REGINT(XSD_UNSIGNEDBYTE, 142);
    REGINT(XSD_UNSIGNEDINT, 140);
    REGINT(XSD_UNSIGNEDLONG, 139);
    REGINT(XSD_UNSIGNEDSHORT, 141);
#undef REGINT

    Native::registerConstant<KindOfString>(
      makeStaticString("XSD_199_NAMESPACE"),
      makeStaticString("http://www.w3.org/1999/XMLSchema")
    );
    Native::registerConstant<KindOfString>(
      makeStaticString("XSD_NAMESPACE"),
      makeStaticString("http://www.w3.org/2001/XMLSchema")
    );
