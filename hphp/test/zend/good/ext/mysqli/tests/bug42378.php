<?php
	...
	function create_table($link, $column, $min, $max, $engine, $offset) {
		...
		if (!mysqli_query($link, 'DROP TABLE IF EXISTS test_bug42378_table_1')) {
			printf("[%03d] Cannot drop table test_bug42378_table_1, [%d] %s\n",
				$offset,
		...
		$sql = sprintf("CREATE TABLE test_bug42378_table_1(id INT AUTO_INCREMENT PRIMARY KEY, col1 %s) ENGINE=%s",
			$column, $engine);
		if (!mysqli_query($link, $sql)) {
			printf("[%03d] Cannot create table test_bug42378_table_1, [%d] %s\n",
				$offset + 1,
				mysqli_errno($link), mysqli_error($link));
			return array();
		}
		...
		for ($i = 1; $i <= 100; $i++) {
			...
			$values[$i] = $col1;
			$sql = sprintf("INSERT INTO test_bug42378_table_1(id, col1) VALUES (%d, %f)",
				$i, $col1);
			if (!mysqli_query($link, $sql)) {
...
?>
<?php
	$test_table_name = 'test_bug42378_table_1'; require_once("clean_table.inc");
?>