...
public:
    ...
    InterationTerminate(char *buffer, const std::uint64_t bufferLength) :
        InterationTerminate(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    InterationTerminate(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        InterationTerminate(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    InterationTerminate &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    InterationTerminate &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        InterationTerminate skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion, m_codecState);
        skipper.skip();
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const InterationTerminate &_writer)
{
