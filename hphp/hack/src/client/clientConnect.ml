...
let rec wait_for_server_message
  ...
  if readable = [] then begin
    ...
    try%lwt
      ...
      in
      if is_not_ping && matches_expected then begin
    ...
    let%lwt () =
      if env.do_post_handoff_handshake then
        ...
        wait_for_server_hello
          ic env.deadline server_finale_file env.progress_callback env.root
      else
        Lwt.return_unit
    in
...
let connect (env : env) : conn Lwt.t =
  ...
  try%lwt
    let%lwt {channels = (_, oc); _} as conn =
      connect ~first_attempt:true env start_time in
