...
let search ctx target include_defs ~hints ~files ~stream_file genv =
  ...
  let res =
    Find_refs_service.find_references
      ctx
      genv.workers
      target
      include_defs
...
let search_single_file ctx target file =
  ...
  let res = Find_refs_service.find_references_single_file ctx target file in
  strip_ns res
...
let handle_prechecked_files genv env dep f =
  ...
  let (env, _telemetry) =
    Server_prechecked_files.update_after_local_changes genv env dep ~start_time
  in
...
let search_function ctx function_name include_defs ~stream_file ~hints genv env
  ...
  search
    ctx
    (Find_refs_service.IFunction function_name)
    include_defs
    ~hints
    ~files
    ~stream_file
...
let search_single_file_for_function ctx function_name filename =
  let function_name = add_ns function_name in
  Hh_logger.debug "ServerFindRefs.search_function: %s" function_name;
  search_single_file ctx (Find_refs_service.IFunction function_name) filename
...
let search_member
  ...
  in
  let target =
    Find_refs_service.IMember
      (Find_refs_service.Class_set class_and_descendants, member)
  in
...
let search_single_file_for_member
  ...
  let target =
    Find_refs_service.IMember
      (Find_refs_service.Subclasses_of origin_class_name, member)
  in
...
let search_gconst ctx cst_name include_defs ~stream_file ~hints genv env =
  ...
  search
    ctx
    (Find_refs_service.IGConst cst_name)
    include_defs
    ~hints
    ~files
    ~stream_file
...
let search_single_file_for_gconst ctx cst_name filename =
  let cst_name = add_ns cst_name in
  search_single_file ctx (Find_refs_service.IGConst cst_name) filename
...
let search_class
  ...
  let target =
    if include_all_ci_types then
      Find_refs_service.IClass class_name
    else
      Find_refs_service.IExplicitClass class_name
  in
...
let search_localvar ~ctx ~entry pos =
  let results = Server_find_locals.go ~ctx ~entry pos in
  match results with
  | first_pos :: _ ->
...
let go_for_single_file
    ...
    ~(action : Server_command_types.Find_refs.action)
...
let go_from_file_ctx
    ~(ctx : Provider_context.t) ~(entry : Provider_context.entry) pos :
    (string * Server_command_types.Find_refs.action) option =
