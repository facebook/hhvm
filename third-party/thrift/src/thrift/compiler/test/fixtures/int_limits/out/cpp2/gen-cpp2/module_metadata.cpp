...
StructMetadata<::cpp2::Limits>::gen(ThriftMetadata& metadata) {
  auto res = genStructMetadata<::cpp2::Limits>(metadata, folly::kIsDebug);
  if (res.preExists) {
    return res.metadata;
  }
  ...
  for (const auto& f : *module_Limits_fields) {
    [[maybe_unused]] auto& field = module_Limits.fields()[i];
    ...
    DCHECK(structuredAnnotationsEquality(
      *field.structured_annotations(),
      newAnnotations,
      getFieldAnnotationTypes<::cpp2::Limits>(i, static_cast<std::int16_t>(f.id))
    ));
