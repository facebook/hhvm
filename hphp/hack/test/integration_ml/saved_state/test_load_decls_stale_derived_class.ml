...
let base_foo_user_contents =
  "<?hh // strict
function base_foo_user(Base $b): int {
  return $b->foo();
}"

let base_bar_user_contents =
  "<?hh // strict
...
let save_state saved_state_dir =
  ...
  Test.save_state disk_state saved_state_dir ~store_decls_in_saved_state:true

let load_state ?(master_changes = []) ?(local_changes = []) saved_state_dir =
  let disk_state = hot_classes_file !hot_classes :: init_disk_state in
  Test.load_state
    saved_state_dir
...
let test_master_change saved_state_dir () =
  ...
  let (env, loop_output) =
    Test.change_files env [("base.php", base_contents "public")]
  in
...
let save_state_incremental_hot_child
  ...
  let env =
    load_state
      existing_state_dir
      ~master_changes:[("base.php", base_contents "final public")]
  in
