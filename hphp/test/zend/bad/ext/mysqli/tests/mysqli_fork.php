<?php
	$test_table_name = 'test_mysqli_fork_table_1'; require_once("table.inc");
	...
	if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_fork_table_2"))
		printf("[006] [%d] %s\n", mysqli_error($link), mysqli_errno($link));

	if (!mysqli_query($link, "CREATE TABLE test_mysqli_fork_table_2(
	...
	if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_fork_table_1 ORDER BY id ASC LIMIT 3", MYSQLI_USE_RESULT))
		printf("[008] [%d] %s\n", mysqli_error($link), mysqli_errno($link));
	...
	switch ($pid) {
		...
		case 0:
			...
			$sql = sprintf("INSERT INTO test_mysqli_fork_table_2(pid, sender, msg) VALUES (%d, 'child', '%%s')", posix_getpid());
			if (!mysqli_query($plink, sprintf($sql, 'start')))
				exit(mysqli_errno($plink));

			$parent_sql = sprintf("SELECT msg_id, msg_time, msg FROM test_mysqli_fork_table_2 WHERE pid = %d  AND sender = 'parent' ORDER BY msg_id DESC LIMIT 1", posix_getppid());
			$msg_id = 0;
		...
		default:
			...
			$status = null;
			$start = time();
			$sql = sprintf("SELECT msg_id, msg_time, msg FROM test_mysqli_fork_table_2 WHERE pid = %d AND sender = 'child' ORDER BY msg_id DESC LIMIT 1", $pid);
			$parent_sql = sprintf("INSERT INTO test_mysqli_fork_table_2 (pid, sender, msg) VALUES (%d, 'parent', '%%s')", posix_getpid());
			$last_msg_id = 0;
			$num_rows = 0;
			do {
	...
	if (!$res = mysqli_query($link, "SELECT sender, msg FROM test_mysqli_fork_table_2 ORDER BY msg_id ASC"))
...
if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_fork_table_1"))
