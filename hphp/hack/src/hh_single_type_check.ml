...
let parse_options () =
  ...
  let options =
    [
      ...
      ( "--patches",
        Arg.Unit (set_mode Dump_patches),
        " Dump the patches generated by global inference (requires --global-inference)."
      );
      ( "--global-inference",
    ...
    | Some gi_solved ->
      ...
      if List.length patches <= 0 then
        ...
        List.iter patches ~f:(fun patch ->
            let json = ClientRefactor.patch_to_json patch in
            Printf.printf "%s:\n" (Pos.filename @@ ClientRefactor.get_pos patch);
            Hh_json.print_json_endline ~pretty:true json))
