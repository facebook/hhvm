...
StructMetadata<::cpp2::Limits>::gen(ThriftMetadata& metadata) {
  if (FLAGS_thrift_enable_schema_to_metadata_conversion) {
    return genStructMetadata<::cpp2::Limits>(metadata, kGenerateAll).metadata;
  }

  auto res = genStructMetadata<::cpp2::Limits>(metadata, {.genAnnotations = folly::kIsDebug});
  if (res.preExists) {
    return res.metadata;
  }
  ::apache::thrift::metadata::ThriftStruct& module_Limits = res.metadata;
  DCHECK_EQ(*module_Limits.is_union(), false);
  ...
  for (const auto& f : *module_Limits_fields) {
    genStructFieldMetadata<::cpp2::Limits>(module_Limits.fields()[i], f, i);
  ...
  DCHECK(structuredAnnotationsEquality(
    *res.metadata.structured_annotations(),
    newAnnotations,
    getAnnotationTypes<::cpp2::Limits>()
  ));
  return res.metadata;
}
