...
constexpr const char* const kAnnotationUri =
    kAllowUnsafeRequiredFieldQualifierUri;
...
class AnnotatedRequiredFields final {
 public:
  AnnotatedRequiredFields(source_manager& src_manager, t_program& program)
...
int main(int argc, char** argv) {
  return apache::thrift::compiler::run_codemod(
      argc, argv, [](source_manager& sm, t_program_bundle& pb) {
        apache::thrift::compiler::AnnotatedRequiredFields(
            sm, *pb.root_program())
            .run();
      });
}
