...
class c_XMLWriter_Instance : public c_XMLWriter {
public:
  c_XMLWriter_Instance (HPHP::VM::Class* cls, unsigned nProps) {
    DECLARE_STACK_GC_ROOT(ObjectData, this);
    m_cls = cls;
    setAttributes(cls->getODAttrs()
                  | (cls->clsInfo()
                     ? 0 : IsInstance));
    m_propVec = (TypedValue *)((uintptr_t)this + sizeof(c_XMLWriter));
  ...
  static HPHP::VM::Instance* new_Instance(HPHP::VM::Class* cls) {
    ...
    new ((void *)inst) c_XMLWriter_Instance(cls, nProps);
    return inst;
  }
  void operator delete(void *p) {
    c_XMLWriter_Instance *this_ = (c_XMLWriter_Instance*)p;
    size_t nProps = this_->m_cls->numDeclProperties();
    size_t builtinPropSize UNUSED = sizeof(c_XMLWriter) - sizeof(ObjectData);
