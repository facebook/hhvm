...
module Messages = struct
  ...
  let lru_cache_directory = " Specify a cache directory for a cache" ^
                            " only if using lru_workers."
...
let parse_options () =
  ...
  let set_lru_cache_directory = fun s -> lru_cache_directory := Some s in
...
let to_string
  {
    ...
    let ai_mode_str = match ai_mode with
      | None -> "<>"
      | Some _ -> "Some(...)" in
    let lru_cache_directory_str = match lru_cache_directory with
      | None -> "<>"
      | Some dir -> dir in
    ...
    ([
      "ServerArgs.options({";
        ...
        "lru_cache_directory: "; lru_cache_directory_str; ", ";
