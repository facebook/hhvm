...
class InteractionCreate {
 ...
 public:
  ...
  InteractionCreate& wrapForEncode(
      char* buffer,
      const std::uint64_t offset,
      const std::uint64_t bufferLength) {
    m_buffer = buffer;
  ...
  InteractionCreate& wrapAndApplyHeader(
      char* buffer,
      const std::uint64_t offset,
      const std::uint64_t bufferLength) {
 ...
 public:
  ...
  SBE_NODISCARD static const char* interactionNameMetaAttribute(
      const MetaAttribute metaAttribute) SBE_NOEXCEPT {
    switch (metaAttribute) {
      case MetaAttribute::PRESENCE:
        return "required";
  ...
  static SBE_CONSTEXPR std::uint64_t interactionNameSinceVersion()
      SBE_NOEXCEPT {
    return 0;
  }

  bool interactionNameInActingVersion() SBE_NOEXCEPT { return true; }

  static SBE_CONSTEXPR std::uint16_t interactionNameId() SBE_NOEXCEPT {
    return 2;
  }

  static SBE_CONSTEXPR std::uint64_t interactionNameHeaderLength()
      SBE_NOEXCEPT {
    return 4;
  }
  ...
  void onInteractionNameLengthAccessed() const {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot decode length of var data \"interactionName\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum InteractionCreate::CodecState.");
  ...
  void onInteractionNameAccessed() {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot access field \"interactionName\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum InteractionCreate::CodecState.");
 ...
 public:
  ...
  InteractionCreate& putInteractionName(
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
      ...
      const InteractionCreate& _writer) {
    ...
    builder << '"' << writer.getInteractionNameAsJsonEscapedString().c_str()
            << '"';
  ...
  SBE_NODISCARD static std::size_t computeLength(
      std::size_t interactionNameLength = 0) {
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    if (interactionNameLength > 1073741824LL) {
      throw std::runtime_error(
          "interactionNameLength too long for length type [E109]");
    }
