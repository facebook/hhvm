...
void syncVMRegsWork(bool soft) {
  ...
  // This will walk the frames upward via the unwinder until we find a TC frame.
  // Historically this directly traversed the chain of frame pointers
  // and extrapolated the CFA using an assumed constant native frame size,
  // but this assumption was unsafe on both aarch64 and x86-64.
