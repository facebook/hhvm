...
(** It can be hard to be precise about what each filtering function does. This function
there is explained in the role it plays. Once the file watcher has given us its raw
updates, then we must put them through this function.
Note that Watchman and Edenfs_watcher only return files satisfying
FilesToIgnore.server_watch_spec, while dfind may return arbitrary additional files. That's
fine, since we will be filtering them away here.
...
that came out of the file watcher. They might be symlinks, e.g. if we watch root ~/dir,
...
val post_file_watcher_filter_from_fully_qualified_raw_updates :
  root:Path.t -> raw_updates:S_set.t -> Relative_path.Set.t
