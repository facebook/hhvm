...
let rewrite_function_decl_header header_node =
  ...
  let function_type =
    make_coroutine_result_type_syntax header_node.function_type
  in
  let function_parameter_list =
    compute_parameter_list
      header_node.function_parameter_list
...
let make_state_machine_method_reference_syntax
    classish_name header_node method_node =
  let method_name =
    make_string_literal_syntax (make_state_machine_method_name header_node)
  in
...
let create_closure_invocation
  ...
  let parameters =
    continuation_variable_syntax :: lambda :: (outer_args @ arg_list)
  in
  (* new Closure($continuation, ...) *)
  let new_closure_syntax =
    make_object_creation_expression_syntax classname parameters
  in
  (* (new Closure($continuation, ...)) *)
  let new_closure_syntax =
    make_parenthesized_expression_syntax new_closure_syntax
  in
  ...
  let select_do_resume_member_syntax =
    make_member_selection_expression_syntax
      new_closure_syntax
      do_resume_with_reentry_guard_member_name_syntax
  in
...
let rewrite_coroutine_body
    context function_parameter_list function_type rewritten_body =
  if
    CoroutineSuspendRewriter.only_tail_call_suspends
      context.Coroutine_context.original_node
  then
    rewritten_body
  else
...
let rewrite_methodish_declaration
  ...
  let methodish_declaration_node =
    get_methodish_declaration (syntax method_node)
  in
  Syntax.synthesize_from
    method_node
    (from_methodish_declaration
       {
         methodish_declaration_node with
...
let rewrite_anon context anon_node =
  ...
  let anonymous_body =
    rewrite_coroutine_body
      ...
      anonymous_body
  in
  let anonymous_parameters =
    compute_parameter_list anonymous_parameters anonymous_type
  in
...
let rewrite_lambda
  ...
  let ({ lambda_body; _ } as lambda) =
    get_lambda_expression (syntax lambda_node)
  in
  let make_sig node = make_syntax (from_lambda_signature node) in
  let lambda_body =
    rewrite_coroutine_body context lambda_parameters lambda_type lambda_body
  in
  let lambda_parameters =
    compute_parameter_list lambda_parameters lambda_type
  in
  let lambda_type = make_coroutine_result_type_syntax lambda_type in
  let lambda_signature =
    make_sig { lambda_signature with lambda_parameters; lambda_type }
  in
  let lambda =
    {
      lambda with
