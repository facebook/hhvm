...
class t_mstch_python_prototypes_generator : public t_whisker_generator {
 ...
 protected:
  ...
  prototype<t_interface>::ptr make_prototype_for_interface(
    ...
    // class body is empty and needs a `pass`.
    def.property("has_interface_methods?", [](const t_interface& self) {
      return std::any_of(
          self.functions().begin(),
          self.functions().end(),
          [](const t_function& func) {
            return !func.is_interaction_constructor() && !func.sink() &&
  ...
  prototype<t_structured>::ptr make_prototype_for_structured(
    ...
    def.property(
        "enable_unsafe_isset_inspection?", [this](const t_structured& self) {
          return has_compiler_option("enable_locally_set_field_inspection") ||
