<?php
	...
	$tmp    = NULL;
	$link   = NULL;
	$test_table_name = 'test_mysqli_fetch_all_table_1'; require('table.inc');
	if (!$res = mysqli_query($link, "SELECT * FROM test_mysqli_fetch_all_table_1 ORDER BY id LIMIT 2")) {
	...
	function func_mysqli_fetch_all($link, $engine, $sql_type, $sql_value, $php_value, $offset, $regexp_comparison = NULL) {
		...
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_fetch_all_table_1")) {
				printf("[%04d] [%d] %s\n", $offset, mysqli_errno($link), mysqli_error($link));
				return false;
		}

		if (!mysqli_query($link, $sql = sprintf("CREATE TABLE test_mysqli_fetch_all_table_1(id INT NOT NULL, label %s, PRIMARY KEY(id)) ENGINE = %s", $sql_type, $engine))) {
				// don't bail, engine might not support the datatype
				return false;
		}

		if (is_null($php_value)) {
			if (!mysqli_query($link, $sql = sprintf("INSERT INTO test_mysqli_fetch_all_table_1(id, label) VALUES (1, NULL)"))) {
				printf("[%04d] [%d] %s\n", $offset + 1, mysqli_errno($link), mysqli_error($link));
				return false;
			}
		} else {
			if (is_string($sql_value)) {
				if (!mysqli_query($link, $sql = "INSERT INTO test_mysqli_fetch_all_table_1(id, label) VALUES (1, '" . $sql_value . "')")) {
					printf("[%04ds] [%d] %s - %s\n", $offset + 1, mysqli_errno($link), mysqli_error($link), $sql);
					return false;
				}
			} else {
				if (!mysqli_query($link, $sql = sprintf("INSERT INTO test_mysqli_fetch_all_table_1(id, label) VALUES (1, '%d')", $sql_value))) {
					printf("[%04di] [%d] %s\n", $offset + 1, mysqli_errno($link), mysqli_error($link));
					return false;
				}
			}
		...
		if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_fetch_all_table_1")) {
...
?>
<?php
	// $test_table_name = 'test_mysqli_fetch_all_table_1'; require_once("clean_table.inc");
?>