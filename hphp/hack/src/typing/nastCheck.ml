...
module CheckFunctionBody = struct
  ...
  and expr_ p f_type env exp = match f_type, exp with
    ...
    | _, Import _
    | _, Omitted
    | _, BracedExpr _ -> failwith "AST should not contain these nodes after naming"
    | _, ParenthesizedExpr e -> expr f_type env e
    | _, Any -> ()
    | _, Class_const _
    | _, Fun_id _
    | _, Method_id _
...
and expr_ env _p = function
  ...
  | Import _
  | Omitted
  | BracedExpr _ -> failwith "AST should not contain these nodes after naming"
  | ParenthesizedExpr e -> expr env e
  | Any
  | Fun_id _
  | Method_id _
  | Smethod_id _
