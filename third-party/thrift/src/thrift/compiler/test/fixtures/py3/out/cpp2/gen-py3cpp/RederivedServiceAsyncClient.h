...
class Client<::py3::simple::RederivedService> : public ::py3::simple::DerivedServiceAsyncClient {
 ...
 private:
  ...
  folly::coro::Task<::std::int32_t> co_get_seven(apache::thrift::RpcOptions* rpcOptions) {
    ...
    if constexpr (hasRpcOptions) {
      fbthrift_serialize_and_send_get_seven(*rpcOptions, std::move(header), ctx.get(), std::move(wrappedCallback));
    } else {
      fbthrift_serialize_and_send_get_seven(*defaultRpcOptions, std::move(header), ctx.get(), std::move(wrappedCallback));
    }
    if (cancellable) {
 ...
 private:
  ...
  FOLLY_NOINLINE folly::SemiFuture<::std::int32_t> fbthrift_semifuture_get_seven(folly::SemiFuture<typename apache::thrift::detail::FutureCallbackHelper<::std::int32_t>::PromiseResult> semifuture, apache::thrift::RpcOptions& rpcOptions, apache::thrift::RequestClientCallback::Ptr callback, apache::thrift::ContextStack* contextStack, std::shared_ptr<::apache::thrift::transport::THeader> header);
 public:
};
