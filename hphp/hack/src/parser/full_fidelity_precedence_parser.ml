...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  ...
  module WithLexer (Lexer : Lexer_S) = struct
    ...
    struct
      ...
      let allow_as_expressions parser = parser.allow_as_expressions

      let with_as_expressions parser ~enabled f =
        let old_enabled = allow_as_expressions parser in
        let parser =
          if old_enabled <> enabled then
            { parser with allow_as_expressions = enabled }
          else
            parser
        in
        let (parser, r) = f parser in
        let parser =
          if old_enabled <> allow_as_expressions parser then
            { parser with allow_as_expressions = old_enabled }
          else
            parser
        in
        (parser, r)
      ...
      let with_precedence parser precedence = { parser with precedence }
      ...
      let with_operator_precedence parser operator parse_function =
        let new_precedence =
          Full_fidelity_operator.precedence parser.env operator
        in
      ...
      let next_xhp_element_token ?(no_trailing = false) parser =
        let (lexer, token, text) =
          Lexer.next_xhp_element_token ~no_trailing parser.lexer
        in
