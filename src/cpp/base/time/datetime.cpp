...
const char *DateTime::DateFormatRFC822n  = "D, d M y H:i:s T";
...
void DateTime::set(int hou, int min, int sec, int mon, int day, int yea) {
  ...
  if (day >= 0) m_time->d = day;
  if (mon >= 0) m_time->m = mon;
  if (sec >= 0) m_time->s = sec;
  if (min >= 0) m_time->i = min;
  if (hou >= 0) m_time->h = hou;
  update();
}
...
int64 DateTime::toInteger(char format) const {
  ...
  throw InvalidArgumentException("unknown format char", (int)format);
}

String DateTime::toString(CStrRef format, bool stdc /* = false */) const {
  if (format.empty()) {
    throw ArgumentMissingException("format");
  }
...
String DateTime::toString(DateFormat format) const {
  ...
  case RFC822n: return rfcFormat(DateFormatRFC822n);
...
String DateTime::stdcFormat(CStrRef format) const {
  ...
  free(buf);
  throw InvalidArgumentException("format", "(over internal buffer)");
}
...
Variant DateTime::getSunInfo(SunInfoFormat retformat,
  ...
  if (retformat != ReturnTimeStamp &&
    ...
    Logger::Warning("Wrong return format given, pick one of "
