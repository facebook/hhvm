  ...
  let param_instrs = gather @@
    List.map params ~f:(fun p ->
      ...
      then
        ...
        instr_pushl (Local.Named (Hhas_param.name p))) in
  let inout_setters =
    List.filter_map params ~f:(fun p ->
      if not @@ Hhas_param.is_inout p
      then None
      else Some (instr_popl @@ Local.Named (Hhas_param.name p))) in
...
let emit_body_instrs_ref params call_instrs =
  ...
  let param_instrs = gather @@
    List.map params ~f:(fun p ->
      ...
      then instr_vgetl local
      else instr_pushl local) in
  let param_get_instrs =
    List.filter_map params ~f:(fun p ->
      if Hhas_param.is_reference p
      then Some (instr_cgetl (Local.Named (Hhas_param.name p)))
      else None) in
...
let emit_body_instrs ~wrapper_type env pos params call_instrs =
  let body =
    ...
    | Emit_inout_helpers.RefWrapper ->
      emit_body_instrs_ref params call_instrs in
...
let emit_wrapper_function
  ...
  let body_instrs =
    emit_body_instrs ~wrapper_type env ast_fun.T.f_span params call_instrs in
  let doc = ast_fun.T.f_doc_comment in
  let body =
    make_wrapper_body env doc decl_vars return_type_info modified_params body_instrs in
...
let emit_wrapper_method
  ...
  let params = Emit_param.from_asts
    ~namespace ~tparams ~generate_defaults:true
  ...
  let wrapper_type, original_id, renamed_id, params =
    ...
    then
      ...
      Emit_inout_helpers.InoutWrapper, original_id, renamed_id, params in
  ...
  let body_instrs =
    emit_body_instrs
      ~wrapper_type env ast_method.T.m_span params call_instrs in
  let params =
    if wrapper_type = Emit_inout_helpers.InoutWrapper
    then
      List.map ~f:Hhas_param.switch_inout_to_reference params
    else
      List.map ~f:Hhas_param.switch_reference_to_inout params in
