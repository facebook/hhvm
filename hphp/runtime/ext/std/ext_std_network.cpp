...
bool HHVM_FUNCTION(checkdnsrr, const String& host,
  ...
  status = DnsQuery_A(host.c_str(), ntype, DNS_QUERY_STANDARD, NULL, &pResult, NULL);
...
Variant HHVM_FUNCTION(dns_get_record, const String& hostname, int type /*= -1*/,
  ...
  type2 < (!addtlRef.isNull() ? (PHP_DNS_NUM_TYPES + 2) : PHP_DNS_NUM_TYPES) || first_query;
    ...
    if (type_to_fetch) {
      status = DnsQuery_A(hostname.c_str(), type_to_fetch, DNS_QUERY_STANDARD, NULL, &pResult, NULL);
...
bool HHVM_FUNCTION(getmxrr, const String& hostname,
  ...
  status = DnsQuery_A(hostname.c_str(), DNS_TYPE_MX, DNS_QUERY_STANDARD, NULL, &pResult, NULL);
