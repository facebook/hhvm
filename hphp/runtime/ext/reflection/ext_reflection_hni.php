...
abstract class ReflectionFunctionAbstract implements Reflector {
  ...
  private static keyset<string> $functions =
    keyset["lambda_expression", "anonymous_function", "methodish_declaration"];

  public static function find_single_function(array $json, int $line): ?array {
    return HH\ffp_json_dfs(
      $json,
      false,
      ($j) ==> {
        if (array_key_exists($j["kind"], self::$functions)) {
          list($ln, $_) = self::findBoundaryToken($j, false);
          if ($ln === $line) {
            return $j;
          }
        }
  ...
  public static function find_all_functions(array $json): dict<int, array> {
    $funs = Map {};
    HH\ffp_json_dfs(
      $json,
      false,
  ...
  public static function body_bounds(array $function): ((int, int), (int, int)) {
    ...
    $left = self::findBoundaryToken($body, false);
    $right = self::findBoundaryToken($body, true);
    invariant($left !== null, "Failed to find left bound of function");
    invariant($right !== null, "Failed to find right bound of function");
    return tuple($left, $right);
  }

  private static function findBoundaryToken(array $body, bool $right): ?(int, int) {
    return HH\ffp_json_dfs(
      $body,
      $right,
      ($json) ==> {
      ...
      // This is necessary, otherwise the line returned by findBoundaryToken and
