...
StructMetadata<::some::ns::IncludedB>::gen(ThriftMetadata& metadata) {
  auto res = genStructMetadata<::some::ns::IncludedB>(metadata, folly::kIsDebug);
  if (res.preExists) {
    return res.metadata;
  }
  ...
  for (const auto& f : *include2_IncludedB_fields) {
    [[maybe_unused]] auto& field = include2_IncludedB.fields()[i];
    ...
    DCHECK(structuredAnnotationsEquality(
      *field.structured_annotations(),
      newAnnotations,
      getFieldAnnotationTypes<::some::ns::IncludedB>(i, static_cast<std::int16_t>(f.id))
    ));
