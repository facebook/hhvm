...
try {
    ...
    $dbh->query( "use $db" );
    $dbh->query( "DROP TABLE IF EXISTS typed_results_false" );
    $dbh->query( "CREATE TABLE typed_results_false (
                    intField   INT(6) UNSIGNED NOT NULL DEFAULT 0,
                    floatField FLOAT NOT NULL DEFAULT 0,
                    strField   VARCHAR(255) NOT NULL DEFAULT ''
                 ) ENGINE=InnoDB" );

    $dbh->query( "INSERT INTO typed_results_false VALUES (
                    123, 1234.5674323, '123214qwe')" );

    $stmt = $dbh->query(
        'SELECT *, floatField / intField AS calc FROM typed_results_false',
        PDO::FETCH_ASSOC
    );
    ...
    $dbh->query( "DROP TABLE typed_results_false" );
}
