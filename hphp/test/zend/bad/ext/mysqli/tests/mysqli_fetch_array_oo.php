<?php
	...
	$test_table_name = 'test_mysqli_fetch_array_oo_table_1'; require('table.inc');
	if (!$mysqli = new my_mysqli($host, $user, $passwd, $db, $port, $socket))
		printf("[001] Cannot connect to the server using host=%s, user=%s, passwd=***, dbname=%s, port=%s, socket=%s\n",
			$host, $user, $db, $port, $socket);

	if (!$res = $mysqli->query("SELECT * FROM test_mysqli_fetch_array_oo_table_1 ORDER BY id LIMIT 5")) {
	...
	function func_mysqli_fetch_array($mysqli, $engine, $sql_type, $sql_value, $php_value, $offset, $regexp_comparison = NULL) {
		...
		if (!$mysqli->query("DROP TABLE IF EXISTS test_mysqli_fetch_array_oo_table_1")) {
			printf("[%04d] [%d] %s\n", $offset, $mysqli->errno, $mysqli->error);
			return false;
		}

		if (!$mysqli->query($sql = sprintf("CREATE TABLE test_mysqli_fetch_array_oo_table_1(id INT NOT NULL, label %s, PRIMARY KEY(id)) ENGINE = %s", $sql_type, $engine))) {
				// don't bail, engine might not support the datatype
				return false;
		}

		if (is_null($php_value)) {
			if (!$mysqli->query($sql = sprintf("INSERT INTO test_mysqli_fetch_array_oo_table_1(id, label) VALUES (1, NULL)"))) {
				printf("[%04d] [%d] %s\n", $offset + 1, $mysqli->errno, $mysqli->error);
				return false;
			}
		} else {
			if (is_string($sql_value)) {
				if (!$mysqli->query($sql = "INSERT INTO test_mysqli_fetch_array_oo_table_1(id, label) VALUES (1, '" . $sql_value . "')")) {
					printf("[%04ds] [%d] %s - %s\n", $offset + 1, $mysqli->errno, $mysqli->error, $sql);
					return false;
				}
			} else {
				if (!$mysqli->query($sql = sprintf("INSERT INTO test_mysqli_fetch_array_oo_table_1(id, label) VALUES (1, '%d')", $sql_value))) {
					printf("[%04di] [%d] %s\n", $offset + 1, $mysqli->errno, $mysqli->error);
					return false;
				}
			}
		...
		if (!$res = $mysqli->query("SELECT id, label FROM test_mysqli_fetch_array_oo_table_1")) {
...
?>
<?php
	$test_table_name = 'test_mysqli_fetch_array_oo_table_1'; require_once("clean_table.inc");
?>