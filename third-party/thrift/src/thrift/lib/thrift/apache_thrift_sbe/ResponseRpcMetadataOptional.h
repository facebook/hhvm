...
class ResponseRpcMetadataOptional {
 ...
 public:
  ...
  ResponseRpcMetadataOptional& wrapForEncode(
      char* buffer,
      const std::uint64_t offset,
      const std::uint64_t bufferLength) {
    m_buffer = buffer;
  ...
  ResponseRpcMetadataOptional& wrapAndApplyHeader(
      char* buffer,
      const std::uint64_t offset,
      const std::uint64_t bufferLength) {
  ...
  SBE_NODISCARD std::uint64_t decodeLength() const {
    ResponseRpcMetadataOptional skipper(
        m_buffer,
        m_offset,
        m_bufferLength,
        sbeBlockLength(),
  ...
  SBE_NODISCARD static const char* proxiedPayloadMetadataMetaAttribute(
  ...
  proxiedPayloadMetadataSinceVersion() SBE_NOEXCEPT {
    return 0;
  }
  ...
  proxiedPayloadMetadataEncodingOffset() SBE_NOEXCEPT {
    return 0;
  }
  ...
  void onProxiedPayloadMetadataAccessed() const {
    if (codecState() == CodecState::NOT_WRAPPED) {
      throw AccessOrderError(
          ...
          "Cannot access field \"proxiedPayloadMetadata\" in state: " +
 ...
 public:
  SBE_NODISCARD static SBE_CONSTEXPR std::size_t
  proxiedPayloadMetadataEncodingLength() SBE_NOEXCEPT {
    return 1;
  }
  ...
  ResponseRpcMetadataOptional& proxiedPayloadMetadata(
      const BooleanType::Value value) {
#if defined(SBE_ENABLE_PRECEDENCE_CHECKS)
    onProxiedPayloadMetadataAccessed();
#endif
  ...
  SBE_NODISCARD static const char* queueMetadataMetaAttribute(
      const MetaAttribute metaAttribute) SBE_NOEXCEPT {
    switch (metaAttribute) {
      case MetaAttribute::PRESENCE:
        return "optional";
  ...
  SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t queueMetadataSinceVersion()
      SBE_NOEXCEPT {
    return 0;
  }
  ...
  SBE_NODISCARD static SBE_CONSTEXPR std::size_t queueMetadataEncodingOffset()
      SBE_NOEXCEPT {
    return 1;
  }
 ...
 private:
  void onQueueMetadataAccessed() const {
    if (codecState() == CodecState::NOT_WRAPPED) {
      throw AccessOrderError(
          std::string("Illegal field access order. ") +
          "Cannot access field \"queueMetadata\" in state: " +
          codecStateName(codecState()) +
          ". Expected one of these transitions: [" +
          codecStateTransitions(codecState()) +
          "]. Please see the diagram in the docs of the enum ResponseRpcMetadataOptional::CodecState.");
 ...
 public:
...
#endif
    m_queueMetadata.wrap(
        m_buffer, m_offset + 1, m_actingVersion, m_bufferLength);
  ...
  static SBE_CONSTEXPR std::uint16_t fdMetadataId() SBE_NOEXCEPT { return 3; }
 ...
 public:
  ...
  SBE_NODISCARD static const char* frameworkMetadataMetaAttribute(
      const MetaAttribute metaAttribute) SBE_NOEXCEPT {
    switch (metaAttribute) {
      case MetaAttribute::PRESENCE:
        return "required";
  ...
  static SBE_CONSTEXPR std::uint64_t frameworkMetadataSinceVersion()
      SBE_NOEXCEPT {
    return 0;
  }

  bool frameworkMetadataInActingVersion() SBE_NOEXCEPT { return true; }

  static SBE_CONSTEXPR std::uint16_t frameworkMetadataId() SBE_NOEXCEPT {
    return 4;
  }

  static SBE_CONSTEXPR std::uint64_t frameworkMetadataHeaderLength()
      SBE_NOEXCEPT {
    return 4;
  }
  ...
  void onFrameworkMetadataLengthAccessed() const {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot decode length of var data \"frameworkMetadata\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum ResponseRpcMetadataOptional::CodecState.");
  ...
  void onFrameworkMetadataAccessed() {
    switch (codecState()) {
      ...
      default:
        throw AccessOrderError(
            std::string("Illegal field access order. ") +
            "Cannot access field \"frameworkMetadata\" in state: " +
            codecStateName(codecState()) +
            ". Expected one of these transitions: [" +
            codecStateTransitions(codecState()) +
            "]. Please see the diagram in the docs of the enum ResponseRpcMetadataOptional::CodecState.");
 ...
 public:
  ...
  ResponseRpcMetadataOptional& putFrameworkMetadata(
  ...
  friend std::basic_ostream<CharT, Traits>& operator<<(
      ...
      const ResponseRpcMetadataOptional& _writer) {
    ...
    builder << '"' << writer.skipFrameworkMetadata() << " bytes of raw data\"";
    builder << '}';

    return builder;
  }

  void skip() { skipFrameworkMetadata(); }
  ...
  SBE_NODISCARD static std::size_t computeLength(
      std::size_t frameworkMetadataLength = 0) {
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    if (frameworkMetadataLength > 1073741824LL) {
      throw std::runtime_error(
          "frameworkMetadataLength too long for length type [E109]");
    }
