...
let record_one_jsonfile
  ...
  (* Open the file *)
  let filename = Printf.sprintf "%s/%s.%s.json"
    path predicate (string_of_int filenum) in

  (* Write all results in json format *)
  let json_export = JSON_Array [
    JSON_Object [
      ("predicate", JSON_String predicate);
...
let record_in_jsonfiles
    (chunk_size: int)
    (path: string)
    (symbols: si_scan_result): string list =
  ...
  let hackSymbols = List.map symbols.sisr_capture ~f:(fun symbol ->
    ...
    let hash = Caml.Hashtbl.find symbols.sisr_filepaths symbol.sif_filepath in
    JSON_Object [
      ("key", JSON_Object [
        ("name_lowercase", JSON_String (String.lowercase symbol.sif_name));
        ("valid", JSON_Object [
          ("acid", JSON_Bool (valid_for_acid symbol));
          ("acnew", JSON_Bool (valid_for_acnew symbol));
          ("actype", JSON_Bool (valid_for_actype symbol));
        ]);
        ("kind_id", JSON_Number (string_of_int (kind_to_int symbol.sif_kind)));
  ...
  let sym_files = List.mapi array_chunk_list ~f:(fun chunk_id chunk ->
        record_one_jsonfile path "hack.symbol.1" chunk_id chunk
  ) in

  (* Namespace schema tables *)
  let hackNamespaces = Caml.Hashtbl.fold (fun ns id acc ->
    incr json_element_id;
    JSON_Object [
      ("key", JSON_Object [
  ...
  ) symbols.sisr_filepaths [] in

  (* Save schema 1 files by chunk size *)
  let array_chunk_list = List.chunks_of hackFilepaths chunk_size in
  let filepath_files = List.mapi array_chunk_list ~f:(fun chunk_id chunk ->
