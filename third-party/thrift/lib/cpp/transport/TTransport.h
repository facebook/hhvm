...
#ifndef HPHP_THRIFT_TRANSPORT_TTRANSPORT_H
#define HPHP_THRIFT_TRANSPORT_TTRANSPORT_H 1
...
class TTransport {
 public:
  ...
  /**
   * Flushes any pending data to be written. Typically used with buffered
   * transport mechanisms.  This also has the benefit of notifying
   * the transport of if the call is oneway, and the transport can
   * make optimizations because of this.
   *
   * @throws TTransportException if an error occurs
   */
  virtual void onewayFlush() {
    // default behaviour is the same as flush
    flush();
  }
...
#endif // #ifndef HPHP_THRIFT_TRANSPORT_TTRANSPORT_H
