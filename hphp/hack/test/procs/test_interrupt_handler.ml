...
let configure_handlers fd1 fd2 (x, y) =
  let handlers = [] in
  let handlers = if x < 3 then (fd1, handler1 fd1)::handlers else handlers in
  let handlers = if y < 4 then (fd2, handler2 fd2)::handlers else handlers in
  handlers
  ...
  let (), (x, y), cancelled =
    MultiWorker.call_with_interrupt workers
      ...
      ~interrupt:{ MultiThreadedCall.
         handlers = configure_handlers interrupt_fd1 interrupt_fd2;
...
let () =
  ...
  try_finalize
    Unit_test.run_all ["test_interrupt_handler", test_interrupt_handler]
    cleanup ()
