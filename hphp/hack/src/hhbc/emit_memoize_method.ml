...
let make_info ast_class class_id ast_methods =
  ...
  let instance_count =
    List.count ast_methods
      (fun ast_method -> is_memoize ast_method &&
...
let make_memoize_instance_method_no_params_code info method_id =
  let renamed_name =
    Hhbc_id.Method.add_suffix method_id memoize_suffix in
  ...
  let ssmc = shared_single_memoize_cache info.memoize_class_prefix in
  let ssmcg = shared_single_memoize_cache_guard info.memoize_class_prefix in
  gather [
    instr_checkthis;
    instr_null;
    instr_ismemotype;
...
let make_memoize_static_method_no_params_code info method_id =
  let label_0 = Label.Regular 0 in
  let label_1 = Label.Regular 1 in
  let label_2 = Label.Regular 2 in
  let label_3 = Label.Regular 3 in
  ...
  gather [
    ...
    instr_maybememotype;
    instr_jmpz label_2;
    instr_string (original_name_lc ^ single_memoize_cache_guard info.memoize_class_prefix);
    get_self info;
    instr_cgets;
    instr_jmpz label_2;
    instr_null;
    ...
    instr_ismemotype;
    instr_jmpnz label_3;
    instr_string (original_name_lc ^ single_memoize_cache info.memoize_class_prefix);
    get_self info;
...
let make_memoize_static_method_with_params_code info method_id params =
  let param_count = List.length params in
  let label = Label.Regular 0 in
  let first_local = Local.Unnamed param_count in
  let original_name_lc = String.lowercase_ascii
...
let make_instance_properties info ast_methods =
  ...
  | [ast_method] when List.is_empty ast_method.Ast.m_params ->
    ...
    let property1 = Hhas_property.make true false false false false true
      (shared_single_memoize_cache_guard info.memoize_class_prefix) false_init None in
    [property1; property2]
  | _ ->
    ...
    if is_memoized_static then
      ...
      if no_params then
        ...
        let property1 = Hhas_property.make true false false true false true
          (Hhbc_id.Prop.add_suffix prop_name (single_memoize_cache_guard info.memoize_class_prefix))
          false_init None in
        [property1; property2]
      else
