...
bool spl_register_class_normalize(CVarRef func, bool get, bool throws = true , bool prepend = false ){
  ...
  if(f_f.find("::"))get ? _set(f_f) : _unset(f_f);
  ...
  Variant x= func.toArray();
	...
	get ? _set(classname+"::"+x[1].toString()) : _unset(classname+"::"+x[1].toString());
    }else{
	get ? _set(x[0].toString()+"::"+x[1].toString()) : _unset(x[0].toString()+"::"+x[1].toString()); 
    }
  get ? res = AutoloadHandler::s_instance->addHandler(func, prepend) : res = true;
  if(res)return res;
  }
