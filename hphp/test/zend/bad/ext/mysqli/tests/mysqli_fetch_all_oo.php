<?php
	...
	$test_table_name = 'test_mysqli_fetch_all_oo_table_1'; require('table.inc');
	...
	function func_mysqli_fetch_all_oo($link, $engine, $sql_type, $sql_value, $php_value, $offset, $regexp_comparison = NULL) {
		...
		if (!$link->query("DROP TABLE IF EXISTS test_mysqli_fetch_all_oo_table_1")) {
			printf("[%04d] [%d] %s\n", $offset, $link->errno, $link->error);
			return false;
		}

		if (!$link->query($sql = sprintf("CREATE TABLE test_mysqli_fetch_all_oo_table_1(id INT NOT NULL, label %s, PRIMARY KEY(id)) ENGINE = %s", $sql_type, $engine))) {
				// don't bail, engine might not support the datatype
				return false;
		}

		if (is_null($php_value)) {
			if (!$link->query($sql = sprintf("INSERT INTO test_mysqli_fetch_all_oo_table_1(id, label) VALUES (1, NULL)"))) {
				printf("[%04d] [%d] %s\n", $offset + 1, $link->errno, $link->error);
				return false;
			}
		} else {
			if (is_string($sql_value)) {
				if (!$link->query($sql = "INSERT INTO test_mysqli_fetch_all_oo_table_1(id, label) VALUES (1, '" . $sql_value . "')")) {
					printf("[%04ds] [%d] %s - %s\n", $offset + 1, $link->errno, $link->error, $sql);
					return false;
				}
			} else {
				if (!$link->query($sql = sprintf("INSERT INTO test_mysqli_fetch_all_oo_table_1(id, label) VALUES (1, '%d')", $sql_value))) {
					printf("[%04di] [%d] %s\n", $offset + 1, $link->errno, $link->error);
					return false;
				}
			}
		...
		if (!$res = $link->query("SELECT id, label FROM test_mysqli_fetch_all_oo_table_1")) {
