...
static Variant php_mcrypt_do_crypt(CStrRef cipher, CStrRef key, CStrRef data,
  ...
  if (td == MCRYPT_FAILED) {
    Logger::Warning(MCRYPT_OPEN_MODULE_FAILED);
    return false;
  }
  ...
  if (key.size() > max_key_length) {
    Logger::Warning("Size of key is too large for this algorithm");
  }
  int count;
  ...
  if (!iv.empty()) {
    if (iv_size != iv.size()) {
      Logger::Warning("The IV parameter must be as long as the blocksize");
    } else {
      iv_s = (char*)malloc(iv_size + 1);
      memcpy(iv_s, iv.data(), iv_size);
    }
  } else if (iv_size != 0) {
    Logger::Warning("Attempt to use an empty IV, which is NOT recommended");
    iv_s = (char*)malloc(iv_size + 1);
  ...
  if (mcrypt_generic_init(td, key_s, use_key_length, iv_s) < 0) {
    Logger::Warning("Mcrypt initialisation failed");
    return false;
  }
  if (dencrypt) {
...
static Variant mcrypt_generic(CObjRef td, CStrRef data, bool dencrypt) {
  ...
  if (!pm->m_init) {
    Logger::Warning("Operation disallowed prior to mcrypt_generic_init().");
    return false;
  }

  if (data.empty()) {
    Logger::Warning("An empty string was passed");
    return false;
  }
...
Variant f_mcrypt_module_open(CStrRef algorithm, CStrRef algorithm_directory,
  ...
  if (td == MCRYPT_FAILED) {
    Logger::Warning("Could not open encryption module");
    return false;
  }
...
Array f_mcrypt_list_algorithms(CStrRef lib_dir /* = null_string */) {
  ...
  if (count == 0) {
    Logger::Warning("No algorithms found in module dir");
  }
...
Array f_mcrypt_list_modes(CStrRef lib_dir /* = null_string */) {
  ...
  if (count == 0) {
    Logger::Warning("No modes found in module dir");
  }
  Array ret = Array::Create();
  for (int i = 0; i < count; i++) {
    ret.append(String(modules[i], CopyString));
...
Variant f_mcrypt_create_iv(int size, int source /* = 0 */) {
  if (size <= 0 || size >= INT_MAX) {
    Logger::Warning("Can not create an IV with a size of less then 1 or "
                    "greater then %d", INT_MAX);
    return false;
  }
  ...
  if (source == RANDOM || source == URANDOM) {
    ...
    if (fd < 0) {
      free(iv);
      Logger::Warning("Cannot open source device");
      return false;
    }
    int read_bytes;
    ...
    if (n < size) {
      free(iv);
      Logger::Warning("Could not gather sufficient random data");
      return false;
    }
  } else {
    n = size;
...
int f_mcrypt_generic_init(CObjRef td, CStrRef key, CStrRef iv) {
  ...
  if (key.size() > max_key_size) {
    Logger::Warning("Key size too large; supplied length: %d, max: %d",
  ...
  if (iv.size() != iv_size) {
    Logger::Warning("Iv size incorrect; supplied length: %d, needed: %d",
                    iv.size(), iv_size);
  }
  memcpy(iv_s, iv.data(), iv_size);
  ...
  if (result < 0) {
    ...
    switch (result) {
    case -3:
      Logger::Warning("Key length incorrect");
      break;
    case -4:
      Logger::Warning("Memory allocation error");
      break;
    case -1:
    default:
...
bool f_mcrypt_generic_deinit(CObjRef td) {
  if (mcrypt_generic_deinit(td.getTyped<MCrypt>()->m_td) < 0) {
    Logger::Warning("Could not terminate encryption specifier");
    return false;
  }
  return true;
}
