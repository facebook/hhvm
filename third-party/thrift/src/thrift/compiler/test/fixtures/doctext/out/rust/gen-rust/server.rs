...
impl<P: ::fbthrift::ProtocolReader> ::fbthrift::Deserialize<P> for self::Args_C_thing {
    ...
    fn read(p: &mut P) -> ::anyhow::Result<Self> {
        ...
        loop {
            ...
            match (fty, fid as ::std::primitive::i32) {
                (::fbthrift::TType::Stop, _) => break,
                (::fbthrift::TType::I32, 1) => field_a = ::std::option::Option::Some(::fbthrift::Deserialize::read(p).context("Error while deserialising a arg of thing")?),
                (::fbthrift::TType::String, 2) => field_b = ::std::option::Option::Some(::fbthrift::Deserialize::read(p).context("Error while deserialising b arg of thing")?),
                (::fbthrift::TType::Set, 3) => field_c = ::std::option::Option::Some(::fbthrift::Deserialize::read(p).context("Error while deserialising c arg of thing")?),
                (fty, _) => p.skip(fty)?,
            }
            p.read_field_end()?;
        }
