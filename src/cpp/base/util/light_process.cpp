...
static void do_popen(FILE *fin, FILE *fout, int afdt_fd) {
  ...
  read_buf(fin, buf);
  FILE *f = ::popen(buf, read_only ? "r" : "w");
  if (f == NULL) {
...
void LightProcess::initialize() {
  if (RuntimeOption::LightProcessFilePrefix.empty() ||
      RuntimeOption::LightProcessCount <= 0) {
    return;
  }
  ...
  g_procs.resize(RuntimeOption::LightProcessCount);

  for (int i = 0; i < RuntimeOption::LightProcessCount; i++) {
    if (!g_procs[i].initShadow(i)) {
      for (int j = 0; j < i; j++) {
...
bool LightProcess::initShadow(int id) {
  Lock lock(m_procMutex);

  ostringstream os;
  os << RuntimeOption::LightProcessFilePrefix << "." << getpid() << "." << id;
...
void LightProcess::change_user(const string &username) {
