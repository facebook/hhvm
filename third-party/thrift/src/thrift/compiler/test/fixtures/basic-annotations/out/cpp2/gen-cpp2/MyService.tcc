...
void MyServiceAsyncProcessor::executeRequest_cppDoNothing(apache::thrift::ServerRequest&& serverRequest) {
  ...
  try {
    auto pargs = args.pargs();
    deserializeRequest<ProtocolIn_>(pargs, "doNothing", apache::thrift::detail::ServerRequestHelper::compressedRequest(std::move(serverRequest)).uncompress(), ctxStack.get());
  }
  catch (...) {
    folly::exception_wrapper ew(std::current_exception());
    apache::thrift::detail::ap::process_handle_exn_deserialization<ProtocolOut_>(
