...
/*static*/ Status OpenSSLKeyUtils::generateECKeyPair(
    folly::ssl::EvpPkeyUniquePtr& ret,
    Error& err,
    int curveNid) {
  FIZZ_RETURN_ON_ERROR(detail::generateECKeyPair(ret, err, curveNid));
...
generateECKeyPair(folly::ssl::EvpPkeyUniquePtr& ret, Error& err, int curveNid) {
  ...
  if (!ecParamKey || !params) {
    return err.error("Error initializing params");
  }
  if (EVP_PKEY_set1_EC_KEY(params.get(), ecParamKey.get()) != 1) {
    return err.error("Error setting ec key for params");
  }
  folly::ssl::EvpPkeyCtxUniquePtr kctx(EVP_PKEY_CTX_new(params.get(), nullptr));
  if (!kctx) {
    return err.error("Error creating kctx");
  }
  if (EVP_PKEY_keygen_init(kctx.get()) != 1) {
    return err.error("Error initializing ctx");
  }
  EVP_PKEY* pkey = nullptr;
  if (EVP_PKEY_keygen(kctx.get(), &pkey) != 1) {
    return err.error("Error generating key");
  }
  folly::ssl::EvpPkeyUniquePtr evpKey(pkey);
  FIZZ_RETURN_ON_ERROR(validateECKey(err, evpKey, curveNid));
  ret = std::move(evpKey);
  return Status::Success;
}
...
Status encodeECPublicKey(
  ...
  FIZZ_RETURN_ON_ERROR(encodeECPublicKey(ret, err, ecKey));
  return Status::Success;
}
...
Status encodeECPublicKey(
  ...
  len = EC_POINT_point2oct(
      ...
      nullptr);
  if (len == 0) {
    return err.error("Failed to encode key");
  }
  buf->append(len);
  ret = std::move(buf);
  return Status::Success;
