<?php
	...
	for ($bits = 1; $bits <= 46; $bits++) {
		...
		if (!mysqli_query($link_ins, "DROP TABLE IF EXISTS test_mysqli_stmt_bind_result_bit_table_1")) {
			printf("[003 - %d] [%d] %s\n", $bits, mysqli_errno($link_ins), mysqli_error($link_ins));
		}

		if (!mysqli_query($link_ins, sprintf("CREATE TABLE test_mysqli_stmt_bind_result_bit_table_1(id BIGINT, bit_value BIT(%d) NOT NULL, bit_null BIT(%d) DEFAULT NULL) ENGINE = %s", $bits, $bits, $engine))) {
		...
		if (!mysqli_stmt_prepare($stmt_ins, "INSERT INTO test_mysqli_stmt_bind_result_bit_table_1(id, bit_value) VALUES (?, ?)")) {
		...
		while ($tests < 10) {
			...
			$sql = sprintf("SELECT id, BIN(bit_value) AS _bin, bit_value, bit_value + 0 AS _bit_value0, bit_null FROM test_mysqli_stmt_bind_result_bit_table_1 WHERE id = %s", $value);
