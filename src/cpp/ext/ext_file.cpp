...
Variant f_fputcsv(CObjRef handle, CArrRef fields, CStrRef delimiter /* = "," */,
                  CStrRef enclosure /* = "\"" */) {
  if (delimiter.size() != 1) {
    throw InvalidArgumentException("delimiter", delimiter.data());
  }
  if (enclosure.size() != 1) {
...
Variant f_parse_ini_file(CStrRef filename,
                         bool process_sections /* = false */) {
  throw NotSupportedException(__func__, "ini file is not supported");
}
...
bool f_file_exists(CStrRef filename) {
  if ((access(File::TranslatePath(filename, true).data(), F_OK)) < 0) {
    return false;
  }
  return true;
}
...
bool f_copy(CStrRef source, CStrRef dest, CObjRef context /* = null_object */) {
  String content = f_file_get_contents(File::TranslatePath(source));
  if (content.isNull()) {
    return false;
  }
...
Variant f_tempnam(CStrRef dir, CStrRef prefix) {
  ...
  String templ = rootDir + "/" + prefix + "XXXXXX";
  if (templ.size() > PATH_MAX) {
    throw InvalidArgumentException("prefix", "(too long)");
  }
...
bool f_mkdir(CStrRef pathname, int64 mode /* = 0 */,
...
static RequestLocal<DirectoryRequestData> s_directory_data;
