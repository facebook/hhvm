...
let column_width line_number = max 3 (Diagnostics.num_digits line_number)
 ...
 *)
let format_error (error : Diagnostics.finalized_diagnostic) : string =
  let { User_diagnostic.severity; _ } = error in
  (* Sort messages such that messages in the same file are together.
     Does not reorder the files or messages within a file. *)
  let msgs =
    User_diagnostic.get_messages error
    |> Diagnostics.combining_sort ~f:(fun msg ->
    ...
    | (msg, is_first) :: msgs ->
      ...
      let (pretty_ctx, pretty_msg) =
        format_message
          ...
          pos
          (User_diagnostic.Severity.tty_color severity)
          ~is_first
          ~col_width
      in
...
let to_string
    ?(claim_color : Tty.raw_color option)
    (error : Diagnostics.finalized_diagnostic) : string =
  let {
  ...
  let color =
    Option.value
      claim_color
      ~default:(User_diagnostic.Severity.tty_color severity)
  in
  Buffer.add_string
    buf
    (Printf.sprintf
  ...
  if not @@ List.is_empty custom_msgs then
    Buffer.add_string
      ...
      (String.concat
         ~sep:"\n"
         (List.map error.User_diagnostic.custom_msgs ~f:(fun msg ->
