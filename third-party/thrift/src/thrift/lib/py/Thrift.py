...
_PY_DEPRECATED_WARNING_MESSAGE = (
    "{module_name} uses thrift-py-deprecated. Migrate to thrift-python. "
...
def _has_user_filter_for_py_deprecated_warning():
    ...
    We check for actions other than `'default'` — `'default'` filters from
    the interpreter (e.g. CPython's startup `'default::DeprecationWarning'`)
    are benign for our purposes, so we still install our own targeted
    `default` filter alongside them. Any other action (`ignore`, `error`,
    `always`, `module`, `once`) on a category that is a superclass of
    `ThriftPyDeprecatedWarning` reflects a deliberate user choice we must
    respect."""
    import warnings
...
def warn_thrift_py_deprecated(name):
    ...
    warnings.warn(
        _PY_DEPRECATED_WARNING_MESSAGE.format(module_name=name),
