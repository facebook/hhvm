...
module WithSyntax (Syntax : Syntax_sig.Syntax_S) = struct
  ...
  type constant?  *)
      ...
      let rec parse_type_specifier
          ?(allow_var = false) ?(allow_attr = true) parser =
     ...
     a simple type specifier here. *)
         ...
         *)
        | Double
        (* TODO: Specification does not mention double; fix it. *)
        ...
        | NullLiteral
        | Name ->
          parse_simple_type_or_type_constant_or_generic parser
        | Namespace ->
        ...
        | Self
        | Parent ->
          parse_simple_type_or_type_constant parser
        | Category
        | XHPClassName ->
          parse_simple_type_or_type_constant_or_generic parser
        ...
        | LessThanLessThan when allow_attr ->
          parse_attributized_specifier parser
        | Classname -> parse_classname_type_specifier parser
        | _ ->
          let parser =
            with_error parser ~on_whole_token:true SyntaxError.error1007
          in
          let (parser1, token) = next_xhp_class_name_or_other_token parser in
          let (parser, token) = Make.token parser1 token in
...
*)
      and parse_remaining_type_constant parser left =
        ...
        if Token.kind right = Name then
          ...
          let (parser, syntax) =
            Make.type_constant parser left separator right
          in
...
*)
  ...
  *)
     ...
     side is not a literal or name. *)
        let (parser, question) =
          if peek_token_kind parser = Question then
            assert_token parser Question
          else
            Make.missing parser (pos parser)
        in
  ...
  *)
        ...
        in
        let (parser, fields) =
          parse_comma_list_opt_allow_trailing_predicate
            parser
            is_closing_token
            SyntaxError.error1025
            parse_field_specifier
        in
        let (parser, ellipsis) =
          if peek_token_kind parser = DotDotDot then
            assert_token parser DotDotDot
          else
            Make.missing parser (pos parser)
        in
