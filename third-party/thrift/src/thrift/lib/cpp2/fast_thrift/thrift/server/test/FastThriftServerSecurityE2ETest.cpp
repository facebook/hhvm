...
/**
 * End-to-end test for FastThriftServer::setSecurityInterface.
 *
 * Boots a real FastThriftServer with a user handler plus a security handler
 * wired into the security slot, then drives both over real
 * RocketClientChannel clients to prove that:
 *  - security methods reach the security handler and user methods do not,
 *  - the security child receives the configured CPU executor like every
 *    other aux child,
 *  - the user handler wins when both claim the same method name, which is
 *    what registering the security child last buys.
 *
 * No security IDL ships with fast_thrift — the slot is generic — so these
 * cases stand in an arbitrary FastServer service for the security handler.
 */
...
using ::apache::thrift::fast_thrift::SecurityServerInterface;
...
// User-side handler. Counts per-method invocations so a security RPC that
...
// Security-side handler. Method names are disjoint from the user service, so
// routing is unambiguous. Also derives from the SecurityServerInterface
// marker that the typed setSecurityInterface requires.
class SecurityHandler : public FastServiceHandler<AuxInterfaceProbeService>,
 ...
 public:
  ...
  void async_tm_probeGreet(
    ...
    cb->result(std::make_unique<std::string>("security:" + *name));
  }
};

// Security handler that serves the *user* service, so every method name
// collides. Used to pin down precedence between the two children.
class ShadowingSecurityHandler
...
// inline on the IO thread, once dispatched to a CPU pool. The security child
// must behave identically either way.
class FastThriftServerSecurityE2ETest
 ...
 protected:
  ...
  // Boot a server with the user handler plus `securityHandler` in the
  // security slot. Separate from SetUp because the precedence case needs a
  // different handler in that slot.
  void startServer(std::shared_ptr<SecurityServerInterface> securityHandler) {
    ...
    server_->setSecurityInterface(std::move(securityHandler));
    server_->start();
  }
...
// A user-only method and a security-only method issued against the same
// server. Each must reach exactly one handler. The thread-placement check
// on the security child proves it was handed the CPU executor rather than
// silently left inline.
TEST_P(FastThriftServerSecurityE2ETest, RoutesUserAndSecurityMethods) {
  auto securityHandler = std::make_shared<SecurityHandler>();
  startServer(securityHandler);
  ASSERT_TRUE(server_->hasSecurityHandler());

  auto userClient = createClient<integration::FastThriftServer>();
  auto secClient = createClient<AuxInterfaceProbeService>();

  userClient->semifuture_ping().get();
  EXPECT_EQ(userHandler_->pingCount.load(), 1);
  EXPECT_EQ(securityHandler->greetCount.load(), 0)
      << "user RPC must not reach the security handler";

  auto greeting = secClient->semifuture_probeGreet("world").get();
  EXPECT_EQ(greeting, "security:world");
  EXPECT_EQ(securityHandler->greetCount.load(), 1);
  EXPECT_EQ(userHandler_->pingCount.load(), 1)
      << "security RPC must not reach the user handler";
  EXPECT_EQ(securityHandler->greetRanOnEventBase.load(), !usingCPUPool());

  destroyClientOnEvb(userClient);
  destroyClientOnEvb(secClient);
}

// The security child is registered last, so on a method-name collision the
// user handler serves the request and the security handler is unreachable.
// Flipping the registration order would silently let a security handler
// shadow the user service.
TEST_P(FastThriftServerSecurityE2ETest, UserHandlerWinsMethodNameConflict) {
  auto securityHandler = std::make_shared<ShadowingSecurityHandler>();
  startServer(securityHandler);
  ...
  userClient->semifuture_ping().get();
  EXPECT_EQ(userHandler_->pingCount.load(), 1);
  EXPECT_EQ(securityHandler->pingCount.load(), 0)
      << "security handler must not shadow a user method";
...
INSTANTIATE_TEST_SUITE_P(
    Inline,
    FastThriftServerSecurityE2ETest,
