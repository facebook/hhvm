...
let go_streaming_on_fd
  ...
  let Client_env.{ max_errors; error_format; output_jsonl; _ } = args in
...
let go_streaming
    ...
    ~(connect_then_close : unit -> unit Lwt.t) :
    (Exit_status.t * Telemetry.t) Lwt.t =
  let Client_env.{ root; show_spinner; deadline; _ } = args in
  let progress_callback : string option -> unit =
    Client_spinner.report ~to_stderr:show_spinner ~angery_reaccs_only:false
  in
