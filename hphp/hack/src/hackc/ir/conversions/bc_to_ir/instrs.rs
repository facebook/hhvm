...
pub(crate) fn convert_sequence(ctx: &mut Context<'_>, addr: Addr) {
    ...
    for idx in Addr::range_to_iter(seq.range) {
        ...
        match instr {
            ...
            Instruct::Pseudo(Pseudo::Break | Pseudo::Continue) => {
...
fn convert_call(ctx: &mut Context<'_>, call: &Opcode) {
    ...
    let call = instr::Call {
        ...
        named_arg_names: fcall_args.named_arg_names.to_vec(),
        readonly,
    };
...
fn convert_iterator(ctx: &mut Context<'_>, opcode: &Opcode) {
    match *opcode {
        Opcode::IterInit(ref args, ref base_id, label) => {
            let hhbc::IterArgs { iter_id, flags } = *args;
            ...
            let args =
                instr::IteratorArgs::new(iter_id, flags, base_lid, done_bid, next_bid, ctx.loc);
