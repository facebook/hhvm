...
# (text, category) dedup key under Python's `default` filter action.
...
def _has_user_filter_for_py_deprecated_warning():
    """True if the user has installed a `warnings` filter that would
    meaningfully silence or escalate `ThriftPyDeprecatedWarning` (or any of
    its superclasses, e.g. `DeprecationWarning`, `Warning`).

    Only exact-category filters on `ThriftPyDeprecatedWarning` count as
    user choices. Superclass filters (e.g. CPython's startup
    `ignore::DeprecationWarning`) are not user choices — they are
    interpreter defaults that would silently suppress this warning.

    User-installed filters that target a superclass (e.g.
    `ignore::DeprecationWarning` added after startup) still take
    precedence because `warnings.filterwarnings` prepends by default.
    A user filter installed after the helper's `default` filter sits
    above it in the filter list and wins via first-match-wins."""
    import warnings

    for action, _msg, category, _mod, _lineno in warnings.filters:
        if action in ("ignore", "error", "always", "module", "once"):
            if category is ThriftPyDeprecatedWarning:
                return True
    return False


def _has_default_filter_for_py_deprecated_warning():
    """True if a targeted `default` filter for
    `ThriftPyDeprecatedWarning` is already present."""
    import warnings

    for action, _msg, category, _mod, _lineno in warnings.filters:
        if action == "default" and category is ThriftPyDeprecatedWarning:
            return True
    return False


def _ensure_default_filter_for_py_deprecated_warning():
    """Install the targeted `default` filter at most once per
    ...
    if _has_user_filter_for_py_deprecated_warning():
        return
    if _has_default_filter_for_py_deprecated_warning():
        return
    ...
    with _py_deprecated_warning_filter_lock:
        ...
        if _has_default_filter_for_py_deprecated_warning():
            return
        warnings.filterwarnings("default", category=ThriftPyDeprecatedWarning)
...
def warn_thrift_py_deprecated(name):
    ...
    # Install a `default` filter for our subclass so the first warning
    # is visible even when DeprecationWarning is globally suppressed
    # (Python's default outside `__main__`). Skip if the user already
    # installed a filter that would silence or escalate our warning
    # (directly or via a superclass like `DeprecationWarning` /
    # `Warning`) — their choice wins.
    _ensure_default_filter_for_py_deprecated_warning()

    frame = sys._getframe(1)
    while frame is not None:
        module_name = frame.f_globals.get("__name__", "")
        is_generated = "warn_thrift_py_deprecated" in frame.f_code.co_names
        is_importlib = module_name == "importlib" or module_name.startswith(
            "importlib."
        )
        if not is_generated and not is_importlib:
            break
        frame = frame.f_back
    if frame is None:
        return
