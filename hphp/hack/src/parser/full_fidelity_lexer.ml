...
module Lexer : sig
  ...
  type t = {
    ...
    start: int;
    (* set once when creating the lexer. *)
    offset: int;
    (* the thing that is incremented when we advance the lexer *)
  ...
  type t = {
    ...
    errors: SyntaxError.t list;
    is_experimental_mode: bool;
        (* write-once: record updates should not update this field *)
  }
  [@@deriving show]
  ...
  let with_offset lexer offset = { lexer with offset }

  let with_start_offset lexer start offset = { lexer with start; offset }

  let make_at ?is_experimental_mode text start_offset =
    with_start_offset
      (make ?is_experimental_mode text)
      start_offset
      start_offset

  let with_offset_errors lexer offset errors = { lexer with offset; errors }

  let start_new_lexeme lexer = { lexer with start = lexer.offset }

  let advance lexer index = { lexer with offset = lexer.offset + index }
...
module WithToken (Token : Lexable_token_sig.LexableToken_S) = struct
  ...
  (* following line warning 32 unused variable "show_lexer" *)
  ...
  let match_string lexer s = s = peek_string lexer (String.length s)

  let make_error_with_location (l : lexer) (msg : string) =
    SyntaxError.make (start l) (offset l) msg

  let width lexer = offset lexer - start lexer

  let current_text lexer =
    SourceText.sub (source lexer) (start lexer) (width lexer)

  let at_end lexer = offset lexer >= SourceText.length (source lexer)
  ...
  let text_len (l : lexer) = SourceText.length (source l)

  let peek (l : lexer) i = SourceText.get (source l) i
  ...
  let skip_whitespace (l : lexer) = skip_while l is_whitespace_no_newline
  ...
  let skip_to_end_of_line (l : lexer) = skip_while l not_newline
  ...
  let skip_to_end_of_line_or_end_tag (l : lexer) =
    ...
    while not (should_stop !i) do
      incr i
    done;
    with_offset l !i

  let skip_name_end (l : lexer) = skip_while l is_name_letter
  ...
  let scan_decimal_digits (l : lexer) = skip_while l is_decimal_digit

  let scan_decimal_digits_with_underscores lexer =
    scan_with_underscores lexer is_decimal_digit

  let scan_octal_digits (l : lexer) = skip_while l is_octal_digit
  ...
  let scan_hexadecimal_digits (l : lexer) = skip_while l is_hexadecimal_digit
  ...
  let scan_hex_literal lexer =
    ...
    if not (is_hexadecimal_digit ch) then
      ...
      ( scan_hexadecimal_digits_with_underscores lexer,
  ...
  let scan_octal_or_float lexer =
    ...
    | '0' .. '9' ->
      ...
      if width lexer_oct = width lexer_dec then
        ...
        if ch = 'e' || ch = 'E' then
          scan_exponent lexer_oct
        else if ch = '.' then
          scan_after_decimal_point lexer_oct
        else
        ...
        make sure we're not dealing with floats. *)
          let lexer_oct_with_underscores =
            scan_octal_digits_with_underscores lexer
          in
        ...
        if ch = 'e' || ch = 'E' then
          ...
          let lexer_dec_with_underscores =
            scan_decimal_digits_with_underscores lexer
          in
  ...
  *)
  ...
  let skip_uninteresting_double_quote_like_string_characters
      (l : lexer) start_char =
    let is_uninteresting ch =
      ...
      | '0' .. '9' ->
        false
      | ch -> ch <> start_char && not (is_name_nondigit ch)
    in
  ...
  let scan_double_quote_like_string_literal_from_start lexer start_char =
    ...
    let rec aux lexer =
      ...
      let lexer =
        skip_uninteresting_double_quote_like_string_characters lexer start_char
      in
      match peek_char lexer 0 with
      | '\000' ->
  ...
  *)
  ...
  let get_string_literal_body_or_double_quoted_tail literal_kind =
    if literal_kind = Literal_double_quoted then
  ...
  let scan_string_literal_in_progress lexer literal_kind =
    ...
    if is_name_nondigit ch0 then
      ...
      | '"' ->
        let kind =
          get_string_literal_body_or_double_quoted_tail literal_kind
        in
        (advance lexer 1, kind)
      | '$' ->
        if is_name_nondigit (peek_char lexer 1) then
          scan_variable lexer
        else
          (advance lexer 1, TokenKind.Dollar)
      ...
      | '0' .. '9' ->
        ...
        if errors lexer == errors lexer1 then
          literal
        else
...
*)
  ...
  let scan_docstring_header lexer =
    ...
    let lexer =
      if is_newline ch then
        ...
        with_error lexer SyntaxError.error0011
    in
  ...
  a : or - not followed by a label, we need to terminate the token. *)
    ...
    if (((not attribute) && ch0 = ':') || ch0 = '-') && is_name_nondigit ch1
    then
  ...
  *)
   ...
   fun in_type lexer ->
    ...
    | '.' ->
      begin
        ...
        | '0' .. '9' -> scan_after_decimal_point lexer
        | '.' ->
          if peek_char lexer 2 = '.' then
    ...
    | '/' ->
      if peek_char lexer 1 = '=' then
        (advance lexer 2, TokenKind.SlashEqual)
      else
        (advance lexer 1, TokenKind.Slash)
    | '%' ->
      if peek_char lexer 1 = '=' then
        (advance lexer 2, TokenKind.PercentEqual)
      else
        (advance lexer 1, TokenKind.Percent)
    | '<' ->
      begin
        match (peek_char lexer 1, peek_char lexer 2) with
    ...
    | '^' ->
      if peek_char lexer 1 = '=' then
        (advance lexer 2, TokenKind.CaratEqual)
      else
        (advance lexer 1, TokenKind.Carat)
    | '|' ->
      begin
        match peek_char lexer 1 with
   ...
   *)
    | ':' ->
      let ch1 = peek_char lexer 1 in
      if ch1 = ':' then
        (advance lexer 2, TokenKind.ColonColon)
      else if Lexer.is_experimental_mode lexer && ch1 = '@' then
        (advance lexer 2, TokenKind.ColonAt)
      else
        (advance lexer 1, TokenKind.Colon)
    ...
    | '1' .. '9' -> scan_decimal_or_float lexer
  ...
  let scan_hash_comment lexer =
    let lexer = skip_to_end_of_line lexer in
    let c =
      Trivia.make_single_line_comment
        (source lexer)
        (start lexer)
        (width lexer)
    in
  ...
  *)
    ...
    let c =
      if remainder >= 11 && peek_string lexer_ws 11 = "FALLTHROUGH" then
        ...
        Trivia.make_single_line_comment (source lexer) (start lexer) w
    in
    (lexer, c)
  ...
  let scan_delimited_comment lexer =
    ...
    let c =
      if match_string lexer_ws "HH_FIXME" then
        ...
        Trivia.make_delimited_comment (source lexer) (start lexer) w
    in
    (lexer, c)
...
*)
    ...
    | '\t' ->
      let new_end =
        str_skip_whitespace ~str:(source_text_string lexer) ~i:(offset lexer)
      in
      let new_start = offset lexer in
      let new_trivia =
        Trivia.make_whitespace (source lexer) new_start (new_end - new_start)
      in
...
*)
  ...
  let scan_leading_trivia scanner lexer =
    let rec aux lexer acc =
      ...
      match trivia with
      | None -> (lexer, acc)
      | Some t -> aux lexer (t :: acc)
    in
    let (lexer, trivia_list) = aux lexer [] in
    (lexer, List.rev trivia_list)

  let scan_leading_php_trivia lexer = scan_leading_trivia scan_php_trivia lexer

  let scan_leading_xhp_trivia lexer = scan_leading_trivia scan_xhp_trivia lexer
  ...
  let as_keyword ~only_reserved kind lexer =
    if kind = TokenKind.Name then
      ...
      | Some keyword ->
        let lexer =
          enforce_lowercase lexer ~original_text ~lowered_text:text
        in
        (lexer, keyword)
      | _ -> (lexer, TokenKind.Name)
    else
      (lexer, kind)
  ...
  let scan_token_and_leading_trivia scanner ~(as_name : kw_set) lexer =
    ...
    let (lexer, kind) =
      ...
      | `NoKeywords -> as_keyword ~only_reserved:false kind lexer
    in
    let w = width lexer in
  ...
  let scan_token_and_trivia scanner ~(as_name : kw_set) lexer =
    ...
    let (lexer, kind, w, leading) =
      scan_token_and_leading_trivia scanner ~as_name lexer
    in
    let (lexer, trailing) =
      match kind with
      ...
      | TokenKind.QuestionGreaterThan ->
        if is_newline (peek_char lexer 0) then
          ...
          (lexer, [])
      | _ -> scan_trailing_php_trivia lexer
    in
  ...
  let scan_assert_progress tokenizer lexer =
    ...
    if
      new_remaining < original_remaining
    ...
    then
      ...
      Printf.kprintf failwith "failed to make progress at %d\n" (offset lexer)
  ...
  let scan_next_token_as_name = scan_next_token ~as_name:`AllKeywords

  let scan_next_token_as_keyword = scan_next_token ~as_name:`NoKeywords
...
design of the parser. We get a big win by memoizing it. *)
  ...
  let next_token =
    (* takes a lexer, returns a (lexer, token) *)
    let next_token_cache =
      Little_cache.make
        empty
        (empty, Token.make TokenKind.EndOfFile SourceText.empty 0 0 [] [])
    in
    Little_cache.memoize
      next_token_cache
  ...
  let next_token_no_trailing lexer =
    let tokenizer lexer =
      let token_start = offset lexer in
      let (lexer, kind, w, leading) =
        scan_token_and_leading_trivia
          scan_token_outside_type
          ~as_name:`NoKeywords
          lexer
      in
  ...
  let next_token_in_string lexer literal_kind =
    ...
    let (lexer, trailing) =
      ...
      | TokenKind.HeredocStringLiteralTail ->
        scan_trailing_php_trivia lexer
      | _ -> (lexer, [])
    in
  ...
  let next_xhp_element_token ~no_trailing lexer =
    ...
    let token_text =
      SourceText.sub (source lexer) token_start_offset token_width
    in
  ...
  let next_xhp_body_token lexer =
    let scanner lexer =
      ...
      let (lexer, trailing) =
        ...
        if kind = TokenKind.XHPBody then
          scan_trailing_xhp_trivia lexer
        else
          (lexer, [])
      in
      (lexer, Token.make kind (source lexer) token_start w leading trailing)
  ...
  let skip_to_end_of_markup lexer =
    let make_markup_and_suffix lexer =
      ...
      | ('=', _, _) ->
        ...
        let lexer = advance lexer 1 in
        let equal =
          Token.make TokenKind.Equal (source lexer) name_token_offset 1 [] []
        in
        (lexer, markup_text, Some (less_than_question_token, Some equal))
      | _ -> (lexer, markup_text, Some (less_than_question_token, None))
    in
    let start_offset =
      (* if leading section starts with #! - it should span the entire line *)
      let index = offset lexer in
      if index != 0 then
        failwith "Should only try to lex header at start of document";
      if
        peek_def ~def:'\x00' lexer index = '#'
        && peek_def ~def:'\x00' lexer (succ index) = '!'
      then
        succ (skip_while_to_offset lexer not_newline)
      ...
      (* numbers in errors *)
        ...
        skip_while_to_offset lexer (fun c ->
            is_newline c || is_whitespace_no_newline c)
    in
    if
      peek lexer start_offset = '<'
      && peek_def ~def:'\x00' lexer (succ start_offset) = '?'
    then
      make_markup_and_suffix (with_offset lexer start_offset)
    else
