...
module ClassDiff = struct
  ...
  let compare cid class1 class2 =
    ...
    let acc =
      add_inverted_deps acc (fun x -> Dep.Method (cid, x)) methods_diff
    in
    ...
    let acc =
      add_inverted_deps acc (fun x -> Dep.Const (cid, x)) typeconsts_diff
    in
    (acc, is_unchanged)
end
    ...
    let (changed, to_redecl, to_recheck) =
      if is_unchanged then
        ...
        if is_unchanged then
           ...
           *)
          let to_redecl =
            Typing_deps.get_extend_deps trace cid_hash to_redecl
          in
          (changed, to_redecl, to_recheck)
      else
        let to_redecl = Typing_deps.get_extend_deps trace cid_hash to_redecl in
