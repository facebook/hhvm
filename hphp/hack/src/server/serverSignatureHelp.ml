...
let go env (file, line, char) =
  ...
  let (lang, mode, tree) = Full_fidelity_ast.parse_text parser_env source_text in
  let results = Full_fidelity_ast.lower_tree parser_env source_text lang mode tree in
  ...
  let siginfo_label =
    Typing_print.full_with_identity typing_env (Reason.Rnone, Tfun ft) occurrence def_opt in
  let params =
    ft.ft_params
    |> List.map ~f:begin fun param ->
      let parinfo_label =
        ...
        | Some s -> s
        | None -> Typing_print.full_strip_ns typing_env ft.ft_ret
      in
