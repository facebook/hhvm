...
external offset_to_file_pos_triple : string -> int -> (int * int * int) = "offset_to_file_pos_triple"
external offset_to_file_pos_triple_with_cursor : string -> int -> int -> (int * int * int) = "offset_to_file_pos_triple_with_cursor"
external offset_to_line_start_offset : string -> int -> int = "offset_to_line_start_offset"
external offset_to_position : string -> int -> (int * int) = "offset_to_position"
external position_to_offset : string -> bool -> int -> int -> (int * bool) = "position_to_offset"
...
let run_1 data =
    ...
    let cmp i =
        ...
        if  a <> e then begin
            Printf.printf "Array: %s : len: %d \n" (Line_break_map.show lbm) (String.length data);
...
let run_5 data =
    ...
    let cmp ((i, j), existing) =
        Line_break_map.reset_global_state();
        let (e1, e2) as e = exception_to_bool (fun() -> Line_break_map.position_to_offset ~existing lbm (i, j)) in
...
let () =
    let result =
      all (
        List.map (fun t -> all (List.map t data)) tests
      )
    in
    if not result then
        failwith "Fail"
