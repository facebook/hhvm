<?php
	...
	if (!mysqli_query($link, 'DROP TABLE IF EXISTS test_mysqli_autocommit_table_1'))
		printf("[012] [%d] %s\n", mysqli_errno($link), mysqli_error($link));

	if (!mysqli_query($link, 'CREATE TABLE test_mysqli_autocommit_table_1(id INT) ENGINE = InnoDB')) {
		printf("[013] Cannot create test table, [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	}

	if (!mysqli_query($link, 'INSERT INTO test_mysqli_autocommit_table_1(id) VALUES (1)'))
		printf("[014] [%d] %s\n", mysqli_errno($link), mysqli_error($link));

	if (!mysqli_query($link, 'ROLLBACK'))
		printf("[015] [%d] %s\n", mysqli_errno($link), mysqli_error($link));

	if (!$res = mysqli_query($link, 'SELECT COUNT(*) AS num FROM test_mysqli_autocommit_table_1'))
		printf("[016] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
...
?>
<?php
	$test_table_name = 'test_mysqli_autocommit_table_1'; require_once("clean_table.inc");
?>