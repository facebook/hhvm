<?php
	...
	*/
	...
	$test_table_name = 'test_mysqli_get_client_stats_table_1'; require('table.inc');
	if (!is_array($info = mysqli_get_client_stats()) || empty($info))
	...
	if (!$res = mysqli_query($link, 'SELECT COUNT(*) AS _num FROM test_mysqli_get_client_stats_table_1', MYSQLI_STORE_RESULT))
		printf("[%03d] SELECT COUNT() FROM test_mysqli_get_client_stats_table_1 failed, [%d] %s\n",
	...
	if (!$row = mysqli_fetch_assoc($res))
		printf("[%03d] fetch_assoc - SELECT COUNT() FROM test_mysqli_get_client_stats_table_1 failed, [%d] %s\n",
			++$test_counter, mysqli_errno($link), mysqli_error($link));
	...
	print "Testing buffered normal... - SELECT id, label FROM test_mysqli_get_client_stats_table_1\n";
	if (!$res = mysqli_query($link, 'SELECT id, label FROM test_mysqli_get_client_stats_table_1', MYSQLI_STORE_RESULT))
		printf("[%03d] SELECT id, label FROM test_mysqli_get_client_stats_table_1 failed, [%d] %s\n",
			++$test_counter, mysqli_errno($link), mysqli_error($link));
	...
	if (!$res = mysqli_query($link, 'SELECT id, label FROM test_mysqli_get_client_stats_table_1', MYSQLI_USE_RESULT))
		printf("[%03d] SELECT id, label FROM test_mysqli_get_client_stats_table_1 failed, [%d] %s\n",
			++$test_counter, mysqli_errno($link), mysqli_error($link));
	...
	print "Testing unbuffered normal... - SELECT id, label FROM test_mysqli_get_client_stats_table_1, not all fetched\n";
	if (!$res = mysqli_query($link, 'SELECT id, label FROM test_mysqli_get_client_stats_table_1', MYSQLI_USE_RESULT))
		printf("[%03d] SELECT id, label FROM test_mysqli_get_client_stats_table_1 failed, [%d] %s\n",
			++$test_counter, mysqli_errno($link), mysqli_error($link));
	...
	if (!mysqli_stmt_prepare($stmt, 'SELECT id, label FROM test_mysqli_get_client_stats_table_1') ||
	...
	*/
	...
	if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_get_client_stats_table_1"))
	...
	*/
	...
	if (!mysqli_query($link, "INSERT INTO test_mysqli_get_client_stats_table_1(id) VALUES (100)"))
	...
	if (!mysqli_query($link, "UPDATE test_mysqli_get_client_stats_table_1 SET label ='z' WHERE id = 100"))
	...
	if (!mysqli_query($link, "REPLACE INTO test_mysqli_get_client_stats_table_1(id, label) VALUES (100, 'b')"))
		printf("[%03d] INSERT failed, [%d] %s\n", ++$test_counter,
			mysqli_errno($link), mysqli_error($link));
	...
	if (!$res = mysqli_query($link, "SELECT id, label FROM test_mysqli_get_client_stats_table_1 WHERE id = 100"))
	...
	if (!mysqli_query($link, "DELETE FROM test_mysqli_get_client_stats_table_1 WHERE id = 100"))
	...
	if (!$res = mysqli_query($link, "TRUNCATE TABLE test_mysqli_get_client_stats_table_1"))
	...
	if ($fp = fopen($file, 'w')) {
		...
		fclose($fp);
		chmod($file, 0644);
		$sql = sprintf('LOAD DATA LOCAL INFILE "%s" INTO TABLE test_mysqli_get_client_stats_table_1', mysqli_real_escape_string($link, $file));
		if (mysqli_query($link, $sql)) {
			if (!is_array($new_info = mysqli_get_client_stats()) || empty($new_info))
				printf("[%03d] Expecting array/any_non_empty, got %s/%s\n",
					++$test_counter, gettype($new_info), $new_info);
	...
	*/
	mysqli_query($link, "DELETE FROM test_mysqli_get_client_stats_table_1");
	if (!mysqli_query($link, "INSERT INTO test_mysqli_get_client_stats_table_1(id, label) VALUES (1, 'a'), (2, 'b')"))
		printf("[%03d] Cannot insert new records, [%d] %s\n", ++$test_counter,
			mysqli_errno($link), mysqli_error($link));

	if (!$res = mysqli_real_query($link, "SELECT id, label FROM test_mysqli_get_client_stats_table_1 ORDER BY id"))
...
if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_get_client_stats_table_1"))
