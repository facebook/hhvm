...
class type ['a] ast_visitor_type =
  object
    ...
    method on_collection :
      'a -> id -> collection_targ option -> afield list -> 'a
...
class virtual ['a] ast_visitor : ['a] ast_visitor_type =
  object (this)
    ...
    method on_stmt acc (_, s) = this#on_stmt_ acc s
    ...
    method on_fun_ acc f =
      let acc =
        List.fold_left this#on_user_attribute acc f.f_user_attributes
      in
    ...
    method on_program acc p =
      let acc = List.fold_left (fun acc d -> this#on_def acc d) acc p in
      acc
    ...
    method on_class_ acc c =
      let acc =
        List.fold_left this#on_user_attribute acc c.c_user_attributes
      in
    ...
    method on_typedef acc t =
      ...
      let acc =
        List.fold_left this#on_user_attribute acc t.t_user_attributes
      in
      acc

    method on_constant acc g = this#on_gconst acc g
    ...
    method on_tparam acc t =
      ...
      let acc =
        List.fold_left this#on_user_attribute acc t.tp_user_attributes
      in
      acc
    ...
    method on_fun_param acc f =
      ...
      let acc =
        match f.param_expr with
        | None -> acc
        | Some expr -> this#on_expr acc expr
      in
      let acc =
        match f.param_hint with
        | Some h -> this#on_hint acc h
        | None -> acc
      in
      let acc =
        match f.param_callconv with
        | Some kind -> this#on_param_kind acc kind
        | None -> acc
      in
      acc
    ...
    method on_typeConst acc t =
      ...
      let acc =
        match t.tconst_constraint with
        | Some h -> this#on_hint acc h
        | None -> acc
      in
      let acc =
    ...
    method on_methodTraitResolution acc mtr =
      ...
      let acc =
        List.fold_left this#on_user_attribute acc mtr.mt_user_attributes
      in
      let acc =
        match mtr.mt_ret with
        | Some h -> this#on_hint acc h
        | None -> acc
      in
      acc
    ...
    method on_classVars acc cv =
      let acc =
        match cv.cv_hint with
        | Some h -> this#on_hint acc h
        | None -> acc
      in
      let acc =
        List.fold_left
          (fun acc (_, id, opt_expr) ->
            let acc = this#on_id acc id in
            match opt_expr with
            | Some expr -> this#on_expr acc expr
            | None -> acc)
    ...
    method on_method_ acc m =
      ...
      let acc =
        List.fold_left this#on_user_attribute acc m.m_user_attributes
      in
      let acc =
        match m.m_ret with
        | Some h -> this#on_hint acc h
    ...
    method on_pu_identifier acc e _ _ = this#on_expr acc e
