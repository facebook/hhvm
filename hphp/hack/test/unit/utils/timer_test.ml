...
let test_adding_in_order () =
  ...
  Asserter.Int_asserter.assert_list_equals
    [1;2;3] !result "Expected ids to be added to list in the timers' firing order";
  true
...
let test_negative_interval () =
  ...
  Asserter.Int_asserter.assert_list_equals
    [-1;0] !result "Expected ids to be added to list in the order the timers are created";
  true
...
let test_cancelling_current_timer () =
  ...
  Asserter.Int_asserter.assert_list_equals
    [2;3] !result "Expected cancelled timer #1 to be missing from list";
  true
...
let test_cancelling_next_timer () =
  ...
  Asserter.Int_asserter.assert_list_equals
    [1;3] !result "Expected cancelled timer #2 to be missing from list";
  true
...
let test_cancelling_finished_timer () =
  ...
  Asserter.Int_asserter.assert_list_equals
    [0] !result "Expected cancelling a finished timer to have no effect";
  true
