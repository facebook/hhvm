...
do { \
  if (((char*)end) - ((char*)(begin)) < (size)) { \
    Logger::Warning("%s/%d: Buffer overrun (%p, %p, %d)", \
...
do { \
  if (((char*)(end)) - ((char*)(begin)) < (size)) { \
    Logger::Warning("%s/%d: Buffer overrun (%p, %p, %d, %d)", \
...
static struct gfxinfo *php_handle_gif(CObjRef stream) {
  ...
  result = (struct gfxinfo *)calloc(1, sizeof(struct gfxinfo));
...
static struct gfxinfo *php_handle_swc(CObjRef stream) {
  ...
  b = (unsigned char *)calloc (1, len + 1);
  ...
  if (uncompress((Bytef*)b, &len, (const Bytef*)a.c_str(), 64) != Z_OK) {
    ...
    if (f_fseek(stream, 8, SEEK_SET)) return NULL;
    ...
    */
    ...
    do {
      ...
      buf = (unsigned char *) realloc(buf,szlength);
...
static struct gfxinfo *php_handle_jpc(CObjRef stream) {
  ...
  if (first_marker_id != JPEG2000_MARKER_SIZ) {
    Logger::Warning("JPEG2000 codestream corrupt(Expected SIZ marker "
                    "not found after SOC)");
    return NULL;
  }
...
static struct gfxinfo *php_handle_jp2(CObjRef stream) {
  ...
  */
  ...
  if (result == NULL) {
    Logger::Warning("JP2 file has no codestreams at root level");
  }
...
static int get_php_tiff_bytes_per_format(int format) {
  ...
  if (format >= size) {
    Logger::Warning("Invalid format %d", format);
    format = 0;
  }
...
static struct gfxinfo *php_handle_wbmp(CObjRef stream) {
  struct gfxinfo *result =
    (struct gfxinfo *)calloc(1, sizeof(struct gfxinfo));
...
static int php_get_xbm(CObjRef stream, struct gfxinfo **result) {
  ...
  while (!(fline=f_fgets(stream, 0)).empty()) {
    iname = (char *)malloc(fline.size() + 1);
...
static int php_getimagetype(CObjRef stream) {
  ...
  if (!memcmp(fileType.c_str(), php_sig_gif, 3)) {
    ...
    if (!memcmp((fileType + data).c_str(), php_sig_png, 8)) {
      ...
      Logger::Warning("PNG file corrupted by ASCII conversion");
      return IMAGE_FILETYPE_UNKNOWN;
    }
...
Variant f_getimagesize(CStrRef filename, Variant imageinfo /* = null */) {
  ...
  if (same(stream, false)) {
    Logger::Warning("failed to open stream: %s", filename.c_str());
    return false;
  }
...
#else
    Logger::Info("The image is a compressed SWF file, but you do not "
  ...
  if (result) {
    ...
    ret.set(2, itype);
    vspprintf(&temp, 0, "width=\"%d\" height=\"%d\"",
             result->width, result->height);
    ret.set(3, String(temp, AttachString));
...
static bool _php_image_output_ctx(CObjRef image, CStrRef filename,
  ...
  if (!filename.empty()) {
    ...
    if (same(stream, false)) {
      Logger::Warning("Unable to open '%s' for writing", filename.c_str());
      return false;
    }
    ctx = gdNewFileCtx(fp);
  } else {
    ctx = (gdIOCtx *)malloc(sizeof(gdIOCtx));
  ...
  case PHP_GDIMG_CONVERT_WBM:
    if(q<0||q>255) {
      Logger::Warning("Invalid threshold value '%d'. "
...
static void _php_image_bw_convert(gdImagePtr im_org, gdIOCtx *out,
  ...
  if (im_dest == NULL) {
    Logger::Warning("Unable to allocate temporary buffer");
    return;
  }

  white = gdImageColorAllocate(im_dest, 255, 255, 255);
  if (white == -1) {
    Logger::Warning("Unable to allocate the colors for "
...
static bool _php_image_convert(CStrRef f_org, CStrRef f_dest,
  ...
  if (threshold < 0 || threshold > 8) {
    Logger::Warning("Invalid threshold value '%d'", threshold);
    return false;
  }
  ...
  if (same(org_stream, false)) {
    Logger::Warning("Unable to open '%s' for reading", f_org.c_str());
    return false;
  }
  ...
  if (same(dest_stream, false)) {
    Logger::Warning("Unable to open '%s' for writing", f_dest.c_str());
    return false;
  }
  ...
  case PHP_GDIMG_TYPE_GIF:
    ...
    if (im_org == NULL) {
      Logger::Warning("Unable to open '%s' Not a valid GIF file",
                      f_org.c_str());
      return false;
    }
    break;
  ...
  case PHP_GDIMG_TYPE_JPG:
    ...
    if (im_org == NULL) {
      Logger::Warning("Unable to open '%s' Not a valid JPEG file",
                      f_org.c_str());
      return false;
    }
    break;
  ...
  case PHP_GDIMG_TYPE_PNG:
    ...
    if (im_org == NULL) {
      Logger::Warning("Unable to open '%s' Not a valid PNG file",
                      f_org.c_str());
      return false;
    }
    break;
  ...
  im_dest = gdImageCreate(dest_width, dest_height);
  if (im_dest == NULL) {
    Logger::Warning("Unable to allocate destination buffer");
    return false;
  }
...
static gdImagePtr _php_image_create_from(CStrRef filename,
  ...
  if (image_type == PHP_GDIMG_TYPE_GD2PART) {
    if (width < 1 || height < 1) {
      Logger::Warning("Zero width or height not allowed");
      return NULL;
    }
  }
  ...
  if (plain_file) {
    ...
    if (buff.empty()) {
      Logger::Warning("Cannot read image data");
      goto out_err;
    }

    io_ctx = gdNewDynamicCtxEx(buff.length(), (char *)buff.c_str(), 0);
    if (!io_ctx) {
      Logger::Warning("Cannot allocate GD IO context");
      goto out_err;
    }
  ...
  Logger::Warning("'%s' is not a valid %s file", filename.c_str(), tn);
out_err:
  f_fclose(stream);
  return NULL;
}
...
gdImagePtr _php_image_create_from_string(CStrRef image, char *tn,
  ...
  im = (*(gdImagePtr (*)(gdIOCtx *))ioctx_func_p)(io_ctx);
  if (!im) {
    Logger::Warning("Passed data is not in '%s' format", tn);
#if HAVE_LIBGD204
    io_ctx->gd_free(io_ctx);
#else
    io_ctx->free(io_ctx);
...
static gdFontPtr php_find_gd_font(int size) {
  ...
  case 5:
    ...
    break;
  default:
    Logger::Warning("Unsupported font: %d", size);
...
static bool php_imagepolygon(CObjRef image, CArrRef points, int num_points,
  ...
  nelem = points.size();
  if (nelem < 6) {
    Logger::Warning("You must have at least 3 points in your array");
    return false;
  }

  if (nelem < num_points * 2) {
    Logger::Warning("Trying to use %d points in array with only %d points",
                    num_points, nelem/2);
    return false;
  }

  pts = (gdPointPtr)malloc(num_points * sizeof(gdPoint));
...
static Variant php_imagettftext_common(int mode, int extended,
  ...
  if (same(stream, false)) {
    Logger::Error("Invalid font filename %s", fontname.c_str());
    return false;
  }
  f_fclose(stream);
...
Variant f_imageloadfont(CStrRef file) {
  ...
  if (same(stream, false)) {
    Logger::Warning("failed to open file: %s", file.c_str());
    return false;
  }
   ...
   */
  font = (gdFontPtr) malloc(sizeof(gdFont));
  b = 0;
  String hdr = f_fread(stream, hdr_size);
  if (hdr.length() < hdr_size) {
    free(font);
    if (f_feof(stream)) {
      Logger::Warning("End of file while reading header");
    } else {
      Logger::Warning("Error while reading header");
    }
    f_fclose(stream);
    return false;
  }
  ...
  if (body_size != body_size_check) {
    Logger::Warning("Error reading font");
    free(font);
    f_fclose(stream);
    return false;
  }
  ...
  if (body.length() < body_size) {
    free(font);
    if (f_feof(stream)) {
      Logger::Warning("End of file while reading body");
    } else {
      Logger::Warning("Error while reading body");
    }
    f_fclose(stream);
    return false;
  }
  font->data = malloc(body_size);
...
bool f_imagesetstyle(CObjRef image, CArrRef style) {
  ...
  stylearr = (int *)malloc(sizeof(int) * style.size());
  index = 0;
...
Variant f_imagetruecolortopalette(CObjRef image, bool dither, int ncolors) {
  ...
  if (ncolors <= 0) {
    Logger::Warning("Number of colors has to be greater than zero");
    return false;
  }
...
/*
  ...
  switch (result) {
  case -1:
    Logger::Warning("Image1 must be TrueColor" );
    return false;
  case -2:
    Logger::Warning("Image2 must be Palette" );
    return false;
  case -3:
    Logger::Warning("Image1 and Image2 must be the same size" );
    return false;
  case -4:
    Logger::Warning("Image2 must have at least one color" );
    return false;
  }
...
Variant f_imagecreatefromstring(CStrRef data) {
  ...
  if (data.length() < 8) {
    Logger::Warning("Empty string or invalid image");
    return false;
  }
...
#else
    Logger::Warning("No JPEG support");
    return false;
#endif
    break;
...
#else
    Logger::Warning("No WBMP support");
    return false;
#endif
    break;
...
#else
    Logger::Warning("No GD2 support");
    return false;
#endif
    break;

  default:
    Logger::Warning("Data is not in a recognized format");
    return false;
  }

  if (!im) {
    Logger::Warning("Couldn't create GD Image Stream out of Data");
    return false;
  }
  return Object(new Image(im));
#else
...
Variant f_imagecolorat(CObjRef image, int x, int y) {
  ...
  if (gdImageTrueColor(im)) {
    if (im->tpixels && gdImageBoundsSafe(im, x, y)) {
      ...
      Logger::Info("%ld,%ld is out of bounds", x, y);
      return false;
    }
  } else {
#endif
...
bool f_imagecolordeallocate(CObjRef image, int color) {
  ...
  if (color >= 0 && color < gdImageColorsTotal(im)) {
    ...
    Logger::Warning("Color index %d out of range",  color);
    return false;
  }
}
...
Variant f_imagecolorsforindex(CObjRef image, int index) {
  ...
  if (col >= 0 && col < gdImageColorsTotal(im)) {
    ...
    Logger::Warning("Color index %d out of range", index);
    return false;
  }
  return ret;
}
...
bool f_imageconvolution(CObjRef image, CArrRef matrix,
  ...
  if (nelem != 3) {
    Logger::Warning("You must have 3x3 array");
    return false;
  }
  for (i=0; i<3; i++) {
    if (matrix.exists(i) && (v = matrix[i]).isArray()) {
      ...
      for (j=0; j<3; j++) {
        if (row.exists(j)) {
          ...
          Logger::Warning("You must have a 3x3 matrix");
          return false;
        }
      }
    }
...
Variant f_iptcembed(CStrRef iptcdata, CStrRef jpeg_file_name,
  ...
  if (same(stream, false)) {
    Logger::Warning("failed to open file: %s", jpeg_file_name.c_str());
    return false;
  }
  if (spool < 2) {
    ...
    poi = spoolbuf =
      (unsigned char *)malloc(iptcdata_len + sizeof(psheader) +
                              st_size + 1024 + 1);
    memset(poi, 0, iptcdata_len + sizeof(psheader) + st_size + 1024 + 1);
  }
  File *file = Object(stream).getTyped<File>();
...
static int exif_file_sections_add(image_info_type *ImageInfo, int type,
  ...
  tmp = (file_section *)
        realloc(ImageInfo->file.list, (count+1) * sizeof(file_section));
...
static char *php_strndup(const char* s, uint length) {
  char *p;
...
static void exif_iif_add_value(image_info_type *image_info, int section_index,
  ...
  list = (image_info_data*)realloc(image_info->info_list[section_index].list,
            (image_info->info_list[section_index].count+1) *
  ...
  case TAG_FMT_STRING:
    if (value) {
      ...
      // } else {
      info_value->s = php_strndup((const char *)value, length);
      // }
      info_data->length = length;
    } else {
      info_data->length = 0;
  ...
  case TAG_FMT_DOUBLE:
    if (length==0) {
      ...
      info_value->list =
        (image_info_value*)calloc(length, sizeof(image_info_value));
    } else {
      info_value = &info_data->value;
    }
...
static void* exif_ifd_make_value(image_info_data *info_data,
  ...
  value_ptr = (char *)malloc((byte_count > 4 ? byte_count : 4));
...
static void exif_thumbnail_build(image_info_type *ImageInfo) {
  ...
  case IMAGE_FILETYPE_TIFF_MM:
    ...
    new_move = new_size;
    new_data = (char *)realloc(ImageInfo->Thumbnail.data,
...
static void exif_thumbnail_extract(image_info_type *ImageInfo,
  ...
  if (ImageInfo->Thumbnail.data) {
    Logger::Warning("Multiple possible thumbnails");
  ...
  if (ImageInfo->Thumbnail.size >= 65536 ||
    ...
    Logger::Warning("Illegal thumbnail size/offset");
    return;
  }
  /* Check to make sure we are not going to go past the ExifLength */
  if ((ImageInfo->Thumbnail.offset + ImageInfo->Thumbnail.size) > length) {
    Logger::Warning("Thumbnail goes IFD boundary or end of file reached");
    return;
  }
  ImageInfo->Thumbnail.data =
    php_strndup(offset + ImageInfo->Thumbnail.offset,
                ImageInfo->Thumbnail.size);
...
static int exif_process_undefined(char **result, char *value,
   ...
   */
  if (byte_count) {
    (*result) = php_strndup(value, byte_count); /* NULL @ byte_count!!! */
    return byte_count+1;
  }
  return 0;
}
...
static int exif_process_string_raw(char **result, char *value,
   ...
   */
  if (byte_count) {
    (*result) = (char*)malloc(byte_count + 1);
...
static int exif_process_string(char **result, char *value,
   ...
   */
    ...
    (*result) = php_strndup("", 1); /* force empty string */
    return byte_count+1;
}
...
static int exif_process_IFD_in_MAKERNOTE(image_info_type *ImageInfo,
  ...
  if ((2+NumDirEntries*12) > value_len) {
    Logger::Warning("Illegal IFD size: 2 + x%04X*12 = x%04X > x%04X",
...
static int exif_process_IFD_TAG(image_info_type *ImageInfo, char *dir_entry,
  ...
  if (ImageInfo->ifd_nesting_level > MAX_IFD_NESTING_LEVEL) {
    Logger::Warning("corrupt EXIF header: maximum directory "
  ...
  if (!format || format > NUM_FORMATS) {
    /* (-1) catches illegal zero case as unsigned underflows to
       positive large. */
    Logger::Warning("Process tag(x%04X=%s): Illegal format code 0x%04X, "
                    "suppose BYTE", tag,
  ...
  if ((ssize_t)byte_count < 0) {
    Logger::Warning("Process tag(x%04X=%s): Illegal byte_count(%ld)",
      ...
      */
      if (offset_val+byte_count>ImageInfo->FileSize ||
        ...
        if (value_ptr < dir_entry) {
          ...
          Logger::Warning("Process tag(x%04X=%s): Illegal pointer offset"
                          "(x%04X < x%04X)", tag,
      ...
      if (byte_count>sizeof(cbuf)) {
        ...
        value_ptr = (char *)malloc(byte_count);
        outside = value_ptr;
      } else {
        /*
      ...
      if (fgot!=offset_val) {
        ...
        Logger::Warning("Wrong file pointer: 0x%08X != 0x08X",
                        fgot, offset_val);
        return 0;
      }
      ...
      if (fgot<byte_count) {
        if (outside) free(outside);
        Logger::Warning("Unexpected end of file reached");
        return 0;
      }
    }
  } else {
  ...
  if (section_index==SECTION_THUMBNAIL) {
    ...
    switch(tag) {
      case TAG_COPYRIGHT:
        ...
        if (byte_count>1 && (length=php_strnlen(value_ptr, byte_count)) > 0) {
          if (length<byte_count-1) {
            ...
            ImageInfo->CopyrightEditor = strdup(value_ptr+length+1);
            vspprintf(&ImageInfo->Copyright, 0, "%s, %s",
                      value_ptr, value_ptr+length+1);
            /* format = TAG_FMT_UNDEFINED; this musn't be ASCII         */
            /* but we are not supposed to change this                   */
            /* keep in mind that image_info does not store editor value */
          } else {
            ImageInfo->Copyright = strdup(value_ptr);
          }
        }
        break;
      ...
      case TAG_XP_SUBJECT:
        tmp_xp =
          (xp_field_type*)realloc(ImageInfo->xp_fields.list,
                                  (ImageInfo->xp_fields.count+1) *
                                  sizeof(xp_field_type));
      ...
      case TAG_SUB_IFD:
        if (format==TAG_FMT_IFD) {
          ...
          Logger::Info("Skip SUB IFD");
        }
        break;

      case TAG_MAKE:
        ImageInfo->make = strdup(value_ptr);
        break;
      case TAG_MODEL:
        ImageInfo->model = strdup(value_ptr);
        break;
      ...
      case TAG_INTEROP_IFD_POINTER:
        if (ReadNextIFD) {
          ...
          if (Subdir_start < offset_base ||
            ...
            Logger::Warning("Illegal IFD Pointer");
            return 0;
          }
...
static int exif_process_IFD_in_JPEG(image_info_type *ImageInfo,
  ...
  if ((dir_start+2+NumDirEntries*12) > (offset_base+IFDlength)) {
    Logger::Warning("Illegal IFD size: x%04X + 2 + x%04X*12 = x%04X > x%04X",
...
static void exif_process_TIFF_in_JPEG(image_info_type *ImageInfo,
  ...
  if (memcmp(CharBuf, "II", 2) == 0) {
    ...
    Logger::Warning("Invalid TIFF a lignment marker");
    return;
  }
  ...
  if ( exif_value_2a != 0x2a || offset_of_ifd < 0x08) {
    Logger::Warning("Invalid TIFF start (1)");
    return;
  }
...
static void exif_process_APP1(image_info_type *ImageInfo, char *CharBuf,
  ...
  if (memcmp(CharBuf+2, ExifHeader, 6)) {
    Logger::Warning("Incorrect APP1 Exif Identifier Code");
    return;
  }
  ...
  for(section=0;;section++) {
    ...
    do {
      if ((marker = ImageInfo->infile->getc()) == EOF) {
        Logger::Warning("File structure corrupted");
        return 0;
      }
      if (last_marker==M_COM && comment_correction>0) {
        if (marker!=0xFF) {
    ...
    if (last_marker==M_COM && !comment_correction) {
      Logger::Info("Image has corrupt COM section: some software set "
    ...
    if (marker == 0xff) {
      ...
      Logger::Warning("To many padding bytes");
      return 0;
    }

    /* Read the length of the section. */
    lh = ImageInfo->infile->getc();
    ll = ImageInfo->infile->getc();
    ...
    got = str.length();
    if (got != itemlen-2) {
      Logger::Warning("Error reading from file: "
    ...
    switch(marker) {
      ...
      case M_EOI:   /* in case it's a tables-only JPEG stream */
        Logger::Warning("No image in jpeg!");
...
static int exif_file_sections_realloc(image_info_type *ImageInfo,
  ...
  if (section_index >= ImageInfo->file.count) {
    Logger::Warning("Illegal reallocating of undefined file section");
    return -1;
  }
    ...
    if (ImageInfo->FileSize >= dir_offset+dir_size) {
      ...
      for(i=0;i<num_entries;i++) {
        ...
        if (entry_type > NUM_FORMATS) {
          Logger::Info("Read from TIFF: tag(0x%04X,%12s): "
      ...
      if (ImageInfo->FileSize >=
        ...
        if (ifd_size > dir_size) {
          if (dir_offset + ifd_size > ImageInfo->FileSize) {
            Logger::Warning("Error in TIFF: filesize(x%04X) less than "
          ...
          if (entry_tag == TAG_EXIF_IFD_POINTER ||
            ...
            if (section_index!=SECTION_THUMBNAIL && entry_tag==TAG_SUB_IFD) {
              if (ImageInfo->Thumbnail.filetype != IMAGE_FILETYPE_UNKNOWN &&
                ...
                if (!ImageInfo->Thumbnail.data) {
                  ImageInfo->Thumbnail.data =
                    (char *)malloc(ImageInfo->Thumbnail.size);
                  ...
                  if (fgot < ImageInfo->Thumbnail.size) {
                    Logger::Warning("Thumbnail goes IFD boundary or "
                                    "end of file reached");
                  }
      ...
      Logger::Warning("Error in TIFF: filesize(x%04X) less than size "
                      "of IFD dir(x%04X)",
  ...
  if (ImageInfo->FileSize >= 2) {
    ...
    if ((file_header[0]==0xff) && (file_header[1]==M_SOI)) {
      ...
      if (exif_scan_JPEG_header(ImageInfo)) {
        ...
        Logger::Warning("Invalid JPEG file");
      }
      ...
      if (!memcmp(file_header, "II\x2A\x00", 4)) {
        ...
        if (exif_process_IFD_in_TIFF(ImageInfo,
          ...
          Logger::Warning("Invalid TIFF file");
        }
      }
    ...
    Logger::Warning("File too small (%d)", ImageInfo->FileSize);
  }
  return ret;
}
...
static int exif_read_file(image_info_type *ImageInfo, String FileName,
  ...
  Variant stream = f_fopen(FileName, "rb");
  if (same(stream, false)) {
    Logger::Warning("Unable to open file %s", FileName.c_str());
    return 0;
  }
  ...
  ImageInfo->encode_unicode    = strdup("ISO-8859-15");
  ImageInfo->decode_unicode_be = strdup("UCS-2BE");
  ImageInfo->decode_unicode_le = strdup("UCS-2LE");
  ImageInfo->encode_jis        = strdup("");
  ImageInfo->decode_jis_be     = strdup("JIS");
  ImageInfo->decode_jis_le     = strdup("JIS");
...
static int exif_file_sections_free(image_info_type *ImageInfo) {
  ...
  if (ImageInfo->file.count) {
    for (i=0; i<ImageInfo->file.count; i++) {
       if (ImageInfo->file.list[i].data) free(ImageInfo->file.list[i].data);
    }
  }
  if (ImageInfo->file.list) free(ImageInfo->file.list);
...
static int exif_discard_imageinfo(image_info_type *ImageInfo) {
  ...
  if (ImageInfo->UserComment) free(ImageInfo->UserComment);
  if (ImageInfo->UserCommentEncoding) free(ImageInfo->UserCommentEncoding);
  if (ImageInfo->Copyright) free(ImageInfo->Copyright);
  if (ImageInfo->CopyrightPhotographer) free(ImageInfo->CopyrightPhotographer);
  if (ImageInfo->CopyrightEditor) free(ImageInfo->CopyrightEditor);
  if (ImageInfo->Thumbnail.data) free(ImageInfo->Thumbnail.data);
  if (ImageInfo->encode_unicode) free(ImageInfo->encode_unicode);
  if (ImageInfo->decode_unicode_be) free(ImageInfo->decode_unicode_be);
  if (ImageInfo->decode_unicode_le) free(ImageInfo->decode_unicode_le);
  if (ImageInfo->encode_jis) free(ImageInfo->encode_jis);
  if (ImageInfo->decode_jis_be) free(ImageInfo->decode_jis_be);
  if (ImageInfo->decode_jis_le) free(ImageInfo->decode_jis_le);
  if (ImageInfo->make) free(ImageInfo->make);
  if (ImageInfo->model) free(ImageInfo->model);
  for (i=0; i<ImageInfo->xp_fields.count; i++) {
    if (ImageInfo->xp_fields.list[i].value) {
      free(ImageInfo->xp_fields.list[i].value);
    }
  }
  if (ImageInfo->xp_fields.list) free(ImageInfo->xp_fields.list);
  for (i=0; i<SECTION_COUNT; i++) {
...
static void exif_iif_add_int(image_info_type *image_info, int section_index,
  ...
  list = (image_info_data *)
    realloc(image_info->info_list[section_index].list,
            (image_info->info_list[section_index].count+1) *
             sizeof(image_info_data));
  image_info->info_list[section_index].list = list;
...
static void exif_iif_add_fmt(image_info_type *image_info, int section_index,
  ...
  if (value) {
    vspprintf_ap(&tmp, 0, value, arglist);
...
static void exif_iif_add_buffer(image_info_type *image_info,
  ...
  if (value) {
    ...
    //   info_data->length = length;
    // } else {
    info_data->value.s = (char *)malloc(length + 1);
...
static int exif_scan_thumbnail(image_info_type *ImageInfo) {
  ...
  if (memcmp(data, "\xFF\xD8\xFF", 3)) {
    if (!ImageInfo->Thumbnail.width && !ImageInfo->Thumbnail.height) {
      Logger::Warning("Thumbnail is not a JPEG image");
    }
    return 0;
  }
  for (;;) {
    ...
    switch (marker) {
      ...
      case M_SOS:
      case M_EOI:
        Logger::Warning("Could not compute size of thumbnail");
        return 0;
        break;
...
Variant f_exif_read_data(CStrRef filename,
  ...
  memset(&ImageInfo, 0, sizeof(ImageInfo));
  if (!sections.isNull()) {
    vspprintf(&sections_str, 0, ",%s,", sections.c_str());
    ...
    if (sections_str) free(sections_str);
  }
...
Variant f_exif_imagetype(CStrRef filename) {
  Variant stream = f_fopen(filename, "rb");
  if (same(stream, false)) {
    Logger::Warning("failed to open file: %s", filename.c_str());
    return false;
  }
