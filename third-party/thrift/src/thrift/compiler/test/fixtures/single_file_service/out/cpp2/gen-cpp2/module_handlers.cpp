...
void apache::thrift::ServiceHandler<::cpp2::A>::async_tm_foo(apache::thrift::HandlerCallback<std::unique_ptr<::cpp2::Foo>>::Ptr callback) {
...
void apache::thrift::ServiceHandler<::cpp2::A>::IIf::async_tm_interact(apache::thrift::HandlerCallback<void>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::cpp2::B>::async_tm_bar(apache::thrift::HandlerCallback<void>::Ptr callback, std::unique_ptr<::cpp2::Foo> p_foo) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::cpp2::B>::async_tm_stream_stuff(apache::thrift::HandlerCallback<::apache::thrift::ServerStream<::std::int32_t>>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::cpp2::B>::async_tm_sink_stuff(apache::thrift::HandlerCallback<::apache::thrift::SinkConsumer<::std::int32_t, ::std::int32_t>>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
...
void apache::thrift::ServiceHandler<::cpp2::C>::IIf::async_tm_interact(apache::thrift::HandlerCallback<void>::Ptr callback) {
  // It's possible the coroutine versions will delegate to a future-based
  // version. If that happens, we need the RequestParams arguments to be
  // available to the future through the thread-local backchannel, so we create
  // a RAII object that sets up RequestParams and clears them on destruction.
