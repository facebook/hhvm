...
bool PDOMySqlConnection::create(const Array& options) {
  ...
  if (!options.empty()) {
    ...
    long read_timeout = pdo_attr_lval(options, PDO_ATTR_READ_TIMEOUT, 123456789);
    long write_timeout = pdo_attr_lval(options, PDO_ATTR_WRITE_TIMEOUT, 123456789);
    ...
    if (mysql_options(m_server, MYSQL_OPT_READ_TIMEOUT,
