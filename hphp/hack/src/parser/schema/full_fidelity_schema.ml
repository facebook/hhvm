...
let full_fidelity_schema_version_number = "2019-08-22-0001"
...
let generate_string template =
  ...
  let operator_folder operators s x =
    replace x.operator_pattern (x.operator_func operators) s in

  let result = List.fold_left
    syntax_folder template.template template.transformations in
  let result = List.fold_left (tokens_folder tokens)
    result template.token_transformations in
  ...
  let result = List.fold_left (operator_folder operators)
    result template.operator_transformations in
  let result = List.fold_left
...
AST_NODES
  ...
  let full_fidelity_json_schema = make_template_file
    ~transformations: [
