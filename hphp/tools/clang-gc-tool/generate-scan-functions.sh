...
#fi
rm -f "$OUTDIR/warnings.log" "$OUTDIR/all-warnings.log" "$ALL_SCAN_H"
...
"$CAT" "$LOG_FILE" | grep cpp_compile | grep -v "scan-methods" | grep -v "heap-trace.cpp" | grep "$FILE_REGEX" | perl -pe "$COMPILE_REXP" | perl -pe "$OFILE_REXP" | perl -pe "$BADARGS_REXP" | perl -pe "$SWIZZLE_DS1_REXP" | perl -pe "$SWIZZLE_DS2_REXP" | perl -pe "$SWIZZLE_IS_REXP" > "$RESULTS"
...
if [[ $VERBOSE == "1" ]]; then
    echo
    echo "Master scan file: $ALL_SCAN_H"
fi
...
(cd "$OUTDIR" && echo "$PROLOG" > "$ALL_SCAN_H" && (find . -name "*.h" ! -name "all-scan.h" -printf "$FMT" | sort) >> "$ALL_SCAN_H" && echo "$EPILOG" >> "$ALL_SCAN_H")
