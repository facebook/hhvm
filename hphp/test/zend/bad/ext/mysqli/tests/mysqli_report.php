<?php
	...
	$test_table_name = 'test_mysqli_report_table_1'; require('table.inc');
	...
	*/
	...
	$stmt = mysqli_stmt_init($link);
	mysqli_stmt_prepare($stmt, "SELECT id FROM test_mysqli_report_table_1 WHERE id > ?");
	$id = 1;
	...
	*/
	...
	if (mysqli_get_server_version($link) <= 50600) {
		...
		if (mysqli_get_server_version($link) >= 50111) {
			...
			if ($log_slow_queries && $log_queries_not_using_indexes) {
				...
				for ($i = 100; $i < 20000; $i++) {
					if (!mysqli_query($link, "INSERT INTO test_mysqli_report_table_1(id, label) VALUES ($i, 'z')"))
						printf("[022 - %d] [%d] %s\n", $i - 99, mysqli_errno($link), mysqli_error($link));
				}

				// this might cause a warning - no index used
				if (!$res = @mysqli_query($link, "SELECT id, label FROM test_mysqli_report_table_1 WHERE id = 1323"))
					printf("[023] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	...
	if (!mysqli_stmt_prepare($stmt, 'SELECT id, label FROM test_mysqli_report_table_1'))
	...
	if (!mysqli_real_query($link, 'SELECT label, id FROM test_mysqli_report_table_1'))
		printf("[028] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	...
	if (!$stmt = mysqli_prepare($link, 'SELECT id * 3 FROM test_mysqli_report_table_1'))
		printf("[032] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
	else
		mysqli_stmt_close($stmt);

	if (!mysqli_query($link, "INSERT INTO test_mysqli_report_table_1(id, label) VALUES (100, 'z')", MYSQLI_USE_RESULT) ||
			!mysqli_query($link, 'DELETE FROM test_mysqli_report_table_1 WHERE id > 50', MYSQLI_USE_RESULT))
		printf("[033] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
