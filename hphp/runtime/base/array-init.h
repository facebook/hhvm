...
struct ArrayInit {
  ...
  ArrayInit& set(int64_t name, const Variant& v) {
    performOp([&]{ return MixedArray::SetInt(m_data, name, v, false); });
    return *this;
  }
  ...
  ArrayInit& set(const String& name, const Variant& v) {
    performOp([&]{
      return MixedArray::SetStr(m_data, name.get(), v, false);
    });
    return *this;
  }
  ...
  ArrayInit& add(int64_t name, const Variant& v, bool keyConverted = false) {
    performOp([&]{ return MixedArray::AddInt(m_data, name, v, false); });
    return *this;
  }
  ...
  ArrayInit& add(const String& name, const Variant& v, bool keyConverted = false) {
    if (keyConverted) {
      performOp([&]{
        return MixedArray::AddStr(m_data, name.get(), v, false);
      });
    } else if (!name.isNull()) {
