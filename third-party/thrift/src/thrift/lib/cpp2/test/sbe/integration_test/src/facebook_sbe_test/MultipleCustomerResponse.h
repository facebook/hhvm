...
private:
     ...
     *       V0_BLOCK -> V0_CUSTOMERRESPONSES_DONE [label="  CustomerResponsesCount(0)  "];
     *       V0_BLOCK -> V0_CUSTOMERRESPONSES_N [label="  CustomerResponsesCount(>0)  "];
     *       V0_CUSTOMERRESPONSES_N_BLOCK -> V0_CUSTOMERRESPONSES_N_BLOCK [label="  CustomerResponses.CustomerResponseLength()  "];
     *       V0_CUSTOMERRESPONSES_1_BLOCK -> V0_CUSTOMERRESPONSES_1_BLOCK [label="  CustomerResponses.CustomerResponseLength()  "];
     *       V0_CUSTOMERRESPONSES_N_BLOCK -> V0_CUSTOMERRESPONSES_N_CUSTOMERRESPONSE_DONE [label="  CustomerResponses.CustomerResponse(?)  "];
     *       V0_CUSTOMERRESPONSES_1_BLOCK -> V0_CUSTOMERRESPONSES_1_CUSTOMERRESPONSE_DONE [label="  CustomerResponses.CustomerResponse(?)  "];
     *       V0_CUSTOMERRESPONSES_N_CUSTOMERRESPONSE_DONE -> V0_CUSTOMERRESPONSES_N_BLOCK [label="  CustomerResponses.next()\n  where count - newIndex > 1  "];
     *       V0_CUSTOMERRESPONSES_N -> V0_CUSTOMERRESPONSES_N_BLOCK [label="  CustomerResponses.next()\n  where count - newIndex > 1  "];
     *       V0_CUSTOMERRESPONSES_N_CUSTOMERRESPONSE_DONE -> V0_CUSTOMERRESPONSES_1_BLOCK [label="  CustomerResponses.next()\n  where count - newIndex == 1  "];
     *       V0_CUSTOMERRESPONSES_N -> V0_CUSTOMERRESPONSES_1_BLOCK [label="  CustomerResponses.next()\n  where count - newIndex == 1  "];
     *       V0_CUSTOMERRESPONSES_N_CUSTOMERRESPONSE_DONE -> V0_CUSTOMERRESPONSES_DONE [label="  CustomerResponses.resetCountToIndex()  "];
     *       V0_CUSTOMERRESPONSES_N -> V0_CUSTOMERRESPONSES_DONE [label="  CustomerResponses.resetCountToIndex()  "];
     *       V0_CUSTOMERRESPONSES_1_CUSTOMERRESPONSE_DONE -> V0_CUSTOMERRESPONSES_DONE [label="  CustomerResponses.resetCountToIndex()  "];
     *       V0_CUSTOMERRESPONSES_DONE -> V0_CUSTOMERRESPONSES_DONE [label="  CustomerResponses.resetCountToIndex()  "];
     *   }
     * }</pre>
     */
...
public:
    ...
    MultipleCustomerResponse(char *buffer, const std::uint64_t bufferLength) :
        MultipleCustomerResponse(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MultipleCustomerResponse(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MultipleCustomerResponse(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MultipleCustomerResponse &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MultipleCustomerResponse &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MultipleCustomerResponse skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion, m_codecState);
        skipper.skip();
    ...
    private:
                ...
                switch (codecState())
                {
                    ...
                    default:
                        throw AccessOrderError(std::string("Illegal field access order. ") +
                            "Cannot access next element in repeating group \"CustomerResponses\" in state: " + codecStateName(codecState()) +
        ...
        void onResetCountToIndex()
        {
            switch (codecState())
            {
                ...
                default:
                    throw AccessOrderError(std::string("Illegal field access order. ") +
                        "Cannot reset count of repeating group \"CustomerResponses\" in state: " + codecStateName(codecState()) +
                        ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                        "]. Please see the diagram in the docs of the enum MultipleCustomerResponse::CodecState.");
            }
        }
    ...
    public:
        ...
        std::uint64_t sbeCheckPosition(const std::uint64_t position)
        {
            if (SBE_BOUNDS_CHECK_EXPECT((position > m_bufferLength), false))
            {
                throw std::runtime_error("buffer too short [E100]CustomerResponses");
            }
            return position;
        }
        ...
        inline CustomerResponses &next()
        {
            if (m_index >= m_count)
            {
                throw std::runtime_error("index >= count [E108] in CustomerResponses");
            }
#if defined(SBE_ENABLE_PRECEDENCE_CHECKS)
            ...
            {
                throw std::runtime_error("buffer too short for next group index [E108] in CustomerResponses");
            }
        ...
        SBE_NODISCARD static const char *CustomerResponseMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
        {
        ...
        static SBE_CONSTEXPR std::uint64_t customerResponseSinceVersion() SBE_NOEXCEPT
        {
            return 0;
        }

        bool customerResponseInActingVersion() SBE_NOEXCEPT
        {
            return true;
        }
        ...
        static SBE_CONSTEXPR std::uint64_t customerResponseHeaderLength() SBE_NOEXCEPT
        {
            return 2;
        }
    ...
    private:
        void onCustomerResponseLengthAccessed() const
        {
            switch (codecState())
            {
                ...
                default:
                    throw AccessOrderError(std::string("Illegal field access order. ") +
                        "Cannot decode length of var data \"CustomerResponses.CustomerResponse\" in state: " + codecStateName(codecState()) +
                        ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                        "]. Please see the diagram in the docs of the enum MultipleCustomerResponse::CodecState.");
            }
        }
    ...
    private:
        void onCustomerResponseAccessed()
        {
            switch (codecState())
            {
                ...
                default:
                    throw AccessOrderError(std::string("Illegal field access order. ") +
                        "Cannot access field \"CustomerResponses.CustomerResponse\" in state: " + codecStateName(codecState()) +
                        ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                        "]. Please see the diagram in the docs of the enum MultipleCustomerResponse::CodecState.");
            }
        }
    ...
    public:
        ...
        CustomerResponses &putCustomerResponse(const char *src, const std::uint16_t length)
        {
#if defined(SBE_ENABLE_PRECEDENCE_CHECKS)
            onCustomerResponseAccessed();
#endif
        ...
        CustomerResponses &putCustomerResponse(const std::string &str)
        {
            if (str.length() > 65534)
            {
                throw std::runtime_error("std::string too long for length type [E109] in CustomerResponses");
            }
            return putCustomerResponse(str.data(), static_cast<std::uint16_t>(str.length()));
        }
            ...
            std::basic_ostream<CharT, Traits> &builder, CustomerResponses &writer)
        {
            ...
            builder << '"' <<
                writer.skipCustomerResponse() << " bytes of raw data\"";
            builder << '}';
        ...
        SBE_NODISCARD static std::size_t computeLength(std::size_t customerResponseLength = 0)
        {
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
            ...
            {
                throw std::runtime_error("customerResponseLength too long for length type [E109]  in MultipleCustomerResponse");
            }
...
public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t CustomerResponsesId() SBE_NOEXCEPT
    {
        return 1;
    }

private:
    void onCustomerResponsesAccessed(std::uint64_t remaining, std::string action)
    {
        if (0 == remaining)
        {
            switch (codecState())
            {
                ...
                default:
                    throw AccessOrderError(std::string("Illegal field access order. ") +
                        "Cannot " + action + " count of repeating group \"CustomerResponses\" in state: " + codecStateName(codecState()) +
                        ". Expected one of these transitions: [" + codecStateTransitions(codecState()) +
                        "]. Please see the diagram in the docs of the enum MultipleCustomerResponse::CodecState.");
            }
        }
...
public:
    ...
    {
        m_customerResponses.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength, codecStatePtr());
#if defined(SBE_ENABLE_PRECEDENCE_CHECKS)
    ...
    {
        m_customerResponses.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength, codecStatePtr());
#if defined(SBE_ENABLE_PRECEDENCE_CHECKS)
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t customerResponsesSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MultipleCustomerResponse &_writer)
{
...
SBE_NODISCARD static std::size_t computeLength(const std::vector<std::tuple<std::size_t>> &customerResponsesItemLengths = {})
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    {
        throw std::runtime_error("customerResponsesItemLengths.size() outside of allowed range [E110] in MultipleCustomerResponse");
    }
