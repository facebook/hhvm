<?php
	...
	for ($bits = 1; $bits <= 46; $bits++) {
		...
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_stmt_get_result_bit_table_1") ||
			!mysqli_query($link, $sql = sprintf('CREATE TABLE test_mysqli_stmt_get_result_bit_table_1(id BIGINT UNSIGNED, bit_value BIT(%d) NOT NULL, bit_null BIT(%d) DEFAULT NULL) ENGINE="%s"', $bits, $bits, $engine)))
		...
		while ($tests < 10) {
			...
			$sql = sprintf("INSERT INTO test_mysqli_stmt_get_result_bit_table_1(id, bit_value) VALUES (%s, b'%s')", $value, $bin);
			...
			$sql = sprintf("SELECT bin(bit_value) AS _bin, id, bit_value, bit_null FROM test_mysqli_stmt_get_result_bit_table_1 WHERE id = %s", $value);
			if (!mysqli_stmt_prepare($stmt, $sql) ||
					!mysqli_stmt_execute($stmt))
				printf("[005 - %d] [%d] %s\n", $bits, mysqli_stmt_errno($stmt), mysqli_stmt_error($stmt));
...
?>
<?php
	$test_table_name = 'test_mysqli_stmt_get_result_bit_table_1'; require_once("clean_table.inc");
?>