...
struct ConcurrentObr {
    ...
    /// When a worker finishes parsing a file, it sends the results over channel `tx`.
    tx: crossbeam::channel::Sender<(RelativePath, SendableOcamlPtrPtr, DeclsHolderObr)>,

    /// When the main ocaml thread is waiting for a result from a worker, it receives
    /// that result over channel `rx`.
    rx: crossbeam::channel::Receiver<(RelativePath, SendableOcamlPtrPtr, DeclsHolderObr)>,
}
    ...
    fn step(
        ...
        for (path, tag, content) in files {
            ...
            rayon::spawn(move || {
                ...
                let decls_holder = DeclsHolderObr {
                    parsed_file,
                    arena,
                    text,
                };
...
struct ConcurrentHandleObr(RefCell<ConcurrentObr>);

impl ocamlrep_custom::CamlSerialize for ConcurrentHandleObr {
    // The "default_impls" support "drop" (when the ocaml GC collects then it will
    // drop the rust object), but doesn't suppoort "serialize" (when ocaml tries
    // to marshal or unmarshal then it will panic).
    ocamlrep_custom::caml_serialize_default_impls!();
...
ocaml_ffi! {
    fn hh_concurrent_parse_start_obr_ffi(opts: DeclParserOptions, root: PathBuf, hhi: PathBuf, tmp: PathBuf, dummy: PathBuf) -> Custom<ConcurrentHandleObr> {
        Custom::from(ConcurrentHandleObr(RefCell::new(ConcurrentObr::start(opts, root, hhi, tmp, dummy))))
    }

    fn hh_concurrent_parse_step_obr_ffi(handle: Custom<ConcurrentHandleObr>, files: Vec<(RelativePath, UnsafeOcamlPtr, Option<String>)>) -> Option<(RelativePath, UnsafeOcamlPtr, UnsafeOcamlPtr)> {
        // This `borrow_mut` will never panic (i.e. the handle will never be borrowed at the time we call it).
        // That's because the only times we borrow are within the scope of ffi functions, and they always release,
        // and ocaml is single-threaded so there are never multiple ffi functions in flight at once.
        let mut handle = handle.0.borrow_mut();
