    ...
    let oldest_entry = Hashtbl.fold
      ...
      ~f:(fun ~key ~data acc ->
        ...
        | Some (old_key, old_data) ->
          if data.last_used_timestamp < old_data.last_used_timestamp
          then Some (key, data)
          else Some (old_key, old_data)
      )
    in
...
let add_internal (t: ('k, 'v) t) (key: 'k) (value: 'v): 'v entry =
  begin match Hashtbl.find_and_remove t.entries key with
  | None ->
    ()
