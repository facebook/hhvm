<?php
	$test_table_name = 'test_mysqli_stmt_get_result_metadata_table_1'; require('table.inc');

	if (!$stmt = mysqli_stmt_init($link))
		printf("[001] [%d] %s\n", mysqli_errno($link), mysqli_error($link));

	if (!mysqli_stmt_prepare($stmt, "SELECT id, label FROM test_mysqli_stmt_get_result_metadata_table_1 ORDER BY id ASC LIMIT 3"))
	...
	// !mysqli_stmt_prepare($stmt, "SELECT id, label, id + 1 as _id,  concat(label, '_') _label FROM test_mysqli_stmt_get_result_metadata_table_1 as _test ORDER BY id ASC LIMIT 3") ||
	if (!($stmt = mysqli_stmt_init($link)) ||
		!mysqli_stmt_prepare($stmt, "SELECT id , label, id + 1 AS _id, label AS _label, null AS _null, CONCAT(label, '_') _label_concat  FROM test_mysqli_stmt_get_result_metadata_table_1 _test ORDER BY id ASC LIMIT 3") ||
...
?>
<?php
	$test_table_name = 'test_mysqli_stmt_get_result_metadata_table_1'; require_once("clean_table.inc");
?>