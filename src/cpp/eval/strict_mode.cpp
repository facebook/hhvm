...
void throw_strict(const ExtendedException &exn,
                  ...
                  bool call_user_error_handler) {

  if (strict_level_exn <= RuntimeOption::StrictLevel) {
    if (RuntimeOption::StrictFatal) {
      throw exn;
    } else {
      if (call_user_error_handler) {
        g_context->onError(dynamic_cast<const Exception&>(exn));
      }
      Logger::Warning("StrictMode WARNING: %s\n", exn.getMessage().c_str());
    }
  }
}
