...
final class ForkJoinPoolSchedulerStats {
  ...
  private static final long MAX_RECORDED_NANOS = 360000000000000L;
  ...
  ForkJoinPoolSchedulerStats(String name) {
    this.name = name;
    this.pending = Ewma.oneMinuteIntervalEWMA();
    this.active = Ewma.oneMinuteIntervalEWMA();
    this.disposed = new LongAdder();
    this.completed = new LongAdder();
    this.executionTime = new Recorder(MAX_RECORDED_NANOS, 3);
  }
  ...
  public void incrementCompletedTasksAndRecordTime(long startTime) {
    completed.add(1);
    try {
      executionTime.recordValue(System.nanoTime() - startTime);
    } catch (Throwable t) {
      LOGGER.warn("error recording value greater than " + MAX_RECORDED_NANOS, t);
    }
  }
