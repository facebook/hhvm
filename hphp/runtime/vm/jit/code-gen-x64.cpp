...
void CodeGenerator::cgStElem(IRInstruction* inst) {
  ...
  auto idxReg = srcLoc(1).reg();
  if (idx->isConst()) {
    cgStore(baseReg[idx->getValInt()], srcValue, srcLoc(2), Width::Full);
  } else {
