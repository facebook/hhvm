...
function cast_for_arithmetic(mixed $value)[]: dynamic {
  ...
  if ($value is bool || $value is resource) {
    /* HH_IGNORE[12005] suppress warning about casting a resource to int */
    return (int)$value;
  }
...
function __cast_and_compare(mixed $l, mixed $r, COMPARISON_TYPE $ctype)[]: int {
  if ($r is bool && !(\HH\is_fun($l) || \HH\is_class_meth($l))) {
    ...
    /* HH_IGNORE[12005] suppress warning about casting to int and float */
    ...
    if ($l is null) {
      ...
      if ($l is \StringishObject && !($l is \ConstCollection<_>)) {
        /* HH_IGNORE[12005] suppress warning about casting to string */
        $l = (string)$l;
        ...
        /* HH_IGNORE[12005] suppress warning about casting to float */
      $l = (float)$l;
      $r = (float)$r;
    } else if (
      $l is string &&
