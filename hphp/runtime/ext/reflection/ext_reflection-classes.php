 ...
 *
 */
abstract class ReflectionFunctionAbstractBak {
  public $info;
   ...
   */
  abstract public function isClosure();
  ...
  public function getStaticVariables() {
    // We cache the function information because it's mostly static, except
    // static variables that can change from call to call.
    $this->updateInfo();
 ...
 *
 */
class ReflectionFunctionBak extends ReflectionFunctionAbstractBak
implements Reflector {
  ...
  public function isClosure() {
    return $this->src instanceof Closure;
  }
  ...
  public static function export($name, $ret=false) {
    $obj = new ReflectionFunctionBak($name);
    $str = (string)$obj;
    if ($ret) {
      return $str;
    }
  ...
  public function invoke() {
    $args = func_get_args();
    if ($this->isClosure()) {
      return hphp_invoke_method($this->src, get_class($this->src),
                                '__invoke', $args);
    }
    return hphp_invoke($this->src, $args);
  }
  ...
  public function invokeArgs($args) {
    ...
    return hphp_invoke($this->src, array_values($args));
  }
...
class ReflectionClass implements Reflector {
  ...
  public function getExtension() {
    return new ReflectionExtension($this->fetch('extension'));
  }
 ...
 *
 */
class ReflectionMethodBak extends ReflectionFunctionAbstractBak
implements Reflector {
  const IS_STATIC = 1;
  const IS_PUBLIC = 256;
  const IS_PROTECTED = 512;
   ...
   *
   * Constructs a new ReflectionMethodBak.
   *
  ...
  public function __construct($cls, $name = '') {
    ...
    $info = hphp_get_method_info($cls, $name);
  ...
  protected function updateInfo() {
    $this->info = hphp_get_method_info($this->class, $this->name);
  }
  ...
  public function getClosure($object = null) {
    if ($this->isStatic()) {
      ...
      if (!$object) {
        trigger_error('ReflectionMethod::getClosure() expects parameter 1'
  ...
  public function getAttributeRecursive($name) {
    ...
    $rm = new ReflectionMethodBak($p, $this->name);
    if ($rm->isPrivate()) {
      return null;
    }
