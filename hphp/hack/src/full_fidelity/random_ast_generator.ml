...
module Make (G : Hack_grammar_descriptor.Grammar) : AstGenerator = struct
   ...
   * becoming 100 *)
  let normalize lst =
    let find_large acc el = if el > acc then el else acc in
    let max = List.fold_left find_large 0 lst in
    if max = 0 then lst
    else List.map (fun x -> x * normalizing_factor / max) lst
  ...
  let gen_list start finish =
    let rec aux start finish acc =
      ...
      else aux start (finish - 1) (finish :: acc)
    in
    aux start (finish - 1) []
  ...
  and get_postfix_helper mem (nonterm, rule_index, pos, count) =
    if count = 0
      ...
      | G.Term _ when pos = last_index ->
        (mem, if count = 1 then [normalizing_factor] else [0])
      | G.Term _ ->
  ...
  let choose l =
    ...
    | _  -> begin
      ...
      if sum = 0 then None
        ...
        with Not_found -> failwith "wrong implementation of choose" end
  ...
  and get_string_postfix mem nonterm rule_index pos count =
    ...
    | G.NonTerm x ->
      let (mem, lst) =
        get_postfix_count_list mem nonterm rule_index pos count in
      match choose lst with
      | None -> ""
      | Some length ->
        let length = length + 1 in (* index to number of tokens *)
