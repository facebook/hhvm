...
std::unique_ptr<Aead> getCipher(CipherSuite suite) {
  ...
  switch (suite) {
    case CipherSuite::TLS_AES_128_GCM_SHA256:
      FIZZ_THROW_ON_ERROR(
          openssl::OpenSSLEVPCipher::makeCipher<AESGCM128>(cipher, err), err);
      break;
    case CipherSuite::TLS_AES_256_GCM_SHA384:
      FIZZ_THROW_ON_ERROR(
          openssl::OpenSSLEVPCipher::makeCipher<AESGCM256>(cipher, err), err);
      break;
    ...
    case CipherSuite::TLS_AEGIS_128X4_SHA256:
      FIZZ_THROW_ON_ERROR(
          libaegis::makeCipher<fizz::AEGIS128X4>(cipher, err), err);
      break;
#endif
    default:
