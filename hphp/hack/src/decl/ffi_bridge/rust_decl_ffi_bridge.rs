...
mod decl_ffi {
    ...
    extern "Rust" {
        ...
        unsafe fn hackc_free_decl_result(decl_result: DeclResult);
...
fn hackc_print_serialized_size<'a>(serialized: &Bytes) {
    println!("Decl-serialized size: {:#?}", serialized.0.len);
}

unsafe fn hackc_free_decl_result<'a>(decl_result: DeclResult<'a>) {
...
impl<'a> DeclResult<'a> {
    ...
    pub unsafe fn into_parts(self) -> (u64, Box<Bytes>, Box<Decls<'a>>) {
        (self.hash, self.serialized, self.decls)
    }
}
