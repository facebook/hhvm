...
static BIO *read_data(CVarRef var, bool *file = NULL) {
  if (var.isString() || var.isObject()) {
    ...
    if (svar.substr(0, 7) == "file://") {
      ...
      if (ret == NULL) {
        Logger::Warning("error opening the file, %s", svar.data());
      }
      return ret;
    }
...
#endif
    default:
      Logger::Warning("key type not supported in this PHP build!");
      break;
    }
    return true;
  }
    ...
    if (var.isResource()) {
      ...
      if (key) {
        ...
        if (!public_key && !is_priv) {
          Logger::Warning("supplied key param is a public key");
          return Object();
        }
        if (public_key && is_priv) {
          Logger::Warning("Don't know how to get public key from "
                          "this private key");
          return Object();
        }
        return var.toObject();
      ...
      if (public_key) {
        ...
        if (var.is(KindOfArray)) {
          ...
          if (!arr.exists(0LL) || !arr.exists(1LL)) {
            Logger::Warning("key array must be of the form "
...
public:
  ...
  static bool load_rand_file(const char *file, int *egdsocket, int *seeded) {
    ...
    *seeded = 0;
    ...
    if (file == NULL || !RAND_load_file(file, -1)) {
      if (RAND_status() == 0) {
        Logger::Warning("unable to load random state; not enough data!");
        return false;
      }
      return false;
    }
  ...
  static bool write_rand_file(const char *file, int egdsocket, int seeded) {
    ...
    if (file == NULL || !RAND_write_file(file)) {
      Logger::Warning("unable to write random state");
      return false;
    }
    return true;
  }
  ...
  bool generatePrivateKey() {
    ...
    if (priv_key_bits < MIN_KEY_LENGTH) {
      Logger::Warning("private key length is too short; it needs to be "
...
static bool add_oid_section(struct php_x509_request *req) {
  ...
  if (sktmp == NULL) {
    Logger::Warning("problem loading oid section %s", str);
    return false;
  }
  ...
  for (int i = 0; i < sk_CONF_VALUE_num(sktmp); i++) {
    ...
    if (OBJ_create(cnf->value, cnf->name, cnf->name) == NID_undef) {
      Logger::Warning("problem creating object %s=%s", cnf->name, cnf->value);
      return false;
    }
  }
  return true;
...
(const char *section_label, const char *config_filename, const char *section,
 LHASH *config) {
  ...
  if (!X509V3_EXT_add_conf(config, &ctx, (char*)section, NULL)) {
    Logger::Warning("Error loading %s section %s of %s",
...
static bool php_openssl_parse_config(struct php_x509_request *req,
  ...
  if (str && !ASN1_STRING_set_default_mask_asc(str)) {
    Logger::Warning("Invalid global string mask setting %s", str);
    return false;
  }
...
static STACK_OF(X509) *load_all_certs_from_file(const char *certfile) {
  ...
  if (!(in = BIO_new_file(certfile, "r"))) {
    Logger::Warning("error opening the file, %s", certfile);
    sk_X509_free(stack);
    goto end;
  }
  ...
  if (!(sk = PEM_X509_INFO_read_bio(in, NULL, NULL, NULL))) {
    Logger::Warning("error reading the file, %s", certfile);
    sk_X509_free(stack);
    goto end;
  }
  ...
  if (!sk_X509_num(stack)) {
    Logger::Warning("no certificates in file, %s", certfile);
    sk_X509_free(stack);
    goto end;
  }
  ret = stack;
...
static X509_STORE *setup_verify(CArrRef calist) {
  ...
  for (ArrayIter iter(calist); iter; ++iter) {
    ...
    if (stat(item.data(), &sb) == -1) {
      Logger::Warning("unable to stat %s", item.data());
      continue;
    }
    ...
    if ((sb.st_mode & S_IFREG) == S_IFREG) {
      ...
      if (file_lookup == NULL ||
          !X509_LOOKUP_load_file(file_lookup, item.data(),
                                 X509_FILETYPE_PEM)) {
        Logger::Warning("error loading file %s", item.data());
      } else {
        nfiles++;
      }
      file_lookup = NULL;
      ...
      if (dir_lookup == NULL ||
        ...
        Logger::Warning("error loading directory %s", item.data());
      } else {
        ndirs++;
      }
      dir_lookup = NULL;
  ...
  for (ArrayIter iter(items); iter; ++iter) {
    ...
    if (nid != NID_undef) {
      if (!X509_NAME_add_entry_by_NID(subj, nid, MBSTRING_ASC,
                                      ...
                                      -1, -1, 0)) {
        Logger::Warning("dn: add_entry_by_NID %d -> %s (failed)",
                        nid, item.data());
        return false;
      }
    } else {
      Logger::Warning("dn: %s is not a recognized name", index.data());
    }
  }
  return true;
}
  ...
  if (X509_REQ_set_version(csr, 0L)) {
    ...
    for (int i = 0; i < sk_CONF_VALUE_num(dn_sk); i++) {
      ...
      if (!X509_NAME_add_entry_by_txt(subj, type, MBSTRING_ASC,
        ...
        Logger::Warning("add_entry_by_txt %s -> %s (failed)", type, v->value);
        return false;
      }
      if (!X509_NAME_entry_count(subj)) {
        Logger::Warning("no objects specified in config file");
        return false;
      }
    }
...
bool f_openssl_csr_export_to_file(CVarRef csr, CStrRef outfilename,
  ...
  if (bio_out == NULL) {
    Logger::Warning("error opening file %s", outfilename.data());
    return false;
  }
...
Variant f_openssl_csr_new(CArrRef dn, Variant privkey,
  ...
  if (php_openssl_parse_config(&req, configargs.toArray(), strings)) {
    ...
    if (req.priv_key == NULL) {
      Logger::Warning("Unable to generate a private key");
    } else {
      csr = X509_REQ_new();
      if (csr && php_openssl_make_REQ(&req, csr, dn, extraattribs.toArray())) {
        ...
        if (req.request_extensions_section &&
          ...
          Logger::Warning("Error loading extension section %s",
                          req.request_extensions_section);
        } else {
          ret = true;
          if (X509_REQ_sign(csr, req.priv_key, req.digest)) {
            ...
            Logger::Warning("Error signing request");
          }
...
Variant f_openssl_csr_sign(CVarRef csr, CVarRef cacert, CVarRef priv_key,
  ...
  if (!cacert.isNull()) {
    ...
    if (ocert.isNull()) {
      Logger::Warning("cannot get cert from parameter 2");
      return false;
    }
  }
  Object okey = Key::Get(priv_key, false);
  if (okey.isNull()) {
    Logger::Warning("cannot get private key from parameter 3");
    return false;
  }
  X509 *cert = NULL;
  if (!ocert.isNull()) {
  ...
  if (cert && !X509_check_private_key(cert, pkey)) {
    Logger::Warning("private key does not correspond to signing cert");
    return false;
  }
  ...
  if (key == NULL) {
    Logger::Warning("error unpacking public key");
    goto cleanup;
  }
  int i;
  i = X509_REQ_verify(pcsr, key);
  if (i < 0) {
    Logger::Warning("Signature verification problems");
    goto cleanup;
  }
  if (i == 0) {
    Logger::Warning("Signature did not match the certificate request");
    goto cleanup;
  }
  ...
  if (!X509_sign(new_cert, pkey, req.digest)) {
    Logger::Warning("failed to sign it");
    goto cleanup;
  }
...
bool f_openssl_open(CStrRef sealed_data, Variant open_data, CStrRef env_key,
  ...
  if (okey.isNull()) {
    Logger::Warning("unable to coerce parameter 4 into a private key");
    return false;
  }
  EVP_PKEY *pkey = okey.getTyped<Key>()->m_key;
...
static bool openssl_pkcs12_export_impl(CVarRef x509, BIO *bio_out,
  ...
  if (ocert.isNull()) {
    Logger::Warning("cannot get cert from parameter 1");
    return false;
  }
  Object okey = Key::Get(priv_key, false);
  if (okey.isNull()) {
  ...
  if (cert && !X509_check_private_key(cert, key)) {
    Logger::Warning("private key does not correspond to cert");
    return false;
  }
...
bool f_openssl_pkcs12_export_to_file(CVarRef x509, CStrRef filename,
  ...
  if (bio_out == NULL) {
    Logger::Warning("error opening file %s", filename.data());
    return false;
  }
...
bool f_openssl_pkcs7_decrypt(CStrRef infilename, CStrRef outfilename,
  ...
  if (ocert.isNull()) {
    Logger::Warning("unable to coerce parameter 3 to x509 cert");
    goto clean_exit;
  }

  okey = Key::Get(recipkey.isNull() ? recipcert : recipkey, false);
  if (okey.isNull()) {
    Logger::Warning("unable to get private key");
    goto clean_exit;
  }

  in = BIO_new_file(infilename.data(), "r");
  if (in == NULL) {
    Logger::Warning("error opening the file, %s", infilename.data());
    goto clean_exit;
  }
  out = BIO_new_file(outfilename.data(), "w");
  if (out == NULL) {
    Logger::Warning("error opening the file, %s", outfilename.data());
    goto clean_exit;
  }
...
#endif
  default:
    Logger::Warning("Invalid cipher type `%ld'", cipherid);
    goto clean_exit;
  }
  if (cipher == NULL) {
    Logger::Warning("Failed to get cipher");
    goto clean_exit;
  }
...
bool f_openssl_pkcs7_sign(CStrRef infilename, CStrRef outfilename,
  ...
  okey = Key::Get(privkey, false);
  if (okey.isNull()) {
    Logger::Warning("error getting private key");
    goto clean_exit;
  }
  EVP_PKEY *key;
  ...
  if (ocert.isNull()) {
    Logger::Warning("error getting cert");
    goto clean_exit;
  }
  X509 *cert;
  cert = ocert.getTyped<Certificate>()->m_cert;
  ...
  infile = BIO_new_file(infilename.data(), "r");
  if (infile == NULL) {
    Logger::Warning("error opening input file %s!", infilename.data());
    goto clean_exit;
  }

  outfile = BIO_new_file(outfilename.data(), "w");
  if (outfile == NULL) {
    Logger::Warning("error opening output file %s!", outfilename.data());
    goto clean_exit;
  }

  p7 = PKCS7_sign(cert, key, others, infile, flags);
  if (p7 == NULL) {
    Logger::Warning("error creating PKCS7 structure!");
    goto clean_exit;
  }
...
Variant f_openssl_pkcs7_verify(CStrRef filename, int flags,
  ...
  if (in == NULL) {
    Logger::Warning("error opening the file, %s", filename.data());
    goto clean_exit;
  }
  ...
  if (!content.empty()) {
    ...
    if (dataout == NULL) {
      Logger::Warning("error opening the file, %s", content.data());
      goto clean_exit;
    }
  }
  ...
  if (PKCS7_verify(p7, others, store, datain, dataout, flags)) {
    ...
    if (!outfilename.empty()) {
      ...
      if (certout) {
        ...
        Logger::Warning("signature OK, but cannot open %s for writing",
                        outfilename.data());
        ret = -1;
      }
    }
...
static bool openssl_pkey_export_impl(CVarRef key, BIO *bio_out,
  ...
  if (okey.isNull()) {
    Logger::Warning("cannot get key from parameter 1");
    return false;
  }
  EVP_PKEY *pkey = okey.getTyped<Key>()->m_key;
...
bool f_openssl_private_decrypt(CStrRef data, Variant decrypted, CVarRef key,
  ...
  if (okey.isNull()) {
    Logger::Warning("key parameter is not a valid private key");
    return false;
  }
  EVP_PKEY *pkey = okey.getTyped<Key>()->m_key;
  ...
  default:
    Logger::Warning("key type not supported");
  }
...
bool f_openssl_private_encrypt(CStrRef data, Variant crypted, CVarRef key,
  ...
  Object okey = Key::Get(key, false);
  if (okey.isNull()) {
    Logger::Warning("key param is not a valid private key");
    return false;
  }
  EVP_PKEY *pkey = okey.getTyped<Key>()->m_key;
  int cryptedlen = EVP_PKEY_size(pkey);
...
bool f_openssl_public_decrypt(CStrRef data, Variant decrypted, CVarRef key,
  ...
  if (okey.isNull()) {
    Logger::Warning("key parameter is not a valid public key");
    return false;
  }
  EVP_PKEY *pkey = okey.getTyped<Key>()->m_key;
  int cryptedlen = EVP_PKEY_size(pkey);
...
Variant f_openssl_seal(CStrRef data, Variant sealed_data, Variant env_keys,
  ...
  if (nkeys == 0) {
    Logger::Warning("Fourth argument to openssl_seal() must be "
                    "a non-empty array");
    return false;
  }
  ...
  for (ArrayIter iter(pub_key_ids); iter; ++iter, ++i) {
    ...
    if (okey.isNull()) {
      Logger::Warning("not a public key (%dth member of pubkeys)", i);
      ret = false;
      goto clean_exit;
    }
...
bool f_openssl_sign(CStrRef data, Variant signature, CVarRef priv_key_id,
  ...
  Object okey = Key::Get(priv_key_id, false);
  if (okey.isNull()) {
    Logger::Warning("supplied key param cannot be coerced into a private key");
    return false;
  }
...
Variant f_openssl_verify(CStrRef data, CStrRef signature, CVarRef pub_key_id,
  ...
  if (okey.isNull()) {
    Logger::Warning("supplied key param cannot be coerced into a public key");
    return false;
  }
  ...
  return EVP_VerifyFinal(&md_ctx, (unsigned char *)signature.data(),
...
static time_t asn1_time_to_time_t(ASN1_UTCTIME *timestr) {
  if (timestr->length < 13) {
    Logger::Warning("extension author too lazy to parse %s correctly",
                    timestr->data);
    return (time_t)-1;
  }
...
Variant f_openssl_x509_read(CVarRef x509certdata) {
  ...
  if (ocert.isNull()) {
    Logger::Warning("supplied parameter cannot be coerced into "
                    "an X509 certificate!");
    return false;
  }
  return ocert;
