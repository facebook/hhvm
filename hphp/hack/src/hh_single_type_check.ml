...
let parse_options () =
  ...
  let options =
    [
      ...
      ( "--dump-global-tvenvs",
        Arg.Unit (set_mode Dump_global_tvenvs),
        " Print out the global environments generated by global inference."
        ^ " Requires setting --infer-missing." );
      ( "--dump-global-tvenvs-no-hhi",
        Arg.Unit (set_mode Dump_global_tvenvs_no_hhi),
        " Print out the global environments generated by global inference."
        ^ " Same as --dump-global-tvenvs, but don't print global environments"
        ^ " that result from type checking .hhi files."
        ^ " Requires setting --infer-missing." );
      ( "--merge-global-tvenvs",
        Arg.Unit (set_mode Merge_global_tvenvs),
        " Merge all global environments generated by global inference"
        ^ " and print the result. This will not print type variables"
        ^ " without any bounds after the merge process."
        ^ " Requires setting --infer-missing." );
      ( "--find-refs",
        Arg.Tuple
          [
...
let check_file_and_get_global_tvenvs opts errors files_info =
...
let check_file opts errors files_info =
  fst @@ check_file_and_get_global_tvenvs opts errors files_info
...
let handle_mode
  ...
  | Dump_global_tvenvs_no_hhi ->
    let (errors, global_tvenvs) =
      check_file_and_get_global_tvenvs tcopt parse_errors files_info
    in
    let should_log (pos, global_tvenv) =
      let file_relevant =
        match mode with
        | Dump_global_tvenvs_no_hhi
  ...
  | Merge_global_tvenvs ->
    ...
    in
    print_header "Merged environment";
    Typing_log.hh_show_env Pos.none env;

    print_header "Merge errors";
    List.iter (StateErrors.elements state_errors) ~f:(fun (var, errl) ->
        Printf.fprintf stderr "#%d\n" var;
        print_error_list error_format errl max_errors);
    Out_channel.flush stderr;

    print_header "Type checker errors";
    print_error_list error_format errors max_errors;
    if errors <> [] then exit 2
  | Decl_compare ->
    let filename = expect_single_file () in
