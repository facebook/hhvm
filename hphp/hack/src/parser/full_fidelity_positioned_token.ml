...
module MinimalLexer = Full_fidelity_lexer.WithToken(MinimalToken)
...
module LazyTrivia : sig
  ...
  let to_internal trivia =
    if Obj.is_int trivia then Packed (Obj.obj trivia) else Expanded (Obj.obj trivia)

  let from_internal = function
    | Packed trivia -> Obj.repr trivia
    | Expanded trivia -> Obj.repr trivia

  let pp formatter trivia =
    pp_internal_t formatter (to_internal trivia)

  let show trivia =
    show_internal_t (to_internal trivia)

  let trivia_kind_mask kind =
    1 lsl (62 - (TriviaKind.to_enum kind))

  let fold_kind acc kind =
    acc lor (trivia_kind_mask kind)
  ...
  let from (leading, trailing) =
    let internal = if has_special [leading; trailing]
      then Expanded (leading, trailing)
      else Packed (get_trivia_kinds [leading; trailing])
    in
    internal
    |> from_internal
  ...
  let update
    ...
    (update_func : Trivia.t list -> (Trivia.t list * Trivia.t list))
  : t =
  ...
  let update_leading trivia new_leading load_trailing =
    update trivia load_trailing new_leading snd (fun loaded -> (new_leading, loaded))

  let update_trailing trivia load_leading new_trailing =
    update trivia load_leading new_trailing fst (fun loaded -> (loaded, new_trailing))
...
let fold_width sum trivia =
  sum + (Trivia.width trivia)
...
let has_trivia_kind token =
  LazyTrivia.has_trivia_kind token.trivia
...
let with_leading new_leading token =
  { token with
    trivia = LazyTrivia.update_leading token.trivia new_leading (fun () -> trailing token) }

let with_trailing new_trailing token =
  { token with
    trivia = LazyTrivia.update_trailing token.trivia (fun () -> leading token) new_trailing }
...
let concatenate b e =
  { b with
    ...
    trivia = LazyTrivia.update_trailing b.trivia (fun () -> leading b) (trailing e);
  }
