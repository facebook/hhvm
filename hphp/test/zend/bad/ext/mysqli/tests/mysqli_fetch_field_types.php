<?php
	$test_table_name = 'test_mysqli_fetch_field_types_table_1'; require('table.inc');

	function mysqli_field_datatypes($link, $sql_type, $php_value, $php_type, $datatypes, $default_charset="latin1") {
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_fetch_field_types_table_1")) {
			printf("[001] %s, [%d] %s\n", $sql_type,
				mysqli_errno($link), mysqli_error($link));
			return false;
		}

		$create = sprintf("CREATE TABLE test_mysqli_fetch_field_types_table_1(id %s) DEFAULT CHARSET %s", $sql_type, $default_charset);
		...
		if (is_string($php_value))
			$sql = sprintf("INSERT INTO test_mysqli_fetch_field_types_table_1(id) VALUES ('%s')", $php_value);
		else
			$sql = sprintf("INSERT INTO test_mysqli_fetch_field_types_table_1(id) VALUES (%s)", $php_value);
		...
		if (!$res = mysqli_query($link, 'SELECT id FROM test_mysqli_fetch_field_types_table_1')) {
			printf("[004] %s, [%d] %s\n", $sql_type,
				mysqli_errno($link), mysqli_error($link));
			return false;
		}
