...
let test () =
  ...
  let (options: ServerArgs.options) = ServerArgs.set_config
    options
    [
      ("timeout", "747");
      ("informant_min_distance_restart", "711");
    ]
  in
  let config, local_config = ServerConfig.load hhconfig_path options in
  let timeout = TypecheckerOptions.timeout (ServerConfig.typechecker_options config) in
  if not (timeout = 747) then
    Test.fail "Global config value not overridden!";

  let informant_min_distance_restart =
    local_config.ServerLocalConfig.informant_min_distance_restart in
