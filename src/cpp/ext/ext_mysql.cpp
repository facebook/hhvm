...
static RequestLocal<MySQLRequestData> s_mysql_data;
...
MYSQL *MySQL::GetConn(CVarRef link_identifier, MySQL **rconn /* = NULL */) {
  ...
  if (ret == NULL) {
    Logger::Warning("supplied argument is not a valid MySQL-Link resource");
  }
  if (rconn) {
    *rconn = mySQL;
  }
...
MySQL::MySQL(const char *host, int port, const char *username,
             const char *password) : m_port(port) {
  if (host) m_host = host;
...
static MySQLResult *get_result(CVarRef result) {
  ...
  if (res == NULL || (res->get() == NULL && !res->isLocalized())) {
    Logger::Warning("supplied argument is not a valid MySQL result resource");
  }
  return res;
}
...
static Variant php_mysql_do_query_general(CStrRef query, CVarRef link_id,
  ...
  if (RuntimeOption::MySQLReadOnly &&
    ...
    Logger::Verbose("cpp/ext_mysql: write query not executed [%s]",
                    query.data());
  ...
  if (RuntimeOption::EnableStats && RuntimeOption::EnableSQLStats) {
    ...
    if (size > 2) {
      ...
      if (size == 2) {
        ...
        Logger::Error("Unable to record MySQL stats with: %s", query.data());
  ...
  SlowTimer timer(RuntimeOption::MySQLSlowQueryThreshold,
                  "cpp/ext_mysql: slow query", query.data());
  unsigned long tid = mysql_thread_id(conn);
  if (mysql_real_query(conn, query.data(), query.size())) {
    Logger::Verbose("cpp/ext_mysql: failed executing [%s] [%s]", query.data(),
                    mysql_error(conn));
        ...
        if (!same(ret, false)) {
          ...
          if (connected) {
            ...
            if (mysql_real_query(connected, killsql.c_str(), killsql.size())) {
              Logger::Error("Unable to kill thread %llu", tid);
            }
          }
  ...
  Logger::Verbose("cpp/ext_mysql: successfully executed [%dms] [%s]",
  ...
  return Object(NEW(MySQLResult)(mysql_result));
}
...
Variant f_mysql_list_dbs(CVarRef link_identifier /* = null */) {
  ...
  if (!res) {
    Logger::Warning("Unable to save MySQL query result");
    return false;
  }
...
static Variant php_mysql_fetch_hash(CVarRef result, int result_type) {
  if ((result_type & MYSQL_BOTH) == 0) {
    throw InvalidArgumentException("result_type", result_type);
  }
...
Variant f_mysql_result(CVarRef result, int row,
  ...
  if (res->isLocalized()) {
    ...
    if (row < 0 || row >= (int)mysql_num_rows(mysql_result)) {
      Logger::Warning("Unable to jump to row %ld on MySQL result index %ld",
  ...
  if (!field.isNull()) {
    if (field.isString()) {
      ...
      if (!found) { /* no match found */
        Logger::Warning("%s%s%s not found in MySQL result index %ld",
      ...
      if (field_offset < 0 ||
        ...
        Logger::Warning("Bad column offset specified");
        return false;
      }
    }
  }
...
bool MySQLResult::seekField(int64 field) {
  if (field < 0 || field >= getFieldCount()) {
    Logger::Warning("Field %ld is invalid for MySQL result index %ld",
                    field, o_getId());
    return false;
  }
