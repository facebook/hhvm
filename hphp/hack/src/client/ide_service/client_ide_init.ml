...
type 'a outcome =
  | Success of 'a
  | Failure of Client_ide_message.rich_error
  | Skip of string

let error_from_load_error (load_error : Saved_state_loader.LoadError.t) :
    Client_ide_message.rich_error =
  let data =
    Some
      (`Assoc
        [
  ...
  in
  let reason =
    Client_ide_utils.make_rich_error
  ...
  if Saved_state_loader.LoadError.is_error_actionable load_error then
    {
      reason with
      Client_ide_message.is_actionable = true;
      short_user_message =
...
let init_via_lsp
    ~(naming_table_load_info :
       Client_ide_message.Initialize_from_saved_state.naming_table_load_info
  ...
  | Some naming_table_load_info ->
    let open Client_ide_message.Initialize_from_saved_state in
      ...
      (Success
         {
           ...
           files_changed = changed_files_since_saved_state;
         })
...
let init_via_build
  ...
  if not (Server_config.ide_fall_back_to_full_index config) then begin
    ...
    | { Naming_table_builder_ffi_externs.exit_status; time_taken_secs = _; _ }
      ...
      in
      Lwt.return
        (Failure (Client_ide_utils.make_rich_error "full_index_error" ~data))
  end
  ...
  if not local_config.Server_local_config.ide_load_naming_table_on_disk then begin
    ...
    | Ok (project_metadata, _telemetry) -> begin
      ...
      | Error err ->
        ...
        Lwt.return
          (Failure (Client_ide_utils.make_rich_error "find_failed" ~data))
    end
  end
...
let map_attempt
  ...
  try%lwt
    ...
    | Failure reason ->
      let { Client_ide_message.category; data; _ } = reason in
  ...
  | exn ->
    ...
    let reason = Client_ide_utils.make_rich_error ~e (key ^ " exn") in
...
let init
    ...
    (init_result, Client_ide_message.rich_error) result Lwt.t =
  let start_time = Unix.gettimeofday () in
  let {
    Client_ide_message.Initialize_from_saved_state.root;
    naming_table_load_info;
  ...
  let sienv =
    Symbol_index.initialize
      ~gleanopt
      ~namespace_map:tcopt.Global_options.po.Parser_options.auto_namespace_map
      ~provider_name:
  ...
  let prev =
    (Telemetry.create (), Client_ide_utils.make_rich_error "trying...")
  in
