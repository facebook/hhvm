...
module Make (S : SearchUtils.Searchable) = struct
  ...
  let strip_special_characters key = Str.global_replace key_regex "" key

  let get_index_for_type type_ = TMap.find_unsafe type_ !term_indexes
  ...
  let check_if_matches_uppercase_chars needle haystack =
    ...
    String.iter
      begin
        fun c ->
        if !first_letter || Char.uppercase_ascii c = c then
          uppercase := !uppercase ^ Char.escaped (Char.lowercase_ascii c);
        first_letter := false
      end
      haystack;

    if String.compare (String.lowercase_ascii needle) !uppercase = 0 then
      true
    else
      false
  ...
  let rec is_cs ?(ni = 0) ?(hi = 0) ?(score = 0) needle haystack =
    if String.length needle > String.length haystack then
      ...
      let cmp =
        Char.compare
          (Char.lowercase_ascii needle.[ni])
          (Char.lowercase_ascii haystack.[hi])
      in
      (* Increment score if characters are not equal case *)
      let score =
        if Char.compare needle.[ni] haystack.[hi] = 0 then
          score
        else
          score + 1
      in
      if cmp = 0 then
        is_cs ~ni:(ni + 1) ~hi:(hi + 1) ~score needle haystack
      else if ni = 0 then
        is_cs ~ni ~hi:(hi + 1) ~score needle haystack
      else
        is_cs ~ni ~hi:(hi + 1) ~score:(score + 2) needle haystack
  ...
  let is_substring needle haystack =
    ...
    (try Str.search_forward re haystack 0 with Not_found -> -1)
  ...
  let add_letter_to_index letter word used type_ =
    if not (CSet.mem letter used) then (
      ...
      ( if Hashtbl.mem letter_val letter then
  ...
  let is_letter_important word idx =
    ...
    if idx <= 1 then
      true
    (* First 2 letters *)
    else if Char.uppercase_ascii c = c then
      true
    (* Uppercase letter *)
    else if word.[idx - 1] = '_' then
      true
    (* Preceding underscore *)
    else if word.[idx - 1] = '\\' then
      true
    (* Namespaced *)
    else
      false
  ...
  let rec add_letters_to_index ?(x = 0) ?(used = CSet.empty) word max_num type_
      =
    if x >= String.length word || CSet.cardinal used >= max_num then
      ()
    else if is_letter_important word x then
      let letter = Char.lowercase_ascii word.[x] in
      let used = add_letter_to_index letter word used type_ in
      add_letters_to_index ~x:(x + 1) ~used word max_num type_
    else
      add_letters_to_index ~x:(x + 1) ~used word max_num type_
  ...
  let add_terms_to_index type_ terms =
    SSet.iter begin
                fun term -> add_letters_to_index term 10 type_
              end terms
    ...
    SSet.iter
      begin
        ...
        Hashtbl.replace
          !term_lookup
          term
          (Relative_path.Set.remove old_val file)
      end
      remove_terms;
    SSet.iter
      begin
  ...
  let process_term key name pos type_ defs_acc =
    let existing_defmap =
      (try TMap.find_unsafe type_ defs_acc with Not_found -> SMap.empty)
    in
    let existing_def_list =
      (try SMap.find_unsafe key existing_defmap with Not_found -> [])
    in
    let updated_defmap =
      SMap.add
        key
        ({ name; pos; result_type = type_ } :: existing_def_list)
        existing_defmap
    in
  ...
  let index_files files =
    if TMap.is_empty !term_indexes then
      term_indexes :=
        List.fold_left
          all_types
          ~f:begin
               fun acc x -> TMap.add x (Hashtbl.create 30) acc
             end
          ~init:TMap.empty;
    List.iter files (fun file ->
        let new_terms =
          (try SearchKeys.find_unsafe file with Not_found -> TMap.empty)
        in
        let old_terms =
          try Hashtbl.find !old_search_terms file
  ...
  (* Catches if the query is an empty string *)
  ...
  let get_terms_from_string_and_type strings =
    List.fold_left
      ...
      ~f:
        begin
          fun acc ((str, type_), score) ->
          let files =
            try Hashtbl.find !term_lookup str
            with Not_found -> Relative_path.Set.empty
          in
          Relative_path.Set.fold files ~init:acc ~f:(fun file acc ->
              let defmap =
