...
folly::SemiFuture<folly::Unit> apache::thrift::Client<::cpp2::MyLeaf>::fbthrift_semifuture_do_leaf(apache::thrift::RpcOptions& rpcOptions) {
  ...
  return fbthrift_semifuture_do_leaf(std::move(semifuture), rpcOptions, std::move(wrappedCallbackAndContextStack.first), wrappedCallbackAndContextStack.second, std::move(ctxAndHeader.second));
}

folly::SemiFuture<folly::Unit> apache::thrift::Client<::cpp2::MyLeaf>::fbthrift_semifuture_do_leaf(folly::SemiFuture<typename apache::thrift::detail::FutureCallbackHelper<folly::Unit>::PromiseResult> semifuture, apache::thrift::RpcOptions& rpcOptions, apache::thrift::RequestClientCallback::Ptr wrappedCallback, apache::thrift::ContextStack* contextStack, std::shared_ptr<::apache::thrift::transport::THeader> header) {
  using CallbackHelper = apache::thrift::detail::FutureCallbackHelper<folly::Unit>;
  apache::thrift::SerializedRequest request = fbthrift_serialize_do_leaf(rpcOptions, *header, contextStack);
#if FOLLY_HAS_COROUTINES
  const bool shouldProcessClientInterceptors = contextStack && contextStack->shouldProcessClientInterceptors();
  if (!shouldProcessClientInterceptors) {
    fbthrift_send_do_leaf(std::move(request), rpcOptions, std::move(header), std::move(wrappedCallback));
