...
{
#ifndef HHVM
    ALLOC_HASHTABLE(pcre_globals->pcre_cache);
...
# endif
    ...
    std::string regex_string(regex, regex_len);
    auto it = PCRE_G(pcre_cache).find(regex_string);
    if (it != PCRE_G(pcre_cache).end()) {
        pce = &(it->second);
		/*
	 ...
	 * ones).
	 */
	if (PCRE_G(pcre_cache).size() == PCRE_CACHE_SIZE) {
		int num_clean = PCRE_CACHE_SIZE / 8;
        auto start = PCRE_G(pcre_cache).begin();
        auto end = PCRE_G(pcre_cache).begin();
        std::advance(end, num_clean);
		PCRE_G(pcre_cache).erase(start, end);
	}
    ...
    std::string regex_str(regex, regex_len);
    assert(PCRE_G(pcre_cache).find(regex_str) == PCRE_G(pcre_cache).end());
    PCRE_G(pcre_cache)[regex_str] = new_entry;
    pce = &(PCRE_G(pcre_cache).find(regex_str)->second);
...
static PHP_FUNCTION(preg_quote)
{
	...
	*q = '\0';
	...
	RETVAL_STRINGL((const char*) erealloc(out_str, q - out_str + 1), q - out_str, 0);
}
/* }}} */
