...
let to_user_error = function
  ...
  | Missing_arrow { pos; cname } ->
    User_error.make
      ...
      []
  | Disallowed_xhp_type { pos; ty_name } ->
    User_error.make
  ...
  | Error_name_already_bound { pos; name; prev_name; prev_pos } ->
    ...
    User_error.make
      ...
      reasons
  | Invalid_fun_pointer { pos; name } ->
    User_error.make
      Error_code.(to_enum InvalidFunPointer)
      ( pos,
        Format.sprintf
          "Unbound global function: %s is not a valid name for fun()"
        @@ Markdown_lite.md_codify
        @@ Render.strip_ns name )
      []
  ...
  | Illegal_fun pos ->
    User_error.make
      ...
      []
  | Illegal_member_variable_class pos ->
    User_error.make
  ...
  | Prop_without_typehint { pos; prop_name; vis } ->
    User_error.make
      ...
      ( pos,
        Format.sprintf
          "Please add a type hint `%s SomeType %s`"
          (Render.vis_to_string vis)
          prop_name )
      []
  | Invalid_require_implements pos ->
    User_error.make
      Error_code.(to_enum InvalidReqImplements)
      (pos, "Only traits may use `require implements`")
      []
  | Invalid_require_extends pos ->
    User_error.make
  ...
  | Did_you_mean { name; suggest_pos; suggest_name; pos } ->
    ...
    User_error.make
      ...
      ( pos,
        Format.sprintf "Could not find %s."
        @@ Markdown_lite.md_codify
        @@ Render.strip_ns name )
      [
        Render.(
  ...
  | Using_internal_class { pos; class_name } ->
    User_error.make
      ...
      ( pos,
        Format.sprintf
          "%s is an implementation internal class that cannot be used directly"
        @@ Markdown_lite.md_codify class_name )
      []
  | Too_few_type_arguments pos ->
    User_error.make
      Error_code.(to_enum TooFewTypeArguments)
      (pos, "Too few type arguments for this type")
      []
  | Method_needs_visibility pos ->
    User_error.make
  ...
  | Xhp_optional_required_attr { pos; attr_name } ->
    User_error.make
      ...
      ( pos,
        Format.sprintf
          "XHP attribute %s cannot be marked as nullable and `@required`"
        @@ Markdown_lite.md_codify attr_name )
      []
  | Xhp_required_with_default { pos; attr_name } ->
    User_error.make
      ...
      ( pos,
        Format.sprintf
          "XHP attribute %s cannot be marked `@required` and provide a default"
        @@ Markdown_lite.md_codify attr_name )
      []
  | Array_typehints_disallowed pos ->
    User_error.make
    ...
    User_error.make
      ...
      [
        ( Pos_or_decl.of_raw_pos meth_pos,
          Format.sprintf
            "But this method is %s"
            (Markdown_lite.md_codify @@ Render.vis_to_string vis) );
      ]
