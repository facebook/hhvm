...
function handle_error_hint($_errno, $msg, ...) {
  $matches = null;
  $pat1 =
    '/Hack Array Compat: Value returned from ([a-z_]+\(\)) must be of type '.
    '(HH\\\\[dv]array|array), ([a-zA-Z0-9]+) given/';
  if (preg_match_with_matches($pat1, $msg, inout $matches)) {
    echo "{$matches[1]}: return notice (wanted: {$matches[2]} ".
         "got: {$matches[3]})!\n";
    return true;
  }

  $pat2 =
    '/Hack Array Compat: Argument 1 to ([a-z_]+\(\)) must be of type '.
    '(HH\\\\[dv]array|array), ([a-zA-Z0-9]+) given/';
  if (preg_match_with_matches($pat2, $msg, inout $matches)) {
    echo "{$matches[1]}: argument notice (wanted: {$matches[2]} ".
         "got: {$matches[3]})!\n";
    return true;
  }

  $pat3 =
    '/class_meth Compat: Value returned from function ([a-z_]+\(\)) must be '.
    'of type (HH\\\\[dv]array|array), clsmeth given/';
  if (preg_match_with_matches($pat3, $msg, inout $matches)) {
    echo "{$matches[1]}: return notice (wanted: {$matches[2]} ".
         "got: clsmeth)!\n";
    return true;
  }
  ...
  $pat4 =
    '/class_meth Compat: Argument 1 passed to ([a-z_]+\(\)) must be of type'.
    ' (HH\\\\[dv]array|array), clsmeth given/';
  if (preg_match_with_matches($pat4, $msg, inout $matches)) {
    echo "{$matches[1]}: argument notice (wanted: {$matches[2]} ".
         "got: clsmeth)!\n";
    return true;
  }
...
class Info { static bool $SawConvert = false; static bool $SawCompare = false; }
function handle_compare_error($_errno, $msg, ...) {
  if (!Info::$SawConvert && $msg === "Implicit clsmeth to varray conversion") {
    Info::$SawConvert = true;
    return true;
  }
  $pat =
    '/Hack Array Compat: Comparing [dv]array and [dv]array( relationally)?/';
  if (!Info::$SawCompare && preg_match($pat, $msg)) {
    Info::$SawCompare = true;
    return true;
  }
  return false;
}
...
<<__NEVER_INLINE>> function C(bool $v) {
  $marks = (Info::$SawConvert ? '*' : '').(Info::$SawCompare ? '+' : '');
  printf(" %-4s |", ($v ? 'T' : 'F').$marks);
  Info::$SawConvert = false;
  Info::$SawCompare = false;
}
...
function handle_is_error($_errno, $msg, ...) {
  if ($msg === 'Hack Array Compat: is_vec() called on varray') {
    echo '(notice!) ';
  }
}
...
<<__EntryPoint>>
function main() {
  set_error_handler('handle_error_hint');
  ...
  set_error_handler('handle_compare_error');
  ...
  set_error_handler('handle_is_error');
