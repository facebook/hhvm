...
// Helper to manage t_namespace objects for tests.
class namespace_map {
 public:
  t_namespace* add(const std::string& lang, const std::string& ns) {
    auto ptr = std::make_unique<t_namespace>(lang, ns);
    t_namespace* raw = ptr.get();
    storage_.push_back(std::move(ptr));
    map_[lang] = raw;
    return raw;
  }

  const std::map<std::string, t_namespace*>& get() const { return map_; }

 private:
  std::vector<std::unique_ptr<t_namespace>> storage_;
  std::map<std::string, t_namespace*> map_;
};
...
TEST(PackageGeneratorTest, common_package) {
  ...
  namespaces.add("cpp2", "foo.bar.baz");
  namespaces.add("hack", "baz");

  auto get_common_pkg = [&]() {
    return codemod::package_name_generator_util::from_namespaces(
               namespaces.get())
        .find_common_package();
  };
  ...
  namespaces.add("python", "foo.bar.BAZ");
   ...
   */
  namespaces.add("java", "org.apache.foobar");
  EXPECT_EQ(get_common_pkg(), "apache.org/foo/bar/baz");

  // Common namespace with domain
  namespaces.add("cpp2", "facebook.foo.bar.baz");
   ...
   */
  namespaces.add("python", "meta.foo.bar.baz");
  EXPECT_EQ(get_common_pkg(), "meta.com/foo/bar/baz");
  ...
  namespaces.add("hack", "apache.foobar");
  EXPECT_EQ(get_common_pkg(), "meta.com/foo/bar/baz");
}

TEST(PackageGeneratorTest, ns_with_language_identifiers) {
  namespace_map namespaces;
  namespaces.add("cpp2", "foo.test_cpp2.baz_cpp2");
  namespaces.add("hack", "baz");
  ...
  namespaces.add("python", "foo.python_test.baz");
  /*
  ...
  namespaces.add("python", "foo.test_cpp2.baz_python_cpp2.python");
  EXPECT_EQ(get_common_pkg(), "meta.com/foo/test_cpp2/baz_cpp2");
}

TEST(PackageGeneratorTest, common_identifiers) {
  namespace_map namespaces;
  namespaces.add("cpp2", "apache.xyz.foo.abc");
  namespaces.add("hack", "abc.apache.xyz.foo");
  namespaces.add("py3", "apache.foo");
  ...
  namespaces.add("py3", "meta.foo.abc.xyz");
...
TEST(PackageGeneratorTest, longest_package) {
  namespace_map namespaces;
  namespaces.add("cpp2", "apache.abcdef.ghij");
  namespaces.add("hack", "xyz.abc.foo");
  namespaces.add("py3", "foo");
  ...
  namespaces.add("py3", "facebook.abcdef.ghi");
