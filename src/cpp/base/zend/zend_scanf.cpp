 ...
 *----------------------------------------------------------------------
*/
static void ValidateFormat(const char *format, int numVars, int *totalSubs) {
...
xpgCheckDone:
    ...
    badSet:
      if (nassign != staticAssign) free((char *)nassign);
      throw InvalidArgumentException("format", "Unmatched [ in format string");

    default:
      if (nassign != staticAssign) free((char *)nassign);
      throw InvalidArgumentException
        (0, "Bad scan conversion character \"%c\"", *ch);
    }
...
void string_sscanf(const char *string, const char *format, int numVars,
   ...
   */
  ValidateFormat(format, numVars, &totalVars);
