...
struct ContinueCommand : public VSCommand {
  VS_COMMAND_COMMON_NOTARGET(ContinueCommand, CommandTarget::WorkItem, false)

public:
  // Client-initiated continues use CommandTarget::None so that a DAP response
  // is sent back per the protocol spec. Internal resumes created by
  // createInstance() use CommandTarget::WorkItem which suppresses the response.
  CommandTarget commandTarget() override { return m_target; }
