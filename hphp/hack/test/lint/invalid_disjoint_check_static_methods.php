...
/* STATIC METHODS */
class C {
  public static function contains<<<__NonDisjoint>> T1, <<__NonDisjoint>> T2>(
    Traversable<T1> $_t,
    T2 $_v,
  ): bool {
    return false;
  ...
  public static function shouldDeleteAssoc(
    dict<int, Map<string, mixed>> $table_data,
    Map<string, mixed> $assoc,
  ): bool {
    return !self::contains($table_data, (int)$assoc['id1']);
  }
}
