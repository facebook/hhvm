...
module type RefsType = sig
  ...
  val set_client_request : 'a Server_command_types.t option -> unit

  val set_client_response : 'a option -> unit

  val get_new_client_type : unit -> connection_type option

  val get_client_request : unit -> 'a Server_command_types.t option
...
let read_client_msg c =
  let metadata = { Server_command_types.from = "test"; desc = "cmd" } in
