...
let describe_mismatch (mismatch_info : Monitor_utils.build_mismatch_info option)
    : string =
  match mismatch_info with
  | None -> ""
  | Some
      { Monitor_utils.existing_version; existing_argv; existing_launch_time; _ }
    ->
    Printf.sprintf
...
let rec connect
  ...
  let conn =
    Monitor_connection.connect_once
      ~tracker
      ~timeout
  ...
  | Error e ->
    ...
    progress_callback None;
    (match e with
    | Monitor_utils.Server_died
    | Monitor_utils.(Connect_to_monitor_failure { server_exists = true; _ }) ->
    ...
    | Monitor_utils.Server_dormant_out_of_retries ->
      Printf.eprintf
    ...
    | Monitor_utils.Server_dormant ->
      Printf.eprintf
        ...
        ^^ " know what you're doing, maybe try --force-dormant-start true\n%!");
      raise Exit_status.(Exit_with No_server_running_should_retry)
    | Monitor_utils.Build_id_mismatched_client_must_terminate mismatch_info ->
      Printf.eprintf
    ...
    | Monitor_utils.Build_id_mismatched_monitor_will_terminate mismatch_info_opt
      ->
      Printf.eprintf
