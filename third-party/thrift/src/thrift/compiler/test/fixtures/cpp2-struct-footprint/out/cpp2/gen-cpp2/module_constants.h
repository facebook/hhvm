/**
 * Autogenerated by Thrift for thrift/compiler/test/fixtures/cpp2-struct-footprint/src/module.thrift
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated @nocommit
 */
#pragma once

#include <thrift/lib/cpp2/gen/module_constants_h.h>

#include "thrift/compiler/test/fixtures/cpp2-struct-footprint/gen-cpp2/module_types.h"

namespace cpp2_struct_footprint {
/** Glean {"file": "thrift/compiler/test/fixtures/cpp2-struct-footprint/src/module.thrift"} */
namespace module_constants {

  FOLLY_EXPORT ::std::string_view _fbthrift_schema_c3dcd37c4dc233e6();
  FOLLY_EXPORT ::folly::Range<const ::std::string_view*> _fbthrift_schema_c3dcd37c4dc233e6_includes();
  FOLLY_EXPORT ::folly::Range<const ::std::string_view*> _fbthrift_schema_c3dcd37c4dc233e6_uris();

} // namespace module_constants
} // namespace cpp2_struct_footprint
