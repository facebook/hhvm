...
impl<R: Reason> Default for SubtypeEnv<R> {
    fn default() -> Self {
        SubtypeEnv {
            this_ty: None,
...
fn simp_ty<R: Reason>(
    ...
    match ety_sup.deref() {
            ...
            match ety_sub.deref() {
                ...
                Toption(ty_sub_inner) => {
                    ...
                    if ty_sub_inner.is_nonnull() {
                        Prop::subtype_ty(ety_sub, ety_sup)
                    } else {
                        let ty_null = Ty::null(r_sub.clone());
                        simp_conj_sub(cfg, env, &ty_sub_inner, &ty_null, &ety_sup)
                    }
                }
                Tvar(tv_sub) if tv_sub == tv_sup => Prop::valid(),
                _ => Prop::subtype_ty(ety_sub, ety_sup),
            }
        }
        ...
        Tunion(ty_sups) => {
            ...
            match ety_sub.deref() {
                ...
                Toption(ty_sub_inner) => {
                    ...
                    if p.is_unsat() { Prop::invalid(None) } else { p }
                }
                _ => {
        ...
        Tgeneric(tp_name_sup, _) => match ety_sub.deref() {
            ...
            Tgeneric(tp_name_sub, _) if tp_name_sub == tp_name_sup => Prop::valid(),
            Tvar(_) | Tunion(_) => simp_ty_common(cfg, env, &ety_sub, &ety_sup),
            _ if cfg.ignore_generic_params => Prop::subtype_ty(ety_sub, ety_sup),
        ...
        Tnonnull => match ety_sub.deref() {
            Tprim(p_sub) => {
                use Prim::*;
                match p_sub {
                    Tnull | Tvoid => Prop::invalid(None),
                    Tint | Tbool | Tfloat | Tstring | Tresource | Tnum | Tarraykey | Tnoreturn => {
                        Prop::valid()
                    }
                }
            }
            Tnonnull | Tfun(_) | Tclass(_, _, _) => Prop::valid(),
        ...
        Tprim(p_sup) => match ety_sub.deref() {
            Tprim(p_sub) => simp_prim(p_sub, p_sup),
...
fn simp_ty_common<R: Reason>(
    ...
    match ty_sub.deref() {
        Tvar(tv) => simp_tvar_ty(cfg, env, ty_sub.reason(), tv, &ty_sup),
        Tprim(p_sub) => {
        ...
        Tgeneric(_, _) if cfg.ignore_generic_params => Prop::subtype_ty(ty_sub, ty_sup),
        Tgeneric(tp_name_sub, _) => {
            simp_typaram_ty(cfg, env, (ty_sub.reason(), tp_name_sub), ty_sup)
        }
        Tclass(_, _, _) | Toption(_) | Tnonnull | Tfun(_) | Tany => Prop::invalid(None),
    }
}
...
fn simp_typaram_ty<R: Reason>(
        ...
        .try_add_visited_generic_sub(tp_sub.1.clone(), &ty_sup)
    {
        ...
        // cycle so fail
        GoalResult::AlreadyVisited => Prop::invalid(None),
        ...
        .try_add_visited_generic_sup(tp_sup.1.clone(), &ty_sub)
    {
        ...
        GoalResult::NewGoal => {
            ...
            if prop.is_valid() {
                ...
                prop.disj(Prop::subtype_ty(ty_sub, ty_sup), None)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_tvar_ty() {
        let mut env = SubtypeEnv::default();
    ...
    fn test_prim() {
        ...
        // int <: arraykey
        let ty_int = Ty::int(NReason::none());
        assert!(simp_ty(&cfg, &mut env, &ty_int, &ty_arraykey).is_valid());
        // string <: arraykey
        let ty_string = Ty::string(NReason::none());
        assert!(simp_ty(&cfg, &mut env, &ty_string, &ty_arraykey).is_valid());

        // Subtypes of num
        let ty_num = Ty::num(NReason::none());
        // int <: num
        assert!(simp_ty(&cfg, &mut env, &ty_int, &ty_num).is_valid());
        // float <: num
        let ty_float = Ty::float(NReason::none());
        assert!(simp_ty(&cfg, &mut env, &ty_float, &ty_num).is_valid());

        // num === (int | float)
        let ty_num_as_union = Ty::union(NReason::none(), vec![ty_int.clone(), ty_float.clone()]);
        assert!(simp_ty(&cfg, &mut env, &ty_num, &ty_num_as_union).is_valid());

        // Subtypes of nonnull
        let ty_nonnull = Ty::nonnull(NReason::none());
        // int <: nonnull
        assert!(simp_ty(&cfg, &mut env, &ty_int, &ty_nonnull).is_valid());
        // float <: nonnull
        assert!(simp_ty(&cfg, &mut env, &ty_int, &ty_nonnull).is_valid());
        // string <: nonnull
        assert!(simp_ty(&cfg, &mut env, &ty_int, &ty_nonnull).is_valid());
        ...
        assert!(simp_ty(&cfg, &mut env, &ty_null, &ty_nonnull).is_unsat());
        // void </: nonnull
        let ty_void = Ty::void(NReason::none());
        assert!(simp_ty(&cfg, &mut env, &ty_void, &ty_nonnull).is_unsat());
    }
    ...
    fn test_disjs_sub() {
        ...
        assert!(simp_disjs_sub(&cfg, &mut env, ty_subs_all_ok.iter(), &ty_num).is_valid());
        assert!(simp_disjs_sub(&cfg, &mut env, ty_subs_one_ok.iter(), &ty_num).is_valid());
        assert!(simp_disjs_sub(&cfg, &mut env, ty_subs_none_ok.iter(), &ty_num).is_unsat());
    }
    ...
    fn test_typaram_visited() {
        ...
        env.visited_goals
            ...
            .1
            .insert(ty_int.clone());
        assert!(simp_typaram_ty(&cfg, &mut env, (&tp_reason, &tp), ty_int.clone()).is_unsat());
        ...
        env.visited_goals
            ...
            .0
            .insert(ty_int.clone());
        assert!(simp_ty_typaram(&cfg, &mut env, ty_int, (&tp_reason, &tp)).is_unsat());
    }
    ...
    fn test_ty_param_sup() {
        ...
        assert!(simp_typaram_ty(&cfg, &mut env, (&tp_reason, &tp_name), ty_arraykey).is_valid());
    }
    ...
    fn test_ty_param_sub() {
        ...
        assert!(simp_ty_typaram(&cfg, &mut env, ty_int, (&tp_reason, &tp_name)).is_valid());
    }
}
