...
module WithExpressionAndDeclAndTypeParser
  ...
  let rec parse_statement parser =
    ...
    | Function ->
      ...
      parse_possible_php_function parser missing
    | Abstract
    | Final
    | Interface
    | Trait
  ...
  and parse_possible_php_function parser attribute_specification =
    ...
    *)
    ...
    | _ ->
      with_decl_parser
        parser
        (fun p ->
          DeclParser.parse_function_declaration p attribute_specification)
  ...
  and parse_return_statement parser =
    ...
    if Token.kind semi_token = Semicolon then
      let (parser, missing) = make_missing parser in
      let (parser, semi_token) = Make.token parser1 semi_token in
