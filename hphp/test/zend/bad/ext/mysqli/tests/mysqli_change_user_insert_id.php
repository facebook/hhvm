<?php
	...
	if (!mysqli_query($link, 'DROP TABLE IF EXISTS test_mysqli_change_user_insert_id_table_1'))
		printf("[002] [%d] %s\n", mysqli_errno($link), mysqli_error($link));

	if (!mysqli_query($link, 'CREATE TABLE test_mysqli_change_user_insert_id_table_1(id INT AUTO_INCREMENT PRIMARY KEY, label CHAR(10))'))
		printf("[003] [%d] %s\n", mysqli_errno($link), mysqli_error($link));

	if (!mysqli_query($link, "INSERT INTO test_mysqli_change_user_insert_id_table_1(id, label) VALUES (100, 'z')"))
		printf("[004] [%d] %s\n", mysqli_errno($link), mysqli_error($link));
...
?>
<?php
	$test_table_name = 'test_mysqli_change_user_insert_id_table_1'; require_once("clean_table.inc");
?>