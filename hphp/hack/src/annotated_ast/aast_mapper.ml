...
struct
  ...
  and map_expr menv (p, e) =
    let e' =
      ...
      | S.Shape sm ->
          T.Shape (List.map ~f:(fun (n, e) -> (n, map_expr menv e)) sm)
      | S.ValCollection (k, ta, el) ->
          T.ValCollection (k, ta, map_exprl menv el)
      ...
      | S.Class_get (ci, y) ->
          T.Class_get (map_class_id menv ci, map_class_get_expr menv y)
      ...
      | S.Obj_get (e1, e2, fl) ->
          T.Obj_get (map_expr menv e1, map_expr menv e2, fl)
      | S.Array_get (e1, e2) ->
          T.Array_get (map_expr menv e1, Option.map e2 (map_expr menv))
      | S.Call (t, e1, hl, el1, el2) ->
          T.Call
            (t, map_expr menv e1, hl, map_exprl menv el1, map_exprl menv el2)
      ...
      | S.Eif (e1, e2, e3) ->
          T.Eif
            (map_expr menv e1, Option.map e2 (map_expr menv), map_expr menv e3)
      | S.Is (e, h) -> T.Is (map_expr menv e, h)
      | S.As (e, h, b) -> T.As (map_expr menv e, h, b)
      | S.New (ci, tl, el1, el2, ctor_annot) ->
          T.New
            ( map_class_id menv ci,
              tl,
              map_exprl menv el1,
              map_exprl menv el2,
              menv.map_expr_annotation ctor_annot )
      | S.Record (ci, is_array, fl) ->
          T.Record
            (map_class_id menv ci, is_array, List.map fl (map_field menv))
      | S.Efun (ef, ids) -> T.Efun (map_fun menv ef, ids)
      | S.Xml (id, pl, el) ->
          T.Xml
            ( id,
              List.map pl (fun attr ->
                  match attr with
      ...
      | S.Collection (id, tal, fl) ->
          T.Collection (id, tal, List.map fl (map_afield menv))
  ...
  and map_fun menv fd =
    {
      ...
      T.f_tparams = List.map fd.S.f_tparams (map_tparam menv);
  ...
  and map_file_attribute menv fa =
    {
      ...
      T.fa_user_attributes =
        List.map fa.S.fa_user_attributes (map_user_attribute menv);
    }

  and map_user_attribute menv ua =
    { T.ua_name = ua.S.ua_name; T.ua_params = map_exprl menv ua.S.ua_params }
  ...
  and map_tparam menv t =
    {
      ...
      T.tp_user_attributes =
        List.map t.S.tp_user_attributes (map_user_attribute menv);
    }
  ...
  and map_class_tparams menv ct =
    {
      ...
      T.c_tparam_constraints =
        SMap.map (Tuple.T2.map_fst ~f:map_reify_kind) ct.S.c_tparam_constraints;
    }
  ...
  and map_stmt_ menv s =
    let map_as_expr ae =
      ...
      | S.Await_as_kv (p, e1, e2) ->
          T.Await_as_kv (p, map_expr menv e1, map_expr menv e2)
    in
    let map_case c =
      match c with
      | S.Default b -> T.Default (map_block menv b)
      | S.Case (e, b) -> T.Case (map_expr menv e, map_block menv b)
    in
    ...
    | S.Awaitall (el, b) ->
        ...
        T.Awaitall (el, b)
    | S.If (e, b1, b2) ->
        T.If (map_expr menv e, map_block menv b1, map_block menv b2)
  ...
  and map_class menv c =
    {
      ...
      T.c_user_attributes =
        List.map c.S.c_user_attributes (map_user_attribute menv);
      T.c_file_attributes =
        List.map c.S.c_file_attributes (map_file_attribute menv);
      T.c_enum = c.S.c_enum;
  ...
  and map_class_typeconst menv tc =
    {
      ...
      T.c_tconst_user_attributes =
        List.map tc.S.c_tconst_user_attributes (map_user_attribute menv);
  ...
  and map_xhp_info = function
    | Some { S.xai_tag = tag } ->
        Some { T.xai_tag = Option.map tag map_xhp_attr_tag }
    | None -> None
  ...
  and map_class_var menv cv =
    {
      ...
      T.cv_user_attributes =
        List.map cv.S.cv_user_attributes (map_user_attribute menv);
  ...
  and map_method menv m =
    {
      ...
      T.m_name = m.S.m_name;
      T.m_tparams = List.map m.S.m_tparams (map_tparam menv);
      ...
      T.m_user_attributes =
        List.map m.S.m_user_attributes (map_user_attribute menv);
      T.m_ret = m.S.m_ret;
  ...
  and map_method_redeclaration menv mt =
    {
      ...
      T.mt_tparams = List.map mt.S.mt_tparams (map_tparam menv);
      ...
      T.mt_user_attributes =
        List.map mt.S.mt_user_attributes (map_user_attribute menv);
    }
  ...
  and map_typedef menv td =
    {
      ...
      T.t_tparams = List.map td.S.t_tparams (map_tparam menv);
  ...
  and map_def menv d =
    let { map_expr_annotation; map_env_annotation; map_funcbody_annotation } =
      menv
    in
    match d with
    ...
    | S.Namespace (id, p) ->
        T.Namespace
          ( id,
            map_program ~map_expr_annotation ~map_env_annotation
              ~map_funcbody_annotation p )
  ...
  and map_program ~map_expr_annotation ~map_env_annotation
      ~map_funcbody_annotation dl =
    let menv =
      { map_expr_annotation; map_env_annotation; map_funcbody_annotation }
    in
