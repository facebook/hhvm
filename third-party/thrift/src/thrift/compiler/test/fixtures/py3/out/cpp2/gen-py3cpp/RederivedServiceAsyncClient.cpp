...
::std::int32_t apache::thrift::Client<::py3::simple::RederivedService>::sync_get_seven(apache::thrift::RpcOptions& rpcOptions) {
  ...
  callback.waitUntilDone(
    evb,
    [&] {
      fbthrift_serialize_and_send_get_seven(rpcOptions, std::move(ctxAndHeader.second), ctxAndHeader.first.get(), std::move(wrappedCallback));
    });
#if FOLLY_HAS_COROUTINES
  if (shouldProcessClientInterceptors) {
...
folly::SemiFuture<::std::int32_t> apache::thrift::Client<::py3::simple::RederivedService>::fbthrift_semifuture_get_seven(apache::thrift::RpcOptions& rpcOptions) {
  ...
  return fbthrift_semifuture_get_seven(std::move(semifuture), rpcOptions, std::move(wrappedCallbackAndContextStack.first), wrappedCallbackAndContextStack.second, std::move(ctxAndHeader.second));
}

folly::SemiFuture<::std::int32_t> apache::thrift::Client<::py3::simple::RederivedService>::fbthrift_semifuture_get_seven(folly::SemiFuture<typename apache::thrift::detail::FutureCallbackHelper<::std::int32_t>::PromiseResult> semifuture, apache::thrift::RpcOptions& rpcOptions, apache::thrift::RequestClientCallback::Ptr wrappedCallback, apache::thrift::ContextStack* contextStack, std::shared_ptr<::apache::thrift::transport::THeader> header) {
  using CallbackHelper = apache::thrift::detail::FutureCallbackHelper<::std::int32_t>;
  apache::thrift::SerializedRequest request = fbthrift_serialize_get_seven(rpcOptions, *header, contextStack);
#if FOLLY_HAS_COROUTINES
  const bool shouldProcessClientInterceptors = contextStack && contextStack->shouldProcessClientInterceptors();
  if (!shouldProcessClientInterceptors) {
    fbthrift_send_get_seven(std::move(request), rpcOptions, std::move(header), std::move(wrappedCallback));
