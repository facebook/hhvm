...
TEST_F(TypeResolverTest, StreamingRes) {
  ...
  EXPECT_EQ(
      get_standard_type(res1), "::apache::thrift::ServerStream<uint64_t>");
  EXPECT_EQ(get_native_type(res1), "::apache::thrift::ServerStream<uint64_t>");

  t_stream_response res2(ui64);
  res2.set_first_response_type(t_type_ref(ui64));
  EXPECT_EQ(
      get_standard_type(res2),
      "::apache::thrift::ResponseAndServerStream<uint64_t, uint64_t>");
  EXPECT_EQ(
      get_native_type(res2),
      "::apache::thrift::ResponseAndServerStream<uint64_t, uint64_t>");
}
...
TEST_F(TypeResolverTest, StreamingSink) {
  ...
  EXPECT_EQ(
      get_standard_type(req2),
      "::apache::thrift::ResponseAndSinkConsumer<"
      "::path::to::detail::Foo, "
      "::path::to::detail::Foo, "
      "::path::to::detail::Foo>");
  EXPECT_EQ(
      get_native_type(req2),
      "::apache::thrift::ResponseAndSinkConsumer<"
      "::path::to::Foo, "
      "::path::to::Foo, "
      "::path::to::Foo>");
