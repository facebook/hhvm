...
let load config_filename options =
  ...
  let global_opts = GlobalOptions.make
    ...
    ?so_remote_version_specifier:(local_config.ServerLocalConfig.remote_version_specifier)
    ...
    ~tco_infer_missing:(GlobalOptions.InferMissing.from_string_opt @@ string_opt "infer_missing" config)
