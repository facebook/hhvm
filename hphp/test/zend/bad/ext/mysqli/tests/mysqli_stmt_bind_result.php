<?php
	...
	function func_mysqli_stmt_bind_result($link, $engine, $bind_type, $sql_type, $bind_value, $offset, $type_hint = null) {
		...
		if (!mysqli_query($link, "DROP TABLE IF EXISTS test_mysqli_stmt_bind_result_table_1")) {
			printf("[%04d] [%d] %s\n", $offset, mysqli_errno($link), mysqli_error($link));
			return false;
		}

		if (!mysqli_query($link, sprintf("CREATE TABLE test_mysqli_stmt_bind_result_table_1(id INT, label %s, PRIMARY KEY(id)) ENGINE = %s", $sql_type, $engine))) {
		...
		if (!mysqli_stmt_prepare($stmt, "INSERT INTO test_mysqli_stmt_bind_result_table_1(id, label) VALUES (?, ?)")) {
		...
		if (!mysqli_stmt_prepare($stmt, "SELECT id, label FROM test_mysqli_stmt_bind_result_table_1")) {
