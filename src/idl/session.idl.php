  ...
  array(
    ...
    'args'   => array(
      ...
      array(
        'name'   => "write",
        'type'   => String,
        'desc'   => "Write function that is called when session data is to be saved. This function expects two parameters: an identifier and the data associated with it. Note: The \"write\" handler is not executed until after the output stream is closed. Thus, output from debugging statements in the \"write\" handler will never be seen in the browser. If debugging output is necessary, it is suggested that the debug output be written to a file instead.",
      ),
      array(
        'name'   => "destroy",
        'type'   => String,
  ...
  array(
    ...
    'args'   => array(
      array(
        ...
        'type'   => String,
        'value'  => "null_string",
        'desc'   => "If id is specified, it will replace the current session id. session_id() needs to be called before session_start() for that purpose. Depending on the session handler, not all characters are allowed within the session id. For example, the file session handler only allows characters in the range a-z A-Z 0-9 , (comma) and - (minus)! Note: When using session cookies, specifying an id for session_id() will always send a new cookie when session_start() is called, regardless if the current session id is identical to the one being set.",
      ),
    ),
  ));
...
DefineFunction(
  array(
    ...
    'return' => array(
      'type'   => null,
      'desc'   => "No value is returned. Session PgSQL session_unset Last updated: Fri, 06 Aug 2010  ",
    ),
  ));
