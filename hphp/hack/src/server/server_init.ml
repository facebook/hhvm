  ...
  let (next_step_descr, next_step, user_instructions) =
    if do_indexing then
      ...
      ( "fatal",
        ...
        Some Server_init_messages.messageSavedStateFailedFullInitDisabled )
  in
...
let saved_state_init
    ...
    profiling =
  let result =
    Server_lazy_init.saved_state_init
      ~do_indexing
      ~load_state_approach
      genv
      env
...
let write_symbol_info_init genv env root (load_state : _ option) profiling =
  ...
  | None ->
    ( Server_lazy_init.write_symbol_info_full_init genv env profiling
      |> post_init genv,
  ...
  if genv.local_config.Server_local_config.rust_provider_backend then (
    ...
    let backend =
      Hh_server_provider_backend.make
        (Decl_fold_options.from_global_options env.tcopt)
        (Decl_parser_options.from_parser_options env.tcopt.Global_options.po)
    in
