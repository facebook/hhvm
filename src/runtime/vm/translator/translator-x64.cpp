...
public:
  ...
  void addLiteral(const Location &loc, x64::X64Assembler &a, RegAlloc &regMap,
                  PhysReg rMis) {
    ASSERT(loc.isLiteral());
...
void TranslatorX64::emitCGetElem(const Tracelet& t,
  ...
  SKTRACE(2, ni.source, "%s %#lx mInd=%u, iInd=%u\n",
          __func__, long(a.code.frontier), mInd, iInd);
  const DynLocation& memb = *ni.inputs[iInd];
  void (*cGetElemOp)(TypedValue*, TypedValue*, TypedValue*, MInstrState*) =
    ni.immVecM[mInd] == MEL ? cGetElemL : cGetElemC;
  m_regMap.smashLoc(memb.location);
...
TranslatorX64::emitPredictionGuards(const NormalizedInstruction& i) {
  ...
  if (m_useHHIR) {
    if (NormalizedInstruction::OutputUsed) {
      ...
      uint32 stackOffset = tx64LocPhysicalOffset(i.outStack->location);
