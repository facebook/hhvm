...
StatementPtr YieldStatement::clone() {
  YieldStatementPtr stmt(new YieldStatement(*this));
...
void YieldStatement::analyzeProgramImpl(AnalysisResultPtr ar) {
  if (m_exp) {
...
ConstructPtr YieldStatement::getNthKid(int n) const {
  switch (n) {
    case 0:
      return m_exp;
    default:
...
int YieldStatement::getKidCount() const {
  return 1;
}

void YieldStatement::setNthKid(int n, ConstructPtr cp) {
  switch (n) {
    case 0:
...
void YieldStatement::inferTypes(AnalysisResultPtr ar) {
  if (m_exp) {
    FunctionScopePtr funcScope = getFunctionScope();
    if (funcScope) {
      TypePtr ret;
...
void YieldStatement::outputPHP(CodeGenerator &cg, AnalysisResultPtr ar) {
  if (m_exp) {
    cg_printf("yield ");
...
void YieldStatement::outputCPPImpl(CodeGenerator &cg, AnalysisResultPtr ar) {
  bool braced = false;
